# srr_stats
# {G1.0} Implements linear models with high-dimensional fixed effects.
# {G2.1a} Ensures the input `formula` is correctly specified and includes fixed effects.
# {G2.1b} Validates that the input `data` is non-empty and of class `data.frame`.
# {G2.3a} Uses structured checks for parameters like `weights` and starting values.
# {G2.4} Handles missing or perfectly classified data by appropriately excluding them.
# {G2.5} Ensures numerical stability and convergence for large datasets and complex models.
# {G3.1a} Provides robust support for the Gaussian family with an identity link function.
...
#' mod <- felm(trade ~ log_dist | 0 | exp_year, yotov2017_subset)
#' summary(mod)
#'
#' @export
felm <- function(
  ...
  # FIT MODEL ----
  fit <- felm_fit_(formula_str, data, wt, control)
