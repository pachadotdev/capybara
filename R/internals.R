...
felm_fit_ <- function(y, X, wt, k.list, control) {
  ...
  # Centering variables
  MX <- center_variables_(MX, wt, k.list, center.tol, 100000L)
...
feglm_fit_ <- function(beta, eta, y, X, wt, k.list, family, control) {
  ...
  # Center variables
  MX <- center_variables_(X, w, k.list, center.tol, 100000L)
  # Recompute Hessian
...
feglm_offset_ <- function(object, offset) {
  ...
  for (iter in seq.int(iter.max)) {
    ...
    Myadj <- center_variables_((Myadj + yadj), w, k.list, center.tol, 100000L)
