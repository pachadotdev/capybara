# srr_stats
# {G1.0} Implements `predict` methods for `feglm` and `felm` objects, similar to base R methods.
# {G2.1a} Ensures input objects are of the expected class (`feglm` or `felm`).
# {G2.3a} Provides options for output type (`link`, `response`, or `terms`) via a standardized `type` argument.
# {G2.3b} Handles missing or invalid new data gracefully with appropriate checks and error messages.
# {G3.1a} Computes predicted values for both new and existing data sets.
# {G3.1b} Supports fixed-effects predictions by accounting for levels in the data.
# {G3.4a} Includes an option for type-specific predictions (e.g., `link` vs. `response`).
# {G5.2a} Tests include validation of predictions against known values and edge cases.
# {G5.4a} Outputs predictions in a format compatible with standard R workflows.
# {RE4.9} The predicted values for the model data or new data are returned as a vector with `predict()`.
# {RE4.16} The fixed effects are passed to the `predict()` function to add the group-specific effects to the predictions.
# {RE5.0} Ensures computational efficiency in handling both `feglm` and `felm` prediction workflows.
# {RE5.2} Integrates seamlessly with user-provided data for generating predictions.
# {RE5.3} Provides predictable and consistent output types for downstream analysis.
