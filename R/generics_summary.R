...
#' @srrstats {G3.1} Includes residual statistics, deviance measures, and (where applicable) R-squared values for
#'  Poisson models.
...
#'  evaluation metrics.
#' @srrstats {RE2.2} Custom handling of model-specific details like Poisson R-squared and Negative Binomial
#'  `theta` values.
#' @srrstats {RE4.11} The deviance, null deviance, R-squared and adjusted R-squared are returned in the summaries.
#' @srrstats {RE4.18} Implemented `summary()` functions specific for GLMs and LMs (i.e., it shows R2 for LMs and Poisson models).
