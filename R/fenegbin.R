# srr_stats
# {G1.0} Implements Negative Binomial regression with high-dimensional fixed effects, adapting `feglm`.
# {G2.1a} Validates input `formula` to ensure inclusion of fixed effects.
# {G2.1b} Ensures `data` is of the appropriate class and contains non-zero rows.
# {G2.3a} Uses `match.arg()` to validate the `link` argument.
# {G2.3b} Checks numeric parameters such as starting guesses and weights for validity.
# {G2.4} Handles missing and non-contributing observations by excluding them appropriately.
# {G3.1a} Supports customizable link functions (`log`, `sqrt`, and `identity`) and initialization of theta.
# {G3.1b} Provides detailed outputs including coefficients, deviance, and theta.
# {G4.0} Uses an iterative algorithm for joint estimation of coefficients and theta, ensuring convergence.
...
# {RE5.0} Optimized for high-dimensional fixed effects and large datasets, ensuring computational feasibility.
# {RE5.1} Validates convergence of both deviance and theta with strict tolerances.
# {RE5.2} Issues warnings if the algorithm fails to converge within the maximum iterations.
# {RE5.3} Outputs reproducible results, including detailed diagnostics and convergence information.
...
fenegbin <- function(
  ...
  # FIT MODEL ----
  fit <- structure(
    fenegbin_fit_(formula_str, data, wt, link, beta, eta_vec, init_theta, offset_vec, control),
