...
inline bool irons_tuck_acceleration(double *x, const double *Gx, const double *GGx,
                                     uword N, double tol) {
...
inline void center_one_column_accel(
  ...
  // Acceleration parameters (more aggressive than fixest for faster convergence)
  ...
  for (uword iter = 0; iter < max_iter; ++iter) {
    ...
    // Irons-Tuck acceleration (every 2 iterations after iteration 1, not overlapping with grand)
    ...
    if (ssr_old < std::numeric_limits<double>::infinity()) {
      ...
      if (rel_change < tol * 10.0) {
        stall_count++;
        if (stall_count > 3) break;  // Stop if stuck for 3 iterations
      } else {
...
void center_variables(
  ...
  for (uword col = 0; col < P; ++col) {
    vec y_col = V.col(col);
    center_one_column_accel(y_col, w, w_ptr, all_group_info,
