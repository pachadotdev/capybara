...
inline InferenceLM felm_fit(mat &X, const vec &y, const vec &w,
  ...
  // Step 2: Demean variables (if needed)
  mat X_demean;
  vec y_demean;
  ...
  if (has_fixed_effects) {
    ...
    DemeanResult y_demean_result = demean_variables(
        y_to_demean, w, fe_indices, nb_ids, fe_id_tables, true, params, vec());
  ...
  // Step 3: Solve normal equation on demeaned data
  ...
  if (has_fixed_effects) {
    ...
    // Use only demeaned data for sum_fe, matching feols logic
    vec sum_fe = y - X * coef_reduced;
