...
void FixedEffects::compute_fe_coef_2(vec& fe_coef_in_C, vec& fe_coef_out_C,
  ...
  // This avoids creating N-length vectors

  // Step 1: Update second FE based on first FE
  size_t nb_coef_1 = nb_coefs_(1);
  size_t coef_start_1 = coef_starts_(1);

  // Initialize with target sums for FE 1
  for (size_t i = 0; i < nb_coef_1; ++i) {
    fe_coef_tmp(i) = in_out_C(coef_start_1 + i);
  }
  ...
  if (has_weights_) {
    for (size_t obs = 0; obs < n_obs_; ++obs) {
      fe_coef_tmp(fe_idx_1(obs)) -= fe_coef_in_C(fe_idx_0(obs)) * weights_(obs);
    }
  } else {
    for (size_t obs = 0; obs < n_obs_; ++obs) {
      fe_coef_tmp(fe_idx_1(obs)) -= fe_coef_in_C(fe_idx_0(obs));
    }
  }

  // Divide by weights
  for (size_t i = 0; i < nb_coef_1; ++i) {
    fe_coef_tmp(i) /= sum_weights_(1)(i);
  }

  // Step 2: Update first FE based on second FE
  size_t nb_coef_0 = nb_coefs_(0);

  // Initialize with target sums for FE 0
  ...
  if (has_weights_) {
    for (size_t obs = 0; obs < n_obs_; ++obs) {
      fe_coef_out_C(fe_idx_0(obs)) -=
          fe_coef_tmp(fe_idx_1(obs)) * weights_(obs);
    }
  } else {
    for (size_t obs = 0; obs < n_obs_; ++obs) {
      fe_coef_out_C(fe_idx_0(obs)) -= fe_coef_tmp(fe_idx_1(obs));
    }
  }

  // Divide by weights
  for (size_t i = 0; i < nb_coef_0; ++i) {
    fe_coef_out_C(i) /= sum_weights_(0)(i);
  }
}
...
void FixedEffects::add_2_fe_to_prediction(const vec& fe_coef_a,
  ...
  // Add the value of the FE coefficients to each observation

  if (update_beta) {
    // Update coefficients of b
    compute_fe_coef_2(const_cast<vec&>(fe_coef_a), const_cast<vec&>(fe_coef_a),
