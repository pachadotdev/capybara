...
public:
  ...
  // TODO: check these
  void accumulate(size_t group_idx, const vec &values, 
                 vec &result, DemeanWorkspace &ws, bool use_weights = true) const;
...
void FixedEffects::accumulate(size_t group_idx, const vec &values, 
                              vec &result, DemeanWorkspace &ws, bool use_weights) const {
  result.zeros();
