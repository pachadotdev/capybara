...
inline solve_alpha_results solve_alpha(const vec &p,
  ...
  for (size_t it = 0; it < iter_max; ++it) {
    ...
    const double ratio = std::sqrt(change_sq / (norm_sq + 1e-10));
    if (ratio < tol)
      break;
  }
