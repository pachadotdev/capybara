...
feglm_fit_(const std::string &formula_str, SEXP df, const doubles &beta_r,
  ...
  auto out = writable::list(
      {"eta"_nm = as_doubles(result.eta),
       ...
       "conv"_nm = writable::logicals({cpp4r::r_bool(result.conv)}),
...
fenegbin_fit_(const std::string &formula_str, SEXP df, const doubles &w_r,
  ...
  auto out = writable::list(
      {"eta"_nm = as_doubles(result.eta),
       ...
       "conv_outer"_nm =
           writable::logicals({cpp4r::r_bool(result.conv_outer)})});
...
fepoisson_asymmetric_fit_(const std::string &formula_str, SEXP df,
  ...
  auto out = writable::list(
      {"eta"_nm = as_doubles(result.eta),
       ...
       "iter_outer"_nm =
           writable::integers({static_cast<int>(result.iter_outer)}),
       "conv_outer"_nm = writable::logicals({cpp4r::r_bool(result.conv_outer)}),
