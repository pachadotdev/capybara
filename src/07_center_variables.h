...
// groups
inline void map_partial_out(mat *X, vec *y, const vec &w, const indices_info &indices, double tol, size_t max_iter, size_t iter_interrupt, bool use_weights) {
  // TIME_FUNCTION;
  ...
  for (size_t iter = 0; iter < max_iter_debug; ++iter) {
    ...
    if ((has_X && x_converged) && (has_y && y_converged)) break;
    if (has_X && !has_y && x_converged) break;
    if (has_y && !has_X && y_converged) break;
  }
}
