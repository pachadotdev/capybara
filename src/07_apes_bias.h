...
inline BiasResult compute_bias_corr(const vec &y, const mat &X,
                                    const mat &TX, // Centered regressor matrix
                                    const vec &eta, const vec &weights,
                                    const mat &hessian, const std::string &link,
                                    const FlatFEMap &fe_map,
                                    const std::string &panel_structure,
                                    uword bandwidth, double center_tol,
...
inline APESResult
compute_apes(const vec &y, const mat &X,
             const mat &TX, // Centered regressor matrix
             const vec &eta, const vec &weights, const vec &beta,
             const mat &hessian, const std::string &link,
             const FlatFEMap &fe_map,
             uword n_full, // Full sample size (before any drops)
             const std::string &panel_structure, const std::string &sampling_fe,
             bool weak_exo,
             double adj,      // Finite population adjustment
             uword bandwidth, // Only used if bias_corrected
             bool bias_corrected, double center_tol, uword iter_center_max,
             uword grand_acc_period) {
  ...
  mat mpsi = psi;
  center_variables(mpsi, w, fe_map_copy, center_tol, iter_center_max,
                   grand_acc_period);
  mat ppsi = psi - mpsi;

  // Build group indices if needed for bias correction or covariance adjustment
  std::vector<field<uvec>> k_groups;
  if (bias_corrected || adj > 0.0 || sampling_fe != "independence" ||
      weak_exo) {
    k_groups.resize(K);
    for (uword k = 0; k < K; ++k) {
      k_groups[k] = build_group_indices(fe_map, k);
  ...
  if (bias_corrected && bandwidth > 0) {
    ...
    if (panel_structure == "classic") {
      ...
      if (bandwidth > 0) {
        b = (b - group_sums_spectral(mpsi % w, v_score, w, bandwidth,
                                     k_groups[0])) /
            n;
      }
    } else {
