...
SEXP unwind_protect(Fun &&code) {
  ...
  SEXP res = R_UnwindProtect(
      ...
      [](void *jmpbuf, Rboolean jump) {
        if (jump == TRUE) {
          // We need to first jump back into the C++ stacks because you can't
          // safely throw exceptions from C stack frames.
...
typename std::enable_if<
    !std::is_same<R, SEXP>::value && !std::is_same<R, void>::value, R>::type
...
#else
// Don't do anything if we don't have unwind protect. This will leak C++
// resources, including those held by cpp11 objects, but the other alternatives
// are also not great.
template <typename Fun>
...
template <typename, size_t> struct appended_sequence;
...
decltype(std::declval<F &&>()(std::declval<Aref>()...))
apply(F &&f, std::tuple<Aref...> &&a, const index_sequence<I...> &) {
  return std::forward<F>(f)(std::get<I>(std::move(a))...);
}

template <typename F, typename... Aref>
decltype(std::declval<F &&>()(std::declval<Aref>()...))
apply(F &&f, std::tuple<Aref...> &&a) {
  return apply(std::forward<F>(f), std::move(a),
               make_index_sequence<sizeof...(Aref)>{});
}

// overload to silence a compiler warning that the (empty) tuple parameter is
// set but unused
template <typename F>
...
template <typename F, typename... Aref> struct closure {
...
struct protect {
  template <typename F> struct function {
    ...
    operator()(A &&...a) const {
      ...
      return unwind_protect(detail::closure<F, A &&...>{
  ...
  /// May not be applied to a function bearing attributes, which interfere with
  /// linkage on some compilers; use an appropriately attributed alternative.
  /// (For example, Rf_error bears the [[noreturn]] attribute and must be
  /// protected with safe.noreturn rather than safe.operator[]).
  template <typename F> constexpr function<F> operator[](F *raw) const {
    return {raw};
  }

  template <typename F> struct noreturn_function {
    template <typename... A> void operator() [[noreturn]] (A &&...a) const {
      // workaround to support gcc4.8, which can't capture a parameter pack
      unwind_protect(detail::closure<F, A &&...>{
          ptr_, std::forward_as_tuple(std::forward<A>(a)...)});
      // Compiler hint to allow [[noreturn]] attribute; this is never executed
      // since the above call will not return.
      throw std::runtime_error("[[noreturn]]");
    }
    F *ptr_;
  };

  template <typename F> constexpr noreturn_function<F> noreturn(F *raw) const {
    return {raw};
  }
};
...
template <typename... Args> void warning(const char *fmt_arg, Args &&...args) {
  std::string msg = fmt::format(fmt_arg, std::forward<Args>(args)...);
...
/// A doubly-linked list of preserved objects, allowing O(1) insertion/release
/// of objects compared to O(N preserved) with R_PreserveObject.
static struct {
  ...
  void print() {
    ...
    for (SEXP cell = list; cell != R_NilValue; cell = CDR(cell)) {
      REprintf("%p CAR: %p CDR: %p TAG: %p\n", reinterpret_cast<void *>(cell),
               reinterpret_cast<void *>(CAR(cell)),
               reinterpret_cast<void *>(CDR(cell)),
...
private:
  ...
  // This avoids issues with sharing preserve list state across compilation
  // units and across packages, which has historically caused many issues
  // (#330).
