...
class data_frame : public list {
  ...
  /* we cannot use Rf_getAttrib because it has a special case for c(NA, -n) and
   * creates the full vector */
  ...
  static int calc_nrow(SEXP x) {
    ...
    bool has_short_rownames = (Rf_isInteger(nms) && Rf_xlength(nms) == 2 &&
                               INTEGER(nms)[0] == NA_INTEGER);
...
public:
  data_frame(const SEXP data)
      : cpp11::data_frame(set_data_frame_attributes(data)) {}
  ...
  attribute_proxy<data_frame> attr(const char *name) const {
    return {*this, name};
  }
