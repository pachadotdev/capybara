...
template <typename T> void default_deleter(T *obj) { delete obj; }
...
public:
  ...
  external_pointer(pointer p, bool use_deleter = true,
                   bool finalize_on_exit = true)
      : data_(safe[R_MakeExternalPtr]((void *)p, R_NilValue, R_NilValue)) {
    if (use_deleter) {
      R_RegisterCFinalizerEx(data_, r_deleter,
                             static_cast<r_bool>(finalize_on_exit));
    }
  }
...
void swap(external_pointer<T, Deleter> &lhs,
          external_pointer<T, Deleter> &rhs) noexcept {
  lhs.swap(rhs);
}
