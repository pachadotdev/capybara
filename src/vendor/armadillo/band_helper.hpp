...
inline bool is_band(uword &out_KL, uword &out_KU, const Mat<eT> &A,
                    const uword N_min) {
  ...
  if ((A_colNm2[0] != eT_zero) || (A_colNm2[1] != eT_zero) ||
      (A_colNm1[0] != eT_zero) || (A_colNm1[1] != eT_zero)) {
    return false;
  }

  // if we reached this point, go through the entire matrix to work out number
  // of subdiagonals and superdiagonals
  ...
  for (uword col = 0; col < N; ++col) {
    ...
    if ((L_count > KL) || (U_count > KU)) {
      ...
      const uword n_nonzero =
          N * (KL + KU + 1) - (KL * (KL + 1) + KU * (KU + 1)) / 2;

      // return as soon as we know that it's not worth analysing the matrix any
      // further
...
inline bool is_band_lower(uword &out_KD, const Mat<eT> &A, const uword N_min) {
  ...
  // if we reached this point, go through the bottom triangle to work out number
  // of subdiagonals
...
template <typename eT>
inline void compress(Mat<eT> &AB, const Mat<eT> &A, const uword KL,
                     const uword KU, const bool use_offset) {
  arma_extra_debug_sigprint();
  ...
  // in rows KL+1 to 2*KL+KU+1  (note: fortran counts from 1) for ?gbsvx, matrix
  // AB size: KL+KU+1 x N; band representaiton of A stored in rows    1 to
  // KL+KU+1  (note: fortran counts from 1)
  //
  // the +1 in the above formulas is to take into account the main diagonal

  const uword AB_n_rows =
      (use_offset) ? uword(2 * KL + KU + 1) : uword(KL + KU + 1);
  const uword N = A.n_rows;
  ...
  if (AB_n_rows == uword(1)) {
    ...
    for (uword j = 0; j < N; ++j) {
      ...
      eT *AB_colptr =
          AB.colptr(j) + AB_row_start + ((use_offset) ? KL : uword(0));
...
template <typename eT>
inline void uncompress(Mat<eT> &A, const Mat<eT> &AB, const uword KL,
                       const uword KU, const bool use_offset) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check((AB_n_rows != ((use_offset) ? uword(2 * KL + KU + 1)
                                               : uword(KL + KU + 1))),
