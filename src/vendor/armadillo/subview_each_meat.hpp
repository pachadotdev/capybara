...
inline subview_each_common<parent, mode>::subview_each_common(
    const parent &in_P)
    : P(in_P) {
...
template <typename parent, unsigned int mode, typename TB>
inline subview_each2<parent, mode, TB>::subview_each2(
    const parent &in_P, const Base<uword, TB> &in_indices)
    : subview_each_common<parent, mode>::subview_each_common(in_P),
...
inline void subview_each2<parent, mode, TB>::operator=(const Base<eT, T1> &in) {
  ...
  if (mode == 0) // each column
  {
    for (uword i = 0; i < N; ++i) {
      ...
      arma_debug_check_bounds((col >= p_n_cols),
                              "each_col(): index out of bounds");
  ...
  } else // each row
  {
    for (uword i = 0; i < N; ++i) {
      ...
      arma_debug_check_bounds((row >= p_n_rows),
                              "each_row(): index out of bounds");
...
subview_each1_aux::operator_minus(const Base<typename parent::elem_type, T1> &X,
...
template <typename parent, unsigned int mode, typename T2>
inline Mat<typename parent::elem_type>
subview_each1_aux::operator_div(const subview_each1<parent, mode> &X,
                                const Base<typename parent::elem_type, T2> &Y) {
  arma_extra_debug_sigprint();
...
template <typename T1, typename parent, unsigned int mode>
inline Mat<typename parent::elem_type>
subview_each1_aux::operator_div(const Base<typename parent::elem_type, T1> &X,
                                const subview_each1<parent, mode> &Y) {
  arma_extra_debug_sigprint();
...
inline Mat<typename parent::elem_type>
subview_each2_aux::operator_minus(const Base<typename parent::elem_type, T1> &X,
...
template <typename parent, unsigned int mode, typename TB, typename T2>
inline Mat<typename parent::elem_type>
subview_each2_aux::operator_div(const subview_each2<parent, mode, TB> &X,
                                const Base<typename parent::elem_type, T2> &Y) {
  arma_extra_debug_sigprint();
...
template <typename T1, typename parent, unsigned int mode, typename TB>
inline Mat<typename parent::elem_type>
subview_each2_aux::operator_div(const Base<typename parent::elem_type, T1> &X,
                                const subview_each2<parent, mode, TB> &Y) {
  arma_extra_debug_sigprint();
