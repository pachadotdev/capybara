...
public:
  ...
  // mem_state = 2: use auxiliary memory and don't allow the number of elements
  // to be changed mem_state = 3: fixed size (eg. via template based size
  // specification)

  arma_aligned const eT *const mem; //!< pointer to the memory used for storing
                                    //!< elements (memory is read-only)
...
public:
  ...
  inline Cube(const uword in_n_rows, const uword in_n_cols,
              const uword in_n_slices, const fill::fill_class<fill_type> &f);
  template <typename fill_type>
  ...
  inline explicit Cube(const BaseCube<pod_type, T1> &A,
  ...
  arma_inline const subview_cube<eT> rows(const uword in_row1,
                                          const uword in_row2) const;

  arma_inline subview_cube<eT> cols(const uword in_col1, const uword in_col2);
  arma_inline const subview_cube<eT> cols(const uword in_col1,
                                          const uword in_col2) const;

  arma_inline subview_cube<eT> slices(const uword in_slice1,
  ...
  subcube(const uword in_row1, const uword in_col1, const uword in_slice1,
          const uword in_row2, const uword in_col2,
          const uword in_slice2) const;

  inline subview_cube<eT> subcube(const uword in_row1, const uword in_col1,
                                  const uword in_slice1, const SizeCube &s);
  inline const subview_cube<eT> subcube(const uword in_row1,
                                        const uword in_col1,
                                        const uword in_slice1,
  ...
  arma_inline subview_cube<eT> tube(const uword in_row1, const uword in_col1,
                                    const SizeMat &s);
  arma_inline const subview_cube<eT>
  tube(const uword in_row1, const uword in_col1, const SizeMat &s) const;

  inline subview_cube<eT> tube(const span &row_span, const span &col_span);
  inline const subview_cube<eT> tube(const span &row_span,
  ...
  operator()(const Base<uword, T1> &a) const;
  ...
  each_slice(const Base<uword, T1> &indices) const;

  inline Cube &each_slice(const std::function<void(Mat<eT> &)> &F);
  inline const Cube &
  each_slice(const std::function<void(const Mat<eT> &)> &F) const;
  ...
  template <typename T1>
  arma_inline subview_cube_slices<eT, T1>
  slices(const Base<uword, T1> &indices);
  template <typename T1>
  arma_inline const subview_cube_slices<eT, T1>
  slices(const Base<uword, T1> &indices) const;
  ...
  insert_slices(const uword slice_num, const uword N, const bool set_to_zero);
  ...
  arma_warn_unused arma_inline eT &
  ...
  arma_warn_unused arma_inline const eT &
  at(const uword in_row, const uword in_col, const uword in_slice) const;
  ...
  in_range(const uword in_row, const uword in_col, const uword in_slice) const;
  arma_warn_unused inline bool in_range(const span &row_span,
                                        const span &col_span,
                                        const span &slice_span) const;
  ...
  arma_warn_unused arma_inline eT *slice_colptr(const uword in_slice,
                                                const uword in_col);
  ...
  arma_cold inline bool load(const hdf5_name &spec,
  ...
  arma_deprecated inline bool
  quiet_save(const hdf5_name &spec, const file_type type = hdf5_binary) const;
  arma_deprecated inline bool
  ...
  inline void steal_mem(Cube &X); //!< don't use this unless you're writing code
                                  //!< internal to Armadillo
  inline void
  steal_mem(Cube &X,
...
protected:
  ...
  template <typename T1, typename T2>
  inline void init(const BaseCube<pod_type, T1> &A,
                   const BaseCube<pod_type, T2> &B);
...
private:
  static constexpr uword fixed_n_elem =
      fixed_n_rows * fixed_n_cols * fixed_n_slices;
...
public:
  ...
  template <typename T1, typename T2>
  inline fixed(const BaseCube<pod_type, T1> &A,
               const BaseCube<pod_type, T2> &B);
  ...
  inline Cube &
  operator=(const fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices> &X);
...
public:
  ...
  inline static void set_real(Cube<std::complex<T>> &out,
                              const BaseCube<T, T1> &X);
  template <typename T, typename T1>
  inline static void set_imag(Cube<std::complex<T>> &out,
                              const BaseCube<T, T1> &X);
};
