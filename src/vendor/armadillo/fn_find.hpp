...
find(const Base<typename T1::elem_type, T1> &X, const uword k,
...
find(const mtGlueCube<uword, T1, T2, glue_rel_type> &X, const uword k = 0,
  ...
  const Mat<eT1> R1(const_cast<eT1 *>(tmp1.M.memptr()), tmp1.M.n_elem, 1,
                    false);
  const Mat<eT2> R2(const_cast<eT2 *>(tmp2.M.memptr()), tmp2.M.n_elem, 1,
                    false);

  return find(mtGlue<uword, Mat<eT1>, Mat<eT2>, glue_rel_type>(R1, R2), k,
              direction);
}
...
template <typename T1>
arma_warn_unused inline Col<uword>
...
template <typename T1>
arma_warn_unused inline Col<uword>
find(const SpBase<typename T1::elem_type, T1> &X, const uword k,
