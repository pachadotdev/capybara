...
inline bool
op_princomp::direct_princomp(Mat<typename T1::elem_type> &coeff_out,
  ...
  if (n_rows > 1) // more than one sample
  {
    ...
    const bool svd_ok = (n_rows >= n_cols)
                            ? svd_econ(U, s, coeff_out, score_out)
...
op_princomp::direct_princomp(Mat<typename T1::elem_type> &coeff_out,
  ...
  if (in.n_elem != 0) {
    ...
    const bool svd_ok = (in.n_rows >= in.n_cols)
                            ? svd_econ(U, s, coeff_out, tmp)
