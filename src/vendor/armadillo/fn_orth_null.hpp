...
arma_warn_unused arma_inline
    ...
    orth(const Base<typename T1::elem_type, T1> &X,
...
orth(Mat<typename T1::elem_type> &out,
...
arma_warn_unused arma_inline
    ...
    null(const Base<typename T1::elem_type, T1> &X,
         const typename T1::pod_type tol = 0.0) {
  arma_extra_debug_sigprint();
...
template <typename T1>
inline typename enable_if2<is_real<typename T1::pod_type>::value, bool>::result
null(Mat<typename T1::elem_type> &out,
     const Base<typename T1::elem_type, T1> &X,
     const typename T1::pod_type tol = 0.0) {
  arma_extra_debug_sigprint();
