...
arma_inline
spdiagview<eT>::spdiagview(const SpMat<eT> &in_m, const uword in_row_offset,
...
inline void spdiagview<eT>::extract(Mat<eT> &out, const spdiagview<eT> &in) {
  ...
  // NOTE: we're assuming that the 'out' matrix has already been set to the
  // correct size; size setting is done by either the Mat contructor or
  // Mat::operator=()
...
template <typename eT> inline eT spdiagview<eT>::at(const uword i) const {
  return m.at(i + row_offset, i + col_offset);
}
...
template <typename eT>
inline SpMat_MapMat_val<eT> spdiagview<eT>::operator()(const uword row,
                                                       const uword col) {
...
inline void spdiagview<eT>::replace(const eT old_val, const eT new_val) {
  ...
  if (old_val == eT(0)) {
    arma_debug_warn_level(
        1, "spdiagview::replace(): replacement not done, as old_val = 0");
  } else {
    Mat<eT> tmp(*this);
