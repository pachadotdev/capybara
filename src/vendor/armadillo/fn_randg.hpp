...
randg(const uword n_rows, const uword n_cols,
  ...
  arma_debug_check(((a <= double(0)) || (b <= double(0))),
                   "randg(): incorrect distribution parameters; a and b must "
...
arma_warn_unused inline typename arma_real_or_cx_only<eT>::result
...
randg(const uword n_rows, const uword n_cols, const uword n_slices,
      const distr_param &param = distr_param(),
