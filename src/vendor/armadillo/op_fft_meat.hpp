...
template <typename cx_type, bool inverse> class fft_engine_wrapper {
public:
  ...
  inline fft_engine_wrapper(const uword N_samples, const uword N_exec) {
    ...
    worker_kissfft = (use_fftw3 == false)
                         ...
                         : nullptr;
    worker_fftw3 = (use_fftw3 == true)
                       ? new fft_engine_fftw3<cx_type, inverse>(N_samples)
                       : nullptr;
  }
...
inline void op_fft_real::apply(
  ...
  // no need to worry about aliasing, as we're going from a real object to
  // complex complex, which by definition cannot alias
...
inline void op_fft_cx::apply(Mat<typename T1::elem_type> &out,
  ...
  if (U.is_alias(out)) {
    ...
    op_fft_cx::apply_noalias<eT, false>(tmp, U.M, in.aux_uword_a,
                                        in.aux_uword_b);

    out.steal_mem(tmp);
  } else {
    op_fft_cx::apply_noalias<eT, false>(out, U.M, in.aux_uword_a,
                                        in.aux_uword_b);
  }
}

template <typename eT, bool inverse>
inline void op_fft_cx::apply_noalias(Mat<eT> &out, const Mat<eT> &X,
                                     const uword a, const uword b) {
  arma_extra_debug_sigprint();
...
inline void op_ifft_cx::apply(Mat<typename T1::elem_type> &out,
  ...
  if (U.is_alias(out)) {
    ...
    op_fft_cx::apply_noalias<eT, true>(tmp, U.M, in.aux_uword_a,
                                       in.aux_uword_b);

    out.steal_mem(tmp);
  } else {
    op_fft_cx::apply_noalias<eT, true>(out, U.M, in.aux_uword_a,
                                       in.aux_uword_b);
  }
}
