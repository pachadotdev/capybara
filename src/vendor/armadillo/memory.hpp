...
arma_malloc inline eT *memory::acquire(const uword n_elem) {
...
#elif defined(ARMA_HAVE_POSIX_MEMALIGN)
  {
    ...
    const size_t alignment =
        (n_bytes >= size_t(1024)) ? size_t(32) : size_t(16);

    // TODO: investigate apparent memory leak when using alignment >= 64 (as
    // shown on Fedora 28, glibc 2.27)
    int status = posix_memalign(
        (void **)&memptr,
        ((alignment >= sizeof(void *)) ? alignment : sizeof(void *)), n_bytes);
...
#elif defined(_MSC_VER)
  {
    ...
    // out_memptr = (eT *) _aligned_malloc( sizeof(eT)*n_elem, 16 );  // lives
    // in malloc.h
  ...
  arma_check_bad_alloc((out_memptr == nullptr),
                       "arma::memory::acquire(): out of memory");
...
template <typename eT> arma_inline bool memory::is_aligned(const eT *mem) {
     ...
     defined(ARMA_HAVE_GCC_ASSUME_ALIGNED)) &&                                 \
    !defined(ARMA_DONT_CHECK_ALIGNMENT)
  {
    return (sizeof(std::size_t) >= sizeof(eT *))
               ? ((std::size_t(mem) & 0x0F) == 0)
               : false;
  }
#else
  {
