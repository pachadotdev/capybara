...
template <typename T1>
inline bool
op_pinv_default::apply_direct(Mat<typename T1::elem_type> &out,
...
template <typename T1>
inline void op_pinv::apply(Mat<typename T1::elem_type> &out,
...
inline bool op_pinv::apply_direct(Mat<typename T1::elem_type> &out,
  ...
  const bool is_sym_size_ok =
      (A.n_rows == A.n_cols) &&
      (A.n_rows > (is_cx<eT>::yes ? uword(20) : uword(40)));
...
inline bool op_pinv::apply_diag(Mat<eT> &out, const Mat<eT> &A,
  ...
  if (tol == T(0)) {
    tol = (std::max)(A.n_rows, A.n_cols) * max_abs_Aii *
