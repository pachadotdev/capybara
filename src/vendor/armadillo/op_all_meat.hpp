...
inline bool op_all::all_vec_helper(const Base<typename T1::elem_type, T1> &X) {
  ...
  // NOTE: for empty vectors it makes more sense to return false, but we need to
  // return true for compatibility with Octave
  return (n_elem == count);
}
...
template <typename T1>
inline void op_all::apply_helper(Mat<uword> &out, const Proxy<T1> &P,
                                 const uword dim) {
  arma_extra_debug_sigprint();
