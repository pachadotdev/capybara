...
arma_inline typename enable_if2<is_arma_type<T1>::value,
                                const eOp<T1, eop_scalar_times>>::result
...
//! diagmat * diagmat
template <typename T1, typename T2>
inline Mat<typename promote_type<typename T1::elem_type,
...
arma_inline typename enable_if2<
    ...
    const mtSpOp<typename std::complex<typename T1::pod_type>, T1,
                 spop_cx_scalar_times>>::result
operator*(const T1 &X, const std::complex<typename T1::pod_type> &k) {
  arma_extra_debug_sigprint();

  return mtSpOp<typename std::complex<typename T1::pod_type>, T1,
                spop_cx_scalar_times>('j', X, k);
}
...
inline typename enable_if2<
    ...
    const mtSpGlue<typename promote_type<typename T1::elem_type,
                                         typename T2::elem_type>::result,
                   T1, T2, spglue_times_mixed>>::result
operator*(const T1 &X, const T2 &Y) {
  arma_extra_debug_sigprint();
...
//! multiplication of one sparse and one dense object with different element
//! types
template <typename T1, typename T2>
inline typename enable_if2<
    (is_arma_sparse_type<T1>::value && is_arma_type<T2>::value &&
...
//! multiplication of one dense and one sparse object with different element
//! types
template <typename T1, typename T2>
inline typename enable_if2<
    (is_arma_type<T1>::value && is_arma_sparse_type<T2>::value &&
