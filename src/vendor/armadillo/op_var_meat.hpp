...
inline void
op_var::apply_noalias(Mat<typename get_pod_type<in_eT>::result> &out,
...
op_var::var_vec(const Base<typename T1::elem_type, T1> &X,
...
inline T op_var::direct_var_robust(const std::complex<T> *const X,
                                   const uword n_elem, const uword norm_type) {
  arma_extra_debug_sigprint();
