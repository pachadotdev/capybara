...
template <typename T1>
inline bool
op_inv_spd_default::apply_direct(Mat<typename T1::elem_type> &out,
...
inline bool
op_inv_spd_full::apply_direct(Mat<typename T1::elem_type> &out,
                              const Base<typename T1::elem_type, T1> &expr,
                              const uword flags) {
  arma_extra_debug_sigprint();
  ...
  if (has_user_flags) {
    ...
    arma_debug_check((fast && allow_approx),
                     "inv_sympd(): options 'fast' and 'allow_approx' are "
                     "mutually exclusive");
    arma_debug_check(
        (fast && no_ugly),
        "inv_sympd(): options 'fast' and 'no_ugly' are mutually exclusive");
    arma_debug_check((no_ugly && allow_approx),
                     "inv_sympd(): options 'no_ugly' and 'allow_approx' are "
                     "mutually exclusive");
  }
  ...
  if ((arma_config::debug) && (arma_config::warn_level > 0)) {
    ...
    } else if ((is_cx<eT>::yes) &&
               (sym_helper::check_diag_imag(out) == false)) {
      arma_debug_warn_level(
          1, "inv_sympd(): imaginary components on diagonal are non-zero");
    }
  }
...
template <typename eT> inline bool op_inv_spd_full::apply_tiny_2x2(Mat<eT> &X) {
  arma_extra_debug_sigprint();
  ...
  // NOTE: since det_min is positive, this also checks whether det_val is
  // positive
