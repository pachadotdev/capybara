...
public:
  ...
  inline static void randi_fill(eT *mem, const uword N, const int a,
                                const int b);
...
inline double arma_rng_cxx03::randn_val() {
  ...
  do {
    tmp1 = double(2) * double(randi_val()) *
               (double(1) / double(randi_max_val())) -
           double(1);
    tmp2 = double(2) * double(randi_val()) *
               (double(1) / double(randi_max_val())) -
           double(1);
...
template <typename eT>
inline void arma_rng_cxx03::randi_fill(eT *mem, const uword N, const int a,
                                       const int b) {
  if ((a == 0) && (b == RAND_MAX)) {
    for (uword i = 0; i < N; ++i) {
      mem[i] = eT(std::rand());
