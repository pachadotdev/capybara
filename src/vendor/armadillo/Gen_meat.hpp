...
inline void Gen<T1, gen_type>::apply(Mat<typename T1::elem_type> &out) const {
  ...
  // size; this is done by either the Mat contructor or operator=()
...
Gen<T1, gen_type>::apply(subview<typename T1::elem_type> &out) const {
  ...
  // NOTE: we're assuming that the submatrix has the same dimensions as the Gen
  // object this is checked by subview::operator=()
