...
protected:
  inline SpSubview(const SpMat<eT> &in_m, const uword in_row1,
...
public:
  ...
  */
  ...
  arma_warn_unused inline SpSubview_MapMat_val<eT>
  ...
  inline SpSubview submat(const uword in_row1, const uword in_col1,
  ...
  public:
    ...
    inline iterator_base(const SpSubview &in_M, const uword col,
                         const uword pos);
  ...
  public:
    ...
    arma_inline uword row() const {
      return iterator_base::M->m
                 .row_indices[iterator_base::internal_pos + skip_pos] -
    ...
    operator!=(const typename SpMat<eT>::const_iterator &rhs) const;
    arma_hot inline bool
    operator==(const typename SpMat<eT>::const_iterator &rhs) const;
    ...
    arma_hot inline bool
    operator!=(const typename SpMat<eT>::const_row_iterator &rhs) const;
    arma_hot inline bool
    operator==(const typename SpMat<eT>::const_row_iterator &rhs) const;

    arma_aligned uword
        skip_pos; // not used in row_iterator or const_row_iterator
  };
  ...
  public:
    ...
    inline const_row_iterator(const SpSubview &in_M, uword in_row,
                              uword in_col);
    ...
    arma_inline eT operator*() const {
      return iterator_base::M->m.values[actual_pos];
    }
...
private:
  ...
  friend class SpValProxy<SpSubview<eT>>; // allow SpValProxy to call
                                          // insert_element() and
                                          // delete_element()
