...
inline bool sp_auxlib::eigs_sym(Col<eT> &eigval, Mat<eT> &eigvec,
                                const SpBase<eT, T1> &X, const uword n_eigvals,
                                const form_type form_val,
  ...
  if ((arma_config::debug) &&
      (sp_auxlib::rudimentary_sym_check(U.M) == false)) {
    if (is_cx<eT>::no) {
  ...
  // shift-invert;
  // TODO: in shift-invert mode, "sm" maps to "lm" of the shift-inverted matrix
  // (with sigma = 0)

#if defined(ARMA_USE_NEWARP)
  {
    return sp_auxlib::eigs_sym_newarp(eigval, eigvec, U.M, n_eigvals, form_val,
                                      opts);
  }
#elif defined(ARMA_USE_ARPACK)
  {
    ...
    return sp_auxlib::eigs_sym_arpack<eT, false>(eigval, eigvec, U.M, n_eigvals,
                                                 form_val, sigma, opts);
  }
#else
  {
    ...
    arma_stop_logic_error(
        "eigs_sym(): use of NEWARP or ARPACK must be enabled");
    return false;
  }
#endif
}
...
inline bool sp_auxlib::eigs_sym(Col<eT> &eigval, Mat<eT> &eigvec,
  ...
  {
    return sp_auxlib::eigs_sym_newarp(eigval, eigvec, U.M, n_eigvals, sigma,
                                      opts);
  }
#elif (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    return sp_auxlib::eigs_sym_arpack<eT, true>(eigval, eigvec, U.M, n_eigvals,
                                                form_val, sigma, opts);
  }
#else
  {
    ...
    arma_stop_logic_error("eigs_sym(): use of NEWARP or ARPACK as well as "
                          "SuperLU must be enabled to use "
                          "'sigma'");
    return false;
  }
#endif
...
inline bool
sp_auxlib::eigs_sym_newarp(Col<eT> &eigval, Mat<eT> &eigvec, const SpMat<eT> &X,
...
#if defined(ARMA_USE_NEWARP)
  {
    ...
    arma_debug_check((n_eigvals >= op.n_rows),
                     "eigs_sym(): n_eigvals must be less than the number of "
                     "rows in the matrix");

    // If the matrix is empty, the case is trivial.
    if ((op.n_cols == 0) ||
        (n_eigvals == 0)) // We already know n_cols == n_rows.
    {
      eigval.reset();
      eigvec.reset();
      return true;
    ...
    // Use max(2*k+1, 20) as default subspace dimension for the sym case; MATLAB
    // uses max(2*k, 20), but we need to be backward-compatible.
    uword ncv_default =
        uword(((2 * n_eigvals + 1) > (20)) ? (2 * n_eigvals + 1) : (20));
    ...
    if (opts.subdim != 0) {
      if (opts.subdim < (n_eigvals + 1)) {
        arma_debug_warn_level(1,
                              "eigs_sym(): opts.subdim must be greater than k; "
                              "using k+1 instead of ",
                              opts.subdim);
...
inline bool
sp_auxlib::eigs_sym_arpack(Col<eT> &eigval, Mat<eT> &eigvec, const SpMat<eT> &X,
                           const uword n_eigvals, const form_type form_val,
                           const eT sigma, const eigs_opts &opts) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // _naupd() functions.
    arma_debug_check((n_eigvals >= X.n_rows),
                     "eigs_sym(): n_eigvals must be less than the number of "
                     "rows in the matrix");
    ...
    // uses max(2*k, 20), but we need to be backward-compatible.
    ncv_default =
        blas_int(((2 * n_eigvals + 1) > (20)) ? (2 * n_eigvals + 1) : (20));
    ...
    {
      run_aupd_shiftinvert(n_eigvals, sigma, X, true /* sym, not gen */, n, tol,
                           maxiter, resid, ncv, v, ldv, iparam, ipntr, workd,
                           workl, lworkl, rwork, info);
    } else {
      const SpMat<eT> Xst = X.st();

      run_aupd_plain(n_eigvals, which, X, Xst, true /* sym, not gen */, n, tol,
    ...
    arpack::seupd(&rvec, &howmny, select.memptr(), eigval.memptr(),
    ...
    // Check for errors.
    if (info != 0) {
      arma_debug_warn_level(1, "eigs_sym(): ARPACK error ", info,
                            " in seupd()");
      return false;
    }
...
inline bool
sp_auxlib::eigs_gen(Col<std::complex<T>> &eigval, Mat<std::complex<T>> &eigvec,
                    const SpBase<T, T1> &X, const uword n_eigvals,
...
#if defined(ARMA_USE_NEWARP)
  {
    return sp_auxlib::eigs_gen_newarp(eigval, eigvec, U.M, n_eigvals, form_val,
                                      opts);
  }
#elif defined(ARMA_USE_ARPACK)
  {
    ...
    return sp_auxlib::eigs_gen_arpack<T, false>(eigval, eigvec, U.M, n_eigvals,
                                                form_val, sigma, opts);
  }
#else
  {
    ...
    arma_stop_logic_error(
        "eigs_gen(): use of NEWARP or ARPACK must be enabled");
    return false;
  }
#endif
}
...
sp_auxlib::eigs_gen(Col<std::complex<T>> &eigval, Mat<std::complex<T>> &eigvec,
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    return sp_auxlib::eigs_gen_arpack<T, true>(eigval, eigvec, U.M, n_eigvals,
                                               form_val, sigma, opts);
  }
#else
  {
...
inline bool sp_auxlib::eigs_gen_newarp(Col<std::complex<T>> &eigval,
                                       ...
                                       const SpMat<T> &X, const uword n_eigvals,
                                       const form_type form_val,
                                       const eigs_opts &opts) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_NEWARP)
  {
    ...
    arma_debug_check((n_eigvals + 1 >= op.n_rows),
                     "eigs_gen(): n_eigvals + 1 must be less than the number "
    ...
    // Use max(2*k+1, 20) as default subspace dimension for the gen case; same
    // as MATLAB.
    uword ncv_default =
        uword(((2 * n_eigvals + 1) > (20)) ? (2 * n_eigvals + 1) : (20));
    ...
    if (opts.subdim != 0) {
      if (opts.subdim < (n_eigvals + 3)) {
        arma_debug_warn_level(1,
                              "eigs_gen(): opts.subdim must be greater than "
                              "k+2; using k+3 instead of ",
                              opts.subdim);
...
inline bool
sp_auxlib::eigs_gen_arpack(Col<std::complex<T>> &eigval,
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // Make sure we aren't asking for every eigenvalue.
    arma_debug_check((n_eigvals + 1 >= X.n_rows),
                     "eigs_gen(): n_eigvals + 1 must be less than the number "
                     "of rows in the matrix");
    ...
    // Also, if sigma contains a non-zero imaginary part, retrieving the
    // eigenvalues becomes utterly messy (see [s/d]eupd, remark #3). We should
    // never get to the point in which the imaginary part of sigma is non-zero;
    // the user-facing functions currently convert X from real to complex if a
    // complex sigma is detected. The check here is just for extra safety, and
    // as a reminder of what's missing.
    T sigmar = real(sigma);
    T sigmai = imag(sigma);
    ...
    // if(form_val == form_sigma)
    {
      ...
      run_aupd_shiftinvert(n_eigvals, sigmar, X, false /* gen, not sym */, n,
                           tol, maxiter, resid, ncv, v, ldv, iparam, ipntr,
                           workd, workl, lworkl, rwork, info);
    } else {
      const SpMat<T> Xst = X.st();

      run_aupd_plain(n_eigvals, which, X, Xst, false /* gen, not sym */, n, tol,
                     maxiter, resid, ncv, v, ldv, iparam, ipntr, workd, workl,
                     lworkl, rwork, info);
    }
    ...
    podarray<blas_int> select(
        ...
        arma_zeros_indicator()); // logical array of dimension NCV
    podarray<T> dr(nev + 1,
                   arma_zeros_indicator()); // real array of dimension NEV + 1
    podarray<T> di(nev + 1,
                   arma_zeros_indicator()); // real array of dimension NEV + 1
    podarray<T> z(
        n * (nev + 1),
    ...
    arpack::neupd(&rvec, &howmny, select.memptr(), dr.memptr(), di.memptr(),
                  z.memptr(), &ldz, (T *)&sigmar, (T *)&sigmai, workev.memptr(),
                  &bmat, &n, which, &nev, &tol, resid.memptr(), &ncv,
                  v.memptr(), &ldv, iparam.memptr(), ipntr.memptr(),
                  workd.memptr(), workl.memptr(), &lworkl, rwork.memptr(),
                  &info);

    // Check for errors.
    if (info != 0) {
      arma_debug_warn_level(1, "eigs_gen(): ARPACK error ", info,
                            " in neupd()");
      return false;
    }
    ...
    for (uword i = 0; i < n_eigvals; ++i) {
      ...
      if ((i < n_eigvals - 1) && (eigval[i] == std::conj(eigval[i + 1]))) {
        for (uword j = 0; j < uword(n); ++j) {
          ...
          eigvec.at(j, i + 1) =
              std::complex<T>(z[n * i + j], -z[n * (i + 1) + j]);
        }
        ++i; // Skip the next one.
      } else if ((i == n_eigvals - 1) &&
                 (std::complex<T>(eigval[i]).imag() != 0.0)) {
...
inline bool sp_auxlib::eigs_gen(Col<std::complex<T>> &eigval,
  ...
  return sp_auxlib::eigs_gen<T, false>(eigval, eigvec, U.M, n_eigvals, form_val,
                                       sigma, opts);
}
...
sp_auxlib::eigs_gen(Col<std::complex<T>> &eigval, Mat<std::complex<T>> &eigvec,
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    return sp_auxlib::eigs_gen<T, true>(eigval, eigvec, U.M, n_eigvals,
                                        form_val, sigma, opts);
  }
#else
  {
...
sp_auxlib::eigs_gen(Col<std::complex<T>> &eigval, Mat<std::complex<T>> &eigvec,
...
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // if(form_val == form_sigma)
    {
      run_aupd_shiftinvert(n_eigvals, sigma, X, false /* gen, not sym */, n,
                           tol, maxiter, resid, ncv, v, ldv, iparam, ipntr,
                           workd, workl, lworkl, rwork, info);
    } else {
    ...
    podarray<std::complex<T>> z(
        n * nev,
        arma_zeros_indicator()); // complex N by NEV array if HOWMNY = 'A'
    ...
    arpack::neupd(&rvec, &howmny, select.memptr(), eigval.memptr(),
                  (std::complex<T> *)NULL, eigvec.memptr(), &ldz,
                  (std::complex<T> *)&sigma, (std::complex<T> *)NULL,
                  workev.memptr(), &bmat, &n, which, &nev, &tol, resid.memptr(),
...
#else
  {
    ...
    arma_stop_logic_error("eigs_gen(): use of ARPACK must be enabled for "
...
inline bool
sp_auxlib::spsolve_simple(Mat<typename T1::elem_type> &X,
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    X = B_expr.get_ref(); // superlu::gssv() uses X as input (the B matrix) and
                          // as output (the solution)

    if (A.is_square() == false) {
      X.soft_reset();
      arma_stop_logic_error("spsolve(): solving under-determined / "
                            "over-determined systems is currently "
                            "not supported");
      return false;
    }
    ...
    if (arma_config::debug) {
      ...
      if (overflow) {
        arma_stop_runtime_error("spsolve(): integer overflow: matrix "
                                "dimensions are too large for integer "
    ...
    superlu_array_wrangler<int> perm_c(
        A.n_cols + 1); // extra paranoia: increase array length by 1
    ...
    superlu::gssv<eT>(&options, a.get_ptr(), perm_c.get_ptr(), perm_r.get_ptr(),
                      l.get_ptr(), u.get_ptr(), x.get_ptr(), stat.get_ptr(),
                      &info);
    ...
    if ((info > 0) && (info <= int(A.n_cols))) {
      ...
      arma_debug_warn_level(
          1, "spsolve(): unknown SuperLU error code from gssv(): ", info);
    }
...
template <typename T1, typename T2>
inline bool
sp_auxlib::spsolve_refine(Mat<typename T1::elem_type> &X,
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    X.zeros(A.n_cols, B.n_cols); // set the elements to zero, as we don't trust
                                 // the SuperLU spaghetti code
    ...
    arrayops::fill_zeros(reinterpret_cast<char *>(&glu),
                         sizeof(superlu::GlobalLU_t));

    superlu::mem_usage_t mu;
    arrayops::fill_zeros(reinterpret_cast<char *>(&mu),
                         sizeof(superlu::mem_usage_t));
    ...
    superlu::gssvx<eT>(&options, a.get_ptr(), perm_c.get_ptr(),
                       perm_r.get_ptr(), etree.get_ptr(), equed, R.get_ptr(),
                       C.get_ptr(), l.get_ptr(), u.get_ptr(), &work[0], lwork,
                       b.get_ptr(), x.get_ptr(), &rpg, &rcond, ferr.get_ptr(),
                       berr.get_ptr(), &glu, &mu, stat.get_ptr(), &info);
    ...
    if ((info > 0) && (info <= int(A.n_cols))) {
      ...
      arma_debug_warn_level(
          2,
          "spsolve(): system is singular to working precision (rcond: ", rcond,
          ")");
      status = true;
      ...
      arma_debug_warn_level(
          1, "spsolve(): unknown SuperLU error code from gssvx(): ", info);
    }
...
template <typename eT>
inline typename get_pod_type<eT>::result
sp_auxlib::lu_rcond(superlu::SuperMatrix *L, superlu::SuperMatrix *U,
  ...
  superlu::gscon<eT>(&norm_id, L, U, norm_val, &rcond_out, stat.get_ptr(),
                     &info);
...
inline bool sp_auxlib::copy_to_supermatrix(superlu::SuperMatrix &out,
  ...
  out.Mtype =
      superlu::SLU_GE; // Just a general matrix.  We don't know more now.

  // We have to actually create the object which stores the data.
  // This gets cleaned by destroy_supermatrix().
  // We have to use SuperLU's problematic memory allocation routines since they
  // are not guaranteed to be new and delete.  See the comments in
  // def_superlu.hpp
  superlu::NCformat *nc =
  ...
  nc->colptr = (superlu::int_t *)superlu::malloc(sizeof(superlu::int_t) *
                                                 (A.n_cols + 1));
  nc->rowind =
      (superlu::int_t *)superlu::malloc(sizeof(superlu::int_t) * A.n_nonzero);

  if ((nc->nzval == nullptr) || (nc->colptr == nullptr) ||
      (nc->rowind == nullptr)) {
    return false;
  }
  ...
  // for(uword i = 0; i <= A.n_cols; ++i)  { nc->colptr[i] = (int_t)
  // A.col_ptrs[i]; } for(uword i = 0; i < A.n_nonzero; ++i) { nc->rowind[i] =
  // (int_t) A.row_indices[i]; }
...
// memory efficient implementation of out = A - shift*I, where A is a square
// matrix
template <typename eT>
inline bool sp_auxlib::copy_to_supermatrix_with_shift(superlu::SuperMatrix &out,
  ...
  arma_debug_check((A.is_square() == false),
                   "sp_auxlib::copy_to_supermatrix_with_shift(): given matrix "
                   "must be square sized");

  if (shift == eT(0)) {
    arma_extra_debug_print("sp_auxlib::copy_to_supermatrix_with_shift(): shift "
                           "is zero; redirecting to "
  ...
  const uword out_n_nonzero =
      A.n_nonzero - n_nonzero_diag_old + n_nonzero_diag_new;

  arma_extra_debug_print(arma_str::format("A.n_nonzero:        %u") %
                         A.n_nonzero);
  arma_extra_debug_print(arma_str::format("n_nonzero_diag_old: %u") %
                         n_nonzero_diag_old);
  arma_extra_debug_print(arma_str::format("n_nonzero_diag_new: %u") %
                         n_nonzero_diag_new);
  arma_extra_debug_print(arma_str::format("out_n_nonzero:      %u") %
                         out_n_nonzero);
  ...
  nc->colptr = (superlu::int_t *)superlu::malloc(sizeof(superlu::int_t) *
                                                 (A.n_cols + 1));
  nc->rowind =
      (superlu::int_t *)superlu::malloc(sizeof(superlu::int_t) * out_n_nonzero);
  ...
  // fill the matrix column by column, and insert diagonal elements when
  // necessary
...
//
//          if(    is_float<eT>::value)  { type_matched = (A.Dtype ==
//          superlu::SLU_S); }
//     else if(   is_double<eT>::value)  { type_matched = (A.Dtype ==
//     superlu::SLU_D); } else if( is_cx_float<eT>::value)  { type_matched =
//     (A.Dtype == superlu::SLU_C); } else if(is_cx_double<eT>::value)  {
//     type_matched = (A.Dtype == superlu::SLU_Z); }
//
//     arma_debug_check( (type_matched == false),      "copy_to_spmat(): type
//     mismatch" ); arma_debug_check( (A.Mtype != superlu::SLU_GE),
//     "copy_to_spmat(): unknown layout"
//     );
//
//     // NOTE: the l and u instances of SuperMatrix resulting from
//     superlu::gstrf()
...
//
//     if( (nc->nzval == nullptr) || (nc->colptr == nullptr) || (nc->rowind ==
//     nullptr) ) { out.reset(); return; }
//
...
//
//     arrayops::copy(access::rwp(out.values), (const eT*)(nc->nzval),
//     A_n_nonzero);
//
...
//
//     out.remove_zeros();  // in case SuperLU has bugs and stores zeros in
//     sparse matrices
//     }

template <typename eT>
inline bool sp_auxlib::wrap_to_supermatrix(superlu::SuperMatrix &out,
                                           const Mat<eT> &A) {
  arma_extra_debug_sigprint();
  ...
  superlu::DNformat *dn =
      (superlu::DNformat *)superlu::malloc(sizeof(superlu::DNformat));
...
inline void sp_auxlib::destroy_supermatrix(superlu::SuperMatrix &out) {
  ...
  if (out.Stype == superlu::SLU_NC) {
    ...
    // since dn->nzval is set to re-use memory from a Mat object (which manages
    // its own memory), we cannot simply call superlu::destroy_dense_mat(). Only
    // the out.Store structure can be freed.
...
inline void sp_auxlib::run_aupd_plain(
    ...
    podarray<blas_int> &ipntr, podarray<T> &workd, podarray<T> &workl,
    blas_int &lworkl, podarray<eT> &rwork, blas_int &info) {
#if defined(ARMA_USE_ARPACK)
  {
    ...
    // which have slighly different minimum constraint and recommended value for
    // NCV: http://www.caam.rice.edu/software/ARPACK/UG/node136.html
    ...
    // info = 0; // resid to be filled with random values by ARPACK
    // (non-deterministic)
    ...
    while (ido != 99) {
      ...
      case 1: {
        ...
        // NEW METHOD
        //
        // both operator*(rowvec, sp_mat) and operator*(sp_mat, colvec) can now
...
// Applying complex shifts to real matrices is currently not directly
// implemented
template <typename eT, typename T>
inline void sp_auxlib::run_aupd_shiftinvert(
    const uword n_eigvals, const T sigma, const SpMat<T> &X, const bool sym,
    blas_int &n, eT &tol, blas_int &maxiter, podarray<T> &resid, blas_int &ncv,
    podarray<T> &v, blas_int &ldv, podarray<blas_int> &iparam,
    podarray<blas_int> &ipntr, podarray<T> &workd, podarray<T> &workl,
    blas_int &lworkl, podarray<eT> &rwork, blas_int &info) {
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
  {
    ...
    char *which = which_lm; // NOTE: which_lm is the assumed operation when
                            // using shift-invert
    ...
    iparam(6) = 3; // Mode 3:  A * x = lambda * M * x, M symmetric
                   // semi-definite. OP = inv[A - sigma*M]*M  (A complex)  or
                   // Real_Part{ inv[A - sigma*M]*M }  (A real)  and  B = M.
    ...
    // if(true)
    //   {
    //   cout << "*** testing output of copy_to_supermatrix_with_shift()" <<
    //   endl; cout << "*** sigma: " << sigma << endl;
    //
    //   SpMat<T> Y(X);
    //   Y.diag() -= sigma;
    //
    ...
    superlu_array_wrangler<int> perm_c(
        X.n_cols + 1); // paranoia: increase array length by 1
    ...
    superlu::sp_preorder_mat(&options, x.get_ptr(), perm_c.get_ptr(),
                             etree.get_ptr(), xC.get_ptr());
    superlu::gstrf<T>(&options, xC.get_ptr(), relax, panel_size,
                      etree.get_ptr(), NULL, lwork, perm_c.get_ptr(),
                      perm_r.get_ptr(), l.get_ptr(), u.get_ptr(), &Glu,
                      stat.get_ptr(), &slu_info);
    ...
    // NOTE: potential problem with inconsistent/mismatched use of eT and T
    // types
    ...
    while (ido != 99) {
      ...
      case 1: {
        ...
        superlu::gstrs<T>(trans, l.get_ptr(), u.get_ptr(), perm_c.get_ptr(),
                          perm_r.get_ptr(), out_slu.get_ptr(), stat.get_ptr(),
                          &info);
    ...
    if ((info != 0) && (info != 1)) {
      ...
      if (sym) {
        arma_debug_warn_level(2, "eigs_sym(): ARPACK error ", info,
                              " in saupd()");
      } else {
        arma_debug_warn_level(2, "eigs_gen(): ARPACK error ", info,
                              " in naupd()");
      }
...
template <typename eT> inline eigs_randu_filler<eT>::eigs_randu_filler() {
  arma_extra_debug_sigprint();
  ...
  typedef
      typename std::uniform_real_distribution<eT>::param_type local_param_type;
...
inline eigs_randu_filler<std::complex<T>>::eigs_randu_filler() {
  ...
  typedef
      typename std::uniform_real_distribution<T>::param_type local_param_type;
...
template <typename T>
inline void
eigs_randu_filler<std::complex<T>>::fill(podarray<std::complex<T>> &X,
                                         const uword N) {
  arma_extra_debug_sigprint();
...
inline superlu_supermatrix_wrangler::superlu_supermatrix_wrangler() {
  ...
  arrayops::fill_zeros(reinterpret_cast<char *>(&m),
                       sizeof(superlu::SuperMatrix));
}
...
inline superlu_stat_wrangler::superlu_stat_wrangler() {
  ...
  arrayops::fill_zeros(reinterpret_cast<char *>(&stat),
                       sizeof(superlu::SuperLUStat_t));

  superlu::init_stat(&stat);
}

inline superlu::SuperLUStat_t *superlu_stat_wrangler::get_ptr() {
  return &stat;
}
...
template <typename eT> inline void superlu_array_wrangler<eT>::reset() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT *superlu_array_wrangler<eT>::get_ptr() {
  return mem;
}

//

template <typename eT> inline superlu_worker<eT>::~superlu_worker() {
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline bool
superlu_worker<eT>::factorise(typename get_pod_type<eT>::result &out_rcond,
                              const SpMat<eT> &A,
                              const superlu_opts &user_opts) {
  arma_extra_debug_sigprint();
  ...
  superlu::sp_preorder_mat(&options, AA.get_ptr(), perm_c.get_ptr(),
                           etree.get_ptr(), AAc.get_ptr());

  arma_extra_debug_print("superlu::gstrf()");
  superlu::gstrf<eT>(&options, AAc.get_ptr(), relax, panel_size,
                     etree.get_ptr(), NULL, lwork, perm_c.get_ptr(),
                     perm_r.get_ptr(), l_ref.get_ptr(), u_ref.get_ptr(), &Glu,
                     stat.get_ptr(), &info);

  if (info != 0) {
    arma_debug_warn_level(
        3, "superlu_worker()::factorise(): LU factorisation failed");
    return false;
  }

  const T AA_norm = sp_auxlib::norm1<T>(AA.get_ptr());
  const T AA_rcond =
      sp_auxlib::lu_rcond<eT>(l_ref.get_ptr(), u_ref.get_ptr(), AA_norm);
