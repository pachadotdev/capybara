...
template <typename cx_type, bool inverse> class fft_engine_fftw3 {
public:
  ...
  podarray<cx_type>
      X_work; // for storing copy of input (can be overwritten by FFTW3)
  ...
  inline ~fft_engine_fftw3() {
    ...
    // fftw3::cleanup<cx_type>();  // NOTE: this also removes any wisdom
    // acquired by FFTW3
  }
  ...
  inline fft_engine_fftw3(const uword in_N) : N(in_N), fftw3_plan(nullptr) {
    ...
    if (N > uword(std::numeric_limits<int>::max())) {
      arma_stop_runtime_error("integer overflow: FFT size too large for "
                              "integer type used by FFTW3");
    }

    arma_extra_debug_print(
        "fft_engine_fftw3::constructor: allocating work arrays");
    X_work.set_size(N);
    Y_work.set_size(N);
    ...
    fftw3_plan = fftw3::plan_dft_1d<cx_type>(
        N, X_work.memptr(), Y_work.memptr(), fftw3_sign, fftw3_flags);

    if (fftw3_plan == nullptr) {
      arma_stop_runtime_error(
          "fft_engine_fftw3::constructor: failed to create plan");
    }
  }
