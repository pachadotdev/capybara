...
inline void op_sum::apply(Mat<typename T1::elem_type> &out,
...
template <typename T1>
inline void op_sum::apply_noalias(Mat<typename T1::elem_type> &out,
...
inline void op_sum::apply_noalias_unwrap(Cube<typename T1::elem_type> &out,
  ...
  if (dim == 0) {
    ...
    for (uword slice = 0; slice < X_n_slices; ++slice) {
      ...
      for (uword col = 0; col < X_n_cols; ++col) {
        out_mem[col] =
            arrayops::accumulate(X.slice_colptr(slice, col), X_n_rows);
      }
    }
  } else if (dim == 1) {
