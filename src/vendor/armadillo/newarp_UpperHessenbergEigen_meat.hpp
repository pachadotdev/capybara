...
inline void UpperHessenbergEigen<eT>::compute(const Mat<eT> &mat_obj) {
  ...
  lapack::lahqr(&want_T, &want_Z, &n, &ilo, &ihi, mat_T.memptr(), &n,
                wr.memptr(), wi.memptr(), &iloz, &ihiz, mat_Z.memptr(), &n,
                &info);

  if (info != 0) {
    arma_stop_runtime_error(
        "lapack::lahqr(): failed to compute all eigenvalues");
    return;
  }
  ...
  lapack::trevc(&side, &howmny, (blas_int *)NULL, &n, mat_T.memptr(), &n,
                (eT *)NULL, &n, mat_Z.memptr(), &n, &n, &m, work.memptr(),
                &info);
...
inline Col<std::complex<eT>> UpperHessenbergEigen<eT>::eigenvalues() {
  ...
  arma_debug_check((computed == false),
                   "newarp::UpperHessenbergEigen::eigenvalues(): need to call "
                   "compute() first");
...
inline Mat<std::complex<eT>> UpperHessenbergEigen<eT>::eigenvectors() {
  ...
  arma_debug_check((computed == false),
                   "newarp::UpperHessenbergEigen::eigenvectors(): need to call "
                   "compute() first");
  ...
  for (uword i = 0; i < n_rows; i++) {
    if (cx_attrib::is_real(evals(i), eT(0))) {
      ...
      for (uword j = 0; j < n_rows; j++) {
        col_ptr[j] =
            std::complex<eT>(z_ptr[j] / z_norm, z_ptr[j + n_rows] / z_norm);
