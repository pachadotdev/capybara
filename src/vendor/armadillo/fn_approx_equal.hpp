...
arma_inline bool
internal_approx_equal_abs_diff(const eT &x, const eT &y,
  ...
  if (x != y) {
                           ...
                           // std::complex<double>
    {
      ...
      if (eop_aux::arma_abs(
              (cond_rel<is_cx<eT>::no>::gt(x, y)) ? (x - y) : (y - x)) > tol) {
        return false;
      }
    }
  }
...
template <typename eT>
arma_inline bool
internal_approx_equal_rel_diff(const eT &a, const eT &b,
                               const typename get_pod_type<eT>::result tol) {
  typedef typename get_pod_type<eT>::result T;
  ...
  if (a != b) {
                           ...
                           // std::complex<double>
    {
      ...
      const T abs_d = eop_aux::arma_abs(
          (cond_rel<is_cx<eT>::no>::gt(a, b)) ? (a - b) : (b - a));
...
inline bool
internal_approx_equal_worker(const Base<typename T1::elem_type, T1> &A,
  ...
  arma_debug_check(((use_abs_diff == false) && (use_rel_diff == false)),
                   "internal_approx_equal_worker(): both 'use_abs_diff' and "
                   "'use_rel_diff' are false");
  ...
  if ((PA.get_n_rows() != PB.get_n_rows()) ||
  ...
  if ((Proxy<T1>::use_at == false) && (Proxy<T2>::use_at == false)) {
    ...
    for (uword col = 0; col < n_cols; ++col)
      for (uword row = 0; row < n_rows; ++row) {
        ...
        const bool state_abs_diff =
            (use_abs_diff) ? internal_approx_equal_abs_diff(x, y, abs_tol)
                           : true;
        const bool state_rel_diff =
            (use_rel_diff) ? internal_approx_equal_rel_diff(x, y, rel_tol)
                           : true;
...
template <bool use_abs_diff, bool use_rel_diff, typename T1, typename T2>
inline bool
internal_approx_equal_worker(const BaseCube<typename T1::elem_type, T1> &A,
...
inline bool internal_approx_equal_handler(const T1 &A, const T2 &B,
                                          const char *method,
                                          const typename T1::pod_type abs_tol,
                                          const typename T1::pod_type rel_tol) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check(((sig != 'a') && (sig != 'r') && (sig != 'b')),
                   "approx_equal(): argument 'method' must be \"absdiff\" or "
                   "\"reldiff\" or \"both\"");
...
approx_equal(const Base<typename T1::elem_type, T1> &A,
             const Base<typename T1::elem_type, T2> &B, const char *method,
             const typename T1::pod_type tol) {
  arma_extra_debug_sigprint();
...
template <typename T1, typename T2>
arma_warn_unused inline bool
approx_equal(const BaseCube<typename T1::elem_type, T1> &A,
             const BaseCube<typename T1::elem_type, T2> &B, const char *method,
             const typename T1::pod_type tol) {
  arma_extra_debug_sigprint();
...
approx_equal(const Base<typename T1::elem_type, T1> &A,
  ...
  return internal_approx_equal_handler(A.get_ref(), B.get_ref(), method,
                                       abs_tol, rel_tol);
}
...
template <typename T1, typename T2>
arma_warn_unused inline bool
approx_equal(const SpBase<typename T1::elem_type, T1> &A,
             const SpBase<typename T1::elem_type, T2> &B, const char *method,
             const typename T1::pod_type tol) {
  arma_extra_debug_sigprint();
