...
inline std::mt19937_64 &arma_rng::get_producer() {
  ...
  static thread_local std::mt19937_64 mt19937_64_producer(
      std::mt19937_64::default_seed + mt19937_64_producer_counter++);
...
#elif defined(ARMA_USE_CXX11_RNG)
    {
      constexpr double scale = double(std::numeric_limits<int>::max()) /
                               double(std::mt19937_64::max());
...
template <typename eT> struct arma_rng::randu {
  ...
  inline static void fill(eT *mem, const uword N, const double a,
                          const double b) {
#if defined(ARMA_RNG_ALT)
    {
      const double r = b - a;
...
template <typename eT> struct arma_rng::randn {
...
#if defined(ARMA_RNG_ALT)
    {
      // NOTE: old method to avoid regressions in user code that assumes
      // specific sequence
  ...
  inline static void fill(eT *mem, const uword N, const double mu,
                          const double sd) {
#if defined(ARMA_RNG_ALT)
    {
      // NOTE: old method to avoid regressions in user code that assumes
...
template <typename T> struct arma_rng::randn<std::complex<T>> {
  ...
  inline static void fill(std::complex<T> *mem, const uword N, const double mu,
    ...
    for (uword i = 0; i < N; ++i) {
      ...
      mem[i] =
          std::complex<T>(((val.real() * sd) + mu), ((val.imag() * sd) + mu));
    }
  }
};
...
template <typename eT> struct arma_rng::randg {
  inline static void fill(eT *mem, const uword N, const double a,
                          const double b) {
#if defined(ARMA_USE_CXX11_RNG)
    {
