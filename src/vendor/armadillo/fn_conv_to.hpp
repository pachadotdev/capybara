...
//! NOTE: use as_scalar() instead; this functionality is kept only for
//! compatibility with old user code
template <typename out_eT> class conv_to {
public:
  template <typename in_eT, typename T1>
  inline static out_eT
...
conv_to<out_eT>::from(const Base<in_eT, T1> &in,
                      const typename arma_not_cx<in_eT>::result *junk) {
...
arma_warn_unused inline out_eT
conv_to<out_eT>::from(const Base<in_eT, T1> &in,
                      const typename arma_cx_only<in_eT>::result *junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
conv_to<out_eT>::from(const BaseCube<in_eT, T1> &in,
  ...
  arrayops::convert_cx_scalar(out,
                              (ProxyCube<T1>::use_at ? P.at(0, 0, 0) : P[0]));

  return out;
}

//! conversion to Armadillo matrices from Armadillo Base objects, as well as
//! from std::vector
template <typename out_eT> class conv_to<Mat<out_eT>> {
public:
  template <typename in_eT, typename T1>
  inline static Mat<out_eT>
  from(const Base<in_eT, T1> &in,
       const typename arma_not_cx<in_eT>::result *junk = nullptr);
  ...
  inline static Mat<out_eT>
  from(const std::vector<in_eT> &in,
       const typename arma_not_cx<in_eT>::result *junk = nullptr);
...
template <typename in_eT>
arma_warn_unused inline Mat<out_eT>
conv_to<Mat<out_eT>>::from(const std::vector<in_eT> &in,
                           const typename arma_not_cx<in_eT>::result *junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
//! conversion to Armadillo row vectors from Armadillo Base objects, as well as
//! from std::vector
template <typename out_eT> class conv_to<Row<out_eT>> {
public:
  template <typename in_eT, typename T1>
  inline static Row<out_eT>
  from(const Base<in_eT, T1> &in,
       const typename arma_not_cx<in_eT>::result *junk = nullptr);
...
template <typename in_eT>
arma_warn_unused inline Row<out_eT>
conv_to<Row<out_eT>>::from(const std::vector<in_eT> &in,
                           const typename arma_not_cx<in_eT>::result *junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
//! conversion to Armadillo column vectors from Armadillo Base objects, as well
//! as from std::vector
template <typename out_eT> class conv_to<Col<out_eT>> {
public:
  template <typename in_eT, typename T1>
  inline static Col<out_eT>
  from(const Base<in_eT, T1> &in,
       const typename arma_not_cx<in_eT>::result *junk = nullptr);
...
template <typename in_eT>
arma_warn_unused inline Col<out_eT>
conv_to<Col<out_eT>>::from(const std::vector<in_eT> &in,
                           const typename arma_not_cx<in_eT>::result *junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
public:
  template <typename in_eT, typename T1>
  inline static SpMat<out_eT>
  from(const SpBase<in_eT, T1> &in,
       const typename arma_not_cx<in_eT>::result *junk = nullptr);
...
conv_to<SpMat<out_eT>>::from(const SpBase<in_eT, T1> &in,
                             const typename arma_not_cx<in_eT>::result *junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
public:
  template <typename in_eT, typename T1>
  inline static std::vector<out_eT>
  from(const Base<in_eT, T1> &in,
       const typename arma_not_cx<in_eT>::result *junk = nullptr);
