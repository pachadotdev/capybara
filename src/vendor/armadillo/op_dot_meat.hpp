...
op_dot::direct_dot_arma(const uword n_elem, const eT *const A,
                        const eT *const B) {
...
template <typename eT>
inline typename arma_cx_only<eT>::result
op_dot::direct_dot_arma(const uword n_elem, const eT *const A,
                        const eT *const B) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline typename arma_real_only<eT>::result
...
template <typename eT>
inline typename arma_integral_only<eT>::result
op_dot::direct_dot(const uword n_elem, const eT *const A, const eT *const B) {
  return op_dot::direct_dot_arma(n_elem, A, B);
}
...
//! for three arrays
template <typename eT>
inline eT op_dot::direct_dot(const uword n_elem, const eT *const A,
  ...
  if (use_at || have_direct_mem) {
    ...
    if (is_subview_row<T1>::value && is_subview_row<T2>::value) {
      ...
      if ((A.m.n_rows == 1) && (B.m.n_rows == 1)) {
        ...
        return op_dot::direct_dot(A.n_elem, &A_mem[A.aux_col1],
                                  &B_mem[B.aux_col1]);
      }
    }
...
template <typename eT>
inline eT op_cdot::direct_cdot(const uword n_elem, const eT *const A,
                               const eT *const B) {
  arma_extra_debug_sigprint();
  ...
  if (n_elem <= 32u) {
...
#if defined(ARMA_USE_BLAS)
    {
      ...
      // using gemv() workaround due to compatibility issues with cdotc() and
      // zdotc()
      ...
      // blas::gemv(&trans, &m, &n, &alpha, A, &lda, B, &inc, &beta, &result[0],
      // &inc);
...
template <typename T1, typename T2>
inline typename promote_type<typename T1::elem_type,
