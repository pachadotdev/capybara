...
inline Cube<eT>::Cube()
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), n_alloc(0),
      mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
}
...
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols,
    ...
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem_slice(in_n_rows * in_n_cols),
      n_slices(in_n_slices), n_elem(in_n_rows * in_n_cols * in_n_slices),
      n_alloc(), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
inline Cube<eT>::Cube(const SizeCube &s)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem_slice(s.n_rows * s.n_cols),
      n_slices(s.n_slices), n_elem(s.n_rows * s.n_cols * s.n_slices), n_alloc(),
      mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! construct the cube to have user specified dimensions and fill with specified
//! pattern
template <typename eT>
...
inline Cube<eT>::Cube(Cube<eT> &&in_cube)
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), n_alloc(0),
      mem_state(0), mem() {
  arma_extra_debug_sigprint(arma_str::format("this = %x   in_cube = %x") %
                            this % &in_cube);
...
template <typename eT> inline void Cube<eT>::init_cold() {
  arma_extra_debug_sigprint(
      arma_str::format("n_rows = %u, n_cols = %u, n_slices = %u") % n_rows %
      n_cols % n_slices);
...
inline void Cube<eT>::init_warm(const uword in_n_rows, const uword in_n_cols,
                                const uword in_n_slices) {
  arma_extra_debug_sigprint(
      arma_str::format("in_n_rows = %u, in_n_cols = %u, in_n_slices = %u") %
  ...
  arma_debug_check((t_mem_state == 2), "Cube::init(): mismatch between size of "
    ...
    for (uword uslice = 0; uslice < local_n_slices; ++uslice)
      for (uword ucol = 0; ucol < local_n_cols; ++ucol)
        for (uword urow = 0; urow < local_n_rows; ++urow) {
          *out_mem = std::complex<T>(PX.at(urow, ucol, uslice),
                                     PY.at(urow, ucol, uslice));
          out_mem++;
        }
  }
}

template <typename eT> inline void Cube<eT>::delete_mat() {
  arma_extra_debug_sigprint();

  if ((n_slices > 0) && (mat_ptrs != nullptr)) {
    for (uword s = 0; s < n_slices; ++s) {
      raw_mat_ptr_type mat_ptr =
          raw_mat_ptr_type(mat_ptrs[s]); // explicit cast to indicate load from
                                         // std::atomic<Mat<eT>*>
...
template <typename eT> inline void Cube<eT>::create_mat() {
  arma_extra_debug_sigprint();
  ...
  if (mem_state <= 2) {
    if (n_slices <= Cube_prealloc::mat_ptrs_size) {
      arma_extra_debug_print(
          "Cube::create_mat(): using local memory for mat_ptrs array");
      ...
      arma_check_bad_alloc((mat_ptrs == nullptr),
                           "Cube::create_mat(): out of memory");
    }
  }
...
inline Mat<eT> *Cube<eT>::create_mat_ptr(const uword in_slice) const {
  ...
  arma_extra_debug_print(
      arma_str::format("Cube::create_mat_ptr(): creating matrix %u") %
      in_slice);
...
arma_check_bad_alloc((mat_ptr == nullptr),
                     "Cube::get_mat_ptr(): out of memory");
}
...
template <typename eT> inline Cube<eT> &Cube<eT>::operator=(const eT val) {
  arma_extra_debug_sigprint();
...
inline Cube<eT>::Cube(const Cube<eT> &x)
    : n_rows(x.n_rows), n_cols(x.n_cols), n_elem_slice(x.n_elem_slice),
      n_slices(x.n_slices), n_elem(x.n_elem), n_alloc(), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
  arma_extra_debug_sigprint(arma_str::format("this = %x   in_cube = %x") %
                            this % &x);
...
//! construct a cube from a given cube
template <typename eT> inline Cube<eT> &Cube<eT>::operator=(const Cube<eT> &x) {
  arma_extra_debug_sigprint(arma_str::format("this = %x   in_cube = %x") %
                            this % &x);
...
template <typename eT>
inline Cube<eT>::Cube(eT *aux_mem, const uword aux_n_rows,
                      const uword aux_n_cols, const uword aux_n_slices,
                      const bool copy_aux_mem, const bool strict,
                      const bool prealloc_mat)
    : n_rows(aux_n_rows), n_cols(aux_n_cols),
      n_elem_slice(aux_n_rows * aux_n_cols), n_slices(aux_n_slices),
      n_elem(aux_n_rows * aux_n_cols * aux_n_slices), n_alloc(0),
...
//! the array is copied.
template <typename eT>
inline Cube<eT>::Cube(const eT *aux_mem, const uword aux_n_rows,
                      const uword aux_n_cols, const uword aux_n_slices)
    : n_rows(aux_n_rows), n_cols(aux_n_cols),
      n_elem_slice(aux_n_rows * aux_n_cols), n_slices(aux_n_slices),
      n_elem(aux_n_rows * aux_n_cols * aux_n_slices), n_alloc(), mem_state(0),
      mem() {
  arma_extra_debug_sigprint_this(this);
...
//! construct a cube from a subview_cube instance (eg. construct a cube from a
//! delayed subcube operation)
template <typename eT>
...
//! in-place element-wise cube mutiplication (using a subcube on the
//! right-hand-side)
template <typename eT>
inline Cube<eT> &Cube<eT>::operator%=(const subview_cube<eT> &X) {
  arma_extra_debug_sigprint();
...
arma_inline subview_cube<eT> Cube<eT>::col(const uword in_col) {
  ...
  arma_debug_check_bounds((in_col >= n_cols),
                          "Cube::col(): index out of bounds");
...
template <typename eT> inline Mat<eT> &Cube<eT>::slice(const uword in_slice) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds((in_slice >= n_slices),
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::rows(const uword in_row1,
                                            const uword in_row2) {
  arma_extra_debug_sigprint();
  ...
  return subview_cube<eT>(*this, in_row1, 0, 0, subcube_n_rows, n_cols,
                          n_slices);
}
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::cols(const uword in_col1,
                                            const uword in_col2) {
  arma_extra_debug_sigprint();
  ...
  return subview_cube<eT>(*this, 0, in_col1, 0, n_rows, subcube_n_cols,
                          n_slices);
}
...
arma_inline subview_cube<eT> Cube<eT>::slices(const uword in_slice1,
  ...
  return subview_cube<eT>(*this, 0, 0, in_slice1, n_rows, n_cols,
                          subcube_n_slices);
}
...
template <typename eT>
arma_inline subview_cube<eT>
Cube<eT>::subcube(const uword in_row1, const uword in_col1,
...
Cube<eT>::subcube(const uword in_row1, const uword in_col1,
  ...
  return subview_cube<eT>(*this, in_row1, in_col1, in_slice1, s_n_rows,
                          s_n_cols, s_n_slices);
}
...
//! creation of subview_cube (generic subcube)
template <typename eT>
inline subview_cube<eT> Cube<eT>::subcube(const span &row_span,
                                          const span &col_span,
  ...
  const uword subcube_n_slices =
      slice_all ? local_n_slices : in_slice2 - in_slice1 + 1;
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::operator()(const span &row_span,
                                             const span &col_span,
                                             const span &slice_span) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline const subview_cube<eT>
Cube<eT>::operator()(const span &row_span, const span &col_span,
                     const span &slice_span) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline subview_cube<eT>
Cube<eT>::operator()(const uword in_row1, const uword in_col1,
                     const uword in_slice1, const SizeCube &s) {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline subview_cube<eT> Cube<eT>::tube(const uword in_row1,
                                            const uword in_col1) {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline subview_cube<eT>
Cube<eT>::tube(const uword in_row1, const uword in_col1, const uword in_row2,
               const uword in_col2) {
  arma_extra_debug_sigprint();
  ...
  return subview_cube<eT>(*this, in_row1, in_col1, 0, subcube_n_rows,
                          subcube_n_cols, n_slices);
}
...
Cube<eT>::tube(const uword in_row1, const uword in_col1, const SizeMat &s) {
  ...
  return subview_cube<eT>(*this, in_row1, in_col1, 0, s_n_rows, s_n_cols,
                          n_slices);
}
...
template <typename eT>
inline subview_cube<eT> Cube<eT>::tube(const span &row_span,
                                       const span &col_span) {
  arma_extra_debug_sigprint();
...
inline subview_cube<eT> Cube<eT>::head_slices(const uword N) {
  ...
  arma_debug_check_bounds((N > n_slices),
                          "Cube::head_slices(): size out of bounds");
...
inline subview_cube<eT> Cube<eT>::tail_slices(const uword N) {
  ...
  arma_debug_check_bounds((N > n_slices),
                          "Cube::tail_slices(): size out of bounds");
...
//! apply a lambda function to each slice, where each slice is interpreted as a
//! matrix
template <typename eT>
...
template <typename eT>
inline const Cube<eT> &
Cube<eT>::each_slice(const std::function<void(const Mat<eT> &)> &F,
...
template <typename eT>
template <typename T1>
inline subview_cube_slices<eT, T1>
...
template <typename eT>
template <typename T1>
inline const subview_cube_slices<eT, T1>
...
template <typename eT> inline void Cube<eT>::shed_row(const uword row_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(row_num >= n_rows,
                          "Cube::shed_row(): index out of bounds");

  shed_rows(row_num, row_num);
}

//! remove specified column
template <typename eT> inline void Cube<eT>::shed_col(const uword col_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(col_num >= n_cols,
                          "Cube::shed_col(): index out of bounds");

  shed_cols(col_num, col_num);
}

//! remove specified slice
template <typename eT> inline void Cube<eT>::shed_slice(const uword slice_num) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void Cube<eT>::shed_slices(const uword in_slice1,
                                  const uword in_slice2) {
  arma_extra_debug_sigprint();
...
inline void Cube<eT>::insert_slices(const uword slice_num, const uword N) {
  ...
  if (B_n_slices > 0) {
    out.slices(slice_num + N, t_n_slices + N - 1) =
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_rows(const uword row_num,
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_cols(const uword col_num,
                                  const BaseCube<eT, T1> &X) {
  arma_extra_debug_sigprint();
...
template <typename eT>
template <typename T1>
inline void Cube<eT>::insert_slices(const uword slice_num,
                                    const BaseCube<eT, T1> &X) {
  arma_extra_debug_sigprint();
...
inline void Cube<eT>::insert_slices(const uword slice_num,
  ...
  const Cube<eT> C(const_cast<eT *>(U.M.memptr()), U.M.n_rows, U.M.n_cols,
                   uword(1), false, true);

  (*this).insert_slices(slice_num, C);
}

//! create a cube from GenCube, ie. run the previously delayed element
//! generation operations
template <typename eT>
template <typename gen_type>
inline Cube<eT>::Cube(const GenCube<eT, gen_type> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem_slice(X.n_rows * X.n_cols),
      n_slices(X.n_slices), n_elem(X.n_rows * X.n_cols * X.n_slices), n_alloc(),
      mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! in-place cube subtraction, with the right-hand-side operand having delayed
//! operations
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT> &Cube<eT>::operator-=(const OpCube<T1, op_type> &X) {
...
//! in-place cube element-wise multiplication, with the right-hand-side operand
...
//! in-place cube element-wise division, with the right-hand-side operand having
//! delayed operations
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT> &Cube<eT>::operator/=(const OpCube<T1, op_type> &X) {
...
inline Cube<eT>::Cube(const eOpCube<T1, eop_type> &X)
    : n_rows(X.get_n_rows()), n_cols(X.get_n_cols()),
      n_elem_slice(X.get_n_elem_slice()), n_slices(X.get_n_slices()),
      n_elem(X.get_n_elem()), n_alloc(), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! create a cube from GlueCube, ie. run the previously delayed binary
//! operations
template <typename eT>
...
//! in-place cube addition, with the right-hand-side operands having delayed
//! operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT> &Cube<eT>::operator+=(const GlueCube<T1, T2, glue_type> &X) {
...
//! in-place cube subtraction, with the right-hand-side operands having delayed
//! operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT> &Cube<eT>::operator-=(const GlueCube<T1, T2, glue_type> &X) {
...
//! in-place cube element-wise multiplication, with the right-hand-side operands
//! having delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT> &Cube<eT>::operator%=(const GlueCube<T1, T2, glue_type> &X) {
...
//! create a cube from eGlueCube, ie. run the previously delayed binary
//! operations
template <typename eT>
...
//! linear element accessor (treats the cube as a vector); bounds checking not
//! done when ARMA_NO_DEBUG is defined
template <typename eT> arma_inline eT &Cube<eT>::operator()(const uword i) {
...
//! linear element accessor (treats the cube as a vector); no bounds check.
template <typename eT> arma_inline eT &Cube<eT>::at(const uword i) {
  return access::rw(mem[i]);
}

//! linear element accessor (treats the cube as a vector); no bounds check
template <typename eT> arma_inline const eT &Cube<eT>::at(const uword i) const {
  return mem[i];
}
...
//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline const eT &Cube<eT>::operator()(const uword in_row,
                                           const uword in_col,
...
//! prefix ++
template <typename eT> arma_inline const Cube<eT> &Cube<eT>::operator++() {
...
template <typename eT> arma_inline void Cube<eT>::operator++(int) {
...
template <typename eT> arma_inline bool Cube<eT>::is_empty() const {
  return (n_elem == 0);
}

template <typename eT> inline bool Cube<eT>::internal_is_finite() const {
  arma_extra_debug_sigprint();

  return arrayops::is_finite(memptr(), n_elem);
}

template <typename eT> inline bool Cube<eT>::internal_has_inf() const {
  arma_extra_debug_sigprint();

  return arrayops::has_inf(memptr(), n_elem);
}

template <typename eT> inline bool Cube<eT>::internal_has_nan() const {
  arma_extra_debug_sigprint();

  return arrayops::has_nan(memptr(), n_elem);
}

template <typename eT> inline bool Cube<eT>::internal_has_nonfinite() const {
  arma_extra_debug_sigprint();
...
inline bool Cube<eT>::in_range(const uword in_row, const uword in_col,
  ...
  if ((in_row >= l_n_rows) || (in_col >= l_n_cols) ||
      (in_slice >= l_n_slices) || ((in_row + s.n_rows) > l_n_rows) ||
...
template <typename eT> arma_inline eT *Cube<eT>::memptr() {
  return const_cast<eT *>(mem);
}

//! returns a pointer to array of eTs used by the cube
template <typename eT> arma_inline const eT *Cube<eT>::memptr() const {
  return mem;
}
...
//! returns a pointer to array of eTs used by the specified slice in the cube
template <typename eT>
arma_inline const eT *Cube<eT>::slice_colptr(const uword uslice,
...
template <typename eT>
inline Cube<eT> &Cube<eT>::set_size(const uword new_n_rows,
...
template <typename eT>
inline Cube<eT> &Cube<eT>::reshape(const uword new_n_rows,
                                   const uword new_n_cols,
                                   const uword new_n_slices) {
  arma_extra_debug_sigprint();
...
//! change the cube to have user specified dimensions (data is preserved)
template <typename eT>
inline Cube<eT> &Cube<eT>::resize(const uword new_n_rows,
                                  const uword new_n_cols,
                                  const uword new_n_slices) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::set_size(const SizeCube &s) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::reshape(const SizeCube &s) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::resize(const SizeCube &s) {
  arma_extra_debug_sigprint();
...
//! change the cube (without preserving data) to have the same dimensions as the
//! given cube
template <typename eT>
...
template <typename eT> inline Cube<eT> &Cube<eT>::fill(const eT val) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::zeros() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::zeros(const SizeCube &s) {
  arma_extra_debug_sigprint();

  return (*this).zeros(s.n_rows, s.n_cols, s.n_slices);
}

template <typename eT> inline Cube<eT> &Cube<eT>::ones() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::ones(const SizeCube &s) {
  arma_extra_debug_sigprint();

  return (*this).ones(s.n_rows, s.n_cols, s.n_slices);
}

template <typename eT> inline Cube<eT> &Cube<eT>::randu() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::randu(const SizeCube &s) {
  arma_extra_debug_sigprint();

  return (*this).randu(s.n_rows, s.n_cols, s.n_slices);
}

template <typename eT> inline Cube<eT> &Cube<eT>::randn() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Cube<eT> &Cube<eT>::randn(const SizeCube &s) {
  arma_extra_debug_sigprint();

  return (*this).randn(s.n_rows, s.n_cols, s.n_slices);
}

template <typename eT> inline void Cube<eT>::reset() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Cube<eT>::min() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Cube<eT>::max() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Cube<eT>::min(uword &index_of_min_val) const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Cube<eT>::max(uword &index_of_max_val) const {
  arma_extra_debug_sigprint();
...
inline bool Cube<eT>::save(const hdf5_name &spec, const file_type type) const {
  ...
  if ((type != hdf5_binary) && (type != hdf5_binary_trans)) {
    arma_stop_runtime_error(
        "Cube::save(): unsupported file type for hdf5_name()");
    return false;
  }
  ...
  if (save_okay == false) {
    if (err_msg.length() > 0) {
      arma_debug_warn_level(3, "Cube::save(): ", err_msg,
                            "; file: ", spec.filename);
    } else {
      arma_debug_warn_level(
          3, "Cube::save(): write failed; file: ", spec.filename);
    }
  }
...
inline bool Cube<eT>::load(const hdf5_name &spec, const file_type type) {
  ...
  if ((type != hdf5_binary) && (type != hdf5_binary_trans)) {
    arma_stop_runtime_error(
        "Cube::load(): unsupported file type for hdf5_name()");
    return false;
  }
  ...
  if (load_okay == false) {
    ...
    if (err_msg.length() > 0) {
      ...
      arma_debug_warn_level(3,
                            "Cube::load(): read failed; file: ", spec.filename);
    }
  }
...
template <typename eT>
inline bool Cube<eT>::quiet_save(const std::string name,
...
template <typename eT>
inline bool Cube<eT>::quiet_save(const hdf5_name &spec,
                                 const file_type type) const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline typename Cube<eT>::iterator Cube<eT>::begin() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline typename Cube<eT>::iterator Cube<eT>::end() {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline typename Cube<eT>::slice_iterator
...
template <typename eT>
inline typename Cube<eT>::const_slice_iterator
...
template <typename eT> inline void Cube<eT>::clear() { reset(); }

//! returns true if the cube has no elements
template <typename eT> inline bool Cube<eT>::empty() const {
  return (n_elem == 0);
}

//! returns the number of elements in this cube
template <typename eT> inline uword Cube<eT>::size() const { return n_elem; }

template <typename eT> inline eT &Cube<eT>::front() {
  arma_debug_check((n_elem == 0), "Cube::front(): cube is empty");

  return access::rw(mem[0]);
}

template <typename eT> inline const eT &Cube<eT>::front() const {
  arma_debug_check((n_elem == 0), "Cube::front(): cube is empty");

  return mem[0];
}

template <typename eT> inline eT &Cube<eT>::back() {
  arma_debug_check((n_elem == 0), "Cube::back(): cube is empty");

  return access::rw(mem[n_elem - 1]);
}

template <typename eT> inline const eT &Cube<eT>::back() const {
  arma_debug_check((n_elem == 0), "Cube::back(): cube is empty");

  return mem[n_elem - 1];
}

template <typename eT> inline void Cube<eT>::swap(Cube<eT> &B) {
  Cube<eT> &A = (*this);
...
template <typename eT> inline void Cube<eT>::steal_mem(Cube<eT> &x) {
  arma_extra_debug_sigprint();
...
inline void Cube<eT>::steal_mem(Cube<eT> &x, const bool is_move) {
  ...
  if ((mem_state <= 1) &&
      ((x.n_alloc > Cube_prealloc::mem_n_elem) || (x.mem_state == 1) ||
       (is_move && (x.mem_state == 2)))) {
    ...
    if (x_n_slices > Cube_prealloc::mat_ptrs_size) {
      ...
      for (uword i = 0; i < x_n_slices; ++i) {
        mat_ptrs[i] =
            raw_mat_ptr_type(x.mat_ptrs[i]); // cast required by std::atomic
    ...
    if ((is_move) && (x.mem_state == 0) &&
        (x.n_alloc <= Cube_prealloc::mem_n_elem)) {
      x.reset();
    }
  }
}
...
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::mem_setup() {
  ...
  if (fixed_n_elem > 0) {
    ...
    access::rw(Cube<eT>::mem) = (fixed_n_elem > Cube_prealloc::mem_n_elem)
                                    ? mem_local_extra
                                    : mem_local;
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
arma_inline const eT &
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::at(
    const uword i) const {
  return (use_extra) ? mem_local_extra[i] : mem_local[i];
}
...
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator()(
    const uword i) {
  arma_debug_check_bounds((i >= fixed_n_elem),
                          "Cube::operator(): index out of bounds");
...
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator[](
  ...
  const uword i =
      in_slice * fixed_n_elem_slice + in_col * fixed_n_rows + in_row;
...
Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::operator()(
    const uword in_row, const uword in_col, const uword in_slice) {
  arma_debug_check_bounds((in_row >= fixed_n_rows) ||
                              (in_col >= fixed_n_cols) ||
...
//! prefix ++
template <typename eT> arma_inline void Cube_aux::prefix_pp(Cube<eT> &x) {
  eT *memptr = x.memptr();
...
//! postfix ++
template <typename eT> arma_inline void Cube_aux::postfix_pp(Cube<eT> &x) {
  eT *memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! prefix --
template <typename eT> arma_inline void Cube_aux::prefix_mm(Cube<eT> &x) {
  eT *memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! postfix --
template <typename eT> arma_inline void Cube_aux::postfix_mm(Cube<eT> &x) {
  eT *memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
inline void Cube_aux::set_real(Cube<std::complex<T>> &out,
  ...
  arma_debug_assert_same_size(out.n_rows, out.n_cols, out.n_slices,
...
template <typename T, typename T1>
inline void Cube_aux::set_imag(Cube<std::complex<T>> &out,
                               const BaseCube<T, T1> &X) {
  arma_extra_debug_sigprint();
