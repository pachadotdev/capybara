...
inline SpMat<eT>::const_iterator::const_iterator(const SpMat<eT> &in_M,
  ...
  while (
      (iterator_base::M->row_indices[iterator_base::internal_pos] < in_row) &&
...
template <typename eT>
inline typename SpMat<eT>::const_iterator &
...
inline SpMat<eT>::const_row_iterator::const_row_iterator(const SpMat<eT> &in_M,
  ...
  // row. What we will do is loop across all columns looking for elements in row
  // 0 (and add to our sum), then in row 1, and so forth, until we get to the
  // desired position.
  ...
  for (uword col = 0; col < iterator_base::M->n_cols; ++col) {
    ...
    if (start_ptr != end_ptr) {
      ...
      if (pos_ptr != end_ptr) {
        ...
        // index greater than or equal to in_row.
...
SpMat<eT>::const_row_iterator::operator++() {
  ...
  for (uword col = iterator_base::internal_col + 1;
       col < iterator_base::M->n_cols; ++col) {
    // Find the first element with row greater than or equal to in_row.
    const uword col_offset = iterator_base::M->col_ptrs[col];
    const uword next_col_offset = iterator_base::M->col_ptrs[col + 1];
  ...
  for (uword col = 0; col <= iterator_base::internal_col; ++col) {
    ...
    if (start_ptr != end_ptr) {
      const uword *pos_ptr =
          std::lower_bound(start_ptr, end_ptr, internal_row + 1);
