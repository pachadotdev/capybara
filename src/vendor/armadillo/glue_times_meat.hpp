...
inline void glue_times_redirect2_helper<do_inv_detect>::apply(
  ...
  if (alias == false) {
    glue_times::apply<
        eT, partial_unwrap<T1>::do_trans, partial_unwrap<T2>::do_trans,
        (partial_unwrap<T1>::do_times || partial_unwrap<T2>::do_times)>(
        out, A, B, alpha);
  } else {
    Mat<eT> tmp;
...
glue_times_redirect2_helper<true>::apply(Mat<typename T1::elem_type> &out,
  ...
  if (arma_config::optimise_invexpr &&
    ...
    arma_extra_debug_print(
        "glue_times_redirect<2>::apply(): detected inv(A)*B");
    ...
    if (status == false) {
      out.soft_reset();
      arma_stop_runtime_error("matrix multiplication: problem with matrix "
                              "inverse; suggest to use solve() "
                              "instead");
    }
  ...
  if (arma_config::optimise_invexpr && strip_inv<T2>::do_inv_spd) {
    ...
    arma_extra_debug_print(
        "glue_times_redirect<2>::apply(): detected A*inv_sympd(B)");
...
inline void glue_times_redirect3_helper<do_inv_detect>::apply(
  ...
  const bool use_alpha = partial_unwrap<T1>::do_times ||
                         partial_unwrap<T2>::do_times ||
                         partial_unwrap<T3>::do_times;
  const eT alpha =
      use_alpha ? (tmp1.get_val() * tmp2.get_val() * tmp3.get_val()) : eT(0);

  const bool alias =
      tmp1.is_alias(out) || tmp2.is_alias(out) || tmp3.is_alias(out);
...
inline void glue_times_redirect3_helper<true>::apply(
  ...
  if (arma_config::optimise_invexpr &&
    ...
    arma_extra_debug_print(
        "glue_times_redirect<3>::apply(): detected inv(A)*B*C");
    ...
    glue_times::apply<
        eT, partial_unwrap<T2>::do_trans, partial_unwrap<T3>::do_trans,
        (partial_unwrap<T2>::do_times || partial_unwrap<T3>::do_times)>(
        BC, B, C, alpha);
  ...
  if (arma_config::optimise_invexpr &&
    ...
    arma_extra_debug_print(
        "glue_times_redirect<3>::apply(): detected A*inv(B)*C");
    ...
    glue_times::apply<eT, partial_unwrap_check<T1>::do_trans, false,
                      partial_unwrap_check<T1>::do_times>(out, A, solve_result,
                                                          alpha);
...
inline void glue_times_redirect<4>::apply(
    Mat<typename T1::elem_type> &out,
    const Glue<Glue<Glue<T1, T2, glue_times>, T3, glue_times>, T4, glue_times>
        &X) {
  arma_extra_debug_sigprint();
  ...
  const eT alpha =
      use_alpha
          ? (tmp1.get_val() * tmp2.get_val() * tmp3.get_val() * tmp4.get_val())
          : eT(0);

  const bool alias = tmp1.is_alias(out) || tmp2.is_alias(out) ||
                     tmp3.is_alias(out) || tmp4.is_alias(out);
...
inline void glue_times::apply(Mat<typename T1::elem_type> &out,
  ...
  constexpr uword N_mat =
      1 + depth_lhs<glue_times, Glue<T1, T2, glue_times>>::num;
...
inline void glue_times::apply_inplace(Mat<typename T1::elem_type> &out,
                                      const T1 &X) {
  arma_extra_debug_sigprint();
...
inline void glue_times::apply_inplace_plus(Mat<typename T1::elem_type> &out,
  ...
  const bool use_alpha = partial_unwrap_check<T1>::do_times ||
                         partial_unwrap_check<T2>::do_times ||
                         (sign < sword(0));

  const eT alpha = use_alpha ? (tmp1.get_val() * tmp2.get_val() *
                                ((sign > sword(0)) ? eT(1) : eT(-1)))
                             : eT(0);

  arma_debug_assert_mul_size(A, B, do_trans_A, do_trans_B,
                             "matrix multiplication");

  const uword result_n_rows = (do_trans_A == false)
                                  ? (TA::is_row ? 1 : A.n_rows)
                                  : (TA::is_col ? 1 : A.n_cols);
  const uword result_n_cols = (do_trans_B == false)
                                  ? (TB::is_col ? 1 : B.n_cols)
                                  : (TB::is_row ? 1 : B.n_rows);

  arma_debug_assert_same_size(out.n_rows, out.n_cols, result_n_rows,
                              result_n_cols,
  ...
  if ((do_trans_A == false) && (do_trans_B == false) && (use_alpha == false)) {
    if (((A.n_rows == 1) || (TA::is_row)) && (is_cx<eT>::no)) {
      ...
      gemv<false, false, true>::apply(out.memptr(), A, B.memptr(), alpha,
                                      eT(1));
    } else {
      gemm<false, false, false, true>::apply(out, A, B, alpha, eT(1));
    }
  } else if ((do_trans_A == false) && (do_trans_B == false) &&
             (use_alpha == true)) {
    if (((A.n_rows == 1) || (TA::is_row)) && (is_cx<eT>::no)) {
    ...
    if (((A.n_cols == 1) || (TA::is_col)) && (is_cx<eT>::no)) {
      ...
      gemm<true, false, true, true>::apply(out, A, B, alpha, eT(1));
             ...
             (use_alpha == false)) {
    if (((A.n_rows == 1) || (TA::is_row)) && (is_cx<eT>::no)) {
      gemv<false, false, true>::apply(out.memptr(), B, A.memptr(), alpha,
                                      eT(1));
...
arma_inline uword glue_times::mul_storage_cost(const TA &A, const TB &B) {
  const uword final_A_n_rows = (do_trans_A == false)
                                   ? (TA::is_row ? 1 : A.n_rows)
                                   : (TA::is_col ? 1 : A.n_cols);
  const uword final_B_n_cols = (do_trans_B == false)
                                   ? (TB::is_col ? 1 : B.n_cols)
                                   : (TB::is_row ? 1 : B.n_rows);
...
template <typename eT, const bool do_trans_A, const bool do_trans_B,
          const bool use_alpha, typename TA, typename TB>
inline void glue_times::apply(Mat<eT> &out, const TA &A, const TB &B,
                              const eT alpha) {
  arma_extra_debug_sigprint();

  // arma_debug_assert_mul_size(A, B, do_trans_A, do_trans_B, "matrix
  // multiplication");
  arma_debug_assert_trans_mul_size<do_trans_A, do_trans_B>(
      A.n_rows, A.n_cols, B.n_rows, B.n_cols, "matrix multiplication");

  const uword final_n_rows = (do_trans_A == false)
                                 ? (TA::is_row ? 1 : A.n_rows)
                                 : (TA::is_col ? 1 : A.n_cols);
  const uword final_n_cols = (do_trans_B == false)
                                 ? (TB::is_col ? 1 : B.n_cols)
                                 : (TB::is_row ? 1 : B.n_rows);
...
template <typename eT, const bool do_trans_A, const bool do_trans_B,
          const bool do_trans_C, const bool do_trans_D, const bool use_alpha,
          typename TA, typename TB, typename TC, typename TD>
inline void glue_times::apply(Mat<eT> &out, const TA &A, const TB &B,
                              const TC &C, const TD &D, const eT alpha) {
  arma_extra_debug_sigprint();
  ...
  if (storage_cost_AC <= storage_cost_BD) {
    ...
    glue_times::apply<eT, do_trans_A, do_trans_B, do_trans_C, use_alpha>(
        tmp, A, B, C, alpha);
    ...
    glue_times::apply<eT, do_trans_B, do_trans_C, do_trans_D, use_alpha>(
        tmp, B, C, D, alpha);
