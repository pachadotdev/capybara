...
inline bool spsolve_helper(
  ...
  // superlu_opts::REF_SINGLE; } if(is_double<T>::value)  {
  // superlu_opts_default.refine = superlu_opts::REF_DOUBLE; }
  ...
  {
    if ((settings.id != 0) &&
        ((opts.symmetric) || (opts.pivot_thresh != double(1)))) {
    ...
    try {
      ...
      arma_debug_warn_level(
          1, "spsolve(): not enough memory to use LAPACK based solver");
    }
  ...
  if ((status == true) && (rcond > T(0)) &&
      (rcond < std::numeric_limits<T>::epsilon())) {
    arma_debug_warn_level(
        2,
...
inline bool spsolve(
  ...
  const bool status =
      spsolve_helper(out, A.get_ref(), B.get_ref(), solver, settings);
