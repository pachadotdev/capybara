...
// For example, if we want to choose the eigenvalues with largest magnitude, the
// target will be -std::abs(x). The minus sign is due to the fact that
// std::sort() sorts in ascending order.
...
// specialisation for LARGEST_MAGN: this covers [float, double, complex] x
// [LARGEST_MAGN]
...
// Here BOTH_ENDS is the same as LARGEST_ALGE, but we need some additional
// steps, which are done in SymEigsSolver => retrieve_ritzpair(). There we move
// the smallest values to the proper locations.
...
// specialisation for SMALLEST_ALGE: this covers [float, double] x
// [SMALLEST_ALGE]
...
private:
  typedef typename get_pod_type<eT>::result
      TargetType; // type of the sorting target, will be a floating number type,
                  // eg. double
  typedef std::pair<TargetType, uword>
      PairType; // type of the sorting pair, including the sorting target and
                // the index
