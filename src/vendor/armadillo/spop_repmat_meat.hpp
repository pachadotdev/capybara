  ...
  for (uword A_col = 0; A_col < A_n_cols; ++A_col)
    for (uword B_col = 0; B_col < B_n_cols; ++B_col) {
      for (uword A_row = 0; A_row < A_n_rows; ++A_row) {
        ...
        for (uword B_i = B.col_ptrs[B_col]; B_i < B.col_ptrs[B_col + 1];
             ++B_i) {
...
// inline
// void
// spop_repmat::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1,
// spop_repmat>& in)
//   {
//   arma_extra_debug_sigprint();
//
...
//
//     for(uword col_offset = 0; col_offset < out_n_cols; col_offset +=
//     X_n_cols) for(uword row_offset = 0; row_offset < out_n_rows; row_offset
//     += X_n_rows)
//       {
