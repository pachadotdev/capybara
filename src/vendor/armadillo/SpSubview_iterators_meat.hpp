...
inline SpSubview<eT>::const_iterator::const_iterator(const SpSubview<eT> &in_M,
  ...
  while (cur_pos < (iterator_base::internal_pos + 1)) {
    ...
    const uword row_index =
        iterator_base::M->m.row_indices[cur_pos + lskip_pos];
...
inline SpSubview<eT>::const_iterator::const_iterator(const SpSubview<eT> &in_M,
  ...
  // We have a destination we want to be just after, but don't know what
  // position that is. Because we have to count the points in this subview and
  // not in this subview, this becomes a little difficult and slow.
  const uword aux_col = iterator_base::M->aux_col1;
  const uword aux_row = iterator_base::M->aux_row1;
  const uword ln_rows = iterator_base::M->n_rows;
  const uword ln_cols = iterator_base::M->n_cols;
...
template <typename eT>
inline SpSubview<eT>::const_iterator::const_iterator(
...
SpSubview<eT>::const_iterator::operator--() {
  ...
  if ((skip_pos + cur_pos + 1) == iterator_base::M->m.n_nonzero) {
    ...
    skip_pos = iterator_base::M->m.col_ptrs[cur_col + aux_col] -
  ...
  while (true) {
    ...
    while ((skip_pos + cur_pos) <
           iterator_base::M->m.col_ptrs[cur_col + aux_col]) {
      --cur_col;
    }
...
SpSubview<eT>::const_iterator::operator==(const const_iterator &rhs) const {
  return (rhs.row() == (*this).row()) &&
         (rhs.col() == iterator_base::internal_col);
}
...
template <typename eT>
inline SpSubview<eT>::const_row_iterator::const_row_iterator(
    const SpSubview<eT> &in_M, uword initial_pos)
  ...
  for (uword row = 0; row < iterator_base::M->n_rows; ++row) {
    for (uword col = 0; col < iterator_base::M->n_cols; ++col) {
      ...
      const uword next_col_offset =
          iterator_base::M->m.col_ptrs[col + aux_col + 1];
      ...
      if (start_ptr != end_ptr) {
        const uword *pos_ptr =
            std::lower_bound(start_ptr, end_ptr, row + aux_row);

        const uword offset = uword(pos_ptr - start_ptr);

        if (iterator_base::M->m.row_indices[col_offset + offset] ==
            row + aux_row) {
...
template <typename eT>
inline SpSubview<eT>::const_row_iterator::const_row_iterator(
    const SpSubview<eT> &in_M, uword in_row, uword in_col)
  ...
  for (uword col = 0; col < iterator_base::M->n_cols; ++col) {
    ...
    if (start_ptr != end_ptr) {
      ...
      const uword *first_subview_ptr =
          std::lower_bound(start_ptr, end_ptr, aux_row);
...
inline SpSubview<eT>::const_row_iterator::const_row_iterator(
    ...
    : iterator_base(*other.M, other.internal_col, other.internal_pos),
      internal_row(other.internal_row), actual_pos(other.actual_pos) {
  // Nothing to do.
}
...
SpSubview<eT>::const_row_iterator::operator++() {
  ...
  for (uword col = iterator_base::internal_col + 1; col < M_n_cols; ++col) {
    ...
    if (start_ptr != end_ptr) {
      ...
      const uword *pos_ptr =
          std::lower_bound(start_ptr, end_ptr, internal_row + aux_row);
...
SpSubview<eT>::const_row_iterator::operator--() {
  ...
  for (uword col = iterator_base::internal_col; col >= 1; --col) {
    ...
    if (start_ptr != end_ptr) {
      ...
      if (pos_ptr != start_ptr) {
        ...
        } else if (*(pos_ptr - 1) == max_row + aux_row &&
                   (col - 1) >= max_col) {
          max_col = col - 1;
          next_actual_pos = col_offset + (pos_ptr - 1 - start_ptr);
        }
  ...
  for (uword col = iterator_base::M->n_cols - 1;
       col >= iterator_base::internal_col; --col) {
    // Find the first element with row greater than or equal to row + 1.
    const uword col_offset = iterator_base::M->m.col_ptrs[col + aux_col];
    const uword next_col_offset =
        iterator_base::M->m.col_ptrs[col + aux_col + 1];
