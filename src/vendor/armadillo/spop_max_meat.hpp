...
inline typename T1::elem_type spop_max::vector_max(
  ...
  if (SpProxy<T1>::use_iterator == false) {
    ...
    if (p.get_n_nonzero() == p.get_n_elem()) {
      ...
      return (std::max)(eT(0),
                        op_max::direct_max(p.get_values(), p.get_n_nonzero()));
    }
  } else {
    // use iterator
    typename SpProxy<T1>::const_iterator_type it = p.begin();
...
spop_max::max_with_index(const SpProxy<T1> &P, uword &index_of_max_val) {
  ...
  if (n_elem != n_nonzero) {
    ...
    // If the max_val is a nonzero element, we need its actual position in the
    // matrix.
    if (max_val == eT(0)) {
...
spop_max::max_with_index(const SpProxy<T1> &P, uword &index_of_max_val) {
  ...
  if (SpProxy<T1>::use_iterator) {
    ...
    // We can do direct access.
    max_val = std::abs(
        op_max::direct_max(P.get_values(), n_nonzero, index_of_max_val));
