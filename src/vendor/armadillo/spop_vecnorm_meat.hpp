...
inline void
spop_vecnorm::apply_direct(Mat<typename get_pod_type<eT>::result> &out,
...
template <typename eT>
inline void
spop_vecnorm_ext::apply_direct(Mat<typename get_pod_type<eT>::result> &out,
  ...
  for (uword col = 0; col < X.n_cols; ++col) {
    ...
    if (n_elem > 0) {
      ...
      if (method_id == uword(1)) {
        ...
        out_val =
            (n_elem < X.n_rows) ? T((std::min)(T(0), tmp_val)) : T(tmp_val);
      }
    }
