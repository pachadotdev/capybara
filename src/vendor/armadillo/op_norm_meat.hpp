...
inline typename T1::pod_type op_norm::vec_norm_1(
  ...
  const bool use_direct_mem =
      (is_Mat<typename Proxy<T1>::stored_type>::value) ||
...
inline eT op_norm::vec_norm_2_direct_std(const Mat<eT> &X) {
  ...
  if ((result != eT(0)) && arma_isfinite(result)) {
    ...
    arma_extra_debug_print("op_norm::vec_norm_2_direct_std(): detected "
...
template <typename T1>
inline typename T1::pod_type op_norm::vec_norm_k(const Proxy<T1> &P,
                                                 const int k) {
  arma_extra_debug_sigprint();
