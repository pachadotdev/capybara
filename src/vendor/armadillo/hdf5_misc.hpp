...
template <> inline hid_t get_hdf5_type<std::complex<float>>() {
  ...
  H5Tinsert(type, "real", HOFFSET(hdf5_complex_t<float>, real),
            H5T_NATIVE_FLOAT);
  H5Tinsert(type, "imag", HOFFSET(hdf5_complex_t<float>, imag),
            H5T_NATIVE_FLOAT);
...
template <> inline hid_t get_hdf5_type<std::complex<double>>() {
  ...
  H5Tinsert(type, "real", HOFFSET(hdf5_complex_t<double>, real),
            H5T_NATIVE_DOUBLE);
  H5Tinsert(type, "imag", HOFFSET(hdf5_complex_t<double>, imag),
            H5T_NATIVE_DOUBLE);
...
inline hid_t search_hdf5_file(const std::vector<std::string> &names,
  ...
  herr_t status = H5Ovisit(hdf5_file, H5_INDEX_NAME, H5_ITER_NATIVE,
