...
template <typename eT>
inline SpRow<eT>::SpRow(const SizeMat &s)
...
inline void SpRow<eT>::shed_cols(const uword in_col1, const uword in_col2) {
  ...
  if (start != end) {
    ...
    uword *new_row_indices =
        memory::acquire<uword>(SpMat<eT>::n_nonzero - elem_diff);
...
// inline
// void
// SpRow<eT>::insert_cols(const uword col_num, const uword N, const bool
// set_to_zero)
//   {
//   arma_extra_debug_sigprint();
//
...
//   arma_debug_check_bounds( (col_num > SpMat<eT>::n_cols),
//   "SpRow::insert_cols(): out of bounds" );
//
//   arma_debug_check( (set_to_zero == false), "SpRow::insert_cols(): cannot set
...
template <typename eT>
inline typename SpRow<eT>::row_iterator
SpRow<eT>::begin_row(const uword row_num) {
  arma_extra_debug_sigprint();

  // Since this is a row, row_num can only be 0.  But the option is provided for
  // compatibility.
  arma_debug_check_bounds((row_num >= 1),
                          "SpRow::begin_row(): index out of bounds");

  return SpMat<eT>::begin();
}

template <typename eT>
inline typename SpRow<eT>::const_row_iterator
...
SpRow<eT>::end_row(const uword row_num) {
  ...
  // compatibility.
  arma_debug_check_bounds((row_num >= 1),
                          "SpRow::end_row(): index out of bounds");
