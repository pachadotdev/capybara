...
template <typename eT> struct ProxyCube<Cube<eT>> {
  ...
  inline explicit ProxyCube(const Cube<eT> &A) : Q(A) {
...
struct ProxyCube<GenCube<eT, gen_type>> {
  ...
  arma_inline uword get_n_elem() const {
    return Q.n_rows * Q.n_cols * Q.n_slices;
  }
  ...
  template <typename eT2> constexpr bool is_alias(const Cube<eT2> &) const {
    return false;
  }
...
template <typename T1, typename op_type> struct ProxyCube<OpCube<T1, op_type>> {
...
template <typename eT> struct ProxyCube<subview_cube<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
struct ProxyCube<eGlueCube<T1, T2, eglue_type>> {
  ...
  static constexpr bool has_subview =
      eGlueCube<T1, T2, eglue_type>::has_subview;
