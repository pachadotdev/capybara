...
inline
    typename enable_if2<is_supported_blas_type<typename T1::elem_type>::value,
                        bool>::result
    chol(Mat<typename T1::elem_type> &out,
  ...
  const bool status =
      op_chol::apply_direct(out, X.get_ref(), ((sig == 'u') ? 0 : 1));
...
inline
    ...
    chol(Mat<typename T1::elem_type> &out, Mat<uword> &P,
         const Base<typename T1::elem_type, T1> &X,
         const char *layout = "upper", const char *P_mode = "matrix") {
  arma_extra_debug_sigprint();
