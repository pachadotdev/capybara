...
arma_warn_unused inline
    ...
    normalise(const T1 &X, const uword p = uword(2),
              ...
              const typename arma_real_or_cx_only<
                  typename T1::elem_type>::result *junk2 = nullptr) {
...
arma_warn_unused inline
    ...
    normalise(const T1 &X, const uword p = uword(2), const uword dim = 0,
              const typename arma_real_or_cx_only<
                  typename T1::elem_type>::result *junk = nullptr) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
//! for compatibility purposes: allows compiling user code designed for earlier
