...
inline void spglue_join_cols::apply(SpMat<eT> &out,
                                    const SpBase<eT, T1> &A_expr,
  ...
  arma_debug_check(
      ((A.n_cols != out_n_cols) && ((A.n_rows > 0) || (A.n_cols > 0))),
      "join_cols() / join_vert(): number of columns must be the same");
  arma_debug_check(
      ((B.n_cols != out_n_cols) && ((B.n_rows > 0) || (B.n_cols > 0))),
      "join_cols() / join_vert(): number of columns must be the same");
  arma_debug_check(
      ((C.n_cols != out_n_cols) && ((C.n_rows > 0) || (C.n_cols > 0))),
      "join_cols() / join_vert(): number of columns must be the same");
...
inline void spglue_join_cols::apply(SpMat<eT> &out,
  ...
  const uword out_n_cols = (std::max)(
      ((std::max)((std::max)(A.n_cols, B.n_cols), C.n_cols)), D.n_cols);
  ...
  arma_debug_check(
      ...
      "join_cols() / join_vert(): number of columns must be the same");
  arma_debug_check(
      ((D.n_cols != out_n_cols) && ((D.n_rows > 0) || (D.n_cols > 0))),
      "join_cols() / join_vert(): number of columns must be the same");
...
inline void spglue_join_rows::apply_noalias(SpMat<eT> &out, const SpMat<eT> &A,
  ...
  arrayops::copy(access::rwp(out.row_indices) + A_n_nz, B.row_indices,
                 B_n_nz + 1);

  arrayops::copy(access::rwp(out.col_ptrs), A.col_ptrs, A_n_cols);
  arrayops::copy(access::rwp(out.col_ptrs) + A_n_cols, B.col_ptrs,
                 B_n_cols + 2);

  arrayops::inplace_plus(access::rwp(out.col_ptrs) + A_n_cols, A_n_nz,
                         B_n_cols + 1);
  ...
  //   }
  //
  // // TODO: the first element of B within C will always have a larger index
  // than the last element of A in C;
...
template <typename eT, typename T1, typename T2, typename T3>
inline void spglue_join_rows::apply(SpMat<eT> &out,
                                    const SpBase<eT, T1> &A_expr,
                                    const SpBase<eT, T2> &B_expr,
                                    const SpBase<eT, T3> &C_expr) {
  arma_extra_debug_sigprint();
