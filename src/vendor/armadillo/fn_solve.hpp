...
inline
  ...
  const bool status =
      glue_solve_gen_default::apply(out, A.get_ref(), B.get_ref());
...
arma_warn_unused inline
  ...
  return Glue<T1, T2, glue_solve_gen_full>(A.get_ref(), B.get_ref(),
                                           opts.flags);
}
...
inline
  ...
  const bool status =
      glue_solve_tri_default::apply(out, A.m, B.get_ref(), flags);
