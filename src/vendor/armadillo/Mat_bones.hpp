...
protected:
  arma_align_mem eT
      mem_local[arma_config::mat_prealloc]; // local storage, for small
...
public:
  ...
  inline Mat &
  operator=(const std::initializer_list<std::initializer_list<eT>> &list);
  ...
  inline Mat(const subview_row_strans<eT> &X); // subview_row_strans can only be
                                               // generated by the Proxy class
  inline Mat(const subview_row_htrans<eT> &X); // subview_row_htrans can only be
                                               // generated by the Proxy class
  inline Mat(const xvec_htrans<eT> &X);        //        xvec_htrans can only be
                                        //        generated by the Proxy class

  template <bool do_conj>
  inline Mat(
      const xtrans_mat<eT, do_conj>
  ...
  arma_frown("use braced initialiser list instead") inline mat_injector<Mat>
  operator<<(const eT val);
  arma_frown("use braced initialiser list instead") inline mat_injector<Mat>
  ...
  arma_inline const subview<eT> rows(const uword in_row1,
                                     const uword in_row2) const;

  arma_inline subview_cols<eT> cols(const uword in_col1, const uword in_col2);
  arma_inline const subview_cols<eT> cols(const uword in_col1,
  ...
  inline const subview<eT> submat(const span &row_span,
                                  const span &col_span) const;

  inline subview<eT> operator()(const span &row_span, const span &col_span);
  inline const subview<eT> operator()(const span &row_span,
                                      const span &col_span) const;
  ...
  elem(const Base<uword, T1> &ri, const Base<uword, T2> &ci) const;
  ...
  template <typename T1, typename T2>
  arma_inline const subview_elem2<eT, T1, T2>
  submat(const Base<uword, T1> &ri, const Base<uword, T2> &ci) const;
  ...
  each_col(const Base<uword, T1> &indices) const;
  template <typename T1>
  inline const subview_each2<Mat<eT>, 1, T1>
  each_row(const Base<uword, T1> &indices) const;

  inline Mat &each_col(const std::function<void(Col<eT> &)> &F);
  inline const Mat &
  each_col(const std::function<void(const Col<eT> &)> &F) const;

  inline Mat &each_row(const std::function<void(Row<eT> &)> &F);
  inline const Mat &
  each_row(const std::function<void(const Row<eT> &)> &F) const;
  ...
  arma_warn_unused arma_inline const eT &at(const uword in_row,
                                            const uword in_col) const;
  arma_warn_unused arma_inline eT &operator()(const uword in_row,
                                              const uword in_col);
  arma_warn_unused arma_inline const eT &operator()(const uword in_row,
  ...
  is_sorted(const char *direction = "ascend") const;
  arma_warn_unused inline bool is_sorted(const char *direction,
  ...
  reshape(const uword new_n_rows, const uword new_n_cols,
  ...
  arma_deprecated inline bool
  quiet_save(const csv_name &spec, const file_type type = csv_ascii) const;
  arma_deprecated inline bool
  ...
  public:
    ...
    inline row_col_iterator(Mat<eT> &in_M, const uword row = 0,
                            const uword col = 0);
    ...
    arma_warn_unused inline bool operator!=(const row_col_iterator &rhs) const;
    arma_warn_unused inline bool
    operator==(const const_row_col_iterator &rhs) const;
    arma_warn_unused inline bool
    operator!=(const const_row_col_iterator &rhs) const;
  ...
  inline void steal_mem(Mat &X); //!< don't use this unless you're writing code
...
protected:
  ...
  inline void
  init(const std::initializer_list<std::initializer_list<eT>> &list);
  ...
  inline Mat(const arma_vec_indicator &, const uword in_n_rows,
             const uword in_n_cols, const uhword in_vec_state);

  inline Mat(const arma_fixed_indicator &, const uword in_n_rows,
             const uword in_n_cols, const uhword in_vec_state,
             const eT *in_mem);
