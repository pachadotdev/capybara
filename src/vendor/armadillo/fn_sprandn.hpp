...
//! set to random values from a Gaussian distribution with zero mean and unit
//! variance
template <typename obj_type>
arma_warn_unused inline obj_type
sprandn(const uword n_rows, const uword n_cols, const double density,
        const typename arma_SpMat_SpCol_SpRow_only<obj_type>::result *junk =
            nullptr) {
...
//! Generate a sparse matrix with the non-zero values in the same locations as
//! in the given sparse matrix X, with the non-zero values set to random values
