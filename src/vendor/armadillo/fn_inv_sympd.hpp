...
inline
    typename enable_if2<is_supported_blas_type<typename T1::elem_type>::value,
                        bool>::result
    inv_sympd(Mat<typename T1::elem_type> &out,
  ...
  if (status == false) {
    ...
    arma_debug_warn_level(
        3, "inv_sympd(): matrix is singular or not positive definite");
  }
...
inline
  ...
  const bool status =
      op_inv_spd_full::apply_direct(out, X.get_ref(), opts.flags);
...
inline
    typename enable_if2<is_supported_blas_type<typename T1::elem_type>::value,
                        bool>::result
    inv_sympd(Mat<typename T1::elem_type> &out_inv,
  ...
  const bool status =
      op_inv_spd_rcond::apply_direct(out_inv, inv_state, X.get_ref());
