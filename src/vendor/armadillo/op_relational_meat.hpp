      ...
      if (Proxy<T1>::use_at == false) {                                        \
        ...
        if (n_rows == 1) {                                                     \
          for (uword count = 0; count < n_cols; ++count) {                     \
            out_mem[count] =                                                   \
                (val operator_rel P.at(0, count)) ? uword(1) : uword(0);       \
          }                                                                    \
        } else {                                                               \
          for (uword col = 0; col < n_cols; ++col)                             \
            for (uword row = 0; row < n_rows; ++row) {                         \
              *out_mem =                                                       \
                  (val operator_rel P.at(row, col)) ? uword(1) : uword(0);     \
              out_mem++;                                                       \
            }                                                                  \
        }                                                                      \
      }                                                                        \
    ...
    if (bad_alias == false) {                                                  \
      ...
      if (Proxy<T1>::use_at == false) {                                        \
        ...
        if (n_rows == 1) {                                                     \
          for (uword count = 0; count < n_cols; ++count) {                     \
            out_mem[count] =                                                   \
                (P.at(0, count) operator_rel val) ? uword(1) : uword(0);       \
          }                                                                    \
        } else {                                                               \
          for (uword col = 0; col < n_cols; ++col)                             \
            for (uword row = 0; row < n_rows; ++row) {                         \
              *out_mem =                                                       \
                  (P.at(row, col) operator_rel val) ? uword(1) : uword(0);     \
              out_mem++;                                                       \
            }                                                                  \
        }                                                                      \
      }                                                                        \
    ...
    if (bad_alias == false) {                                                  \
      ...
      if (ProxyCube<T1>::use_at == false) {                                    \
        ...
        for (uword slice = 0; slice < n_slices; ++slice)                       \
          for (uword col = 0; col < n_cols; ++col)                             \
            for (uword row = 0; row < n_rows; ++row) {                         \
              *out_mem = (val operator_rel P.at(row, col, slice)) ? uword(1)   \
                                                                  : uword(0);  \
              out_mem++;                                                       \
            }                                                                  \
      }                                                                        \
    ...
    if (bad_alias == false) {                                                  \
      ...
      if (ProxyCube<T1>::use_at == false) {                                    \
        ...
        for (uword slice = 0; slice < n_slices; ++slice)                       \
          for (uword col = 0; col < n_cols; ++col)                             \
            for (uword row = 0; row < n_rows; ++row) {                         \
              *out_mem = (P.at(row, col, slice) operator_rel val) ? uword(1)   \
                                                                  : uword(0);  \
              out_mem++;                                                       \
            }                                                                  \
      }                                                                        \
...
template <typename T1>
inline void
op_rel_lteq_post::apply(Mat<uword> &out,
...
template <typename T1>
inline void
op_rel_gteq_post::apply(Mat<uword> &out,
...
template <typename T1>
inline void op_rel_eq::apply(Mat<uword> &out,
...
template <typename T1>
inline void op_rel_noteq::apply(Mat<uword> &out,
...
template <typename T1>
inline void
op_rel_lteq_pre::apply(Cube<uword> &out,
...
template <typename T1>
inline void
op_rel_gteq_pre::apply(Cube<uword> &out,
...
template <typename T1>
inline void
op_rel_lt_post::apply(Cube<uword> &out,
...
template <typename T1>
inline void
op_rel_gt_post::apply(Cube<uword> &out,
...
template <typename T1>
inline void
op_rel_lteq_post::apply(Cube<uword> &out,
...
template <typename T1>
inline void
op_rel_gteq_post::apply(Cube<uword> &out,
...
template <typename T1>
inline void op_rel_eq::apply(Cube<uword> &out,
