...
//! format. Format: "ARMA_MAT_TXT_ABXYZ". A is one of: I (for integral types) or
//! F (for floating point types). B is one of: U (for unsigned types), S (for
//! signed types), N (for not applicable) or C (for complex types). XYZ
//! specifies the width of each element in terms of bytes, eg. "008" indicates
//! eight bytes.
...
//! format. Format: "ARMA_MAT_BIN_ABXYZ". A is one of: I (for integral types) or
//! F (for floating point types). B is one of: U (for unsigned types), S (for
//! signed types), N (for not applicable) or C (for complex types). XYZ
//! specifies the width of each element in terms of bytes, eg. "008" indicates
//! eight bytes.
...
//! Generate the first line of the header used for saving matrices in binary
//! format. Format: "ARMA_SPM_BIN_ABXYZ". A is one of: I (for integral types) or
//! F (for floating point types). B is one of: U (for unsigned types), S (for
//! signed types), N (for not applicable) or C (for complex types). XYZ
//! specifies the width of each element in terms of bytes, eg. "008" indicates
//! eight bytes.
...
//! applicable) or C (for complex types). XYZ specifies the width of each
//! element in terms of bytes, eg. "008" indicates eight bytes.
template <typename eT>
...
//! format. Format: "ARMA_CUB_BIN_ABXYZ". A is one of: I (for integral types) or
//! F (for floating point types). B is one of: U (for unsigned types), S (for
//! signed types), N (for not applicable) or C (for complex types). XYZ
//! specifies the width of each element in terms of bytes, eg. "008" indicates
//! eight bytes.
...
inline file_type diskio::guess_file_type_internal(std::istream &f) {
  ...
  const uword N_max = ((pos1 >= 0) && (pos2 >= 0) && (pos2 > pos1))
                          ? uword(pos2 - pos1)
                          : uword(0);
  ...
  // if the data has semicolons, it suggests a CSV file with semicolon as the
  // separating character; the semicolon may be used to allow the comma
  // character to represent the decimal seperator (eg. 1,2345 vs 1.2345)
...
inline void diskio::sanitise_token(std::string &token) {
  ...
  if ((c_front == ' ') || (c_front == '\t') || (c_front == '\r') ||
      (c_back == ' ') || (c_back == '\t') || (c_back == '\r')) {
...
inline bool diskio::convert_token(eT &val, const std::string &token) {
  ...
  //   // std::from_chars() handles only the decimal point (.) as the decimal
  //   seperator
  //
...
template <typename T>
inline bool diskio::convert_token(std::complex<T> &val,
...
inline std::streamsize diskio::prepare_stream(std::ostream &f) {
  ...
  if (is_real<eT>::value) {
    ...
    // NOTE: for 'float' the optimum settings are f.precision(8) and cell_width
    // = 15 NOTE: however, to avoid introducing errors in case single precision
    // data is loaded as double precision, NOTE: the same settings must be used
...
//! complex elements.
template <typename eT>
inline bool diskio::save_raw_ascii(const Mat<eT> &x,
...
template <typename eT>
inline bool diskio::save_raw_binary(const Mat<eT> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
...
inline bool diskio::save_raw_binary(const Mat<eT> &x, std::ostream &f) {
  ...
  f.write(reinterpret_cast<const char *>(x.mem),
...
template <typename eT>
inline bool diskio::save_arma_ascii(const Mat<eT> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
...
diskio::save_csv_ascii(const Mat<eT> &x, const std::string &final_name,
                       const field<std::string> &header, const bool with_header,
...
//! Save a matrix in CSV text format (human readable); complex numbers stored in
//! "a+bi" format
template <typename T>
inline bool diskio::save_csv_ascii(const Mat<std::complex<T>> &x,
                                   std::ostream &f, const char separator) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_coord_ascii(const Mat<eT> &x,
                                     const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename T>
inline bool diskio::save_coord_ascii(const Mat<std::complex<T>> &x,
                                     std::ostream &f) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_arma_binary(const Mat<eT> &x,
                                     const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_pgm_binary(const Mat<eT> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
...
inline bool diskio::save_hdf5_binary(const Mat<eT> &x, const hdf5_name &spec,
...
#if defined(ARMA_USE_HDF5)
  {
    ...
    const std::string tmp_name = (use_existing_file)
                                     ? std::string()
                                     : diskio::gen_tmp_name(spec.filename);

    // Set up the file according to HDF5's preferences
    hid_t file = (use_existing_file)
                     ? H5Fopen(spec.filename.c_str(), H5F_ACC_RDWR, H5P_DEFAULT)
                     : H5Fcreate(tmp_name.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                                 H5P_DEFAULT);
    ...
    hid_t dataspace = H5Screate_simple(
        2, dims, NULL); // treat the matrix as a 2d array dataspace
    ...
    while ((loc = full_name.find("/")) != std::string::npos) {
      ...
      {
        hid_t gid =
            H5Gcreate((groups.size() == 0) ? file : groups[groups.size() - 1],
    ...
    const hid_t last_group =
        (groups.size() == 0) ? file : groups[groups.size() - 1];

    if (use_existing_file && replace) {
      H5Ldelete(last_group, dataset_name.c_str(), H5P_DEFAULT);
      // NOTE: H5Ldelete() in HDF5 v1.8 doesn't reclaim the deleted space; use
      // h5repack to reclaim space: h5repack oldfile.h5 newfile.h5 NOTE: has
      // this behaviour changed in HDF5 1.10 ? NOTE:
    ...
    hid_t dataset = H5Dcreate(last_group, dataset_name.c_str(), datatype,
    ...
    if (dataset < 0) {
      ...
      save_okay = (H5Dwrite(dataset, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                            x.mem) >= 0);
...
inline bool diskio::load_raw_ascii(Mat<eT> &x, const std::string &name,
  ...
  (arma_config::text_as_binary) ? f.open(name, std::fstream::binary)
                                : f.open(name);
...
//! NOTE: this is much slower than reading a file with a header.
template <typename eT>
inline bool diskio::load_raw_ascii(Mat<eT> &x, std::istream &f,
                                   std::string &err_msg) {
  arma_extra_debug_sigprint();
  ...
  while (f.good() && load_okay) {
    ...
    // TODO: does it make sense to stop processing the file if an empty line is
    // found ?
...
template <typename eT>
inline bool diskio::load_raw_binary(Mat<eT> &x, std::istream &f,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
//! with a header that indicates the matrix type as well as its dimensions
template <typename eT>
inline bool diskio::load_arma_ascii(Mat<eT> &x, std::istream &f,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
//! Load a matrix in CSV text format (human readable)
template <typename eT>
inline bool diskio::load_csv_ascii(Mat<eT> &x, std::istream &f,
  ...
  const bool use_mp =
      (arma_config::openmp) && (f_n_rows >= 2) && (f_n_cols >= 64);
...
//! Load a matrix in CSV text format (human readable); complex numbers stored in
//! "a+bi" format
template <typename T>
inline bool diskio::load_csv_ascii(Mat<std::complex<T>> &x, std::istream &f,
  ...
  while (f.good()) {
    ...
    while (line_stream.good()) {
      ...
      bool found_x = false;
      std::string::size_type loc_x = 0; // location of the separator (+ or -)
      ...
      if (loc_i == std::string::npos) {
        ...
        if (found_plus && found_minus) {
          if ((loc_i > loc_plus) && (loc_i > loc_minus)) {
            // choose the sign closest to the "i" to be the separator between
            // the real and imaginary part
            loc_x = ((loc_i - loc_plus) < (loc_i - loc_minus)) ? loc_plus
                                                               : loc_minus;
            found_x = true;
          }
        } else if (found_plus) {
...
//! Load a matrix in CSV text format (human readable)
template <typename eT>
inline bool diskio::load_coord_ascii(Mat<eT> &x, std::istream &f,
                                     std::string &err_msg) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::load_arma_binary(Mat<eT> &x, std::istream &f,
                                     std::string &err_msg) {
  arma_extra_debug_sigprint();
  ...
  if (f_header == diskio::gen_bin_header(x)) {
    // f.seekg(1, ios::cur);  // NOTE: this may not be portable, as on a Windows
    // machine a newline could be two characters
    f.get();
...
//! Load a PGM greyscale image as a matrix
template <typename eT>
inline bool diskio::load_pgm_binary(Mat<eT> &x, std::istream &f,
                                    std::string &err_msg) {
  bool load_okay = true;
  ...
  if (f_header == "P5") {
    ...
    if ((f_maxval > 0) && (f_maxval <= 65535)) {
      ...
      if (f_maxval <= 255) {
        ...
        f.read(reinterpret_cast<char *>(tmp.memptr()),
               std::streamsize(n_elem * 2));
...
inline bool diskio::load_hdf5_binary(Mat<eT> &x, const hdf5_name &spec,
...
#if defined(ARMA_USE_HDF5)
  {
    ...
    if (fid >= 0) {
      ...
      // being referred to as "value". If the user hasn't specified a dataset,
      // we will search for "dataset" and "value", and if those are not found we
      ...
      if (dataset >= 0) {
        ...
        hsize_t dims[2];
        const herr_t query_status =
            H5Sget_simple_extent_dims(filespace, dims, NULL);

        // arma_check(query_status < 0, "Mat::load(): cannot get size of HDF5
        // dataset");
        if (query_status < 0) {
        ...
        if (H5Tequal(datatype, mat_type) > 0) {
          ...
          hid_t read_status = hdf5_misc::load_and_convert_hdf5(
              x.memptr(), dataset, datatype, x.n_elem);
...
#if defined(ARMA_USE_HDF5)
  // We're currently using the C bindings for the HDF5 library, which don't
  // support C++ streams
...
//! Try to load a matrix by automatically determining its type
template <typename eT>
inline bool diskio::load_auto_detect(Mat<eT> &x, std::istream &f,
                                     std::string &err_msg) {
  arma_extra_debug_sigprint();
  ...
  if (std::strncmp(ARMA_MAT_TXT_str, header_mem, size_t(ARMA_MAT_TXT_len)) ==
      0) {
    return load_arma_ascii(x, f, err_msg);
  } else if (std::strncmp(ARMA_MAT_BIN_str, header_mem,
                          size_t(ARMA_MAT_BIN_len)) == 0) {
...
template <typename T>
inline bool diskio::save_csv_ascii(const SpMat<std::complex<T>> &x,
                                   std::ostream &f, const char separator) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_coord_ascii(const SpMat<eT> &x,
                                     const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename T>
inline bool diskio::save_coord_ascii(const SpMat<std::complex<T>> &x,
                                     std::ostream &f) {
  arma_extra_debug_sigprint();
...
//! with a header that stores the matrix type as well as its dimensions
template <typename eT>
inline bool diskio::save_arma_binary(const SpMat<eT> &x,
                                     const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool
diskio::load_csv_ascii(SpMat<eT> &x, const std::string &name,
...
template <typename eT>
inline bool diskio::load_csv_ascii(SpMat<eT> &x, std::istream &f,
                                   std::string &err_msg, const char separator) {
  arma_extra_debug_sigprint();
...
inline bool diskio::load_arma_binary(SpMat<eT> &x, std::istream &f,
  ...
  if (f_header == diskio::gen_bin_header(x)) {
    ...
    if ((check1 == true) && ((check2 == false) || (check3 == false))) {
      if (sizeof(uword) == 8) {
        arma_extra_debug_print("detected inconsistent data while loading; "
                               "re-reading integer parts as u32");

        // inconstency could be due to a different uword size used during
        // saving, so try loading the row_indices and col_ptrs under the
        // assumption of 32 bit unsigned integers
        ...
        if (load_okay && (check2 == true) && (check3 == true)) {
          ...
          arrayops::convert(access::rwp(x.row_indices), tmp_a.memptr(),
                            x.n_nonzero);
          arrayops::convert(access::rwp(x.col_ptrs), tmp_b.memptr(),
                            x.n_cols + 1);
        } else {
...
template <typename eT>
inline bool diskio::save_raw_ascii(const Cube<eT> &x,
                                   const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_raw_binary(const Cube<eT> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_arma_ascii(const Cube<eT> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_arma_binary(const Cube<eT> &x,
                                     const std::string &final_name) {
  arma_extra_debug_sigprint();
...
inline bool diskio::save_hdf5_binary(const Cube<eT> &x, const hdf5_name &spec,
...
#if defined(ARMA_USE_HDF5)
  {
    ...
    hid_t dataspace = H5Screate_simple(
        3, dims, NULL); // treat the cube as a 3d array dataspace
    hid_t datatype = hdf5_misc::get_hdf5_type<eT>();
...
//! NOTE: this is much slower than reading a file with a header.
template <typename eT>
inline bool diskio::load_raw_ascii(Cube<eT> &x, std::istream &f,
                                   std::string &err_msg) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::load_raw_binary(Cube<eT> &x, std::istream &f,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
//! with a header that indicates the cube type as well as its dimensions
template <typename eT>
inline bool diskio::load_arma_ascii(Cube<eT> &x, std::istream &f,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::load_arma_binary(Cube<eT> &x, std::istream &f,
                                     std::string &err_msg) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_HDF5)
  {
    ...
    if (fid >= 0) {
      ...
      if (dataset >= 0) {
        ...
        // arma_check(query_status < 0, "Cube::load(): cannot get size of HDF5
        // dataset");
        if (query_status < 0) {
          err_msg = "cannot get size of HDF5 dataset";
...
//! Try to load a cube by automatically determining its type
template <typename eT>
inline bool diskio::load_auto_detect(Cube<eT> &x, std::istream &f,
                                     std::string &err_msg) {
  arma_extra_debug_sigprint();
  ...
  if (std::strncmp(ARMA_CUB_TXT_str, header_mem, size_t(ARMA_CUB_TXT_len)) ==
      0) {
    return load_arma_ascii(x, f, err_msg);
  } else if (std::strncmp(ARMA_CUB_BIN_str, header_mem,
                          size_t(ARMA_CUB_BIN_len)) == 0) {
    return load_arma_binary(x, f, err_msg);
...
inline bool diskio::save_arma_binary(const field<T1> &x,
                                     const std::string &final_name) {
  arma_extra_debug_sigprint();
...
inline bool diskio::save_arma_binary(const field<T1> &x, std::ostream &f) {
  ...
  arma_type_check(
      ((is_Mat<T1>::value == false) && (is_Cube<T1>::value == false)));
...
inline bool diskio::load_std_string(field<std::string> &x,
                                    const std::string &name,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::load_ppm_binary(Cube<eT> &x, std::istream &f,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool diskio::save_ppm_binary(const Cube<eT> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
...
template <typename T1>
inline bool diskio::load_ppm_binary(field<T1> &x, std::istream &f,
                                    std::string &err_msg) {
  arma_extra_debug_sigprint();
...
template <typename T1>
inline bool diskio::save_ppm_binary(const field<T1> &x,
                                    const std::string &final_name) {
  arma_extra_debug_sigprint();
