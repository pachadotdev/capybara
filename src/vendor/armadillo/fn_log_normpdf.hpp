...
inline
  ...
  arma_debug_check(((PX.get_n_rows() != PM.get_n_rows()) ||
                    (PX.get_n_cols() != PM.get_n_cols()) ||
                    (PM.get_n_rows() != PS.get_n_rows()) ||
                    (PM.get_n_cols() != PS.get_n_cols())),
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
      for (uword i = 0; i < N; ++i) {
        ...
        out_mem[i] = (eT(-0.5) * (tmp * tmp)) -
                     (std::log(sigma) + Datum<eT>::log_sqrt2pi);
      }
    }
#endif
  } else {
...
arma_warn_unused inline typename enable_if2<(is_real<eT>::value), eT>::result
...
arma_warn_unused inline
    ...
    log_normpdf(const eT x, const Base<eT, T2> &M_expr,
                const Base<eT, T3> &S_expr) {
  arma_extra_debug_sigprint();
  ...
  log_normpdf_helper(out, x * ones<Mat<eT>>(arma::size(M)), M,
                     S_expr.get_ref());
...
arma_warn_unused inline
    typename enable_if2<(is_real<typename T1::elem_type>::value),
                        Mat<typename T1::elem_type>>::result
    log_normpdf(const Base<typename T1::elem_type, T1> &X_expr,
                const typename T1::elem_type mu,
