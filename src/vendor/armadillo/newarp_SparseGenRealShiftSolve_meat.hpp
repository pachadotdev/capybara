...
inline SparseGenRealShiftSolve<eT>::SparseGenRealShiftSolve(
    const SpMat<eT> &mat_obj, const eT shift)
#if defined(ARMA_USE_SUPERLU)
    : perm_c(mat_obj.n_cols + 1), perm_r(mat_obj.n_rows + 1),
      n_rows(mat_obj.n_rows), n_cols(mat_obj.n_cols)
#else
    : n_rows(0), n_cols(0)
#endif
{
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    if (status_x == false) {
      arma_stop_runtime_error("newarp::SparseGenRealShiftSolve::"
                              "SparseGenRealShiftSolve(): could not "
    ...
    superlu::gstrf<eT>(&options, xC.get_ptr(), relax, panel_size,
...
inline void SparseGenRealShiftSolve<eT>::perform_op(eT *x_in, eT *y_out) const {
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    const bool status_out_slu =
        sp_auxlib::wrap_to_supermatrix(out_slu.get_ref(), y);

    if (status_out_slu == false) {
      arma_stop_runtime_error("newarp::SparseGenRealShiftSolve::perform_op(): "
                              "could not construct SuperLU "
                              "matrix");
      return;
    }
    ...
    superlu::gstrs<eT>(superlu::NOTRANS, l.get_ptr(), u.get_ptr(),
                       perm_c.get_ptr(), perm_r.get_ptr(), out_slu.get_ptr(),
                       stat.get_ptr(), &info);

    if (info != 0) {
      arma_stop_runtime_error("newarp::SparseGenRealShiftSolve::perform_op(): "
                              "could not solve linear "
                              "equation");
      return;
    }
