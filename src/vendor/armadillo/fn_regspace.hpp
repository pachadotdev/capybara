...
inline void
internal_regspace_default_delta(Mat<eT> &x,
...
internal_regspace_var_delta(Mat<eT> &x, const typename Mat<eT>::pod_type start,
  ...
  if (((start < end) && (delta < sT(0))) ||
      ((start > end) && (delta > sT(0))) || (delta == sT(0))) {
    return;
  }
...
inline typename enable_if2<
    is_Mat<vec_type>::value && (is_signed<sT>::value == true), vec_type>::result
regspace(const typename vec_type::pod_type start, const sT delta,
  ...
  if (((delta == sT(+1)) && (start <= end)) ||
      ((delta == sT(-1)) && (start > end))) {
...
inline typename enable_if2<is_Mat<vec_type>::value &&
                               (is_signed<uT>::value == false),
                           vec_type>::result
...
arma_warn_unused inline
    typename enable_if2<is_Mat<vec_type>::value, vec_type>::result
