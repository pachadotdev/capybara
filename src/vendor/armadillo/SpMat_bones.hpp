...
public:
  ...
  // The correct locations of the values are assembled from the row indices and
  // column pointers.
  //
  // The length of this array is (n_nonzero + 1).
  // The final value values[n_nonzero] must be zero to ensure integrity of
  // iterators. Use mem_resize(new_n_nonzero) to resize this array.
  //
  ...
  // The element col_ptrs[n_cols + 1] must be an invalid very large value to
  // ensure integrity of iterators.
  //
  // The col_ptrs array is set by the init() function
  // (which is called by constructors, set_size() and other functions that
  ...
  inline SpMat(const Base<uword, T1> &rowind, const Base<uword, T2> &colptr,
               const Base<eT, T3> &values, const uword n_rows,
               const uword n_cols, const bool check_for_zeros = true);
  ...
  inline SpMat(const bool add_values, const Base<uword, T1> &locations,
               const Base<eT, T2> &values, const uword n_rows,
               const uword n_cols, const bool sort_locations = true,
               const bool check_for_zeros = true);
  ...
  template <typename T1, typename T2>
  inline explicit SpMat(const SpBase<pod_type, T1> &A,
  ...
  inline SpSubview_row<eT> operator()(const uword row_num,
                                      const span &col_span);
  ...
  inline SpSubview_col<eT> operator()(const span &row_span,
                                      const uword col_num);
  ...
  arma_inline const SpSubview<eT> rows(const uword in_row1,
                                       const uword in_row2) const;

  arma_inline SpSubview<eT> cols(const uword in_col1, const uword in_col2);
  arma_inline const SpSubview<eT> cols(const uword in_col1,
  ...
  arma_inline const SpSubview<eT> submat(const uword in_row1,
                                         const uword in_col1,
                                         const uword in_row2,
                                         const uword in_col2) const;
  ...
  submat(const uword in_row1, const uword in_col1, const SizeMat &s) const;

  inline SpSubview<eT> submat(const span &row_span, const span &col_span);
  inline const SpSubview<eT> submat(const span &row_span,
                                    const span &col_span) const;

  inline SpSubview<eT> operator()(const span &row_span, const span &col_span);
  inline const SpSubview<eT> operator()(const span &row_span,
                                        const span &col_span) const;

  arma_inline SpSubview<eT> operator()(const uword in_row1, const uword in_col1,
                                       const SizeMat &s);
  arma_inline const SpSubview<eT>
  operator()(const uword in_row1, const uword in_col1, const SizeMat &s) const;
  ...
  cols(const Base<uword, T1> &ci) const;
...
// position, 0 is returned
  ...
  arma_warn_unused arma_inline SpMat_MapMat_val<eT>
  ...
  arma_warn_unused arma_inline SpMat_MapMat_val<eT> at(const uword in_row,
                                                       const uword in_col);
  arma_warn_unused arma_inline eT at(const uword in_row,
                                     const uword in_col) const;
  ...
  is_symmetric(const typename get_pod_type<eT>::result tol) const;

  arma_warn_unused inline bool is_hermitian() const;
  arma_warn_unused inline bool
  is_hermitian(const typename get_pod_type<eT>::result tol) const;
  ...
  arma_warn_unused arma_inline bool
  in_range(const uword in_row, const uword in_col, const SizeMat &s) const;
  ...
  inline SpMat &sprandu(const uword in_rows, const uword in_cols,
                        const double density);
  inline SpMat &sprandu(const SizeMat &s, const double density);

  inline SpMat &sprandn(const uword in_rows, const uword in_cols,
                        const double density);
  ...
  //! don't use this unless you're writing internal Armadillo code
  inline SpMat(const arma_reserve_indicator &, const uword in_rows,
  ...
  arma_cold inline bool save(std::ostream &os,
                             const file_type type = arma_binary) const;

  arma_cold inline bool load(const std::string name,
                             const file_type type = arma_binary);
  arma_cold inline bool load(const csv_name &spec,
                             const file_type type = csv_ascii);
  arma_cold inline bool load(std::istream &is,
                             const file_type type = arma_binary);

  arma_deprecated inline bool
  quiet_save(const std::string name, const file_type type = arma_binary) const;
  arma_deprecated inline bool
  quiet_save(std::ostream &os, const file_type type = arma_binary) const;
  ...
  // iterator_base provides basic operators but not how to compare or how to
  // iterate
  class iterator_base {
  public:
    ...
    // don't hold location internally; call "dummy" methods to get that
    // information
  ...
  public:
    ...
    inline const_iterator(
        ...
        uword in_col); // iterator will be at the first nonzero value after the
                       // given position (using forward columnwise traversal)
    inline const_iterator(
        const SpMat &in_M, uword in_row, uword in_col,
    ...
    operator==(const typename SpSubview<eT>::const_iterator &rhs) const;
    arma_hot inline bool
    operator!=(const typename SpSubview<eT>::const_iterator &rhs) const;
    ...
    arma_hot inline bool
    operator==(const typename SpSubview<eT>::const_row_iterator &rhs) const;
    arma_hot inline bool
    operator!=(const typename SpSubview<eT>::const_row_iterator &rhs) const;
  };
   ...
   * implementation. This can't be as simple as for Mat, which is just a pointer
  ...
  public:
    ...
    inline const_row_iterator &
    operator=(const const_row_iterator &other) = default;
  ...
  // Resize memory.
  // If the new size is larger, the column pointers and new memory still need to
  // be correctly set. If the new size is smaller, the first new_n_nonzero
  // elements will be copied. n_nonzero is updated.
...
protected:
  ...
  inline SpMat(const arma_vec_indicator &, const uword in_n_rows,
               const uword in_n_cols, const uword in_vec_state);

private:
  arma_warn_unused arma_hot inline const eT *
  find_value_csc(const uword in_row, const uword in_col) const;
  ...
  arma_warn_unused arma_hot inline bool
  try_set_value_csc(const uword in_row, const uword in_col, const eT in_val);
  arma_warn_unused arma_hot inline bool
  try_add_value_csc(const uword in_row, const uword in_col, const eT in_val);
  arma_warn_unused arma_hot inline bool
  try_sub_value_csc(const uword in_row, const uword in_col, const eT in_val);
  arma_warn_unused arma_hot inline bool
  try_mul_value_csc(const uword in_row, const uword in_col, const eT in_val);
  arma_warn_unused arma_hot inline bool
  try_div_value_csc(const uword in_row, const uword in_col, const eT in_val);

  arma_warn_unused inline eT &insert_element(const uword in_row,
                                             const uword in_col,
...
public:
  ...
  inline static void set_real(SpMat<std::complex<T>> &out,
                              const SpBase<T, T1> &X);

  template <typename eT, typename T1>
  inline static void set_imag(SpMat<eT> &out, const SpBase<eT, T1> &X);
  template <typename T, typename T1>
  inline static void set_imag(SpMat<std::complex<T>> &out,
                              const SpBase<T, T1> &X);
};
