...
arma_hot inline typename T1::elem_type accu_proxy_linear(const Proxy<T1> &P) {
  ...
  if (arma_config::openmp && Proxy<T1>::use_mp && mp_gate<eT>::eval(n_elem)) {
    ...
    {
      // NOTE: using parallelisation with manual reduction workaround to take
      // into account complex numbers; NOTE: OpenMP versions lower than 4.0 do
      // not support user-defined reduction

      const int n_threads_max = mp_thread_limit::get();
      const uword n_threads_use = (std::min)(
          uword(podarray_prealloc_n_elem::val), uword(n_threads_max));
...
arma_hot inline typename T1::elem_type accu_proxy_at(const Proxy<T1> &P) {
  ...
  if (arma_config::openmp && Proxy<T1>::use_mp &&
      mp_gate<eT>::eval(P.get_n_elem())) {
...
arma_warn_unused inline typename T1::elem_type
accu(const eGlue<T1, T2, eglue_schur> &expr) {
  ...
  const bool have_direct_mem_1 = (is_Mat<P1_stored_type>::value) ||
                                 (is_subview_col<P1_stored_type>::value);
  const bool have_direct_mem_2 = (is_Mat<P2_stored_type>::value) ||
                                 (is_subview_col<P2_stored_type>::value);
    ...
    if (PA_n_rows == 1) {
      ...
      for (uword col = 0; col < PA_n_cols; ++col)
        for (uword row = 0; row < PA_n_rows; ++row) {
          n_nonzero +=
              (PA.at(row, col) != PB.at(row, col)) ? uword(1) : uword(0);
        }
    }
  }
...
template <typename T1>
arma_hot inline typename T1::elem_type
accu_cube_proxy_linear(const ProxyCube<T1> &P) {
  ...
  if (arma_config::openmp && ProxyCube<T1>::use_mp &&
      mp_gate<eT>::eval(n_elem)) {
#if defined(ARMA_USE_OPENMP)
    {
      // NOTE: using parallelisation with manual reduction workaround to take
      // into account complex numbers; NOTE: OpenMP versions lower than 4.0 do
...
#pragma omp parallel for schedule(static) num_threads(int(n_threads_use))
    ...
    {
      if (P.is_aligned()) {
        typename ProxyCube<T1>::aligned_ea_type Pea_aligned =
            P.get_aligned_ea();
...
template <typename T1>
arma_warn_unused arma_hot inline typename T1::elem_type
accu(const BaseCube<typename T1::elem_type, T1> &X) {
  ...
  return (ProxyCube<T1>::use_at) ? accu_cube_proxy_at(P)
...
accu(const SpGlue<T1, T2, spglue_plus> &expr) {
  ...
  arma_debug_assert_same_size(UA.M.n_rows, UA.M.n_cols, UB.M.n_rows,
                              UB.M.n_cols, "addition");
