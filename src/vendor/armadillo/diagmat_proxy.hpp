...
public:
  ...
  inline diagmat_proxy_default(const T1 &X)
      : P(X), P_is_vec((resolves_to_vector<T1>::yes) || (P.get_n_rows() == 1) ||
                       (P.get_n_cols() == 1)),
...
public:
  ...
  inline diagmat_proxy_fixed(const T1 &X) : P(X) {
    arma_extra_debug_sigprint();
  }
...
class diagmat_proxy
    : public diagmat_proxy_redirect<T1, is_Mat_fixed<T1>::value>::result {
public:
...
public:
  ...
  inline diagmat_proxy(const Mat<eT> &X)
      : P(X), P_is_vec((X.n_rows == 1) || (X.n_cols == 1)),
