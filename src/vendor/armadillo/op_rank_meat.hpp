...
inline bool op_rank::apply(uword &out,
  ...
  if (do_sym) {
    arma_extra_debug_print(
        "op_rank::apply(): symmetric/hermitian optimisation");
...
inline bool op_rank::apply_sym(uword &out, Mat<eT> &A,
  ...
  // set tolerance to default if it hasn't been specified
  if (tol == T(0)) {
    tol = (std::max)(A.n_rows, A.n_cols) * max_abs_v *
          std::numeric_limits<T>::epsilon();
  }
...
inline bool op_rank::apply_gen(uword &out, Mat<eT> &A,
  ...
  // set tolerance to default if it hasn't been specified
  if (tol == T(0)) {
    tol = (std::max)(A.n_rows, A.n_cols) * s_mem[0] *
          std::numeric_limits<T>::epsilon();
  }
