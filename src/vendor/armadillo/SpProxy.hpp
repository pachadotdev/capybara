...
// TODO: clarify and check which variables and functions are valid when
// 'use_iterator' is either true or false
...
// std::complex stored_type             = the type of the Q object
//
// const_iterator_type     = the type of iterator provided by begin() and
// begin_col() const_row_iterator_type = the type of iterator provided by
// begin_row()
//
// use_iterator            = boolean indicating that the provided iterators must
// be used for accessing elements Q_is_generated          = boolean indicating
// that the Q object was generated by SpProxy
//
// is_row                  = boolean indicating whether the Q object can be
// treated a row vector is_col                  = boolean indicating whether the
// Q object can be treated a column vector is_xvec                 = boolean
// indicating whether the Q object is a vector with unknown orientation
//
// Q                       = object that can be unwrapped via the unwrap_spmat
// family of classes (ie. Q must be convertible to SpMat)
//
...
//
// get_values()            = return pointer to the CSC          values array in
// Q; valid only if the 'use_iterator' boolean is false get_row_indices() =
// return pointer to the CSC     row indices array in Q; valid only if the
// 'use_iterator' boolean is false get_col_ptrs()          = return pointer to
// the CSC column pointers array in Q; valid only if the 'use_iterator' boolean
// is false
//
// begin()                 = column-wise iterator indicating the first element
// in Q begin_col(col_num)      = column-wise iterator indicating the first
// element in column 'col_num' in Q begin_row(row_num = 0)  =    row-wise
// iterator indicating the first element in row    'row_num' in Q
//
// end()                   = column-wise iterator indicating the "one-past-end"
// element in Q end_row()               =    row-wise iterator indicating the
// "one-past-end" element in Q end_row(row_num)        =    row-wise iterator
// indicating the "one-past-end" element in row 'row_num' in Q
//
// is_alias(X)             = return true/false indicating whether the Q object
// aliases matrix X

template <typename eT> struct SpProxy<SpMat<eT>> {
...
template <typename eT> struct SpProxy<SpCol<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
template <typename eT> struct SpProxy<SpRow<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
template <typename eT> struct SpProxy<SpSubview<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
template <typename eT> struct SpProxy<SpSubview_col<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
template <typename eT, typename T1> struct SpProxy<SpSubview_col_list<eT, T1>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef SpMat<eT> stored_type;
...
template <typename eT> struct SpProxy<SpSubview_row<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  arma_inline elem_type at(const uword, const uword col) const {
    return Q.at(0, col);
  }
...
template <typename eT> struct SpProxy<spdiagview<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef SpMat<eT> stored_type;
  ...
  inline explicit SpProxy(const spdiagview<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename T1, typename spop_type> struct SpProxy<SpOp<T1, spop_type>> {
...
struct SpProxy<mtSpGlue<out_eT, T1, T2, spglue_type>> {
  ...
  static constexpr bool is_xvec =
      mtSpGlue<out_eT, T1, T2, spglue_type>::is_xvec;
