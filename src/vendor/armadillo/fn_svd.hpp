...
inline bool
svd(Col<typename T1::pod_type> &S, const Base<typename T1::elem_type, T1> &X,
...
svd(const Base<typename T1::elem_type, T1> &X,
    const typename arma_blas_type_only<typename T1::elem_type>::result *junk =
        nullptr) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
...
template <typename T1>
inline bool
svd(Mat<typename T1::elem_type> &U, Col<typename T1::pod_type> &S,
  ...
  arma_debug_check(((sig != 's') && (sig != 'd')),
                   "svd(): unknown method specified");

  Mat<eT> A(X.get_ref());

  const bool status =
      (sig == 'd') ? auxlib::svd_dc(U, S, V, A) : auxlib::svd(U, S, V, A);
