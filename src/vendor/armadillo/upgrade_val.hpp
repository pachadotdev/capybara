...
//! upgrade_val is used to ensure an operation such as multiplication is
//! possible between two types. values are upgraded only where necessary.
...
template <> struct upgrade_val<std::complex<float>, double> {
  ...
  arma_inline static const std::complex<double>
...
//! ensure we don't lose precision when multiplying complex numbers with
...
//! work around limitations in the complex class (at least as present in gcc 4.1
//! & 4.3)
