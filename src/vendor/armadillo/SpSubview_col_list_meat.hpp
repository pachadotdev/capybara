...
template <typename eT, typename T1>
arma_inline
SpSubview_col_list<eT, T1>::SpSubview_col_list(const SpMat<eT> &in_m,
  ...
  for (uword ci_count = 0; ci_count < ci_n_elem; ++ci_count) {
    ...
    arma_debug_check_bounds((i >= in_m.n_cols),
                            "SpMat::cols(): index out of bounds");
  }
}
...
template <typename eT, typename T1>
inline void SpSubview_col_list<eT, T1>::replace(const eT old_val,
                                                const eT new_val) {
  arma_extra_debug_sigprint();
...
template <typename eT, typename T1>
inline void
SpSubview_col_list<eT, T1>::extract(SpMat<eT> &out,
                                    const SpSubview_col_list<eT, T1> &in) {
  arma_extra_debug_sigprint();

  // NOTE: aliasing is handled by SpMat<eT>::operator=(const
  // SpSubview_col_list<eT,T1>& in)
...
template <typename eT, typename T1>
inline void
SpSubview_col_list<eT, T1>::plus_inplace(SpMat<eT> &out,
...
template <typename eT, typename T1>
inline void
SpSubview_col_list<eT, T1>::minus_inplace(SpMat<eT> &out,
                                          const SpSubview_col_list &in) {
  arma_extra_debug_sigprint();
...
template <typename eT, typename T1>
inline void
SpSubview_col_list<eT, T1>::schur_inplace(SpMat<eT> &out,
                                          const SpSubview_col_list &in) {
  arma_extra_debug_sigprint();
...
template <typename eT, typename T1>
inline void
SpSubview_col_list<eT, T1>::div_inplace(SpMat<eT> &out,
                                        const SpSubview_col_list &in) {
  arma_extra_debug_sigprint();
