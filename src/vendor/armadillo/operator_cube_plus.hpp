...
template <typename T1>
arma_inline const eOpCube<T1, eop_scalar_plus>
operator+(const typename T1::elem_type k,
...
template <typename T1>
arma_inline const mtOpCube<typename std::complex<typename T1::pod_type>, T1,
                           op_cx_scalar_plus>
operator+(const BaseCube<typename T1::pod_type, T1> &X,
  ...
  return mtOpCube<typename std::complex<typename T1::pod_type>, T1,
                  op_cx_scalar_plus>('j', X.get_ref(), k);
}
...
operator+(const std::complex<typename T1::pod_type> &k,
  ...
  return mtOpCube<typename std::complex<typename T1::pod_type>, T1,
                  op_cx_scalar_plus>('j', X.get_ref(),
                                     k); // NOTE: order is swapped
}
...
template <typename T1, typename T2>
inline const mtGlueCube<typename promote_type<typename T1::elem_type,
                                              typename T2::elem_type>::result,
                        T1, T2, glue_mixed_plus>
operator+(const BaseCube<
              typename force_different_type<typename T1::elem_type,
...
arma_inline Cube<eT> operator+(const subview_cube_each1<eT> &X,
                               const Base<eT, T2> &Y) {
  arma_extra_debug_sigprint();
...
arma_inline Cube<eT> operator+(const Base<eT, T1> &X,
  ...
  return subview_cube_each1_aux::operator_plus(
      Y, X.get_ref()); // NOTE: swapped order
}
...
arma_inline Cube<eT> operator+(const Base<eT, T1> &X,
  ...
  return subview_cube_each2_aux::operator_plus(
      Y, X.get_ref()); // NOTE: swapped order
}
