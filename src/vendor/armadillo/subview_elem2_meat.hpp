...
template <typename eT, typename T1, typename T2>
arma_inline subview_elem2<eT, T1, T2>::subview_elem2(
    const Mat<eT> &in_m, const Base<uword, T1> &in_ri,
    const Base<uword, T2> &in_ci, const bool in_all_rows,
    const bool in_all_cols)
    : m(in_m), base_ri(in_ri), base_ci(in_ci), all_rows(in_all_rows),
...
inline void subview_elem2<eT, T1, T2>::inplace_op(const Base<eT, expr> &x) {
  ...
  if ((all_rows == false) && (all_cols == false)) {
    ...
    arma_debug_assert_same_size(ri_n_elem, ci_n_elem, X.n_rows, X.n_cols,
                                "Mat::elem()");
    ...
    arma_debug_assert_same_size(m_n_rows, ci_n_elem, X.n_rows, X.n_cols,
                                "Mat::elem()");
    ...
    arma_debug_assert_same_size(ri_n_elem, m_n_cols, X.n_rows, X.n_cols,
                                "Mat::elem()");
...
template <typename eT, typename T1, typename T2>
inline void subview_elem2<eT, T1, T2>::replace(const eT old_val,
                                               const eT new_val) {
  arma_extra_debug_sigprint();
...
template <typename eT, typename T1, typename T2>
inline void subview_elem2<eT, T1, T2>::clamp(const eT min_val,
                                             const eT max_val) {
  arma_extra_debug_sigprint();
...
template <typename eT, typename T1, typename T2>
inline void
subview_elem2<eT, T1, T2>::extract(Mat<eT> &actual_out,
...
// TODO: implement a dedicated function instead of creating a temporary (but
// lots of potential aliasing issues)
template <typename eT, typename T1, typename T2>
