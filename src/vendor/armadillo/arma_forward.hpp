...
enum struct file_type : unsigned int {
  ...
  arma_ascii,  //!< Armadillo text format, with a header specifying matrix type
               //!< and size
  ...
  hdf5_binary_trans, //!< [NOTE: DO NOT USE - deprecated] as per hdf5_binary,
                     //!< but save/load the data with columns transposed to rows
  coord_ascii, //!< simple co-ordinate format for sparse matrices (indices start
               //!< at zero)
  ssv_ascii, //!< similar to csv_ascii; uses semicolon (;) instead of comma (,)
             //!< as the separator
};
