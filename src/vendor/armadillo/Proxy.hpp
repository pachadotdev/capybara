...
// pod_type         = the underlying type of elements if elem_type is
// std::complex stored_type      = the type of the Q object ea_type          =
// the type of the object that provides access to elements via operator[i]
// aligned_ea_type  = the type of the object that provides access to elements
// via at_alt(i)
//
// use_at           = boolean indicating whether at(row,col) must be used to get
// elements use_mp           = boolean indicating whether OpenMP can be used
// while processing elements has_subview      = boolean indicating whether the Q
// object has a subview
//
// is_row           = boolean indicating whether the Q object can be treated a
// row vector is_col           = boolean indicating whether the Q object can be
// treated a column vector is_xvec          = boolean indicating whether the Q
// object is a vector with unknown orientation
//
// Q                = object that can be unwrapped via the unwrap family of
// classes (ie. Q must be convertible to Mat)
//
...
//
// operator[i]      = linear element accessor; valid only if the 'use_at'
// boolean is false at(row,col)      = access elements via (row,col); valid only
// if the 'use_at' boolean is true at_alt(i)        = aligned linear element
// accessor; valid only if the 'use_at' boolean is false and is_aligned()
// returns true
//
// get_ea()         = return the object that provides linear access to elements
// via operator[i] get_aligned_ea() = return the object that provides linear
// access to elements via at_alt(i); valid only if is_aligned() returns true
//
// is_alias(X)      = return true/false indicating whether the Q object aliases
// matrix X has_overlap(X)   = return true/false indicating whether the Q object
// has overlap with subview X is_aligned()     = return true/false indicating
...
template <typename T1> struct Proxy_fixed {
  ...
  inline explicit Proxy_fixed(const T1 &A) : Q(A) {
...
template <typename eT> struct Proxy<Mat<eT>> {
  ...
  inline explicit Proxy(const Mat<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
  ...
  template <typename eT2> arma_inline bool is_alias(const Mat<eT2> &X) const {
    return (is_same_type<eT, eT2>::value) ? (void_ptr(&Q) == void_ptr(&X))
                                          : false;
  }
...
template <typename eT> struct Proxy<Col<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const Col<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename eT> struct Proxy<Row<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const Row<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename T1, typename gen_type> struct Proxy<Gen<T1, gen_type>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  template <typename eT2> constexpr bool is_alias(const Mat<eT2> &) const {
    return false;
  }
...
template <typename T1, typename eop_type> struct Proxy<eOp<T1, eop_type>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
struct Proxy<eGlue<T1, T2, eglue_type>> {
  ...
  arma_inline bool is_aligned() const {
    return (Q.P1.is_aligned() && Q.P2.is_aligned());
  }
};

template <typename T1, typename op_type> struct Proxy<Op<T1, op_type>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const Op<T1, op_type> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename T1, typename T2> struct Proxy<Glue<T1, T2, glue_min>> {
  ...
  static constexpr bool has_subview =
      (Proxy<T1>::has_subview || Proxy<T2>::has_subview);
  ...
  arma_inline bool is_aligned() const {
    return (P1.is_aligned() && P2.is_aligned());
  }
};

template <typename T1, typename T2> struct Proxy<Glue<T1, T2, glue_max>> {
...
template <typename T1> struct Proxy<CubeToMatOp<T1, op_vectorise_cube_col>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<elem_type> stored_type;
  typedef const elem_type *ea_type;
...
template <typename T1, typename op_type> struct Proxy<SpToDOp<T1, op_type>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<elem_type> stored_type;
  typedef const elem_type *ea_type;
...
template <typename T1> struct Proxy<SpToDOp<T1, op_nonzeros_spmat>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<elem_type> stored_type;
  typedef const elem_type *ea_type;
...
template <typename eT> struct Proxy<subview<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const subview<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
  ...
  template <typename eT2> arma_inline bool is_alias(const Mat<eT2> &X) const {
    return (is_same_type<eT, eT2>::value) ? (void_ptr(&(Q.m)) == void_ptr(&X))
                                          : false;
  }
...
template <typename eT> struct Proxy<subview_col<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const subview_col<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename eT> struct Proxy<subview_cols<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<eT> stored_type;
  typedef const eT *ea_type;
  ...
  template <typename eT2> arma_inline bool is_alias(const Mat<eT2> &X) const {
    return (is_same_type<eT, eT2>::value) ? (void_ptr(&(sv.m)) == void_ptr(&X))
                                          : false;
  }
...
template <typename eT> struct Proxy<subview_row<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const subview_row<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename eT, typename T1> struct Proxy<subview_elem1<eT, T1>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  arma_inline elem_type operator[](const uword i) const {
    ...
    arma_debug_check_bounds((ii >= Q.m.n_elem),
...
template <typename eT> struct Proxy<diagview<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  inline explicit Proxy(const diagview<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename T1> struct Proxy_diagvec_mat {
  inline Proxy_diagvec_mat(const T1 &) {}
};

template <typename T1> struct Proxy_diagvec_mat<Op<T1, op_diagvec>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
...
template <typename T1> struct Proxy_diagvec_expr {
  inline Proxy_diagvec_expr(const T1 &) {}
};

template <typename T1> struct Proxy_diagvec_expr<Op<T1, op_diagvec>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<elem_type> stored_type;
  typedef const elem_type *ea_type;
...
struct Proxy<Op<T1, op_diagvec>>
    : public Proxy_diagvec_redirect<Op<T1, op_diagvec>,
                                    is_Mat<T1>::value>::result {
  typedef
      typename Proxy_diagvec_redirect<Op<T1, op_diagvec>,
                                      is_Mat<T1>::value>::result Proxy_diagvec;
...
template <typename T1> struct Proxy<Op<T1, op_diagvec2>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<elem_type> stored_type;
  typedef const elem_type *ea_type;
...
template <typename T1> struct Proxy_xtrans_vector<Op<T1, op_htrans>> {
  ...
  arma_aligned const quasi_unwrap<T1>
      U; // avoid copy if T1 is a Row, Col or subview_col
  arma_aligned const Mat<elem_type> Q;

  inline Proxy_xtrans_vector(const Op<T1, op_htrans> &A)
      : U(A.m), Q(const_cast<elem_type *>(U.M.memptr()), U.M.n_cols, U.M.n_rows,
                  false, false) {
    arma_extra_debug_sigprint();
  }
...
struct Proxy<Op<T1, op_strans>>
  ...
  typedef typename Proxy_xtrans_redirect<
      Op<T1, op_strans>, ((Op<T1, op_strans>::is_row) ||
...
template <typename eT> struct Proxy_subview_row_htrans_cx {
  ...
  inline explicit Proxy_subview_row_htrans_cx(
      const Op<subview_row<eT>, op_htrans> &A)
      : Q(A.m) {
    arma_extra_debug_sigprint();
  }
...
template <typename eT> struct Proxy<Op<subview_row<eT>, op_strans>> {
  typedef eT elem_type;
  typedef typename get_pod_type<eT>::result pod_type;
  typedef subview_row_strans<eT> stored_type;
  typedef const subview_row_strans<eT> &ea_type;
...
template <typename T> struct Proxy<Op<Row<std::complex<T>>, op_htrans>> {
...
template <typename T> struct Proxy<Op<Col<std::complex<T>>, op_htrans>> {
  typedef typename std::complex<T> eT;
...
template <typename T1> struct Proxy<Op<T1, op_htrans2>> {
  typedef typename T1::elem_type elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  static constexpr bool use_at =
      eOp<Op<T1, op_htrans>, eop_scalar_times>::use_at;
  static constexpr bool use_mp =
      eOp<Op<T1, op_htrans>, eop_scalar_times>::use_mp;
  static constexpr bool has_subview =
      eOp<Op<T1, op_htrans>, eop_scalar_times>::has_subview;

  // NOTE: the Op class takes care of swapping row and col for op_htrans
  static constexpr bool is_row =
      eOp<Op<T1, op_htrans>, eop_scalar_times>::is_row;
  static constexpr bool is_col =
      eOp<Op<T1, op_htrans>, eop_scalar_times>::is_col;
  static constexpr bool is_xvec =
      eOp<Op<T1, op_htrans>, eop_scalar_times>::is_xvec;
...
template <typename eT> struct Proxy<subview_row_strans<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef subview_row_strans<eT> stored_type;
  typedef const subview_row_strans<eT> &ea_type;
...
template <typename eT> struct Proxy<subview_row_htrans<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef subview_row_htrans<eT> stored_type;
  typedef const subview_row_htrans<eT> &ea_type;
...
template <typename eT, bool do_conj> struct Proxy<xtrans_mat<eT, do_conj>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<eT> stored_type;
  typedef const eT *ea_type;
...
template <typename eT> struct Proxy<xvec_htrans<eT>> {
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  typedef Mat<eT> stored_type;
  typedef const eT *ea_type;
  ...
  inline explicit Proxy(const xvec_htrans<eT> &A) : Q(A) {
    arma_extra_debug_sigprint();
  }
...
template <typename T1> struct Proxy_vectorise_col_mat {
...
template <typename T1> struct Proxy_vectorise_col_expr {
...
struct Proxy<Op<T1, op_vectorise_col>>
  ...
  typedef typename Proxy_vectorise_col_redirect<Op<T1, op_vectorise_col>,
                                                (Proxy<T1>::use_at)>::result
      Proxy_vectorise_col;
