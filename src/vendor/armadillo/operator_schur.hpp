...
// operator %, which we define it to do a schur product (element-wise
// multiplication)

//! element-wise multiplication of user-accessible Armadillo objects with same
//! element type
...
//! element-wise multiplication of user-accessible Armadillo objects with
//! different element types
template <typename T1, typename T2>
inline typename enable_if2<
    (is_arma_type<T1>::value && is_arma_type<T2>::value &&
     (is_same_type<typename T1::elem_type, typename T2::elem_type>::no)),
    const mtGlue<typename promote_type<typename T1::elem_type,
                                       typename T2::elem_type>::result,
                 T1, T2, glue_mixed_schur>>::result
operator%(const T1 &X, const T2 &Y) {
  arma_extra_debug_sigprint();
...
operator%(const T1 &x, const T2 &y) {
  ...
  // TODO: if there is a matrix size mismatch, the debug assert will print the
  // matrix sizes in wrong order
  spglue_schur_misc::dense_schur_sparse(out, y, x);

  return out;
}

//! optimization: sparse % (sparse +/- scalar) can be done without forming the
//! dense result of the (sparse +/- scalar) term
...
//! optimization: (sparse +/- scalar) % sparse can be done without forming the
//! dense result of the (sparse +/- scalar) term
template <typename T1, typename T2, typename op_type>
inline typename enable_if2<
    (is_arma_sparse_type<T1>::value && is_arma_sparse_type<T2>::value &&
...
//! element-wise multiplication of two sparse objects with different element
//! types
template <typename T1, typename T2>
inline typename enable_if2<
    (is_arma_sparse_type<T1>::value && is_arma_sparse_type<T2>::value &&
     is_same_type<typename T1::elem_type, typename T2::elem_type>::no),
    const mtSpGlue<typename promote_type<typename T1::elem_type,
                                         typename T2::elem_type>::result,
                   T1, T2, spglue_schur_mixed>>::result
operator%(const T1 &X, const T2 &Y) {
  arma_extra_debug_sigprint();
...
//! element-wise multiplication of one dense and one sparse object with
//! different element types
template <typename T1, typename T2>
inline typename enable_if2<
    (is_arma_type<T1>::value && is_arma_sparse_type<T2>::value &&
...
//! element-wise multiplication of one sparse and one dense object with
//! different element types
template <typename T1, typename T2>
inline typename enable_if2<
    (is_arma_sparse_type<T1>::value && is_arma_type<T2>::value &&
...
operator%(const subview_each1<parent, mode> &X,
...
arma_inline Mat<typename parent::elem_type>
operator%(const Base<typename parent::elem_type, T1> &X,
          const subview_each1<parent, mode> &Y) {
  arma_extra_debug_sigprint();

  return subview_each1_aux::operator_schur(Y,
                                           X.get_ref()); // NOTE: swapped order
}

template <typename parent, unsigned int mode, typename TB, typename T2>
inline Mat<typename parent::elem_type>
operator%(const subview_each2<parent, mode, TB> &X,
          const Base<typename parent::elem_type, T2> &Y) {
  arma_extra_debug_sigprint();
...
operator%(const Base<typename parent::elem_type, T1> &X,
  ...
  return subview_each2_aux::operator_schur(Y,
                                           X.get_ref()); // NOTE: swapped order
}
