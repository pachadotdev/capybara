    ...
    if ((A.n_elem <= threshold)) {
...
#if defined(ARMA_USE_ATLAS)
      {
        ...
        atlas::cblas_herk<T>(
            ...
            (do_trans_A) ? atlas_CblasConjTrans : atlas_CblasNoTrans, C.n_cols,
            (do_trans_A) ? A.n_rows : A.n_cols, (use_alpha) ? alpha : T(1),
            A.mem, (do_trans_A) ? A.n_rows : C.n_cols, (use_beta) ? beta : T(0),
            C.memptr(), C.n_cols);
...
#elif defined(ARMA_USE_BLAS)
      {
        ...
        arma_extra_debug_print(arma_str::format("blas::herk(): trans_A = %c") %
                               trans_A);
