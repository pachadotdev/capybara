...
//! the operand (eg. the matrix to which the operation is to be applied) and the
//! unary operator (eg. inverse). The operand is stored as a reference (which
//! can be optimised away), while the operator is "stored" through the template
//! definition (op_type). The operands can be 'SpMat', 'SpRow', 'SpCol', 'SpOp',
//! and 'SpGlue'. Note that as 'SpGlue' can be one of the operands, more than
//! one matrix can be stored.
//!
...
public:
  ...
  arma_aligned elem_type
      aux; //!< auxiliary data, using the element type as used by T1
