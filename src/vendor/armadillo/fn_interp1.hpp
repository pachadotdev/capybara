...
template <typename eT>
inline void interp1_helper_linear(const Mat<eT> &XG, const Mat<eT> &YG,
                                  const Mat<eT> &XI, Mat<eT> &YI,
                                  const eT extrap_val) {
  arma_extra_debug_sigprint();
  ...
  for (uword i = 0; i < NI; ++i) {
    ...
    if ((XI_val < XG_min) || (XI_val > XG_max)) {
      ...
      YI_mem[i] =
          (eT(1) - weight) * YG_mem[a_best_j] + (weight)*YG_mem[b_best_j];
    }
  }
}

template <typename eT>
inline void interp1_helper(const Mat<eT> &X, const Mat<eT> &Y,
                           const Mat<eT> &XI, Mat<eT> &YI, const uword sig,
                           const eT extrap_val) {
  arma_extra_debug_sigprint();

  arma_debug_check(((X.is_vec() == false) || (Y.is_vec() == false) ||
                    (XI.is_vec() == false)),
  ...
  // NOTE: find_unique(X,false) provides indices of elements sorted in ascending
  // order NOTE: find_unique(X,false) will reset X_indices if X has NaN

  const uword N_subset = X_indices.n_elem;

  arma_debug_check((N_subset < 2),
                   "interp1(): X must have at least two unique elements");
...
interp1(const Base<typename T1::elem_type, T1> &X,
        const Base<typename T1::elem_type, T2> &Y,
        const Base<typename T1::elem_type, T3> &XI,
        Mat<typename T1::elem_type> &YI, const char *method = "linear",
