...
inline void gmm_full<eT>::set_params(const Base<eT, T1> &in_means_expr,
  ...
  arma_debug_check((in_means.n_cols != in_fcovs.n_slices) ||
                       (in_means.n_rows != in_fcovs.n_rows) ||
                       (in_fcovs.n_rows != in_fcovs.n_cols) ||
                       (in_hefts.n_cols != in_means.n_cols) ||
                       (in_hefts.n_rows != 1),
                   "gmm_full::set_params(): given parameters have inconsistent "
                   "and/or wrong sizes");
  ...
  arma_debug_check(
      (in_fcovs.internal_has_nonfinite()),
  ...
  for (uword g = 0; g < in_fcovs.n_slices; ++g) {
    arma_debug_check((any(diagvec(in_fcovs.slice(g)) <= eT(0))),
                     "gmm_full::set_params(): given fcovs have negative or "
...
inline void gmm_full<eT>::set_fcovs(const BaseCube<eT, T1> &in_fcovs_expr) {
  ...
  for (uword i = 0; i < in_fcovs.n_slices; ++i) {
    arma_debug_check((any(diagvec(in_fcovs.slice(i)) <= eT(0))),
                     "gmm_full::set_fcovs(): given fcovs have negative or zero "
                     "values on diagonals");
  }
...
template <typename eT> inline uword gmm_full<eT>::n_dims() const {
  return means.n_rows;
}

template <typename eT> inline uword gmm_full<eT>::n_gaus() const {
  return means.n_cols;
}
...
inline eT gmm_full<eT>::log_p(
  ...
  arma_debug_check((U.M.n_rows != N_dims),
...
template <typename eT>
template <typename T1>
inline eT gmm_full<eT>::sum_log_p(const Base<eT, T1> &expr,
...
template <typename eT>
template <typename T1>
inline eT gmm_full<eT>::avg_log_p(const Base<eT, T1> &expr,
                                  const uword gaus_id) const {
  arma_extra_debug_sigprint();
...
inline bool gmm_full<eT>::learn(const Base<eT, T1> &data, const uword N_gaus,
  ...
  arma_debug_check((seed_mode_ok == false),
                   "gmm_full::learn(): unknown seed_mode");
  ...
  if (X.internal_has_nonfinite()) {
    arma_debug_warn_level(
        3, "gmm_full::learn(): given matrix has non-finite values");
    return false;
  }
  ...
  if (seed_mode == keep_existing) {
    ...
    if (X.n_cols < N_gaus) {
      arma_debug_warn_level(3, "gmm_full::learn(): number of vectors is less "
  ...
  if (km_iter > 0) {
    ...
    if (status == false) {
      arma_debug_warn_level(
          3, "gmm_full::learn(): k-means algorithm failed; not enough "
  ...
  if (seed_mode != keep_existing) {
    if (print_mode) {
      get_cout_stream()
          << "gmm_full::learn(): generating initial covariances\n";
      get_cout_stream().flush();
    }
...
inline void gmm_full<eT>::init_constants(const bool calc_chol) {
  ...
  for (uword g = 0; g < N_gaus; ++g) {
    ...
    const bool log_det_ok = (log_det_status && (arma_isfinite(log_det_val)) &&
                             (log_det_sign > eT(0)));
...
inline umat gmm_full<eT>::internal_gen_boundaries(const uword N) const {
  ...
  // get_cout_stream() << "gmm_full::internal_gen_boundaries(): n_threads: " <<
  // n_threads
  // << '\n';
...
template <typename eT>
inline eT gmm_full<eT>::internal_scalar_log_p(const eT *x,
                                              const uword g) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline eT gmm_full<eT>::internal_sum_log_p(const Mat<eT> &X,
                                           const uword gaus_id) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline eT gmm_full<eT>::internal_avg_log_p(const Mat<eT> &X,
                                           const uword gaus_id) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline uword
gmm_full<eT>::internal_scalar_assign(const Mat<eT> &X,
                                     const gmm_dist_mode &dist_mode) const {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check((X.n_rows != N_dims),
                   "gmm_full::assign(): incompatible dimensions");
...
template <typename eT>
inline void
gmm_full<eT>::internal_vec_assign(urowvec &out, const Mat<eT> &X,
                                  const gmm_dist_mode &dist_mode) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void
gmm_full<eT>::internal_raw_hist(urowvec &hist, const Mat<eT> &X,
                                const gmm_dist_mode &dist_mode) const {
  arma_extra_debug_sigprint();
...
inline void
gmm_full<eT>::generate_initial_means(const Mat<eT> &X,
                                     const gmm_seed_mode &seed_mode) {
  arma_extra_debug_sigprint();
...
template <typename eT>
template <uword dist_id>
inline void gmm_full<eT>::generate_initial_params(const Mat<eT> &X,
                                                  const eT var_floor) {
  arma_extra_debug_sigprint();
...
#pragma omp parallel for schedule(static)
  ...
  for (uword g = 0; g < N_gaus; ++g) {
    ...
    for (uword d = 0; d < N_dims; ++d) {
      ...
      fcov.at(d, d) = (acc_heft >= 2)
                          ? eT((acc_dcov[d] / eT(acc_heft)) - (tmp * tmp))
                          : eT(var_floor);
    }
...
#pragma omp parallel for schedule(static)
    ...
    if (dead_gs.n_elem > 0) {
      if (verbose) {
        get_cout_stream()
            << "gmm_full::learn(): k-means: recovering from dead means\n";
        get_cout_stream().flush();
      }
...
template <typename eT>
inline void gmm_full<eT>::em_update_params(
    const Mat<eT> &X, const umat &boundaries, field<Mat<eT>> &t_acc_means,
    field<Cube<eT>> &t_acc_fcovs, field<Col<eT>> &t_acc_norm_lhoods,
...
#pragma omp parallel for schedule(static)
    for (uword t = 0; t < n_threads; t++) {
      ...
      em_generate_acc(X, boundaries.at(0, t), boundaries.at(1, t), acc_means,
                      acc_fcovs, acc_norm_lhoods, gaus_log_lhoods,
                      progress_log_lhood);
    }
  }
#else
  ...
  for (uword g = 0; g < N_gaus; ++g) {
    ...
    const bool inv_ok =
        (log_det_ok) ? bool(auxlib::inv_sympd(mean_outer, acc_fcov))
...
template <typename eT>
inline void gmm_full<eT>::em_generate_acc(
    const Mat<eT> &X, const uword start_index, const uword end_index,
    Mat<eT> &acc_means, Cube<eT> &acc_fcovs, Col<eT> &acc_norm_lhoods,
