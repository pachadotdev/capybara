...
inline void glue_hist::apply_noalias(Mat<uword> &out, const Mat<eT> &X,
                                     const Mat<eT> &C, const uword dim) {
  ...
  arma_debug_check(
      ...
      "hist(): given 'centers' vector does not contain monotonically "
      "increasing values");
...
template <typename T1, typename T2>
inline void
glue_hist_default::apply(Mat<uword> &out,
  ...
  const uword dim = (T1::is_xvec) ? uword(UA.M.is_rowvec() ? 1 : 0)
                                  : uword((T1::is_row) ? 1 : 0);
