...
inline void op_mean::apply(Mat<typename T1::elem_type> &out,
...
inline void op_mean::apply_noalias_unwrap(Cube<typename T1::elem_type> &out,
  ...
  if (dim == 0) {
    ...
    for (uword slice = 0; slice < X_n_slices; ++slice) {
      ...
      for (uword col = 0; col < X_n_cols; ++col) {
        out_mem[col] =
            op_mean::direct_mean(X.slice_colptr(slice, col), X_n_rows);
      }
    }
  } else if (dim == 1) {
...
inline eT op_mean::direct_mean(const eT *const X, const uword n_elem) {
  ...
  return arma_isfinite(result) ? result
                               : op_mean::direct_mean_robust(X, n_elem);
}
...
inline eT op_mean::direct_mean_robust(const eT *const X, const uword n_elem) {
  ...
  // use an adapted form of the mean finding algorithm from the running_stat
  // class
  ...
  for (i = 0, j = 1; j < n_elem; i += 2, j += 2) {
    ...
    r_mean = r_mean + (Xi - r_mean) /
                          T(j); // we need i+1, and j is equivalent to i+1 here
