...
mvnrnd(Mat<typename T1::elem_type> &out,
       const Base<typename T1::elem_type, T1> &M,
       const Base<typename T1::elem_type, T2> &C) {
  arma_extra_debug_sigprint();

  const bool status =
      glue_mvnrnd::apply_direct(out, M.get_ref(), C.get_ref(), uword(1));

  if (status == false) {
    out.soft_reset();
    arma_debug_warn_level(3, "mvnrnd(): given covariance matrix is not "
...
mvnrnd(Mat<typename T1::elem_type> &out,
  ...
  const bool status =
      glue_mvnrnd::apply_direct(out, M.get_ref(), C.get_ref(), N);
