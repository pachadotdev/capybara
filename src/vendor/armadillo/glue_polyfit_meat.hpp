...
inline bool glue_polyfit::apply_noalias(Mat<eT> &out, const Col<eT> &X,
                                        const Col<eT> &Y, const uword N) {
...
inline bool
glue_polyfit::apply_direct(Mat<typename T1::elem_type> &out,
  ...
  const Col<eT> X_as_colvec(const_cast<eT *>(X.memptr()), X.n_elem, false,
                            false);
  const Col<eT> Y_as_colvec(const_cast<eT *>(Y.memptr()), Y.n_elem, false,
                            false);
...
inline void glue_polyfit::apply(Mat<typename T1::elem_type> &out,
  ...
  const bool status =
      glue_polyfit::apply_direct(out, expr.A, expr.B, expr.aux_uword);
