...
// This class uses T1::insert_element(), T1::delete_element(),
// T1::invalidate_cache()
...
public:
   ...
   */
  arma_inline SpValProxy(uword row, uword col, T1 &in_parent,
                         eT *in_val_ptr = nullptr);
...
private:
  // Deletes the element if it is zero; NOTE: does not check if val_ptr ==
  // nullptr
  ...
  arma_aligned T1 &parent; // We will call this object if we need to insert or
                           // delete an element.
};
