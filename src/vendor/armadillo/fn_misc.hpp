...
arma_warn_unused inline
    typename enable_if2<is_Mat<out_type>::value, out_type>::result
    linspace(const typename out_type::pod_type start,
             const typename out_type::pod_type end, const uword num = 100u) {
  ...
  if (num == 1) {
    ...
    if (is_non_integral<T>::value) {
      ...
      const double delta = (end >= start)
                               ? double(end - start) / double(num_m1)
...
template <typename out_type>
arma_warn_unused inline
    typename enable_if2<(is_Mat<out_type>::value &&
                         is_real<typename out_type::pod_type>::value),
                        out_type>::result
    logspace(const typename out_type::pod_type A,
             const typename out_type::pod_type B, const uword N = 50u) {
  arma_extra_debug_sigprint();
...
arma_warn_unused inline typename arma_real_only<eT>::result
...
template <typename eT>
arma_warn_unused inline typename arma_real_only<eT>::result log_add(eT log_a,
                                                                    eT log_b) {
...
is_finite(const eT x,
          const typename arma_scalar_only<eT>::result *junk = nullptr) {
  arma_ignore(junk);
...
arma_warn_unused inline uvec ind2sub(const SizeMat &s, const uword i) {
  ...
  arma_debug_check((i >= (s_n_rows * s.n_cols)),
...
arma_warn_unused inline uvec ind2sub(const SizeCube &s, const uword i) {
  ...
  arma_debug_check((i >= (s_n_elem_slice * s.n_slices)),
                   "ind2sub(): index out of range");
...
arma_warn_unused arma_inline uword sub2ind(const SizeCube &s, const uword row,
  ...
  arma_debug_check(
      ((row >= s_n_rows) || (col >= s_n_cols) || (slice >= s.n_slices)),
      "sub2ind(): subscript out of range");
...
arma_warn_unused inline uvec sub2ind(const SizeCube &s,
  ...
  for (uword count = 0; count < U_M_n_cols; ++count) {
    ...
    arma_debug_check(
        ((row >= s_n_rows) || (col >= s_n_cols) || (slice >= s_n_slices)),
        "sub2ind(): subscript out of range");

    out_mem[count] =
        uword((slice * s_n_rows * s_n_cols) + (col * s_n_rows) + row);
  }
