...
inline eT spop_mean::direct_mean(const eT *const X, const uword length,
                                 const uword N) {
  arma_extra_debug_sigprint();

  typedef typename get_pod_type<eT>::result T;

  const eT result = ((length > 0) && (N > 0))
                        ? eT(arrayops::accumulate(X, length) / T(N))
                        : eT(0);

  return arma_isfinite(result) ? result
                               : spop_mean::direct_mean_robust(X, length, N);
}
...
spop_mean::mean_all(const SpBase<typename T1::elem_type, T1> &X) {
  ...
  } else // use_iterator == false; that is, we can directly access the values
         // array
  {
    return spop_mean::direct_mean(p.get_values(), p.get_n_nonzero(),
                                  p.get_n_elem());
  }
}
...
template <typename T1, typename eT>
inline eT spop_mean::iterator_mean_robust(T1 &it, const T1 &end,
                                          const uword n_zero, const eT junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
