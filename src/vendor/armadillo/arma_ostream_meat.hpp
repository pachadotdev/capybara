...
inline std::streamsize arma_ostream::modify_stream(std::ostream &o,
  ...
  for (uword i = 0; i < n_elem; ++i) {
    ...
    if ((val >= eT(+100)) ||
        ...
        //) || ( (is_non_integral<eT>::value) && (is_signed<eT>::value) && (val
        //< eT(0)) && (val >= eT(-1e-4)) )
...
inline std::streamsize arma_ostream::modify_stream(
    std::ostream &o, typename SpMat<eT>::const_iterator begin,
    const uword n_elem, const typename arma_not_cx<eT>::result *junk) {
  ...
  for (typename SpMat<eT>::const_iterator it = begin; it.pos() < n_elem; ++it) {
    ...
    if (val >= eT(+100) || ((is_signed<eT>::value) && (val <= eT(-100))) ||
        ((is_non_integral<eT>::value) && (val > eT(0)) && (val <= eT(+1e-4))) ||
        ((is_non_integral<eT>::value) && (is_signed<eT>::value) &&
...
inline std::streamsize arma_ostream::modify_stream(
    std::ostream &o, typename SpMat<eT>::const_iterator begin,
    const uword n_elem, const typename arma_cx_only<eT>::result *junk) {
  arma_ignore(begin);
  arma_ignore(n_elem);
  arma_ignore(junk);
...
template <typename eT>
inline void arma_ostream::print_elem(std::ostream &o, const eT &x,
                                     const bool modify) {
  if (x == eT(0)) {
...
template <typename T>
inline void arma_ostream::raw_print_elem(std::ostream &o,
...
template <typename eT>
inline void arma_ostream::print(std::ostream &o, const Mat<eT> &m,
                                const bool modify) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void arma_ostream::print(std::ostream &o, const Cube<eT> &x,
                                const bool modify) {
  arma_extra_debug_sigprint();
  ...
  if (x.is_empty() == false) {
    for (uword slice = 0; slice < x.n_slices; ++slice) {
      const Mat<eT> tmp(const_cast<eT *>(x.slice_memptr(slice)), x.n_rows,
                        x.n_cols, false);
...
//! Assumes type oT can be printed, ie. oT has std::ostream& operator<<
//! (std::ostream&, const oT&)
...
template <typename eT>
inline void arma_ostream::print(std::ostream &o, const SpMat<eT> &m,
                                const bool modify) {
  arma_extra_debug_sigprint();
  ...
  const double density =
      (m.n_elem > 0) ? (double(m_n_nonzero) / double(m.n_elem) * double(100))
                     : double(0);

  o << "[matrix size: " << m.n_rows << 'x' << m.n_cols
    << "; n_nonzero: " << m_n_nonzero;
  ...
  if (density == double(0)) {
    ...
    o.precision(1);
             ...
             (double(0.01) - std::numeric_limits<double>::epsilon())) {
    o.precision(2);
  } else if (density >
             (double(0.001) - std::numeric_limits<double>::epsilon())) {
    o.precision(3);
  } else if (density >
             (double(0.0001) - std::numeric_limits<double>::epsilon())) {
    o.precision(4);
  } else {
    o.unsetf(ios::fixed);
    o.setf(ios::scientific);
  ...
  if (m_n_nonzero > 0) {
    const std::streamsize cell_width =
        modify ? arma_ostream::modify_stream<eT>(o, m.begin(), m_n_nonzero)
               : o.width();
    ...
    while (it != it_end) {
      ...
      // TODO: change the maximum number of spaces before and after each
      // location to be dependent on n_rows and n_cols
