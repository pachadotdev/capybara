...
arma_warn_unused arma_inline const Gen<vec, gen_zeros>
...
arma_warn_unused arma_inline const Gen<obj_type, gen_zeros>
zeros(const uword n_elem, const arma_empty_class junk1 = arma_empty_class(),
...
arma_warn_unused arma_inline const Gen<mat, gen_zeros>
...
arma_warn_unused arma_inline const
    GenCube<typename cube_type::elem_type, gen_zeros>
    zeros(const uword n_rows, const uword n_cols, const uword n_slices,
          const typename arma_Cube_only<cube_type>::result *junk = nullptr) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);

  return GenCube<typename cube_type::elem_type, gen_zeros>(n_rows, n_cols,
                                                           n_slices);
}
...
zeros(const uword n_rows, const uword n_cols,
      const typename arma_SpMat_SpCol_SpRow_only<sp_obj_type>::result *junk =
          nullptr) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
