...
inline typename T1::elem_type spop_min::vector_min(
  ...
  if (SpProxy<T1>::use_iterator == false) {
    ...
    if (p.get_n_nonzero() == p.get_n_elem()) {
      ...
      return (std::min)(eT(0),
                        op_min::direct_min(p.get_values(), p.get_n_nonzero()));
    }
  } else {
    // use iterator
    typename SpProxy<T1>::const_iterator_type it = p.begin();
...
spop_min::min_with_index(const SpProxy<T1> &P, uword &index_of_min_val) {
  ...
  if (n_elem != n_nonzero) {
    ...
    // If the min_val is a nonzero element, we need its actual position in the
    // matrix.
    if (min_val == eT(0)) {
...
spop_min::min_with_index(const SpProxy<T1> &P, uword &index_of_min_val) {
  ...
  if (SpProxy<T1>::use_iterator) {
    ...
    // We can do direct access.
    min_val = std::abs(
        op_min::direct_min(P.get_values(), n_nonzero, index_of_min_val));
