...
public:
  template <const uword row, const uword col> struct pos {
    static constexpr uword n2 =
        (do_trans_A == false) ? (row + col * 2) : (col + row * 2);
    static constexpr uword n3 =
        (do_trans_A == false) ? (row + col * 3) : (col + row * 3);
    static constexpr uword n4 =
        (do_trans_A == false) ? (row + col * 4) : (col + row * 4);
  };

  template <typename eT, const uword i>
  arma_inline static void assign(eT *y, const eT acc, const eT alpha,
                                 const eT beta) {
    if (use_beta == false) {
  ...
  arma_cold inline static void apply(eT *y, const TA &A, const eT *x,
    ...
    case 3: {
      ...
      const eT acc0 = Am[pos<0, 0>::n3] * x0 + Am[pos<0, 1>::n3] * x1 +
                      Am[pos<0, 2>::n3] * x2;
      const eT acc1 = Am[pos<1, 0>::n3] * x0 + Am[pos<1, 1>::n3] * x1 +
                      Am[pos<1, 2>::n3] * x2;
      const eT acc2 = Am[pos<2, 0>::n3] * x0 + Am[pos<2, 1>::n3] * x1 +
                      Am[pos<2, 2>::n3] * x2;
...
public:
  template <typename eT, typename TA>
  arma_hot inline static typename arma_not_cx<eT>::result
...
//! 'y' is assumed to have been set to the correct size (ie. taking into account
//! the transpose)
...
//! \brief
//! Wrapper for BLAS gemv function, using template arguments to control the
//! arguments passed to gemv. 'y' is assumed to have been set to the correct
//! size (ie. taking into account the transpose)
...
public:
  ...
  inline static void apply_blas_type(eT *y, const TA &A, const eT *x,
    ...
    if ((A.n_rows <= 4) && (A.n_rows == A.n_cols) && (is_cx<eT>::no)) {
      gemv_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(y, A, x, alpha,
                                                               beta);
    } else {
#if defined(ARMA_USE_ATLAS)
      {
        ...
        if (is_cx<eT>::no) {
          ...
          atlas::cblas_gemm<eT>(
              ...
              A.mem, A.n_rows, x, (do_trans_A) ? A.n_rows : A.n_cols,
              (use_beta) ? beta : eT(0), y, (do_trans_A) ? A.n_cols : A.n_rows);
        } else {
...
#elif defined(ARMA_USE_BLAS)
      {
        ...
        arma_extra_debug_print(arma_str::format("blas::gemv(): trans_A = %c") %
                               trans_A);
  ...
  template <typename eT, typename TA>
  arma_inline static void apply(eT *y, const TA &A, const eT *x,
  ...
  arma_inline static void apply(float *y, const TA &A, const float *x,
    ...
    gemv<do_trans_A, use_alpha, use_beta>::apply_blas_type(y, A, x, alpha,
                                                           beta);
  }
  ...
  apply(std::complex<float> *y, const TA &A, const std::complex<float> *x,
  ...
  template <typename TA>
  arma_inline static void
  apply(std::complex<double> *y, const TA &A, const std::complex<double> *x,
