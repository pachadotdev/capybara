...
arma_warn_unused arma_inline
    typename enable_if2<is_arma_type<T1>::value,
                        const Glue<T1, T2, glue_powext>>::result
...
arma_warn_unused inline Mat<typename parent::elem_type>
pow(const subview_each1<parent, mode> &X,
...
template <typename T1, typename T2>
arma_warn_unused arma_inline const GlueCube<T1, T2, glue_powext>
pow(const BaseCube<typename T1::elem_type, T1> &X,
...
arma_warn_unused inline
    typename enable_if2<is_cx<typename parent::elem_type>::yes,
...
arma_warn_unused arma_inline const
  ...
  return mtGlueCube<typename T1::elem_type, T1, T2, glue_powext_cx>(
      X.get_ref(), Y.get_ref());
}

template <typename T, typename T2>
arma_warn_unused inline Cube<std::complex<T>>
pow(const subview_cube_each1<std::complex<T>> &X, const Base<T, T2> &Y) {
  arma_extra_debug_sigprint();
