...
// computationally inexpensive algorithm to guess whether a matrix is positive
// definite: (1) ensure the matrix is symmetric/hermitian (within a tolerance)
...
// 2*abs(real(A_ij)) the above conditions are necessary, but not sufficient;
...
inline typename enable_if2<is_cx<eT>::no, bool>::result
guess_sympd_worker(const Mat<eT> &A) {
  ...
  const eT tol =
      eT(100) * std::numeric_limits<eT>::epsilon(); // allow some leeway
...
template <typename eT>
inline typename enable_if2<is_cx<eT>::yes, bool>::result
guess_sympd_worker(const Mat<eT> &A) {
  arma_extra_debug_sigprint();
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      ...
      // std::sqrt()
      const T square_A_ij_abs =
          (A_ij_real * A_ij_real) + (A_ij_imag * A_ij_imag);
      ...
      const T A_imag_delta = (std::abs)(
          A_ij_imag + A_ji_imag); // take into account complex conjugate
...
template <typename eT> inline bool guess_sympd(const Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline typename enable_if2<is_cx<eT>::no, void>::result
analyse_matrix_worker(bool &is_approx_sym, bool &is_approx_sympd,
                      const Mat<eT> &A) {
  arma_extra_debug_sigprint();
  ...
  for (uword j = 0; j < Nm1; ++j) {
    ...
    for (uword i = jp1; i < N; ++i) {
      ...
      if (is_approx_sympd) {
        // if( (A_ij_abs >= max_diag) || (A_ji_abs >= max_diag) )  {
        // is_approx_sympd = false; }
        if (A_ij_abs >= max_diag) {
          is_approx_sympd = false;
        }
...
template <typename eT>
inline typename enable_if2<is_cx<eT>::yes, void>::result
analyse_matrix_worker(bool &is_approx_sym, bool &is_approx_sympd,
                      const Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void analyse_matrix(bool &is_approx_sym, bool &is_approx_sympd,
                           const Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline bool check_diag_imag(const Mat<eT> &A) {
  arma_extra_debug_sigprint();
