...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    if (A.is_square() == false) {
      arma_debug_warn_level(
          1, "spsolve_factoriser::factorise(): solving under-determined / "
    ...
    if ((opts.pivot_thresh < double(0)) || (opts.pivot_thresh > double(1))) {
      arma_debug_warn_level(1, "spsolve_factoriser::factorise(): pivot_thresh "
    ...
    if (worker_ptr == nullptr) {
      arma_debug_warn_level(
          3,
          "spsolve_factoriser::factorise(): could not construct worker object");
      return false;
    }
...
inline bool spsolve_factoriser::solve(
...
#if defined(ARMA_USE_SUPERLU)
  {
    ...
    if (worker_ptr == nullptr) {
      arma_debug_warn_level(
          2, "spsolve_factoriser::solve(): no factorisation available");
      X.soft_reset();
      return false;
    }
    ...
    if (type_mismatch) {
      arma_debug_warn_level(
          1, "spsolve_factoriser::solve(): matrix type mismatch");
      X.soft_reset();
      return false;
    }
    ...
    if (n_rows != B.n_rows) {
      arma_debug_warn_level(
          1, "spsolve_factoriser::solve(): matrix size mismatch");
      X.soft_reset();
      return false;
    }
    ...
    if (status == false) {
      arma_debug_warn_level(3,
                            "spsolve_factoriser::solve(): solution not found");
      X.soft_reset();
      return false;
    }
...
#else
  {
    ...
    arma_ignore(B_expr);
    arma_stop_logic_error(
        "spsolve_factoriser::solve(): use of SuperLU must be enabled");
    return false;
  }
#endif
}
