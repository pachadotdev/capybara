...
inline Mat<eT>::Mat()
    : n_rows(0), n_cols(0), n_elem(0), n_alloc(0), vec_state(0), mem_state(0),
      mem() {
  arma_extra_debug_sigprint_this(this);
}
...
inline Mat<eT>::Mat(const uword in_n_rows, const uword in_n_cols)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols),
      n_alloc(), vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const SizeMat &s)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem(s.n_rows * s.n_cols),
      n_alloc(), vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! construct the matrix to have user specified dimensions and fill with
//! specified pattern
template <typename eT>
...
inline Mat<eT>::Mat(const arma_vec_indicator &, const uhword in_vec_state)
    : n_rows((in_vec_state == 2) ? 1 : 0), n_cols((in_vec_state == 1) ? 1 : 0),
      n_elem(0), n_alloc(0), vec_state(in_vec_state), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
}
...
inline Mat<eT>::Mat(const arma_vec_indicator &, const uword in_n_rows,
    ...
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols),
      n_alloc(), vec_state(in_vec_state), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const arma_fixed_indicator &, const uword in_n_rows,
                    ...
                    const eT *in_mem)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols),
      n_alloc(0), vec_state(in_vec_state), mem_state(3), mem(in_mem) {
  arma_extra_debug_sigprint_this(this);
}

template <typename eT> inline void Mat<eT>::init_cold() {
  arma_extra_debug_sigprint(arma_str::format("n_rows = %u, n_cols = %u") %
                            n_rows % n_cols);
...
inline void Mat<eT>::init_warm(uword in_n_rows, uword in_n_cols) {
  ...
  if (t_vec_state > 0) {
    if ((in_n_rows == 0) && (in_n_cols == 0)) {
      ...
      if (t_vec_state == 1) {
        arma_debug_set_error(err_state, err_msg, (in_n_cols != 1),
                             error_message_2);
      }
      if (t_vec_state == 2) {
        arma_debug_set_error(err_state, err_msg, (in_n_rows != 1),
                             error_message_3);
      }
    }
  }
  ...
  arma_debug_check((t_mem_state == 2), "Mat::init(): mismatch between size of "
...
//! create the matrix from a textual description
template <typename eT> inline Mat<eT> &Mat<eT>::operator=(const char *text) {
...
template <typename eT> inline void Mat<eT>::init(const std::string &text_orig) {
  arma_extra_debug_sigprint();
...
inline Mat<eT>::Mat(const std::vector<eT> &x)
    : n_rows(uword(x.size())), n_cols(1), n_elem(uword(x.size())), n_alloc(),
      vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(Mat<eT> &&X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), n_alloc(X.n_alloc),
      vec_state(0), mem_state(0), mem() {
  ...
  } else // condition: (X.n_alloc <= arma_config::mat_prealloc) || (X.mem_state
         // == 0) || (X.mem_state == 3)
  {
    init_cold();
...
template <typename eT> inline Mat<eT> &Mat<eT>::operator=(Mat<eT> &&X) {
  arma_extra_debug_sigprint(arma_str::format("this = %x   X = %x") % this % &X);
...
template <typename eT> inline Mat<eT> &Mat<eT>::operator=(const eT val) {
  arma_extra_debug_sigprint();
...
inline Mat<eT>::Mat(const Mat<eT> &in_mat)
    : n_rows(in_mat.n_rows), n_cols(in_mat.n_cols), n_elem(in_mat.n_elem),
      n_alloc(), vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint(arma_str::format("this = %x   in_mat = %x") % this %
                            &in_mat);
...
Mat<eT>::init(const std::initializer_list<std::initializer_list<eT>> &list) {
  ...
  if (t.mem_state == 3) {
    arma_debug_check(((x_n_rows != t.n_rows) || (x_n_cols != t.n_cols)),
                     "Mat::init(): size mismatch between fixed size matrix and "
                     "initialiser list");
  } else {
...
inline void Mat<eT>::init(const Base<typename Mat<eT>::pod_type, T1> &X,
  ...
  arma_type_check(
      (is_cx<eT>::no)); //!< compile-time abort if eT is not std::complex
  arma_type_check(
      (is_cx<T>::yes)); //!< compile-time abort if  T is     std::complex

  arma_type_check(
      (is_same_type<std::complex<T>, eT>::no)); //!< compile-time abort if
...
template <typename eT> inline void Mat<eT>::swap(Mat<eT> &B) {
  Mat<eT> &A = (*this);
  ...
  } else if ((A_mem_state <= 2) && (B_mem_state <= 2) &&
             (A.n_elem == B.n_elem) && layout_ok) {
    std::swap(access::rw(A.n_rows), access::rw(B.n_rows));
    std::swap(access::rw(A.n_cols), access::rw(B.n_cols));
...
template <typename eT> inline void Mat<eT>::steal_mem(Mat<eT> &x) {
  arma_extra_debug_sigprint();
...
inline void Mat<eT>::steal_mem(Mat<eT> &x, const bool is_move) {
  ...
  if (layout_ok && (t_mem_state <= 1) &&
    ...
    if ((is_move) && (x_mem_state == 0) &&
        (x_n_alloc <= arma_config::mat_prealloc)) {
      access::rw(x.n_rows) = (x_vec_state == 2) ? 1 : 0;
      access::rw(x.n_cols) = (x_vec_state == 1) ? 1 : 0;
      access::rw(x.n_elem) = 0;
      access::rw(x.mem) = nullptr;
...
inline void Mat<eT>::steal_mem_col(Mat<eT> &x, const uword max_n_rows) {
  ...
  if ((this != &x) && (t_vec_state <= 1) && (t_mem_state <= 1) &&
      (x_mem_state <= 1)) {
...
//! allocating memory and copying). the default is to copy the array.

template <typename eT>
inline Mat<eT>::Mat(eT *aux_mem, const uword aux_n_rows, const uword aux_n_cols,
                    const bool copy_aux_mem, const bool strict)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem(aux_n_rows * aux_n_cols),
      n_alloc(0), vec_state(0), mem_state(copy_aux_mem ? 0 : (strict ? 2 : 1)),
...
//! the array is copied.
template <typename eT>
inline Mat<eT>::Mat(const eT *aux_mem, const uword aux_n_rows,
...
inline Mat<eT>::Mat(const char junk, const eT *aux_mem, const uword aux_n_rows,
                    const uword aux_n_cols)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem(aux_n_rows * aux_n_cols),
      n_alloc(0), vec_state(0), mem_state(3), mem(aux_mem) {
  arma_extra_debug_sigprint_this(this);
...
template <typename eT> inline Mat<eT> &Mat<eT>::operator+=(const Mat<eT> &m) {
  arma_extra_debug_sigprint();
...
inline Mat<eT> &Mat<eT>::operator+=(const BaseCube<eT, T1> &X) {
  ...
  if (in_n_slices == 1) {
    for (uword ucol = 0; ucol < in_n_cols; ++ucol) {
      arrayops::inplace_plus(out.colptr(ucol), in.slice_colptr(0, ucol),
                             in_n_rows);
    }
  } else {
    if (out_vec_state == 0) {
      if ((in_n_rows == out_n_rows) && (in_n_cols == 1) &&
          (in_n_slices == out_n_cols)) {
        for (uword i = 0; i < in_n_slices; ++i) {
          arrayops::inplace_plus(out.colptr(i), in.slice_colptr(i, 0),
                                 in_n_rows);
        }
...
inline Mat<eT> &Mat<eT>::operator-=(const BaseCube<eT, T1> &X) {
  ...
  if (in_n_slices == 1) {
    for (uword ucol = 0; ucol < in_n_cols; ++ucol) {
      arrayops::inplace_minus(out.colptr(ucol), in.slice_colptr(0, ucol),
                              in_n_rows);
    }
  } else {
    if (out_vec_state == 0) {
      if ((in_n_rows == out_n_rows) && (in_n_cols == 1) &&
          (in_n_slices == out_n_cols)) {
        for (uword i = 0; i < in_n_slices; ++i) {
          arrayops::inplace_minus(out.colptr(i), in.slice_colptr(i, 0),
                                  in_n_rows);
        }
      } else if ((in_n_rows == 1) && (in_n_cols == out_n_rows) &&
                 (in_n_slices == out_n_cols)) {
...
inline Mat<eT> &Mat<eT>::operator%=(const BaseCube<eT, T1> &X) {
  ...
  if (in_n_slices == 1) {
    for (uword ucol = 0; ucol < in_n_cols; ++ucol) {
      arrayops::inplace_mul(out.colptr(ucol), in.slice_colptr(0, ucol),
                            in_n_rows);
    }
  } else {
    if (out_vec_state == 0) {
      if ((in_n_rows == out_n_rows) && (in_n_cols == 1) &&
          (in_n_slices == out_n_cols)) {
        for (uword i = 0; i < in_n_slices; ++i) {
          arrayops::inplace_mul(out.colptr(i), in.slice_colptr(i, 0),
                                in_n_rows);
        }
      } else if ((in_n_rows == 1) && (in_n_cols == out_n_rows) &&
                 (in_n_slices == out_n_cols)) {
...
inline Mat<eT> &Mat<eT>::operator/=(const BaseCube<eT, T1> &X) {
  ...
  if (in_n_slices == 1) {
    for (uword ucol = 0; ucol < in_n_cols; ++ucol) {
      arrayops::inplace_div(out.colptr(ucol), in.slice_colptr(0, ucol),
                            in_n_rows);
    }
  } else {
    if (out_vec_state == 0) {
      if ((in_n_rows == out_n_rows) && (in_n_cols == 1) &&
          (in_n_slices == out_n_cols)) {
        for (uword i = 0; i < in_n_slices; ++i) {
          arrayops::inplace_div(out.colptr(i), in.slice_colptr(i, 0),
                                in_n_rows);
        }
      } else if ((in_n_rows == 1) && (in_n_cols == out_n_rows) &&
                 (in_n_slices == out_n_cols)) {
...
inline Mat<eT>::Mat(const subview<eT> &X, const bool use_colmem)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), n_alloc(0),
      vec_state(0), mem_state(use_colmem ? 3 : 0),
...
//! construct a matrix from subview (eg. construct a matrix from a delayed
//! submatrix operation)
template <typename eT>
inline Mat<eT>::Mat(const subview<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), n_alloc(),
      vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! in-place element-wise matrix mutiplication (using a submatrix on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator%=(const subview<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place element-wise matrix division (using a submatrix on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator/=(const subview<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place matrix addition (using a single-slice subcube on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator+=(const subview_cube<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place matrix subtraction (using a single-slice subcube on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator-=(const subview_cube<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place matrix mutiplication (using a single-slice subcube on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator*=(const subview_cube<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place element-wise matrix mutiplication (using a single-slice subcube on
//! the right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator%=(const subview_cube<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place element-wise matrix mutiplication (using a diagview on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator%=(const diagview<eT> &X) {
  arma_extra_debug_sigprint();
...
//! in-place element-wise matrix division (using a diagview on the
//! right-hand-side)
template <typename eT>
inline Mat<eT> &Mat<eT>::operator/=(const diagview<eT> &X) {
  arma_extra_debug_sigprint();
...
inline Mat<eT> &Mat<eT>::operator%=(const SpBase<eT, T1> &m) {
  ...
  while (it != it_end) {
    ...
    const uword next_loc = (it == it_end) ? (p.get_n_cols() * n_rows)
...
inline Mat<eT> &Mat<eT>::operator/=(const SpBase<eT, T1> &m) {
  ...
  // NOTE: use of this function is not advised; it is implemented only for
  // completeness
...
template <typename eT>
inline subview_row<eT> Mat<eT>::operator()(const uword row_num,
                                           const span &col_span) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check_bounds(
      (row_num >= n_rows) ||
          (col_all ? false
                   : ((in_col1 > in_col2) || (in_col2 >= local_n_cols))),
...
template <typename eT>
inline subview_col<eT> Mat<eT>::operator()(const span &row_span,
                                           const uword col_num) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check_bounds(
      (col_num >= n_cols) ||
          (row_all ? false
                   : ((in_row1 > in_row2) || (in_row2 >= local_n_rows))),
      "Mat::operator(): indices out of bounds or incorrectly used");
...
//! and does not take into account that the parent matrix object could be
//! deleted. if deleted memory is accessed by the created Col object, it will
//! cause memory corruption and/or a crash
template <typename eT> inline Col<eT> Mat<eT>::unsafe_col(const uword col_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(col_num >= n_cols,
                          "Mat::unsafe_col(): index out of bounds");
...
template <typename eT>
arma_inline subview_cols<eT> Mat<eT>::cols(const uword in_col1,
                                           const uword in_col2) {
  arma_extra_debug_sigprint();
...
//! creation of subview (submatrix comprised of specified row vectors)
template <typename eT> inline subview<eT> Mat<eT>::rows(const span &row_span) {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline subview<eT>
Mat<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2,
                const uword in_col2) {
  arma_extra_debug_sigprint();
...
arma_inline subview<eT> Mat<eT>::submat(const uword in_row1,
                                        const uword in_col1, const SizeMat &s) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check_bounds(((in_row1 >= l_n_rows) || (in_col1 >= l_n_cols) ||
                           ((in_row1 + s_n_rows) > l_n_rows) ||
                           ((in_col1 + s_n_cols) > l_n_cols)),
...
template <typename eT>
inline subview<eT> Mat<eT>::operator()(const span &row_span,
                                       const span &col_span) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline const subview<eT> Mat<eT>::operator()(const uword in_row1,
                                             const uword in_col1,
                                             const SizeMat &s) const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline subview<eT> Mat<eT>::head_rows(const uword N) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline subview<eT> Mat<eT>::tail_rows(const uword N) {
  arma_extra_debug_sigprint();
...
template <typename eT>
template <typename T1>
arma_inline subview_elem1<eT, T1>
...
template <typename eT>
template <typename T1, typename T2>
arma_inline subview_elem2<eT, T1, T2>
...
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2>
Mat<eT>::operator()(const Base<uword, T1> &ri,
...
//! apply a lambda function to each column, where each column is interpreted as
//! a column vector
template <typename eT>
...
//! apply a lambda function to each row, where each row is interpreted as a row
//! vector
template <typename eT>
...
inline void Mat<eT>::swap_rows(const uword in_row1, const uword in_row2) {
  ...
  arma_debug_check_bounds((in_row1 >= local_n_rows) ||
                              (in_row2 >= local_n_rows),
...
inline void Mat<eT>::swap_cols(const uword in_colA, const uword in_colB) {
  ...
  arma_debug_check_bounds((in_colA >= local_n_cols) ||
                              (in_colB >= local_n_cols),
...
template <typename eT> inline void Mat<eT>::shed_row(const uword row_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(row_num >= n_rows,
                          "Mat::shed_row(): index out of bounds");

  shed_rows(row_num, row_num);
}

//! remove specified column
template <typename eT> inline void Mat<eT>::shed_col(const uword col_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(col_num >= n_cols,
                          "Mat::shed_col(): index out of bounds");
...
inline void Mat<eT>::shed_rows(const Base<uword, T1> &indices) {
  ...
  const Col<uword> tmp2(const_cast<uword *>(tmp1.memptr()), tmp1.n_elem, false,
                        false);

  const Col<uword> &rows_to_shed =
      (tmp2.is_sorted("strictascend") == false)
          ? Col<uword>(unique(tmp2))
          : Col<uword>(const_cast<uword *>(tmp2.memptr()), tmp2.n_elem, false,
                       false);
...
inline void Mat<eT>::insert_rows(const uword row_num, const Base<eT, T1> &X) {
  ...
  arma_debug_set_error(err_state, err_msg, (row_num > t_n_rows),
                       error_message_1);

  arma_debug_set_error(err_state, err_msg,
                       ((C_n_cols != t_n_cols) &&
                        ((t_n_rows > 0) || (t_n_cols > 0)) &&
...
inline void Mat<eT>::insert_cols(const uword col_num, const Base<eT, T1> &X) {
  ...
  arma_debug_set_error(err_state, err_msg, (col_num > t_n_cols),
                       error_message_1);
...
inline Mat<eT>::Mat(const Gen<T1, gen_type> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(n_rows * n_cols), n_alloc(),
      vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! in-place matrix addition, with the right-hand-side operand having delayed
//! operations
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT> &Mat<eT>::operator+=(const Op<T1, op_type> &X) {
...
//! in-place matrix element-wise multiplication, with the right-hand-side
//! operand having delayed operations
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT> &Mat<eT>::operator%=(const Op<T1, op_type> &X) {
  arma_extra_debug_sigprint();
...
inline Mat<eT>::Mat(const eOp<T1, eop_type> &X)
    : n_rows(X.get_n_rows()), n_cols(X.get_n_cols()), n_elem(X.get_n_elem()),
      n_alloc(), vec_state(0), mem_state(0), mem() {
  arma_extra_debug_sigprint_this(this);
...
//! create a matrix from an SpToDOp, ie. run the previously delayed unary
//! operations
template <typename eT>
template <typename T1, typename op_type>
...
//! in-place matrix addition, with the right-hand-side operands having delayed
//! operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT> &Mat<eT>::operator+=(const Glue<T1, T2, glue_type> &X) {
...
//! in-place matrix subtraction, with the right-hand-side operands having
//! delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT> &Mat<eT>::operator-=(const Glue<T1, T2, glue_type> &X) {
...
//! in-place matrix multiplications, with the right-hand-side operands having
//! delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT> &Mat<eT>::operator*=(const Glue<T1, T2, glue_type> &X) {
...
//! in-place matrix element-wise division, with the right-hand-side operands
//! having delayed operations
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT> &Mat<eT>::operator/=(const Glue<T1, T2, glue_type> &X) {
...
inline Mat<eT> &Mat<eT>::operator=(const eGlue<T1, T2, eglue_type> &X) {
  ...
  const bool bad_alias =
      ((eGlue<T1, T2, eglue_type>::proxy1_type::has_subview &&
        X.P1.is_alias(*this)) ||
       (eGlue<T1, T2, eglue_type>::proxy2_type::has_subview &&
        X.P2.is_alias(*this)));
...
//! linear element accessor (treats the matrix as a vector); bounds checking not
//! done when ARMA_NO_DEBUG is defined
template <typename eT> arma_inline eT &Mat<eT>::operator()(const uword ii) {
  arma_debug_check_bounds((ii >= n_elem),
...
//! linear element accessor (treats the matrix as a vector); no bounds check.
template <typename eT> arma_inline eT &Mat<eT>::at(const uword ii) {
  return access::rw(mem[ii]);
}

//! linear element accessor (treats the matrix as a vector); no bounds check
template <typename eT> arma_inline const eT &Mat<eT>::at(const uword ii) const {
  return mem[ii];
}
...
//! element accessor; bounds checking not done when ARMA_NO_DEBUG is defined
template <typename eT>
arma_inline const eT &Mat<eT>::operator()(const uword in_row,
...
//! element accessor; no bounds check
template <typename eT>
arma_inline const eT &Mat<eT>::at(const uword in_row,
                                  const uword in_col) const {
  return mem[in_row + in_col * n_rows];
}
...
//! prefix ++
template <typename eT> arma_inline const Mat<eT> &Mat<eT>::operator++() {
...
template <typename eT> arma_inline void Mat<eT>::operator++(int) {
...
template <typename eT> arma_inline bool Mat<eT>::is_empty() const {
  return (n_elem == 0);
}

//! returns true if the object can be interpreted as a column or row vector
template <typename eT> arma_inline bool Mat<eT>::is_vec() const {
  return ((n_rows == 1) || (n_cols == 1));
}

//! returns true if the object can be interpreted as a row vector
template <typename eT> arma_inline bool Mat<eT>::is_rowvec() const {
  return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT> arma_inline bool Mat<eT>::is_colvec() const {
  return (n_cols == 1);
}

//! returns true if the object has the same number of non-zero rows and columnns
template <typename eT> arma_inline bool Mat<eT>::is_square() const {
  return (n_rows == n_cols);
}

template <typename eT> inline bool Mat<eT>::internal_is_finite() const {
  arma_extra_debug_sigprint();

  return arrayops::is_finite(memptr(), n_elem);
}

template <typename eT> inline bool Mat<eT>::internal_has_inf() const {
  arma_extra_debug_sigprint();

  return arrayops::has_inf(memptr(), n_elem);
}

template <typename eT> inline bool Mat<eT>::internal_has_nan() const {
  arma_extra_debug_sigprint();

  return arrayops::has_nan(memptr(), n_elem);
}

template <typename eT> inline bool Mat<eT>::internal_has_nonfinite() const {
  arma_extra_debug_sigprint();
...
inline bool Mat<eT>::is_sorted(const char *direction) const {
  ...
  return (*this).is_sorted(
      direction, (((vec_state == 2) || (n_rows == 1)) ? uword(1) : uword(0)));
}
...
inline bool Mat<eT>::is_sorted(const char *direction, const uword dim) const {
  ...
  arma_debug_check((dim > 1),
                   "Mat::is_sorted(): parameter 'dim' must be 0 or 1");
...
inline bool Mat<eT>::is_sorted_helper(const comparator &comp,
...
template <typename eT> arma_inline bool Mat<eT>::in_range(const span &x) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline bool Mat<eT>::in_range(const uword in_row,
                                   const uword in_col) const {
  return ((in_row < n_rows) && (in_col < n_cols));
}

template <typename eT>
arma_inline bool Mat<eT>::in_range(const span &row_span,
                                   const uword in_col) const {
  arma_extra_debug_sigprint();
...
arma_inline bool Mat<eT>::in_range(const uword in_row, const uword in_col,
  ...
  if ((in_row >= l_n_rows) || (in_col >= l_n_cols) ||
...
template <typename eT> arma_inline eT *Mat<eT>::colptr(const uword in_col) {
...
template <typename eT> arma_inline eT *Mat<eT>::memptr() {
  return const_cast<eT *>(mem);
}

//! returns a pointer to array of eTs used by the matrix
template <typename eT> arma_inline const eT *Mat<eT>::memptr() const {
  return mem;
}
...
//! change the matrix to have user specified dimensions (data is not preserved)
template <typename eT>
inline Mat<eT> &Mat<eT>::set_size(const uword new_n_rows,
...
template <typename eT> inline Mat<eT> &Mat<eT>::set_size(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::resize(const uword new_n_elem) {
  arma_extra_debug_sigprint();
...
//! change the matrix to have user specified dimensions (data is preserved)
template <typename eT>
inline Mat<eT> &Mat<eT>::resize(const uword new_n_rows,
                                const uword new_n_cols) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::resize(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
//! change the matrix to have user specified dimensions (data is preserved)
template <typename eT>
inline Mat<eT> &Mat<eT>::reshape(const uword new_n_rows,
                                 const uword new_n_cols) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::reshape(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
//! change the matrix (without preserving data) to have the same dimensions as
...
template <typename eT> inline Mat<eT> &Mat<eT>::fill(const eT val) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::zeros() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::zeros(const uword new_n_elem) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::zeros(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).zeros(s.n_rows, s.n_cols);
}

template <typename eT> inline Mat<eT> &Mat<eT>::ones() {
  arma_extra_debug_sigprint();

  return fill(eT(1));
}

template <typename eT> inline Mat<eT> &Mat<eT>::ones(const uword new_n_elem) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::ones(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).ones(s.n_rows, s.n_cols);
}

template <typename eT> inline Mat<eT> &Mat<eT>::randu() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::randu(const uword new_n_elem) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::randu(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).randu(s.n_rows, s.n_cols);
}

template <typename eT> inline Mat<eT> &Mat<eT>::randn() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::randn(const uword new_n_elem) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::randn(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).randn(s.n_rows, s.n_cols);
}

template <typename eT> inline Mat<eT> &Mat<eT>::eye() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Mat<eT> &Mat<eT>::eye(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).eye(s.n_rows, s.n_cols);
}

template <typename eT> inline void Mat<eT>::reset() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void Mat<eT>::soft_reset() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Mat<eT>::min() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Mat<eT>::max() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Mat<eT>::min(uword &index_of_min_val) const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline eT Mat<eT>::max(uword &index_of_max_val) const {
  arma_extra_debug_sigprint();
...
inline bool Mat<eT>::save(const std::string name, const file_type type) const {
  ...
  case hdf5_binary_trans: // kept for compatibility with earlier versions of
                          // Armadillo
...
inline bool Mat<eT>::save(const hdf5_name &spec, const file_type type) const {
  ...
  // handling of hdf5_binary_trans kept for compatibility with earlier versions
  // of Armadillo

  if ((type != hdf5_binary) && (type != hdf5_binary_trans)) {
    arma_stop_runtime_error(
        "Mat::save(): unsupported file type for hdf5_name()");
    return false;
  }

  const bool do_trans = bool(spec.opts.flags & hdf5_opts::flag_trans) ||
                        (type == hdf5_binary_trans);
  ...
  if (save_okay == false) {
    if (err_msg.length() > 0) {
      arma_debug_warn_level(3, "Mat::save(): ", err_msg,
                            "; file: ", spec.filename);
    } else {
      arma_debug_warn_level(3,
                            "Mat::save(): write failed; file: ", spec.filename);
    }
  }
...
inline bool Mat<eT>::save(const csv_name &spec, const file_type type) const {
  ...
  if ((type != csv_ascii) && (type != ssv_ascii)) {
    arma_stop_runtime_error(
        "Mat::save(): unsupported file type for csv_name()");
    return false;
  }
  ...
  if (with_header) {
    if ((spec.header_ro.n_cols != 1) && (spec.header_ro.n_rows != 1)) {
      arma_debug_warn_level(
          1, "Mat::save(): given header must have a vector layout");
      return false;
    }
    ...
    for (uword i = 0; i < spec.header_ro.n_elem; ++i) {
      ...
      if (token.find(separator) != std::string::npos) {
        arma_debug_warn_level(1,
                              "Mat::save(): token within the header contains "
  ...
  if (do_trans) {
    ...
    save_okay = diskio::save_csv_ascii(tmp, spec.filename, spec.header_ro,
                                       with_header, separator);
  } else {
    save_okay = diskio::save_csv_ascii(*this, spec.filename, spec.header_ro,
                                       with_header, separator);
  }
...
inline bool Mat<eT>::load(const hdf5_name &spec, const file_type type) {
  ...
  if ((type != hdf5_binary) && (type != hdf5_binary_trans)) {
    arma_stop_runtime_error(
        "Mat::load(): unsupported file type for hdf5_name()");
    return false;
  }
  ...
  if (load_okay == false) {
    if (err_msg.length() > 0) {
      ...
      arma_debug_warn_level(3,
                            "Mat::load(): read failed; file: ", spec.filename);
    }
  }
...
inline bool Mat<eT>::load(const csv_name &spec, const file_type type) {
  ...
  if ((type != csv_ascii) && (type != ssv_ascii)) {
    arma_stop_runtime_error(
        "Mat::load(): unsupported file type for csv_name()");
    return false;
  }
  ...
  if (do_trans) {
    ...
    load_okay =
        diskio::load_csv_ascii(tmp_mat, spec.filename, err_msg, spec.header_rw,
    ...
    if (load_okay) {
      ...
      if (with_header) {
        // field::set_size() preserves data if the number of elements hasn't
        // changed
    ...
    load_okay =
        diskio::load_csv_ascii(*this, spec.filename, err_msg, spec.header_rw,
                               with_header, separator, strict);
  }
...
template <typename eT>
inline bool Mat<eT>::quiet_save(const std::string name,
...
template <typename eT>
inline bool Mat<eT>::quiet_save(const hdf5_name &spec,
                                const file_type type) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool Mat<eT>::quiet_save(const csv_name &spec,
                                const file_type type) const {
  arma_extra_debug_sigprint();
...
inline Mat<eT>::row_iterator::row_iterator()
  ...
  // NOTE: this instance of row_iterator is invalid (it does not point to a
  // valid element)
}
...
template <typename eT> inline eT &Mat<eT>::row_iterator::operator*() {
...
inline Mat<eT>::const_row_iterator::const_row_iterator()
  ...
  // NOTE: this instance of const_row_iterator is invalid (it does not point to
  // a valid element)
}
...
template <typename eT>
inline typename Mat<eT>::const_row_iterator &
...
inline Mat<eT>::row_col_iterator::row_col_iterator(
    ...
    : M(in_it.M), current_ptr(in_it.current_ptr),
      current_col(in_it.current_col), current_row(in_it.current_row) {
  arma_extra_debug_sigprint();
}

template <typename eT>
inline Mat<eT>::row_col_iterator::row_col_iterator(Mat<eT> &in_M,
                                                   const uword in_row,
                                                   const uword in_col)
    : M(&in_M), current_ptr(&in_M.at(in_row, in_col)), current_col(in_col),
      current_row(in_row) {
  arma_extra_debug_sigprint();
}

template <typename eT> inline eT &Mat<eT>::row_col_iterator::operator*() {
  return *current_ptr;
}
...
template <typename eT> inline uword Mat<eT>::row_col_iterator::row() const {
  return current_row;
}

template <typename eT> inline uword Mat<eT>::row_col_iterator::col() const {
  return current_col;
}
...
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(
    const row_col_iterator &in_it)
    : M(in_it.M), current_ptr(in_it.current_ptr), current_col(in_it.col()),
...
template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(
    const Mat<eT> &in_M, const uword in_row, const uword in_col)
    : M(&in_M), current_ptr(&in_M.at(in_row, in_col)), current_col(in_col),
      current_row(in_row) {
  arma_extra_debug_sigprint();
}
...
template <typename eT> inline typename Mat<eT>::iterator Mat<eT>::begin() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline typename Mat<eT>::iterator Mat<eT>::end() {
  arma_extra_debug_sigprint();
...
inline typename Mat<eT>::col_iterator Mat<eT>::begin_col(const uword col_num) {
  ...
  arma_debug_check_bounds((col_num >= n_cols),
                          "Mat::begin_col(): index out of bounds");

  return colptr(col_num);
}

template <typename eT>
inline typename Mat<eT>::const_col_iterator
...
inline typename Mat<eT>::row_iterator Mat<eT>::begin_row(const uword row_num) {
  ...
  arma_debug_check_bounds((row_num >= n_rows),
                          "Mat::begin_row(): index out of bounds");
...
inline typename Mat<eT>::row_iterator Mat<eT>::end_row(const uword row_num) {
  ...
  arma_debug_check_bounds((row_num >= n_rows),
                          "Mat::end_row(): index out of bounds");
...
template <typename eT> inline void Mat<eT>::clear() { reset(); }

//! returns true if the matrix has no elements
template <typename eT> inline bool Mat<eT>::empty() const {
  return (n_elem == 0);
}

//! returns the number of elements in this matrix
template <typename eT> inline uword Mat<eT>::size() const { return n_elem; }

template <typename eT> inline eT &Mat<eT>::front() {
  arma_debug_check((n_elem == 0), "Mat::front(): matrix is empty");

  return access::rw(mem[0]);
}

template <typename eT> inline const eT &Mat<eT>::front() const {
  arma_debug_check((n_elem == 0), "Mat::front(): matrix is empty");

  return mem[0];
}

template <typename eT> inline eT &Mat<eT>::back() {
  arma_debug_check((n_elem == 0), "Mat::back(): matrix is empty");

  return access::rw(mem[n_elem - 1]);
}

template <typename eT> inline const eT &Mat<eT>::back() const {
  arma_debug_check((n_elem == 0), "Mat::back(): matrix is empty");
...
template <typename T1, typename T2>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(
    const Base<pod_type, T1> &A, const Base<pod_type, T2> &B)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0,
              ((use_extra) ? mem_local_extra : Mat<eT>::mem_local)) {
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT> &Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(
  ...
  arma_debug_check((N > fixed_n_elem),
                   "Mat::fixed: initialiser list is too long");
...
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const
    Op<typename Mat<eT>::template fixed<fixed_n_rows,
                                        fixed_n_cols>::Mat_fixed_type,
       op_htrans>
    Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::t() const {
  return Op<typename Mat<eT>::template fixed<fixed_n_rows,
                                             fixed_n_cols>::Mat_fixed_type,
            op_htrans>(*this);
}
...
arma_inline eT &
Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(const uword ii) {
  arma_debug_check_bounds((ii >= fixed_n_elem),
                          "Mat::operator(): index out of bounds");
...
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline eT &
Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::at(const uword in_row,
                                               const uword in_col) {
  const uword iq = in_row + in_col * fixed_n_rows;
...
Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator()(const uword in_row,
                                                       const uword in_col) {
  arma_debug_check_bounds(
      ((in_row >= fixed_n_rows) || (in_col >= fixed_n_cols)),
      "Mat::operator(): index out of bounds");
...
//! prefix ++
template <typename eT> inline void Mat_aux::prefix_pp(Mat<eT> &x) {
  eT *memptr = x.memptr();
...
//! prefix ++ for complex numbers (work around for limitations of the
//! std::complex class)
...
//! postfix ++
template <typename eT> inline void Mat_aux::postfix_pp(Mat<eT> &x) {
  eT *memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! postfix ++ for complex numbers (work around for limitations of the
//! std::complex class)
...
//! prefix --
template <typename eT> inline void Mat_aux::prefix_mm(Mat<eT> &x) {
  eT *memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
//! postfix --
template <typename eT> inline void Mat_aux::postfix_mm(Mat<eT> &x) {
  eT *memptr = x.memptr();
  const uword n_elem = x.n_elem;
...
inline void Mat_aux::set_real(Mat<std::complex<T>> &out, const Base<T, T1> &X) {
  ...
  arma_debug_assert_same_size(out.n_rows, out.n_cols, local_n_rows,
                              local_n_cols, "Mat::set_real()");
