...
template <typename T1>
inline bool
op_roots::apply_direct(Mat<std::complex<typename T1::pod_type>> &out,
...
inline bool op_roots::apply_noalias(
    Mat<std::complex<typename get_pod_type<eT>::result>> &out,
    const Mat<eT> &X) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check((X.is_vec() == false),
                   "roots(): given object must be a vector");
  ...
  const uword n_tail_zeros =
      (indices.n_elem > 0) ? uword((Y.n_elem - 1) - indices[indices.n_elem - 1])
                           : uword(0);
