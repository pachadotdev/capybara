...
inline uword op_find::helper(
  ...
  if ((is_same_type<op_type, op_rel_eq>::yes ||
    ...
    arma_debug_warn_level(1, "find(): NaN is not equal to anything; suggest to "
                             "use find_nonfinite() instead");
  }
...
inline void op_find::apply(Mat<uword> &out, const mtOp<uword, T1, op_find> &X) {
  ...
  if (n_nz > 0) {
    if (type == 0) // "first"
    {
      out = (k > 0 && k <= n_nz) ? indices.rows(0, k - 1)
...
inline void op_find_finite::apply(Mat<uword> &out,
  ...
  if (arma_config::fast_math_warn) {
    arma_debug_warn_level(
        1, "find_finite(): detection of non-finite values is not reliable "
           "in fast math mode");
  }
...
template <typename T1>
inline void
op_find_nonfinite::apply(Mat<uword> &out,
                         const mtOp<uword, T1, op_find_nonfinite> &X) {
  arma_extra_debug_sigprint();

  if (arma_config::fast_math_warn) {
    arma_debug_warn_level(
        1, "find_nonfinite(): detection of non-finite values is not "
...
template <typename T1>
inline void op_find_nan::apply(Mat<uword> &out,
                               const mtOp<uword, T1, op_find_nan> &X) {
  arma_extra_debug_sigprint();

  if (arma_config::fast_math_warn) {
    arma_debug_warn_level(1, "find_nan(): detection of non-finite values is "
