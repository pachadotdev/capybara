...
//! Matrix 'C' is assumed to have been set to the correct size (ie. taking into
//! account transposes)
...
public:
  ...
  apply(Mat<out_eT> &C, const Mat<in_eT1> &A, const Mat<in_eT2> &B,
        const out_eT alpha = out_eT(1), const out_eT beta = out_eT(0)) {
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
      ...
      gemm_mixed_large<false, false, use_alpha, use_beta>::apply(C, A, B_tmp,
                                                                 alpha, beta);
...
public:
  ...
  template <typename out_eT, typename in_eT1, typename in_eT2>
  inline static void apply(Mat<out_eT> &C, const Mat<in_eT1> &A,
                           const Mat<in_eT2> &B, const out_eT alpha = out_eT(1),
                           const out_eT beta = out_eT(0)) {
    arma_extra_debug_sigprint();

    if ((is_cx<in_eT1>::yes && do_trans_A) ||
        (is_cx<in_eT2>::yes && do_trans_B)) {
      ...
      gemm_mixed_large<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(
          C, A, B, alpha, beta);
    }
  }
};
