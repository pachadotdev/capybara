...
//! Years Later. SIAM Review, Vol. 45, No. 1, 2003, pp. 3-49.
...
template <typename T1>
inline bool
op_expmat::apply_direct(Mat<typename T1::elem_type> &out,
  ...
  if ((arma_config::optimise_sym) && (auxlib::crippled_lapack(A) == false)) {
    ...
    do_sym = ((is_cx<eT>::no) ? (is_approx_sym)
                              : (is_approx_sym && is_approx_sympd));
  }
...
template <typename T1>
inline bool
op_expmat_sym::apply_direct(Mat<typename T1::elem_type> &out,
                            const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
