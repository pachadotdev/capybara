...
arma_inline
diagview<eT>::diagview(const Mat<eT> &in_m, const uword in_row_offset,
                       const uword in_col_offset, const uword in_len)
    : m(in_m), row_offset(in_row_offset), col_offset(in_col_offset),
      n_rows(in_len), n_elem(in_len) {
  arma_extra_debug_sigprint_this(this);
}

template <typename eT>
inline diagview<eT>::diagview(const diagview<eT> &in)
    : m(in.m), row_offset(in.row_offset), col_offset(in.col_offset),
      n_rows(in.n_rows), n_elem(in.n_elem) {
...
template <typename eT> arma_inline eT &diagview<eT>::at(const uword ii) {
  return (const_cast<Mat<eT> &>(m)).at(ii + row_offset, ii + col_offset);
}

template <typename eT> arma_inline eT diagview<eT>::at(const uword ii) const {
  return m.at(ii + row_offset, ii + col_offset);
}
...
template <typename eT>
arma_inline eT diagview<eT>::operator()(const uword row,
