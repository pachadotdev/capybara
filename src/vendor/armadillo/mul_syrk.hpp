...
public:
  ...
  arma_hot inline static void apply(Mat<eT> &C, const TA &A,
                                    const eT alpha = eT(1),
                                    const eT beta = eT(0)) {
...
public:
  template <typename eT, typename TA>
  inline static void apply_blas_type(Mat<eT> &C, const TA &A,
                                     const eT alpha = eT(1),
                                     const eT beta = eT(0)) {
    arma_extra_debug_sigprint();
    ...
    if (A.n_elem <= threshold) {
      ...
      {
        if (use_beta == true) {
          // use a temporary matrix, as we can't assume that matrix C is already
          // symmetric
          Mat<eT> D(C.n_rows, C.n_cols, arma_nozeros_indicator());

          syrk<do_trans_A, use_alpha, false>::apply_blas_type(D, A, alpha);

          // NOTE: assuming beta=1; this is okay for now, as currently
          // glue_times only uses beta=1
        ...
        atlas::cblas_syrk<eT>(
            atlas_CblasColMajor, atlas_CblasUpper,
            (do_trans_A) ? atlas_CblasTrans : atlas_CblasNoTrans, C.n_cols,
            (do_trans_A) ? A.n_rows : A.n_cols, (use_alpha) ? alpha : eT(1),
            A.mem, (do_trans_A) ? A.n_rows : C.n_cols,
...
#elif defined(ARMA_USE_BLAS)
      {
        ...
        arma_extra_debug_print(arma_str::format("blas::syrk(): trans_A = %c") %
                               trans_A);
  ...
  arma_inline static void apply(Mat<float> &C, const TA &A,
  ...
  apply(Mat<std::complex<float>> &C, const TA &A,
  ...
  template <typename TA>
  arma_inline static void
  apply(Mat<std::complex<double>> &C, const TA &A,
