...
//! subcube (ie. where the subcube starts and ends as well as a
//! reference/pointer to the original cube),
...
protected:
  arma_inline subview_cube(const Cube<eT> &in_m, const uword in_row1,
...
public:
  ...
  arma_inline eT &at(const uword in_row, const uword in_col,
                     const uword in_slice);
  arma_inline eT at(const uword in_row, const uword in_col,
                    const uword in_slice) const;

  arma_inline eT *slice_colptr(const uword in_slice, const uword in_col);
  arma_inline const eT *slice_colptr(const uword in_slice,
  ...
  public:
    ...
    inline iterator(subview_cube<eT> &in_sv, const uword in_row,
