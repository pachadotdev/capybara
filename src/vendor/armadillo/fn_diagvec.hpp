...
arma_warn_unused arma_inline const Op<T1, op_diagvec>
...
template <typename T1>
arma_warn_unused arma_inline const Op<T1, op_diagvec2>
...
template <typename T1>
arma_warn_unused arma_inline const SpOp<T1, spop_diagvec>
diagvec(const SpBase<typename T1::elem_type, T1> &X, const sword diag_id = 0) {
  arma_extra_debug_sigprint();

  return SpOp<T1, spop_diagvec>(X.get_ref(),
                                ((diag_id < 0) ? -diag_id : diag_id),
                                ((diag_id < 0) ? 1 : 0));
}
