...
template <> struct as_scalar_redirect<3> {
  ...
  inline static typename T1::elem_type
  apply(const Glue<Glue<T1, T2, glue_times>, T3, glue_times> &X);
};
...
as_scalar_redirect<2>::apply(const Glue<T1, T2, glue_times> &X) {
  ...
  if (do_partial_unwrap) {
    ...
    const uword A_n_rows = (tmp1.do_trans == false)
                               ? (TA::is_row ? 1 : A.n_rows)
                               : (TA::is_col ? 1 : A.n_cols);
    const uword A_n_cols = (tmp1.do_trans == false)
                               ? (TA::is_col ? 1 : A.n_cols)
                               : (TA::is_row ? 1 : A.n_rows);

    const uword B_n_rows = (tmp2.do_trans == false)
                               ? (TB::is_row ? 1 : B.n_rows)
                               : (TB::is_col ? 1 : B.n_cols);
    const uword B_n_cols = (tmp2.do_trans == false)
                               ? (TB::is_col ? 1 : B.n_cols)
                               : (TB::is_row ? 1 : B.n_rows);

    arma_debug_check((A_n_rows != 1) || (B_n_cols != 1) ||
                         (A_n_cols != B_n_rows),
                     "as_scalar(): incompatible dimensions");

    const eT val = op_dot::direct_dot(A.n_elem, A.memptr(), B.memptr());

    return (tmp1.do_times || tmp2.do_times)
               ? (val * tmp1.get_val() * tmp2.get_val())
               : val;
  } else {
...
inline typename T1::elem_type as_scalar_redirect<3>::apply(
  ...
  const bool tmp2_do_inv_gen =
      strip1.do_inv_gen && arma_config::optimise_invexpr;
  ...
  if (tmp2_do_diagmat == false) {
    ...
    const uword B_n_rows =
        (B_is_vec) ? B.n_elem
                   : ((tmp2.do_trans == false) ? B.n_rows : B.n_cols);
    const uword B_n_cols =
        (B_is_vec) ? B.n_elem
                   : ((tmp2.do_trans == false) ? B.n_cols : B.n_rows);
    ...
    arma_debug_check((A_n_rows != 1) || (C_n_cols != 1) ||
    ...
    if (B_is_vec) {
      if (tmp2_do_inv_gen) {
        return val *
               op_dotext::direct_rowvec_invdiagvec_colvec(A.mem, B, C.mem);
      } else {
      ...
      if (tmp2_do_inv_gen) {
        return val *
               op_dotext::direct_rowvec_invdiagmat_colvec(A.mem, B, C.mem);
      } else {
