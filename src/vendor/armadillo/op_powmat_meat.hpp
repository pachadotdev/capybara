...
template <typename T1>
inline bool
op_powmat_cx::apply_direct(Mat<std::complex<typename T1::pod_type>> &out,
                           const Base<typename T1::elem_type, T1> &X,
  ...
  if (y == in_T(int(y))) {
    arma_extra_debug_print("op_powmat_cx::apply_direct(): integer exponent "
                           "detected; redirecting to "
                           "op_powmat");
