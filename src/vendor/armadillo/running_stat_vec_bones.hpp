...
template <typename obj_type> struct rsv_get_elem_type {
  typedef typename rsv_get_elem_type_worker<
      obj_type, is_Mat<obj_type>::value>::result elem_type;
};
...
template <typename obj_type> struct rsv_get_return_type1 {
  typedef typename rsv_get_return_type1_worker<
      obj_type, is_Mat<obj_type>::value>::result return_type1;
};
...
public:
  ...
  typedef
      typename rsv_get_return_type2<return_type1>::return_type2 return_type2;

  inline ~running_stat_vec();
  inline running_stat_vec(
      const bool in_calc_cov = false); // TODO: investigate char* overload, eg.
                                       // "calc_cov", "no_calc_cov"
...
public:
  ...
  inline static void update_stats(
      ...
      const typename arma_not_cx<
          typename running_stat_vec<obj_type>::eT>::result *junk = nullptr);
