...
inline void spglue_times::apply_noalias(SpMat<eT> &c, const SpMat<eT> &x,
  ...
  // SpMat<typename T1::elem_type> c(x_n_rows, y_n_cols); // Initializes
  // col_ptrs to 0.
  ...
  do {
    ...
    if (old_col != y_it.col()) {
      // Set column pointer (this is not a cumulative count; that is done
      // later).
  ...
  // last_ind is already set to x_n_rows, and cur_col_length is already set to
  // 0. We will loop through all columns as necessary.
  uword cur_col = 0;
  while (cur_col < c.n_cols) {
    // Skip to next column with elements in it.
    while ((cur_col < c.n_cols) &&
           (c.col_ptrs[cur_col] == c.col_ptrs[cur_col + 1])) {
    ...
    while (y_col_it.col() == cur_col) {
      ...
      typename SpMat<eT>::const_iterator x_col_it =
          x.begin_col_no_sync(y_col_it_row);
...
template <typename T1, typename T2>
inline void
spglue_times_mixed::apply(SpMat<typename eT_promoter<T1, T2>::eT> &out,
                          const mtSpGlue<typename eT_promoter<T1, T2>::eT, T1,
                                         T2, spglue_times_mixed> &expr) {
  arma_extra_debug_sigprint();
