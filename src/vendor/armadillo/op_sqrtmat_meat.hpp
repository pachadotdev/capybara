...
template <typename T1>
inline bool
op_sqrtmat::apply_direct(Mat<std::complex<typename T1::elem_type>> &out,
...
op_sqrtmat::apply_direct(Mat<std::complex<typename T1::elem_type>> &out,
  ...
  if (A.is_diagmat()) {
    ...
    out.zeros(N,
              N); // aliasing can't happen as op_sqrtmat is defined as cx_mat =
                  // op(mat)
...
template <typename T1>
inline bool
op_sqrtmat_cx::apply_direct_noalias(Mat<typename T1::elem_type> &out,
...
template <typename T1>
inline bool
op_sqrtmat_cx::apply_direct(Mat<typename T1::elem_type> &out,
                            const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
...
template <typename T1>
inline bool
op_sqrtmat_sympd::apply_direct(Mat<typename T1::elem_type> &out,
                               const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if ((arma_config::debug) && (is_cx<eT>::yes) &&
      ...
      arma_debug_warn_level(
          1,
          "sqrtmat_sympd(): imaginary components on the diagonal are non-zero");
    }
    ...
    const bool status =
        eig_sym_helper(eigval, eigvec, X, 'd', "sqrtmat_sympd()");
