...
inline void spglue_merge::subview_merge(SpSubview<eT> &sv, const SpMat<eT> &B) {
  ...
  while (x_it_valid || y_it_valid) {
    ...
    const uword y_it_row =
        (y_it_valid) ? uword(sv_row_start + y_it.row()) : uword(0);
    const uword y_it_col =
        (y_it_valid) ? uword(sv_col_start + y_it.col()) : uword(0);
  ...
  arma_check((count != merge_n_nonzero),
             "internal error: spglue_merge::subview_merge(): count != "
             "merge_n_nonzero");
...
inline void spglue_merge::diagview_merge(SpMat<eT> &out, const SpMat<eT> &A,
  ...
  out.reserve(A.n_rows, A.n_cols,
              A.n_nonzero + B.n_nonzero); // worst case scenario
