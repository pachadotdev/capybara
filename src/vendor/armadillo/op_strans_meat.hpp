...
inline void op_strans::apply_mat_noalias_large(Mat<eT> &out, const Mat<eT> &A) {
  ...
  for (uword row = 0; row < n_rows_base; row += block_size) {
    ...
    for (uword col = 0; col < n_cols_base; col += block_size) {
      ...
      op_strans::block_worker(&Y[col + Y_offset], &X[row + X_offset], n_rows,
                              n_cols, block_size, block_size);
    }

    const uword X_offset = n_cols_base * n_rows;

    op_strans::block_worker(&Y[n_cols_base + Y_offset], &X[row + X_offset],
  ...
  for (uword col = 0; col < n_cols_base; col += block_size) {
    ...
    op_strans::block_worker(&Y[col + Y_offset], &X[n_rows_base + X_offset],
                            n_rows, n_cols, n_rows_extra, block_size);
  }

  const uword X_offset = n_cols_base * n_rows;

  op_strans::block_worker(&Y[n_cols_base + Y_offset],
...
template <typename eT> inline void op_strans::apply_mat_inplace(Mat<eT> &out) {
  arma_extra_debug_sigprint();
...
inline void op_strans::apply_proxy(Mat<typename T1::elem_type> &out,
                                   const Proxy<T1> &P) {
  arma_extra_debug_sigprint();
...
template <typename T1>
inline void op_strans::apply_direct(Mat<typename T1::elem_type> &out,
                                    const T1 &X) {
  arma_extra_debug_sigprint();
...
inline void op_strans_cube::apply_noalias(Cube<eT> &out, const Cube<eT> &X) {
  ...
  for (uword s = 0; s < X.n_slices; ++s) {
    ...
    const Mat<eT> X_slice(const_cast<eT *>(X.slice_memptr(s)), X.n_rows,
                          X.n_cols, false, true);
