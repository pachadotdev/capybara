...
inline void
SymEigsSolver<eT, SelectionRule, OpType>::fill_rand(eT *dest, const uword N,
...
template <typename eT, int SelectionRule, typename OpType>
inline void SymEigsSolver<eT, SelectionRule, OpType>::factorise_from(
    uword from_k, uword to_m, const Col<eT> &fk) {
  arma_extra_debug_sigprint();
  ...
  for (uword i = from_k; i <= to_m - 1; i++) {
    ...
    while (count < 5 && ortho_err > eps * beta) {
      // There is an edge case: when beta=||f|| is close to zero, f mostly
      // consists of rounding errors, so the test [ortho_err < eps * beta] is
      // very likely to fail. In particular, if beta=0, then the test is ensured
      // to fail. Hence when this happens, we force f to be zero, and then
...
inline void SymEigsSolver<eT, SelectionRule, OpType>::sort_ritzpair() {
  ...
  // SortEigenvalue<eT, EigsSelect::LARGEST_MAGN> sorting(ritz_val.memptr(),
  // nev);
...
template <typename eT, int SelectionRule, typename OpType>
inline SymEigsSolver<eT, SelectionRule, OpType>::SymEigsSolver(
    const OpType &op_, uword nev_, uword ncv_)
    : op(op_), nev(nev_), dim_n(op.n_rows), ncv(ncv_ > dim_n ? dim_n : ncv_),
      nmatop(0), niter(0), eps(std::numeric_limits<eT>::epsilon()),
  ...
  arma_debug_check((ncv_ <= nev_ || ncv_ > dim_n),
                   "newarp::SymEigsSolver: ncv must satisfy nev < ncv <= n, n "
...
template <typename eT, int SelectionRule, typename OpType>
inline uword SymEigsSolver<eT, SelectionRule, OpType>::compute(uword maxit,
                                                               eT tol) {
  arma_extra_debug_sigprint();
