...
inline void DoubleShiftQR<eT>::compute_reflector(const eT &x1, const eT &x2,
                                                 const eT &x3, uword ind) {
...
inline void DoubleShiftQR<eT>::update_block(uword il, uword iu) {
  ...
  if (bsize == 2) {
    ...
    // m10 = x10 * (x00 + x11 - s)
    eT m10 =
        mat_H(il + 1, il) * (mat_H(il, il) + mat_H(il + 1, il + 1) - shift_s);
    // This causes nr=2
...
template <typename eT>
inline void DoubleShiftQR<eT>::apply_PX(Mat<eT> &X, uword oi, uword oj,
                                        uword nrow, uword ncol, uword u_ind) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void DoubleShiftQR<eT>::apply_XP(Mat<eT> &X, uword oi, uword oj,
                                        uword nrow, uword ncol, uword u_ind) {
  arma_extra_debug_sigprint();
...
inline DoubleShiftQR<eT>::DoubleShiftQR(uword size)
    : n(size), prec(std::numeric_limits<eT>::epsilon()), eps_rel(prec),
      eps_abs(prec), computed(false) {
  arma_extra_debug_sigprint();
}

template <typename eT>
inline DoubleShiftQR<eT>::DoubleShiftQR(const Mat<eT> &mat_obj, eT s, eT t)
    : n(mat_obj.n_rows), mat_H(n, n), shift_s(s), shift_t(t), ref_u(3, n),
      ref_nr(n), prec(std::numeric_limits<eT>::epsilon()), eps_rel(prec),
      eps_abs(prec), computed(false) {
  arma_extra_debug_sigprint();
...
void DoubleShiftQR<eT>::compute(const Mat<eT> &mat_obj, eT s, eT t) {
  ...
  for (uword i = 0; i < n - 2; i++, Hii += (n + 1)) {
    ...
    if (h <= eps_abs ||
        h <= eps_rel * (std::abs(Hii[0]) + std::abs(Hii[n + 1]))) {
      Hii[1] = 0;
