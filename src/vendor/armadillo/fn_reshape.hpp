...
arma_frown("don't use this form: it will be removed") inline Mat<
    typename T1::elem_type> reshape(const Base<typename T1::elem_type, T1> &X,
                                    ...
                                    const uword new_n_cols, const uword dim) {
  arma_extra_debug_sigprint();
...
template <typename T1>
arma_warn_unused inline const OpCube<T1, op_reshape>
reshape(const BaseCube<typename T1::elem_type, T1> &X, const uword new_n_rows,
        const uword new_n_cols, const uword new_n_slices) {
  arma_extra_debug_sigprint();

  return OpCube<T1, op_reshape>(X.get_ref(), new_n_rows, new_n_cols,
                                new_n_slices);
}
...
reshape(const SpBase<typename T1::elem_type, T1> &X, const uword new_n_rows,
