...
inline void interp2_helper_nearest(const Mat<eT> &XG, const Mat<eT> &ZG,
                                   const Mat<eT> &XI, Mat<eT> &ZI,
                                   const eT extrap_val, const uword mode) {
...
template <typename eT>
inline void interp2_helper_linear(const Mat<eT> &XG, const Mat<eT> &ZG,
                                  const Mat<eT> &XI, Mat<eT> &ZI,
                                  const eT extrap_val, const uword mode) {
  arma_extra_debug_sigprint();
  ...
  for (uword i = 0; i < NI; ++i) {
    ...
    if ((XI_val < XG_min) || (XI_val > XG_max)) {
      ...
      const eT weight = (a_best_err > eT(0))
                            ? (a_best_err / (a_best_err + b_best_err))
                            : eT(0);

      if (mode == 0) {
        ZI.row(i) =
            (eT(1) - weight) * ZG.row(a_best_j) + (weight)*ZG.row(b_best_j);
      }
      if (mode == 1) {
        ZI.col(i) =
            (eT(1) - weight) * ZG.col(a_best_j) + (weight)*ZG.col(b_best_j);
      }
    }
  }
}
...
interp2(const Base<typename T1::elem_type, T1> &X,
        const Base<typename T1::elem_type, T2> &Y,
        const Base<typename T1::elem_type, T3> &Z,
        const Base<typename T1::elem_type, T4> &XI,
        const Base<typename T1::elem_type, T5> &YI,
        Mat<typename T1::elem_type> &ZI, const char *method = "linear",
        const typename T1::elem_type extrap_val =
            Datum<typename T1::elem_type>::nan) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check((UXG.M.is_vec() == false),
                   "interp2(): X must resolve to a vector");
  arma_debug_check((UYG.M.is_vec() == false),
                   "interp2(): Y must resolve to a vector");

  arma_debug_check((UXI.M.is_vec() == false),
                   "interp2(): XI must resolve to a vector");
  arma_debug_check((UYI.M.is_vec() == false),
                   "interp2(): YI must resolve to a vector");
  ...
  arma_debug_check((UXG.M.n_elem != UZG.M.n_cols),
                   "interp2(): number of elements in X must equal the number "
                   "of columns in Z");
  arma_debug_check(
