...
template <typename eT> inline podarray<eT>::podarray() : n_elem(0), mem(0) {
  arma_extra_debug_sigprint_this(this);
}
...
arma_inline eT podarray<eT>::operator()(const uword i) const {
  arma_debug_check_bounds((i >= n_elem),
                          "podarray::operator(): index out of bounds");
...
template <typename eT> arma_inline const eT *podarray<eT>::memptr() const {
  return mem;
}
...
inline void podarray<eT>::copy_row(const Mat<eT> &A, const uword row) {
  ...
  // note: this function assumes that the podarray has been set to the correct
  // size beforehand
...
inline void podarray<eT>::init_cold(const uword new_n_elem) {
  ...
  mem = (new_n_elem <= podarray_prealloc_n_elem::val)
            ? mem_local
