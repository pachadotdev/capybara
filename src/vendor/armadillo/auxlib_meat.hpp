...
template <typename eT> inline bool auxlib::inv(Mat<eT> &A) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (n > 16) {
      ...
      lapack::getri(&n, A.memptr(), &lda, ipiv.memptr(), &work_query[0],
                    &lwork_query, &info);
      ...
      blas_int lwork_proposed =
          static_cast<blas_int>(access::tmp_real(work_query[0]));
    ...
    arma_extra_debug_print("lapack::getri()");
    lapack::getri(&n, A.memptr(), &lda, ipiv.memptr(), work.memptr(), &lwork,
                  &info);
...
inline bool auxlib::inv_rcond(Mat<eT> &A,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_gen(A)
                   : lapack::lange<eT>(&norm_id, &n, &n, A.memptr(), &lda,
                                       junk.memptr());
...
template <typename eT>
inline bool auxlib::inv_tr_rcond(Mat<eT> &A,
                                 typename get_pod_type<eT>::result &out_rcond,
                                 const uword layout) {
  arma_extra_debug_sigprint();
...
inline bool auxlib::inv_sympd(Mat<eT> &A, bool &out_sympd_state) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // NOTE: for complex matrices, zpotrf() assumes the matrix is hermitian (not
    // simply symmetric)
...
template <typename eT>
inline bool auxlib::inv_sympd_rcond(Mat<eT> &A, bool &out_sympd_state,
                                    eT &out_rcond) {
  arma_extra_debug_sigprint();
...
template <typename T>
inline bool auxlib::inv_sympd_rcond(Mat<std::complex<T>> &A,
                                    bool &out_sympd_state, T &out_rcond) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline bool auxlib::det(eT &out_val, Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // on output A appears to be L+U_alt, where U_alt is U with the main
...
template <typename eT>
inline bool auxlib::log_det(eT &out_val,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    sword sign = (is_cx<eT>::no)
                     ? ((access::tmp_real(A.at(0, 0)) < T(0)) ? -1 : +1)
                     : +1;
    eT val = (is_cx<eT>::no) ? std::log((access::tmp_real(A.at(0, 0)) < T(0))
                                            ? A.at(0, 0) * T(-1)
                                            : A.at(0, 0))
                             : std::log(A.at(0, 0));
    ...
    for (uword i = 1; i < A.n_rows; ++i) {
      ...
      val += (is_cx<eT>::no)
                 ? std::log((access::tmp_real(x) < T(0)) ? x * T(-1) : x)
                 : std::log(x);
    }
...
template <typename eT, typename T1>
inline bool auxlib::lu(Mat<eT> &L, Mat<eT> &U, Mat<eT> &P,
                       const Base<eT, T1> &X) {
  arma_extra_debug_sigprint();
...
inline bool auxlib::lu(Mat<eT> &L, Mat<eT> &U, const Base<eT, T1> &X) {
  ...
  for (uword i = 0; i < n; ++i) {
    ...
    if (ipiv2_mem[i] != ipiv2_mem[k]) {
      std::swap(ipiv2_mem[i], ipiv2_mem[k]);
      L.swap_rows(static_cast<uword>(ipiv2_mem[i]),
                  static_cast<uword>(ipiv2_mem[k]));
    }
  }
...
inline bool auxlib::eig_gen(Mat<std::complex<typename T1::pod_type>> &vals,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork = 64 * N; // lwork_min = (vecs_on) ? (std::max)(blas_int(1),
                             // 4*N) : (std::max)(blas_int(1), 3*N)
    blas_int info = 0;
    ...
    lapack::geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals_real.memptr(),
                 vals_imag.memptr(), vl, &ldvl, vr, &ldvr, work.memptr(),
                 &lwork, &info);
    ...
    if (vecs_on) {
      for (uword j = 0; j < X.n_rows; ++j) {
        if ((j < (X.n_rows - 1)) &&
            (vals_mem[j] == std::conj(vals_mem[j + 1]))) {
          for (uword i = 0; i < X.n_rows; ++i) {
            vecs.at(i, j) = std::complex<T>(tmp.at(i, j), tmp.at(i, j + 1));
            vecs.at(i, j + 1) =
                std::complex<T>(tmp.at(i, j), -tmp.at(i, j + 1));
          }
...
template <typename T1>
inline bool
auxlib::eig_gen(Mat<std::complex<typename T1::pod_type>> &vals,
                Mat<std::complex<typename T1::pod_type>> &vecs,
                const bool vecs_on,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals.memptr(), vl,
                    &ldvl, vr, &ldvr, work.memptr(), &lwork, rwork.memptr(),
                    &info);
...
template <typename T1>
inline bool
auxlib::eig_gen_balance(Mat<std::complex<typename T1::pod_type>> &vals,
                        Mat<std::complex<typename T1::pod_type>> &vecs,
                        const bool vecs_on,
                        const Base<typename T1::pod_type, T1> &expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N,
                  vals_real.memptr(), vals_imag.memptr(), vl, &ldvl, vr, &ldvr,
                  &ilo, &ihi, scale.memptr(), &abnrm, rconde.memptr(),
                  rcondv.memptr(), work.memptr(), &lwork, iwork.memptr(),
                  &info);
...
inline bool auxlib::eig_gen_balance(
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::eig_gen_balance(): redirecting to "
                           "auxlib::eig_gen() due to crippled "
                           "LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    T abnrm = T(0);
    blas_int lwork =
        64 * N; // lwork_min = (std::max)(blas_int(1), blas_int(2*N))
    blas_int info = blas_int(0);
    ...
    lapack::cx_geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N,
                     vals.memptr(), vl, &ldvl, vr, &ldvr, &ilo, &ihi,
...
template <typename T1>
inline bool
auxlib::eig_gen_twosided(Mat<std::complex<typename T1::pod_type>> &vals,
    ...
    for (uword j = 0; j < X.n_rows; ++j) {
      if ((j < (X.n_rows - 1)) && (vals_mem[j] == std::conj(vals_mem[j + 1]))) {
        for (uword i = 0; i < X.n_rows; ++i) {
          ...
          lvecs.at(i, j + 1) =
              std::complex<T>(ltmp.at(i, j), -ltmp.at(i, j + 1));
          rvecs.at(i, j) = std::complex<T>(rtmp.at(i, j), rtmp.at(i, j + 1));
          rvecs.at(i, j + 1) =
              std::complex<T>(rtmp.at(i, j), -rtmp.at(i, j + 1));
        }
        ++j;
      } else {
        for (uword i = 0; i < X.n_rows; ++i) {
...
inline bool auxlib::eig_gen_twosided(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::cx_geev() -- START");
    lapack::cx_geev(&jobvl, &jobvr, &N, X.memptr(), &N, vals.memptr(),
                    lvecs.memptr(), &ldvl, rvecs.memptr(), &ldvr, work.memptr(),
...
//! two-sided eigen decomposition of general square matrix (real, balance given
//! matrix)
template <typename T1>
inline bool auxlib::eig_gen_twosided_balance(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    T abnrm = T(0);
    blas_int lwork =
        64 * N; // lwork_min = (std::max)(blas_int(1), blas_int(3*N))
    blas_int info = blas_int(0);
    ...
    arma_extra_debug_print("lapack::geevx() -- START");
    lapack::geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N,
                  vals_real.memptr(), vals_imag.memptr(), ltmp.memptr(), &ldvl,
                  rtmp.memptr(), &ldvr, &ilo, &ihi, scale.memptr(), &abnrm,
...
//! two-sided eigen decomposition of general square matrix (complex, balance
//! given matrix)
template <typename T1>
inline bool auxlib::eig_gen_twosided_balance(
    Mat<std::complex<typename T1::pod_type>> &vals,
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::eig_gen_twosided_balance(): redirecting to "
                           "auxlib::eig_gen() due to "
                           "crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::cx_geevx() -- START");
    lapack::cx_geevx(&bal, &jobvl, &jobvr, &sense, &N, X.memptr(), &N,
                     vals.memptr(), lvecs.memptr(), &ldvl, rvecs.memptr(),
                     &ldvr, &ilo, &ihi, scale.memptr(), &abnrm, rconde.memptr(),
                     rcondv.memptr(), work.memptr(), &lwork, rwork.memptr(),
                     &info);
    arma_extra_debug_print("lapack::cx_geevx() -- END");
...
inline bool auxlib::eig_pair(Mat<std::complex<typename T1::pod_type>> &vals,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N,
                 alphar.memptr(), alphai.memptr(), beta.memptr(), vl, &ldvl, vr,
    ...
    if (beta_has_zero) {
      arma_debug_warn_level(
          1, "eig_pair(): given matrices appear ill-conditioned");
    }
...
template <typename T1, typename T2>
inline bool
auxlib::eig_pair(Mat<std::complex<typename T1::pod_type>> &vals,
                 Mat<std::complex<typename T1::pod_type>> &vecs,
                 const bool vecs_on,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N,
                    alpha.memptr(), beta.memptr(), vl, &ldvl, vr, &ldvr,
                    work.memptr(), &lwork, rwork.memptr(), &info);
...
template <typename T1, typename T2>
inline bool
auxlib::eig_pair_twosided(Mat<std::complex<typename T1::pod_type>> &vals,
                          Mat<std::complex<typename T1::pod_type>> &lvecs,
                          Mat<std::complex<typename T1::pod_type>> &rvecs,
                          const Base<typename T1::pod_type, T1> &A_expr,
                          const Base<typename T1::pod_type, T2> &B_expr) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::ggev()");
    lapack::ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N,
                 alphar.memptr(), alphai.memptr(), beta.memptr(), ltmp.memptr(),
                 &ldvl, rtmp.memptr(), &ldvr, work.memptr(), &lwork, &info);
...
inline bool auxlib::eig_pair_twosided(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::cx_ggev()");
    lapack::cx_ggev(&jobvl, &jobvr, &N, A.memptr(), &N, B.memptr(), &N,
                    alpha.memptr(), beta.memptr(), lvecs.memptr(), &ldvl,
                    rvecs.memptr(), &ldvr, work.memptr(), &lwork,
                    rwork.memptr(), &info);
...
inline bool auxlib::eig_sym(Col<eT> &eigval, Mat<eT> &A) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::syev(&jobz, &uplo, &N, A.memptr(), &N, eigval.memptr(),
                 work.memptr(), &lwork, &info);
...
inline bool auxlib::eig_sym(Col<T> &eigval, Mat<std::complex<T>> &A) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::heev(&jobz, &uplo, &N, A.memptr(), &N, eigval.memptr(),
                 work.memptr(), &lwork, rwork.memptr(), &info);
...
template <typename eT>
inline bool auxlib::eig_sym(Col<eT> &eigval, Mat<eT> &eigvec,
                            const Mat<eT> &X) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::syev()");
    lapack::syev(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                 work.memptr(), &lwork, &info);
...
inline bool auxlib::eig_sym(Col<T> &eigval, Mat<std::complex<T>> &eigvec,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::heev()");
    lapack::heev(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                 work.memptr(), &lwork, rwork.memptr(), &info);
...
//! eigenvalues and eigenvectors of a symmetric real matrix (divide and conquer
//! algorithm)
template <typename eT>
inline bool auxlib::eig_sym_dc(Col<eT> &eigval, Mat<eT> &eigvec,
                               const Mat<eT> &X) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (N >= 32) {
      ...
      lapack::syevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                    &work_query[0], &lwork_query, &iwork_query[0],
                    &liwork_query, &info);
...
//! eigenvalues and eigenvectors of a hermitian complex matrix (divide and
//! conquer algorithm)
template <typename T>
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (N >= 32) {
      ...
      lapack::heevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                    &work_query[0], &lwork_query, &rwork_query[0],
                    &lrwork_query, &iwork_query[0], &liwork_query, &info);
    ...
    arma_extra_debug_print("lapack::heevd()");
    lapack::heevd(&jobz, &uplo, &N, eigvec.memptr(), &N, eigval.memptr(),
                  work.memptr(), &lwork_final, rwork.memptr(), &lrwork_final,
                  iwork.memptr(), &liwork_final, &info);
...
template <typename eT> inline bool auxlib::chol_simple(Mat<eT> &X) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::chol_band(): redirecting to auxlib::chol() "
                           "due to crippled LAPACK");
...
template <typename eT>
inline bool auxlib::chol_band_common(Mat<eT> &X, const uword KD,
                                     const uword layout) {
  arma_extra_debug_sigprint();
...
inline bool auxlib::chol_pivot(Mat<eT> &X, Mat<uword> &P, const uword layout) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::pstrf(&uplo, &n, X.memptr(), &n, ipiv.memptr(), &rank, &tol,
                  work.memptr(), &info);
...
inline bool auxlib::hess(Mat<eT> &H, const Base<eT, T1> &X, Col<eT> &tao) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (H.n_rows > 2) {
      ...
      lapack::gehrd(&n, &ilo, &ihi, H.memptr(), &lda, tao.memptr(),
                    work.memptr(), &lwork, &info);
...
inline bool auxlib::qr(Mat<eT> &Q, Mat<eT> &R, const Base<eT, T1> &X) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min = (std::max)(
        blas_int(1), (std::max)(m, n)); // take into account requirements of
                                        // geqrf() _and_ orgqr()/ungqr()
    ...
    lapack::geqrf(&m, &n, R.memptr(), &m, tau.memptr(), &work_query[0],
                  &lwork_query, &info);
    ...
    arma_extra_debug_print("lapack::geqrf()");
    lapack::geqrf(&m, &n, R.memptr(), &m, tau.memptr(), work.memptr(),
                  &lwork_final, &info);
...
inline bool auxlib::qr_econ(Mat<eT> &Q, Mat<eT> &R, const Base<eT, T1> &X) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::geqrf()");
    lapack::geqrf(&m, &n, Q.memptr(), &m, tau.memptr(), &work_query[0],
                  &lwork_query, &info);
    ...
    arma_extra_debug_print("lapack::geqrf()");
    lapack::geqrf(&m, &n, Q.memptr(), &m, tau.memptr(), work.memptr(),
                  &lwork_final, &info);
...
inline bool auxlib::qr_pivot(Mat<eT> &Q, Mat<eT> &R, Mat<uword> &P,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min = (std::max)(
        blas_int(3 * n + 1),
        (std::max)(m,
                   n)); // take into account requirements of geqp3() and orgqr()
    blas_int k = (std::min)(m, n);
    blas_int info = 0;
    ...
    lapack::geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(),
...
inline bool auxlib::qr_pivot(Mat<std::complex<T>> &Q, Mat<std::complex<T>> &R,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min = (std::max)(
        blas_int(3 * n + 1),
        (std::max)(m,
                   n)); // take into account requirements of geqp3() and ungqr()
    blas_int k = (std::min)(m, n);
    blas_int info = 0;
    ...
    arma_extra_debug_print("lapack::geqp3()");
    lapack::cx_geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(),
                     &work_query[0], &lwork_query, rwork.memptr(), &info);
    ...
    arma_extra_debug_print("lapack::geqp3()");
    lapack::cx_geqp3(&m, &n, R.memptr(), &m, jpvt.memptr(), tau.memptr(),
                     work.memptr(), &lwork_final, rwork.memptr(), &info);
...
template <typename eT> inline bool auxlib::svd(Col<eT> &S, Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork_min = (std::max)(
        blas_int(1), (std::max)((3 * min_mn + (std::max)(m, n)), 5 * min_mn));
    blas_int info = 0;
    ...
    if (A.n_elem >= 1024) {
      ...
      lapack::gesvd<eT>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(),
                        U.memptr(), &ldu, V.memptr(), &ldvt, &work_query[0],
                        &lwork_query, &info);
    ...
    arma_extra_debug_print("lapack::gesvd()");
    lapack::gesvd<eT>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(),
                      U.memptr(), &ldu, V.memptr(), &ldvt, work.memptr(),
                      &lwork_final, &info);
...
inline bool auxlib::svd(Col<T> &S, Mat<std::complex<T>> &A) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_elem >= 256) {
      ...
      lapack::cx_gesvd<T>(&jobu, &jobvt, &m, &n, A.memptr(), &lda, S.memptr(),
                          U.memptr(), &ldu, V.memptr(), &ldvt, &work_query[0],
...
template <typename T>
inline bool auxlib::svd(Mat<std::complex<T>> &U, Col<T> &S,
...
template <typename eT> inline bool auxlib::svd_dc(Col<eT> &S, Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_elem >= 1024) {
      ...
      lapack::gesdd<eT>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(),
...
inline bool auxlib::svd_dc(Col<T> &S, Mat<std::complex<T>> &A) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_elem >= 256) {
      ...
      lapack::cx_gesdd<T>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(),
                          U.memptr(), &ldu, V.memptr(), &ldvt, &work_query[0],
                          &lwork_query, rwork.memptr(), iwork.memptr(), &info);
    ...
    arma_extra_debug_print("lapack::cx_gesdd()");
    lapack::cx_gesdd<T>(&jobz, &m, &n, A.memptr(), &lda, S.memptr(), U.memptr(),
                        &ldu, V.memptr(), &ldvt, work.memptr(), &lwork_final,
                        rwork.memptr(), iwork.memptr(), &info);
...
inline bool auxlib::svd_dc(Mat<eT> &U, Col<eT> &S, Mat<eT> &V, Mat<eT> &A) {
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork1 =
        3 * min_mn * min_mn +
        (std::max)(max_mn,
                   4 * min_mn * min_mn + 4 * min_mn); // as per LAPACK 3.2 docs
    blas_int lwork2 =
        4 * min_mn * min_mn + 6 * min_mn +
        max_mn; // as per LAPACK 3.8 docs; consistent with LAPACK 3.4 docs
    blas_int lwork_min = (std::max)(
        lwork1, lwork2); // due to differences between LAPACK 3.2 and 3.8
    blas_int info = 0;
...
template <typename T>
inline bool auxlib::svd_dc(Mat<std::complex<T>> &U, Col<T> &S,
                           Mat<std::complex<T>> &V, Mat<std::complex<T>> &A) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int ldu = blas_int(U.n_rows);
    blas_int ldvt = blas_int(V.n_rows);
    blas_int lwork_min = min_mn * min_mn + 2 * min_mn +
                         max_mn; // as per LAPACK 3.2, 3.4, 3.8 docs
    blas_int lrwork =
...
template <typename eT>
inline bool auxlib::svd_dc_econ(Mat<eT> &U, Col<eT> &S, Mat<eT> &V,
                                Mat<eT> &A) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int lwork2 = 4 * min_mn * min_mn + 6 * min_mn +
                      max_mn; // as per LAPACK 3.4 docs; LAPACK 3.8 requires
                              // 4*min_mn*min_mn + 7*min_mn
    blas_int lwork_min = (std::max)(
        lwork1, lwork2); // due to differences between LAPACK 3.2 and 3.4
    blas_int info = 0;
...
inline bool auxlib::svd_dc_econ(Mat<std::complex<T>> &U, Col<T> &S,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int ldvt = min_mn;
    blas_int lwork_min =
        min_mn * min_mn + 2 * min_mn + max_mn; // as per LAPACK 3.2 docs
    blas_int lrwork =
        min_mn *
        ((std::max)(5 * min_mn + 7,
                    2 * max_mn + 2 * min_mn +
...
template <typename T1>
inline bool
auxlib::solve_square_fast(Mat<typename T1::elem_type> &out,
...
template <typename T1>
inline bool
auxlib::solve_square_rcond(Mat<typename T1::elem_type> &out,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::getrs<eT>(&trans, &n, &nrhs, A.memptr(), &lda, ipiv.memptr(),
                      out.memptr(), &ldb, &info);
...
//! solve a system of linear equations via LU decomposition with refinement
//! (real matrices)
template <typename T1>
inline bool auxlib::solve_square_refine(
    Mat<typename T1::pod_type> &out, typename T1::pod_type &out_rcond,
    Mat<typename T1::pod_type> &A,
    const Base<typename T1::pod_type, T1> &B_expr, const bool equilibrate) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    quasi_unwrap<T1> UB(
        B_expr.get_ref()); // deliberately not declaring as const

    const Mat<eT> &UB_M_as_Mat =
        UB.M; // so we don't confuse the ?: operator below
    ...
    lapack::gesvx(&fact, &trans, &n, &nrhs, A.memptr(), &lda, AF.memptr(),
                  &ldaf, IPIV.memptr(), &equed, R.memptr(), C.memptr(),
                  const_cast<eT *>(B.memptr()), &ldb, out.memptr(), &ldx,
                  &rcond, FERR.memptr(), BERR.memptr(), WORK.memptr(),
                  IWORK.memptr(), &info);

    // NOTE: using const_cast<eT*>(B.memptr()) to allow B to be overwritten for
    // equilibration; NOTE: B is created as a copy of B_expr if equilibration is
    // enabled; otherwise B is a reference to B_expr
...
inline bool auxlib::solve_square_refine(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gesvx(&fact, &trans, &n, &nrhs, A.memptr(), &lda, AF.memptr(),
                     &ldaf, IPIV.memptr(), &equed, R.memptr(), C.memptr(),
                     const_cast<eT *>(B.memptr()), &ldb, out.memptr(), &ldx,
                     &rcond, FERR.memptr(), BERR.memptr(), WORK.memptr(),
                     RWORK.memptr(), &info);
...
template <typename T1>
inline bool
auxlib::solve_sympd_fast(Mat<typename T1::elem_type> &out,
                         Mat<typename T1::elem_type> &A,
                         const Base<typename T1::elem_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_sympd_fast(): redirecting to "
                           "auxlib::solve_square_fast() due to "
                           "crippled LAPACK");
...
inline bool auxlib::solve_sympd_fast_common(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::posv<eT>(&uplo, &n, &nrhs, A.memptr(), &lda, out.memptr(), &ldb,
                     &info);
...
//! solve a system of linear equations via Cholesky decomposition with rcond
//! estimate (real matrices)
template <typename T1>
inline bool auxlib::solve_sympd_rcond(
    Mat<typename T1::pod_type> &out, bool &out_sympd_state,
    typename T1::pod_type &out_rcond, Mat<typename T1::pod_type> &A,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::potrs<eT>(&uplo, &n, &nrhs, A.memptr(), &n, out.memptr(), &n,
                      &info);
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_sympd_rcond(): redirecting to "
                           "auxlib::solve_square_rcond() due to "
                           "crippled LAPACK");
...
//! solve a system of linear equations via Cholesky decomposition with
//! refinement (real matrices)
template <typename T1>
inline bool auxlib::solve_sympd_refine(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::posvx(&fact, &uplo, &n, &nrhs, A.memptr(), &lda, AF.memptr(), &ldaf,
                  &equed, S.memptr(), const_cast<eT *>(B.memptr()), &ldb,
                  out.memptr(), &ldx, &rcond, FERR.memptr(), BERR.memptr(),
...
inline bool auxlib::solve_sympd_refine(
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_sympd_refine(): redirecting to "
                           "auxlib::solve_square_refine() due "
                           "to crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    // Mat<eT> B = B_expr.get_ref();  // B is overwritten by lapack::cx_posvx()
    // if equilibrate is enabled
    ...
    lapack::cx_posvx(&fact, &uplo, &n, &nrhs, A.memptr(), &lda, AF.memptr(),
                     &ldaf, &equed, S.memptr(), const_cast<eT *>(B.memptr()),
                     &ldb, out.memptr(), &ldx, &rcond, FERR.memptr(),
                     BERR.memptr(), WORK.memptr(), RWORK.memptr(), &info);
...
template <typename T1>
inline bool
auxlib::solve_rect_fast(Mat<typename T1::elem_type> &out,
                        Mat<typename T1::elem_type> &A,
                        const Base<typename T1::elem_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    blas_int min_mn = (std::min)(m, n);
    blas_int lwork_min =
        (std::max)(blas_int(1), min_mn + (std::max)(min_mn, nrhs));
    blas_int info = 0;
    ...
    if (A.n_elem >= ((is_cx<eT>::yes) ? uword(256) : uword(1024))) {
      ...
      lapack::gels<eT>(&trans, &m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(),
                       &ldb, &work_query[0], &lwork_query, &info);
    ...
    arma_extra_debug_print("lapack::gels()");
    lapack::gels<eT>(&trans, &m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(),
                     &ldb, work.memptr(), &lwork_final, &info);
...
//! solve a non-square full-rank system via QR or LQ decomposition with rcond
//! estimate (experimental)
template <typename T1>
inline bool
auxlib::solve_rect_rcond(Mat<typename T1::elem_type> &out,
                         typename T1::pod_type &out_rcond,
                         Mat<typename T1::elem_type> &A,
                         const Base<typename T1::elem_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    if (A.n_rows >= A.n_cols) {
      ...
      // xGELS  docs: for M >= N, A contains details of its QR decomposition as
      // returned by xGEQRF xGEQRF docs: elements on and above the diagonal
      ...
      out_rcond = auxlib::rcond_trimat(
          R, 0); // 0: upper triangular; 1: lower triangular
    } else if (A.n_rows < A.n_cols) {
      arma_extra_debug_print("estimating rcond via L");

      // xGELS  docs: for M < N, A contains details of its LQ decomposition as
      // returned by xGELQF xGELQF docs: elements on and below the diagonal
      ...
      // determine quality of solution
      out_rcond = auxlib::rcond_trimat(
          L, 1); // 0: upper triangular; 1: lower triangular
    }
...
template <typename T1>
inline bool
auxlib::solve_approx_svd(Mat<typename T1::pod_type> &out,
                         Mat<typename T1::pod_type> &A,
                         const Base<typename T1::pod_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    eT rcond =
        (std::max)(A.n_rows, A.n_cols) * std::numeric_limits<eT>::epsilon();
    ...
    lapack::gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb,
                  S.memptr(), &rcond, &rank, &work_query[0], &lwork_query,
                  iwork.memptr(), &info);
    ...
    arma_extra_debug_print("lapack::gelsd()");
    lapack::gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb,
                  S.memptr(), &rcond, &rank, work.memptr(), &lwork_final,
                  iwork.memptr(), &info);
...
inline bool auxlib::solve_approx_svd(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    T rcond =
        (std::max)(A.n_rows, A.n_cols) * std::numeric_limits<T>::epsilon();
    blas_int rank = blas_int(0);
    blas_int info = blas_int(0);
    ...
    blas_int liwork =
        (std::max)(blas_int(1), (blas_int(3) * blas_int(min_mn) * nlvl +
    ...
    lapack::cx_gelsd(&m, &n, &nrhs, A.memptr(), &lda, tmp.memptr(), &ldb,
                     S.memptr(), &rcond, &rank, &work_query[0], &lwork_query,
                     rwork.memptr(), iwork.memptr(), &info);
...
template <typename T1>
inline bool auxlib::solve_trimat_fast(
    Mat<typename T1::elem_type> &out, const Mat<typename T1::elem_type> &A,
    const Base<typename T1::elem_type, T1> &B_expr, const uword layout) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::trtrs(&uplo, &trans, &diag, &n, &nrhs, A.memptr(), &n, out.memptr(),
                  &n, &info);
...
template <typename T1>
inline bool auxlib::solve_trimat_rcond(
...
template <typename T1>
inline bool
auxlib::solve_band_fast(Mat<typename T1::pod_type> &out,
...
//! solve a system of linear equations via LU decomposition (complex band
//! matrix)
template <typename T1>
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_band_fast(): redirecting to "
                           "auxlib::solve_square_fast() due to "
                           "crippled LAPACK");
...
template <typename T1>
inline bool
auxlib::solve_band_fast_common(Mat<typename T1::elem_type> &out,
                               const Mat<typename T1::elem_type> &A,
                               const uword KL, const uword KU,
                               const Base<typename T1::elem_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // for gbsv, matrix AB size: 2*KL+KU+1 x N; band representation of A stored
    // in rows KL+1 to 2*KL+KU+1  (note: fortran counts from 1)
    ...
    lapack::gbsv<eT>(&n, &kl, &ku, &nrhs, AB.memptr(), &ldab, ipiv.memptr(),
                     out.memptr(), &ldb, &info);
...
inline bool auxlib::solve_band_rcond(
    Mat<typename T1::pod_type> &out, typename T1::pod_type &out_rcond,
    Mat<typename T1::pod_type> &A, const uword KL, const uword KU,
    const Base<typename T1::pod_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_band_rcond(): redirecting to "
                           "auxlib::solve_square_rcond() due to "
                           "crippled LAPACK");
...
inline bool auxlib::solve_band_rcond_common(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // for gbtrf, matrix AB size: 2*KL+KU+1 x N; band representation of A stored
    // in rows KL+1 to 2*KL+KU+1  (note: fortran counts from 1)
    ...
    // // NOTE: lapack::langb() and lapack::gbtrf() use incompatible storage
    // formats for the band matrix arma_extra_debug_print("lapack::langb()");
    // norm_val = lapack::langb<eT>(&norm_id, &n, &kl, &ku, AB.memptr(), &ldab,
    // junk.memptr());

    norm_val = auxlib::norm1_band(A, KL, KU);

    arma_extra_debug_print("lapack::gbtrf()");
    lapack::gbtrf<eT>(&n, &n, &kl, &ku, AB.memptr(), &ldab, ipiv.memptr(),
                      &info);
    ...
    lapack::gbtrs<eT>(&trans, &n, &kl, &ku, &nrhs, AB.memptr(), &ldab,
                      ipiv.memptr(), out.memptr(), &ldb, &info);
...
inline bool auxlib::solve_band_refine(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    // for gbsvx, matrix AB size: KL+KU+1 x N; band representation of A stored
    // in rows 1 to KL+KU+1  (note: fortran counts from 1)
    ...
    lapack::gbsvx(&fact, &trans, &n, &kl, &ku, &nrhs, AB.memptr(), &ldab,
                  AFB.memptr(), &ldafb, IPIV.memptr(), &equed, R.memptr(),
                  C.memptr(), B.memptr(), &ldb, out.memptr(), &ldx, &rcond,
                  FERR.memptr(), BERR.memptr(), WORK.memptr(), IWORK.memptr(),
                  &info);
...
inline bool auxlib::solve_band_refine(
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_band_refine(): redirecting to "
                           "auxlib::solve_square_refine() due "
                           "to crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    podarray<T> RWORK(N); // NOTE: according to lapack 3.6.1 docs, the size of
                          // RWORK in zgbsvx is different to RWORK in dgesvx

    arma_extra_debug_print("lapack::cx_gbsvx()");
    lapack::cx_gbsvx(&fact, &trans, &n, &kl, &ku, &nrhs, AB.memptr(), &ldab,
                     AFB.memptr(), &ldafb, IPIV.memptr(), &equed, R.memptr(),
                     C.memptr(), B.memptr(), &ldb, out.memptr(), &ldx, &rcond,
                     FERR.memptr(), BERR.memptr(), WORK.memptr(),
                     RWORK.memptr(), &info);
...
//! solve a system of linear equations via Gaussian elimination with partial
//! pivoting (real tridiagonal band matrix)
template <typename T1>
inline bool
auxlib::solve_tridiag_fast(Mat<typename T1::pod_type> &out,
                           Mat<typename T1::pod_type> &A,
                           const Base<typename T1::pod_type, T1> &B_expr) {
  arma_extra_debug_sigprint();
...
//! solve a system of linear equations via Gaussian elimination with partial
//! pivoting (complex tridiagonal band matrix)
template <typename T1>
...
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::solve_tridiag_fast(): redirecting to "
                           "auxlib::solve_square_fast() due to "
                           "crippled LAPACK");
...
//! solve a system of linear equations via Gaussian elimination with partial
//! pivoting (tridiagonal band matrix)
template <typename T1>
inline bool auxlib::solve_tridiag_fast_common(
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::gtsv<eT>(&n, &nrhs, tridiag.colptr(0), tridiag.colptr(1),
                     tridiag.colptr(2), out.memptr(), &ldb, &info);
...
inline bool auxlib::schur(Mat<eT> &U, Mat<eT> &S, const Base<eT, T1> &X,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::gees(&jobvs, &sort, select, &n, S.memptr(), &n, &sdim, wr.memptr(),
                 wi.memptr(), U.memptr(), &ldvs, work.memptr(), &lwork,
                 bwork.memptr(), &info);
...
inline bool auxlib::schur(Mat<std::complex<T>> &U, Mat<std::complex<T>> &S,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gees(&jobvs, &sort, select, &n, S.memptr(), &n, &sdim,
                    w.memptr(), U.memptr(), &ldvs, work.memptr(), &lwork,
                    rwork.memptr(), bwork.memptr(), &info);
...
inline bool auxlib::syl(Mat<eT> &X, const Mat<eT> &A, const Mat<eT> &B,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::trsyl<eT>(&trana, &tranb, &isgn, &m, &n, T1.memptr(), &m,
                      T2.memptr(), &n, Y.memptr(), &m, &scale, &info);
...
inline bool auxlib::qz(Mat<T> &A, Mat<T> &B, Mat<T> &vsl, Mat<T> &vsr,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::gges(&jobvsl, &jobvsr, &eigsort, selctg, &N, A.memptr(), &N,
                 B.memptr(), &N, &sdim, alphar.memptr(), alphai.memptr(),
                 beta.memptr(), vsl.memptr(), &N, vsr.memptr(), &N,
                 work.memptr(), &lwork, bwork.memptr(), &info);
...
inline bool auxlib::qz(Mat<std::complex<T>> &A, Mat<std::complex<T>> &B,
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gges(&jobvsl, &jobvsr, &eigsort, selctg, &N, A.memptr(), &N,
                    B.memptr(), &N, &sdim, alpha.memptr(), beta.memptr(),
                    vsl.memptr(), &N, vsr.memptr(), &N, work.memptr(), &lwork,
                    rwork.memptr(), bwork.memptr(), &info);
...
template <typename eT> inline eT auxlib::rcond(Mat<eT> &A) {
#if defined(ARMA_USE_LAPACK)
  {
    arma_debug_assert_blas_size(A);
    ...
    lapack::gecon(&norm_id, &n, A.memptr(), &lda, &norm_val, &rcond,
                  work.memptr(), iwork.memptr(), &info);
...
template <typename T> inline T auxlib::rcond(Mat<std::complex<T>> &A) {
#if defined(ARMA_USE_LAPACK)
  {
    typedef typename std::complex<T> eT;
    ...
    lapack::cx_gecon(&norm_id, &n, A.memptr(), &lda, &norm_val, &rcond,
...
inline eT auxlib::rcond_sympd(Mat<eT> &A, bool &calc_ok) {
#if defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_sym(A)
                   : lapack::lansy(&norm_id, &uplo, &n, A.memptr(), &lda,
                                   work.memptr());
...
inline T auxlib::rcond_sympd(Mat<std::complex<T>> &A, bool &calc_ok) {
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_extra_debug_print("auxlib::rcond_sympd(): redirecting to "
                           "auxlib::rcond() due to crippled LAPACK");
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    norm_val = (has_blas_float_bug<eT>::value)
                   ? auxlib::norm1_sym(A)
                   : lapack::lanhe(&norm_id, &uplo, &n, A.memptr(), &lda,
                                   rwork.memptr());
    ...
    lapack::cx_pocon(&uplo, &n, A.memptr(), &lda, &norm_val, &rcond,
                     work.memptr(), rwork.memptr(), &info);
...
inline eT auxlib::rcond_trimat(const Mat<eT> &A, const uword layout) {
#if defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::trcon(&norm_id, &uplo, &diag, &n, A.memptr(), &n, &rcond,
                  work.memptr(), iwork.memptr(), &info);
...
template <typename T>
inline T auxlib::rcond_trimat(const Mat<std::complex<T>> &A,
                              const uword layout) {
#if defined(ARMA_USE_LAPACK)
  {
    typedef typename std::complex<T> eT;
    ...
    lapack::cx_trcon(&norm_id, &uplo, &diag, &n, A.memptr(), &n, &rcond,
                     work.memptr(), rwork.memptr(), &info);
...
template <typename T>
inline T auxlib::lu_rcond_sympd(const Mat<std::complex<T>> &A,
                                const T norm_val) {
#if defined(ARMA_CRIPPLED_LAPACK)
  {
    arma_ignore(A);
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    arma_extra_debug_print("lapack::cx_pocon()");
    lapack::cx_pocon(&uplo, &n, A.memptr(), &n, &norm_val, &rcond,
                     work.memptr(), rwork.memptr(), &info);
...
template <typename eT>
inline eT auxlib::lu_rcond_band(const Mat<eT> &AB, const uword KL,
...
inline T auxlib::lu_rcond_band(const Mat<std::complex<T>> &AB, const uword KL,
...
#elif defined(ARMA_USE_LAPACK)
  {
    ...
    lapack::cx_gbcon<T>(&norm_id, &n, &kl, &ku, AB.memptr(), &ldab,
                        ipiv.memptr(), &norm_val, &rcond, work.memptr(),
                        rwork.memptr(), &info);
...
inline bool auxlib::rudimentary_sym_check(const Mat<eT> &X) {
  ...
  const eT A2 =
      *(X_offsetA + 1); // bottom-left corner (ie. last value in first column)
  const eT B1 = *(X_offsetB);
  const eT B2 =
      *(X_offsetB + N); // top-right   corner (ie. first value in last column)
  ...
  const eT tol =
      eT(10000) * std::numeric_limits<eT>::epsilon(); // allow some leeway
...
inline bool auxlib::rudimentary_sym_check(const Mat<std::complex<T>> &X) {
  ...
  // NOTE: the function name is a misnomer, as it checks for hermitian complex
  // matrices; NOTE: for simplicity of use, the function name is the same as for
  // real matrices
  ...
  const T tol =
      T(10000) * std::numeric_limits<T>::epsilon(); // allow some leeway
  ...
  const eT &A =
      X_mem[Nm1]; // bottom-left corner (ie. last value in first column)
  const eT &B =
      X_mem[Nm1 * N]; // top-right   corner (ie. first value in last column)
  ...
  const bool okay_real =
      ((delta_real <= tol) || (delta_real <= (C_real * tol)));
  const bool okay_imag =
      ((delta_imag <= tol) || (delta_imag <= (C_imag * tol)));
...
auxlib::norm1_band(const Mat<eT> &A, const uword KL, const uword KU) {
  ...
  for (uword c = 0; c < n_cols; ++c) {
    ...
    // use values only from main diagonal + KU upper diagonals + KL lower
    // diagonals
...
inline blas_int select_iuc(const T *x_ptr, const T *y_ptr, const T *z_ptr) {
  ...
  // return (std::abs(std::complex<T>(x,y) / z) < T(1)) ? blas_int(1) :
  // blas_int(0);
...
inline blas_int select_ouc(const T *x_ptr, const T *y_ptr, const T *z_ptr) {
  ...
  if (z == T(0)) {
    return (x == T(0)) ? blas_int(0)
                       : blas_int(1); // consider an infinite eig value to
...
inline blas_int cx_select_ouc(const std::complex<T> *x_ptr,
  ...
  if ((y.real() == T(0)) && (y.imag() == T(0))) {
    return ((x.real() == T(0)) && (x.imag() == T(0)))
               ? blas_int(0)
               : blas_int(1); // consider an infinite eig value to be outside of
                              // the unit circle
  }
...
// - we're using LAPACK ?gges() defined to expect pointer-to-function to be
// passed as pointer-to-object
// - explicit casting between pointer-to-function and pointer-to-object is a
// non-standard extension in C
// - the extension is essentially mandatory on POSIX systems
// - some compilers will complain about the extension in pedantic mode

template <typename T>
inline void_ptr ptr_cast(blas_int (*function)(const T *, const T *,
                                              const T *)) {
  union converter {
