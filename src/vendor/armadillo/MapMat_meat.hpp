...
template <typename eT>
arma_inline MapMat_val<eT> MapMat<eT>::operator()(const uword index) {
  arma_debug_check_bounds((index >= n_elem),
...
template <typename eT>
arma_inline MapMat_val<eT> MapMat<eT>::at(const uword in_row,
                                          const uword in_col) {
...
template <typename eT> inline bool MapMat<eT>::is_empty() const {
  return (n_elem == 0);
}

template <typename eT> inline bool MapMat<eT>::is_vec() const {
  return ((n_rows == 1) || (n_cols == 1));
}

template <typename eT> inline bool MapMat<eT>::is_rowvec() const {
  return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT> inline bool MapMat<eT>::is_colvec() const {
  return (n_cols == 1);
}

template <typename eT> inline bool MapMat<eT>::is_square() const {
...
inline void MapMat<eT>::print(const std::string &extra_text) const {
  ...
  const double density =
      (n_elem > 0) ? ((double(n_nonzero) / double(n_elem)) * double(100))
                   : double(0);
...
#else
  const char *error_message = "MapMat(): requested size is too large; suggest "
                              "to enable ARMA_64BIT_WORD";
#endif
...
template <typename eT>
inline void MapMat<eT>::init_warm(const uword in_n_rows,
...
arma_inline void MapMat<eT>::set_val(const uword index, const eT &in_val) {
  ...
  if (in_val != eT(0)) {
    ...
    if ((map_ref.empty() == false) &&
        (index > uword(map_ref.crbegin()->first))) {
...
template <typename eT>
arma_inline MapMat_val<eT>::MapMat_val(MapMat<eT> &in_parent,
...
arma_inline void MapMat_val<eT>::operator+=(const eT in_val) {
  ...
  if (in_val != eT(0)) {
    eT &val =
        map_ref.operator[](index); // creates the element if it doesn't exist
...
template <typename eT> inline SpMat_MapMat_val<eT>::operator eT() const {
  ...
  const SpMat<eT> &const_s_parent =
      s_parent; // declare as const for clarity of intent
...
template <typename eT> inline eT SpMat_MapMat_val<eT>::operator++(int) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat_MapMat_val<eT>::set(const eT in_val) {
  arma_extra_debug_sigprint();

  const bool done = (s_parent.sync_state == 0)
                        ? s_parent.try_set_value_csc(row, col, in_val)
                        : false;
...
template <typename eT> inline void SpMat_MapMat_val<eT>::add(const eT in_val) {
  arma_extra_debug_sigprint();

  const bool done = (s_parent.sync_state == 0)
                        ? s_parent.try_add_value_csc(row, col, in_val)
                        : false;
...
template <typename eT> inline void SpMat_MapMat_val<eT>::sub(const eT in_val) {
  arma_extra_debug_sigprint();

  const bool done = (s_parent.sync_state == 0)
                        ? s_parent.try_sub_value_csc(row, col, in_val)
                        : false;
...
template <typename eT> inline void SpMat_MapMat_val<eT>::mul(const eT in_val) {
  arma_extra_debug_sigprint();

  const bool done = (s_parent.sync_state == 0)
                        ? s_parent.try_mul_value_csc(row, col, in_val)
                        : false;
  ...
  if (done == false) {
    ...
    if (it != it_end) {
      ...
      // element not found, ie. it's zero; zero multiplied by anything is zero,
      // except for nan and inf
...
template <typename eT> inline void SpMat_MapMat_val<eT>::div(const eT in_val) {
  arma_extra_debug_sigprint();

  const bool done = (s_parent.sync_state == 0)
                        ? s_parent.try_div_value_csc(row, col, in_val)
                        : false;
  ...
  if (done == false) {
    ...
    if (it != it_end) {
      ...
      // element not found, ie. it's zero; zero divided by anything is zero,
      // except for zero and nan
...
template <typename eT>
arma_inline SpSubview_MapMat_val<eT>::SpSubview_MapMat_val(
    SpSubview<eT> &in_sv_parent, MapMat<eT> &in_m_parent, const uword in_row,
    const uword in_col)
    : SpMat_MapMat_val<eT>(access::rw(in_sv_parent.m), in_m_parent, in_row,
                           in_col),
...
template <typename eT>
inline SpSubview_MapMat_val<eT> &
...
template <typename eT> inline eT SpSubview_MapMat_val<eT>::operator++(int) {
  arma_extra_debug_sigprint();
