...
template <typename eT> inline void gmm_diag<eT>::reset() {
  arma_extra_debug_sigprint();
...
inline void gmm_diag<eT>::set_params(const Base<eT, T1> &in_means_expr,
  ...
  arma_debug_check((arma::size(in_means) != arma::size(in_dcovs)) ||
                       (in_hefts.n_cols != in_means.n_cols) ||
                       (in_hefts.n_rows != 1),
                   "gmm_diag::set_params(): given parameters have inconsistent "
                   "and/or wrong sizes");

  arma_debug_check(
      (in_means.internal_has_nonfinite()),
      "gmm_diag::set_params(): given means have non-finite values");
  arma_debug_check(
      (in_dcovs.internal_has_nonfinite()),
      "gmm_diag::set_params(): given dcovs have non-finite values");
  arma_debug_check(
      (in_hefts.internal_has_nonfinite()),
      "gmm_diag::set_params(): given hefts have non-finite values");

  arma_debug_check(
      (any(vectorise(in_dcovs) <= eT(0))),
...
template <typename eT> inline uword gmm_diag<eT>::n_dims() const {
  return means.n_rows;
}

template <typename eT> inline uword gmm_diag<eT>::n_gaus() const {
  return means.n_cols;
}

template <typename eT> inline bool gmm_diag<eT>::load(const std::string name) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline Col<eT> gmm_diag<eT>::generate() const {
  arma_extra_debug_sigprint();
...
inline eT gmm_diag<eT>::log_p(
    ...
    typename enable_if<((is_arma_type<T1>::value) &&
                        (resolves_to_colvector<T1>::value == true))>::result
        *junk2) const {
  arma_extra_debug_sigprint();
  arma_ignore(junk1);
  arma_ignore(junk2);
...
inline Row<eT> gmm_diag<eT>::log_p(
    const T1 &expr, const gmm_empty_arg &junk1,
    typename enable_if<((is_arma_type<T1>::value) &&
                        (resolves_to_colvector<T1>::value == false))>::result
        *junk2) const {
  arma_extra_debug_sigprint();
  arma_ignore(junk1);
  arma_ignore(junk2);
...
template <typename eT>
template <typename T1>
inline eT gmm_diag<eT>::sum_log_p(const Base<eT, T1> &expr,
...
template <typename eT>
template <typename T1>
inline eT gmm_diag<eT>::avg_log_p(const Base<eT, T1> &expr,
                                  const uword gaus_id) const {
  arma_extra_debug_sigprint();
...
inline bool gmm_diag<eT>::learn(const Base<eT, T1> &data, const uword N_gaus,
                                ...
                                const uword km_iter, const uword em_iter,
                                const eT var_floor, const bool print_mode) {
  arma_extra_debug_sigprint();

  const bool dist_mode_ok =
      (dist_mode == eucl_dist) || (dist_mode == maha_dist);

  const bool seed_mode_ok =
      (seed_mode == keep_existing) || (seed_mode == static_subset) ||
      (seed_mode == static_spread) || (seed_mode == random_subset) ||
      (seed_mode == random_spread);

  arma_debug_check(
      (dist_mode_ok == false),
      "gmm_diag::learn(): dist_mode must be eucl_dist or maha_dist");
  arma_debug_check((seed_mode_ok == false),
                   "gmm_diag::learn(): unknown seed_mode");
  arma_debug_check((var_floor < eT(0)),
  ...
  if (X.internal_has_nonfinite()) {
    arma_debug_warn_level(
        3, "gmm_diag::learn(): given matrix has non-finite values");
    return false;
  }
  ...
  if (dist_mode == maha_dist) {
    ...
    for (uword i = 0; i < mah_aux_n_elem; ++i) {
      ...
      mah_aux_mem[i] =
          ((val != eT(0)) && arma_isfinite(val)) ? eT(1) / val : eT(1);
    }
  }
  ...
  if (seed_mode == keep_existing) {
    ...
    if (X.n_cols < N_gaus) {
      arma_debug_warn_level(3, "gmm_diag::learn(): number of vectors is less "
                               "than number of gaussians");
      return false;
    }
  ...
  if (km_iter > 0) {
    ...
    if (dist_mode == eucl_dist) {
      status =
          km_iterate<1>(X, km_iter, print_mode, "gmm_diag::learn(): k-means");
    } else if (dist_mode == maha_dist) {
      status =
          km_iterate<2>(X, km_iter, print_mode, "gmm_diag::learn(): k-means");
    }
    ...
    if (status == false) {
      arma_debug_warn_level(
          3, "gmm_diag::learn(): k-means algorithm failed; not enough "
...
template <typename T1>
inline bool
gmm_diag<eT>::kmeans_wrapper(Mat<eT> &user_means, const Base<eT, T1> &data,
  ...
  if (seed_mode == keep_existing) {
    ...
    if (X.n_cols < N_gaus) {
      arma_debug_warn_level(
          3, "kmeans(): number of vectors is less than number of means");
      return false;
    }
  ...
  if (km_iter > 0) {
    ...
    if (status == false) {
      arma_debug_warn_level(3, "kmeans(): clustering failed; not enough data, "
                               "or too many means requested");
      return false;
    }
  }
...
template <typename eT> inline void gmm_diag<eT>::init(const gmm_diag<eT> &x) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void gmm_diag<eT>::init(const gmm_full<eT> &x) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void gmm_diag<eT>::init_constants() {
  arma_extra_debug_sigprint();
  ...
  for (uword i = 0; i < dcovs_n_elem; ++i) {
    inv_dcovs_mem[i] =
        eT(1) / (std::max)(dcovs_mem[i], std::numeric_limits<eT>::min());
  }
...
inline umat gmm_diag<eT>::internal_gen_boundaries(const uword N) const {
  ...
  const uword n_threads = (n_threads_avail > 0)
                              ? ((n_threads_avail <= N) ? n_threads_avail : 1)
                              : 1;
#else
  static constexpr uword n_threads = 1;
#endif

  // get_cout_stream() << "gmm_diag::internal_gen_boundaries(): n_threads: " <<
  // n_threads
  // << '\n';
...
template <typename eT>
inline eT gmm_diag<eT>::internal_scalar_log_p(const eT *x,
                                              const uword g) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline eT gmm_diag<eT>::internal_sum_log_p(const Mat<eT> &X,
                                           const uword gaus_id) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline eT gmm_diag<eT>::internal_avg_log_p(const Mat<eT> &X,
                                           const uword gaus_id) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline uword
gmm_diag<eT>::internal_scalar_assign(const Mat<eT> &X,
                                     const gmm_dist_mode &dist_mode) const {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check((X.n_rows != N_dims),
                   "gmm_diag::assign(): incompatible dimensions");
  ...
  if (dist_mode == eucl_dist) {
    ...
    for (uword g = 0; g < N_gaus; ++g) {
      const eT tmp_dist =
          distance<eT, 1>::eval(N_dims, X_mem, means.colptr(g), X_mem);
...
template <typename eT>
inline void
gmm_diag<eT>::internal_vec_assign(urowvec &out, const Mat<eT> &X,
                                  const gmm_dist_mode &dist_mode) const {
  arma_extra_debug_sigprint();
...
#pragma omp parallel for schedule(static)
      for (uword i = 0; i < X_n_cols; ++i) {
        ...
        for (uword g = 0; g < N_gaus; ++g) {
          const eT tmp_p =
              internal_scalar_log_p(X_colptr, g) + log_hefts_mem[g];
...
template <typename eT>
inline void
gmm_diag<eT>::internal_raw_hist(urowvec &hist, const Mat<eT> &X,
                                const gmm_dist_mode &dist_mode) const {
  arma_extra_debug_sigprint();
...
#pragma omp parallel for schedule(static)
      for (uword t = 0; t < n_threads; ++t) {
        ...
        for (uword i = start_index; i <= end_index; ++i) {
          ...
          for (uword g = 0; g < N_gaus; ++g) {
            const eT tmp_dist = distance<eT, 1>::eval(
                N_dims, X_colptr, means.colptr(g), X_colptr);
...
inline void
gmm_diag<eT>::generate_initial_means(const Mat<eT> &X,
                                     const gmm_seed_mode &seed_mode) {
  arma_extra_debug_sigprint();
  ...
  if ((seed_mode == static_subset) || (seed_mode == random_subset)) {
    ...
    // instead, if there are enough samples, randomly choose samples with
    // probability 0.1
    ...
    for (uword g = 1; g < N_gaus; ++g) {
      ...
      for (uword i = start_i; i < X.n_cols; i += step) {
        ...
        for (uword h = 0; h < g; ++h) {
          const eT dist = distance<eT, dist_id>::eval(
              N_dims, X_colptr, means.colptr(h), mah_aux_mem);
      ...
      // set the mean to the sample that is the furthest away from the means so
      // far
...
template <typename eT>
template <uword dist_id>
inline void gmm_diag<eT>::generate_initial_params(const Mat<eT> &X,
                                                  const eT var_floor) {
  arma_extra_debug_sigprint();
  ...
  // the means also need to be calculated via accumulators to ensure numerical
  // consistency
...
#pragma omp parallel for schedule(static)
    for (uword t = 0; t < n_threads; ++t) {
      ...
      for (uword i = start_index; i <= end_index; ++i) {
        ...
        for (uword g = 0; g < N_gaus; ++g) {
          const eT dist = distance<eT, dist_id>::eval(
              N_dims, X_colptr, means.colptr(g), mah_aux_mem);
...
#pragma omp parallel for schedule(static)
      for (uword t = 0; t < n_threads; ++t) {
        ...
        for (uword i = start_index; i <= end_index; ++i) {
          ...
          for (uword g = 0; g < N_gaus; ++g) {
            const eT dist = distance<eT, dist_id>::eval(
                N_dims, X_colptr, old_means.colptr(g), mah_aux_mem);
    ...
    if (dead_gs.n_elem > 0) {
      ...
      for (uword dead_gs_count = 0; dead_gs_count < dead_gs.n_elem;
           ++dead_gs_count) {
...
inline bool gmm_diag<eT>::em_iterate(const Mat<eT> &X, const uword max_iter,
  ...
  for (uword iter = 1; iter <= max_iter; ++iter) {
    ...
    const eT new_avg_log_p =
        accu(t_progress_log_lhood) / eT(t_progress_log_lhood.n_elem);
...
template <typename eT>
inline void gmm_diag<eT>::em_update_params(
    const Mat<eT> &X, const umat &boundaries, field<Mat<eT>> &t_acc_means,
    field<Mat<eT>> &t_acc_dcovs, field<Col<eT>> &t_acc_norm_lhoods,
    field<Col<eT>> &t_gaus_log_lhoods, Col<eT> &t_progress_log_lhood) {
  arma_extra_debug_sigprint();
...
#pragma omp parallel for schedule(static)
    for (uword t = 0; t < n_threads; t++) {
      ...
      em_generate_acc(X, boundaries.at(0, t), boundaries.at(1, t), acc_means,
                      acc_dcovs, acc_norm_lhoods, gaus_log_lhoods,
                      progress_log_lhood);
    }
  }
#else
  ...
  // the "reduce" operation, which combines the partial accumulators produced by
  ...
  // conditionally update each component;  if only a subset of the hefts was
  // updated, em_fix_params() will sanitise them
  for (uword g = 0; g < N_gaus; ++g) {
...
template <typename eT>
inline void gmm_diag<eT>::em_generate_acc(
    const Mat<eT> &X, const uword start_index, const uword end_index,
    Mat<eT> &acc_means, Mat<eT> &acc_dcovs, Col<eT> &acc_norm_lhoods,
...
inline void gmm_diag<eT>::em_fix_params(const eT var_floor) {
  ...
  for (uword g1 = 0; g1 < N_gaus; ++g1) {
    if (hefts_mem[g1] > eT(0)) {
      ...
      for (uword g2 = (g1 + 1); g2 < N_gaus; ++g2) {
        if ((hefts_mem[g2] > eT(0)) &&
            (std::abs(hefts_mem[g1] - hefts_mem[g2]) <=
             std::numeric_limits<eT>::epsilon())) {
          const eT dist = distance<eT, 1>::eval(
              N_dims, means_colptr_g1, means.colptr(g2), means_colptr_g1);
  ...
  if ((heft_sum < (eT(1) - Datum<eT>::eps)) ||
      (heft_sum > (eT(1) + Datum<eT>::eps))) {
