...
inline void glue_trapz::apply_noalias(Mat<eT> &out, const Mat<eT> &X,
                                      const Mat<eT> &Y, const uword dim) {
  arma_extra_debug_sigprint();
  ...
  if (dim == 0) {
    ...
    arma_debug_check((N != Y.n_cols), "trapz(): length of X must equal the "
                                      "number of columns in Y when dim=1");
  }
  ...
  if (dim == 0) {
    const Row<eT> diff_X_t(const_cast<eT *>(diff_X.memptr()), diff_X.n_elem,
                           false, true);
...
template <typename eT>
inline void op_trapz::apply_noalias(Mat<eT> &out, const Mat<eT> &Y,
                                    const uword dim) {
  arma_extra_debug_sigprint();
