...
public:
  ...
  inline Row(eT *aux_mem, const uword aux_length,
  ...
  arma_frown("use braced initialiser list instead") inline mat_injector<Row>
  ...
  arma_inline const subview_row<eT> cols(const uword in_col1,
  ...
  arma_inline const subview_row<eT> subvec(const uword start_col,
...
protected:
  inline Row(const arma_fixed_indicator &, const uword in_n_elem,
             const eT *in_mem);
