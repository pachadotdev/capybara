...
template <typename elem_type, typename derived>
inline void
Base<elem_type, derived>::print(std::ostream &user_stream,
...
template <typename elem_type, typename derived>
inline void
Base<elem_type, derived>::raw_print(std::ostream &user_stream,
                                    const std::string extra_text) const {
  arma_extra_debug_sigprint();
...
template <typename elem_type, typename derived>
inline void
Base<elem_type, derived>::brief_print(std::ostream &user_stream,
                                      const std::string extra_text) const {
  arma_extra_debug_sigprint();
...
inline bool Base<elem_type, derived>::is_zero(
  ...
  if (Proxy<derived>::use_at ||
      is_Mat<typename Proxy<derived>::stored_type>::value) {
    const quasi_unwrap<derived> U((*this).get_ref());
...
inline bool Base<elem_type, derived>::is_vec() const {
  ...
  if ((Proxy<derived>::is_row) || (Proxy<derived>::is_col) ||
      (Proxy<derived>::is_xvec)) {
    return true;
  }
...
template <typename elem_type, typename derived>
inline const Op<derived, op_inv_gen_default>
