...
inline void op_sort::direct_sort(eT *X, const uword n_elem,
...
template <typename eT>
inline void op_sort::apply_noalias(Mat<eT> &out, const Mat<eT> &X,
                                   const uword sort_type, const uword dim) {
  arma_extra_debug_sigprint();
...
inline void op_sort::apply(Mat<typename T1::elem_type> &out,
  ...
  arma_debug_check((sort_type > 1),
                   "sort(): parameter 'sort_type' must be 0 or 1");
...
inline void op_sort_vec::apply(Mat<typename T1::elem_type> &out,
  ...
  const unwrap<T1> U(in.m); // not using quasi_unwrap, to ensure there is no
                            // aliasing with subviews
  const Mat<eT> &X = U.M;
