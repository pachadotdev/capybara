...
arma_warn_unused inline
  ...
  const bool status =
      sp_auxlib::eigs_gen(eigval, eigvec, X, n_eigvals, form_val, opts);
...
arma_warn_unused inline
  ...
  // The reason is that we are still not able to apply truly complex shifts to
  // real matrices
  if ((is_real<typename T1::elem_type>::yes) && (std::imag(sigma) != T(0))) {
    status = sp_auxlib::eigs_gen(eigval, eigvec,
                                 conv_to<SpMat<std::complex<T>>>::from(X),
...
//! eigenvalues of general sparse matrix X
template <typename T1>
inline typename enable_if2<is_real<typename T1::pod_type>::value, bool>::result
...
//! this form is deprecated; use eigs_gen(eigval, X, n_eigvals, form, opts)
//! instead
template <typename T1>
arma_deprecated inline
...
eigs_gen(Col<std::complex<typename T1::pod_type>> &eigval,
  ...
  arma_debug_check(
      void_ptr(&eigval) == void_ptr(&eigvec),
...
//! this form is deprecated; use eigs_gen(eigval, eigvec, X, n_eigvals, form,
//! opts) instead
template <typename T1>
arma_deprecated inline
    typename enable_if2<is_real<typename T1::pod_type>::value, bool>::result
