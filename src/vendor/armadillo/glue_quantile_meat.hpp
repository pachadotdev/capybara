...
inline void glue_quantile::worker(eTb *out_mem, Col<eTa> &Y,
                                  const Mat<eTb> &P) {
  ...
  for (uword i = 0; i < P_n_elem; ++i) {
    ...
    if (P_i < P_min) {
      out_val = (P_i < eTb(0)) ? eTb(-std::numeric_limits<eTb>::infinity())
                               : eTb(Y.min());
    } else if (P_i > P_max) {
      out_val = (P_i > eTb(1)) ? eTb(std::numeric_limits<eTb>::infinity())
                               : eTb(Y.max());
    } else {
