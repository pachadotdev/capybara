...
//! Class for storing data required to construct or apply operations to a
//! submatrix (ie. where the submatrix starts and ends as well as a
//! reference/pointer to the original matrix),
...
protected:
  arma_inline subview(const Mat<eT> &in_m, const uword in_row1,
...
public:
  ...
  arma_warn_unused inline eT &operator()(const uword in_row,
                                         const uword in_col);
  arma_warn_unused inline eT operator()(const uword in_row,
  ...
  inline subview<eT> submat(const uword in_row1, const uword in_col1,
  ...
  each_col(const Base<uword, T1> &indices);
  template <typename T1>
  inline subview_each2<subview<eT>, 1, T1>
  each_row(const Base<uword, T1> &indices);
  ...
  public:
    ...
    inline row_iterator(subview<eT> &in_sv, const uword in_row,
                        const uword in_col);
...
public:
  ...
  arma_warn_unused arma_inline const Op<subview_col<eT>, op_strans>
  as_row() const;
  ...
  inline const subview_col<eT> rows(const uword in_row1,
                                    const uword in_row2) const;

  inline subview_col<eT> subvec(const uword in_row1, const uword in_row2);
  inline const subview_col<eT> subvec(const uword in_row1,
                                      const uword in_row2) const;

  inline subview_col<eT> subvec(const uword start_row, const SizeMat &s);
  inline const subview_col<eT> subvec(const uword start_row,
...
template <typename eT> class subview_cols : public subview<eT> {
public:
  typedef eT elem_type;
  typedef typename get_pod_type<elem_type>::result pod_type;
  ...
  arma_warn_unused arma_inline const Op<subview_cols<eT>, op_vectorise_col>
  as_col() const;
...
protected:
  inline subview_cols(const Mat<eT> &in_m, const uword in_col1,
                      const uword in_n_cols);
...
public:
  ...
  arma_warn_unused arma_inline const Op<subview_row<eT>, op_strans>
  as_col() const;
  ...
  inline const subview_row<eT> cols(const uword in_col1,
                                    const uword in_col2) const;

  inline subview_row<eT> subvec(const uword in_col1, const uword in_col2);
  inline const subview_row<eT> subvec(const uword in_col1,
                                      const uword in_col2) const;

  inline subview_row<eT> subvec(const uword start_col, const SizeMat &s);
  inline const subview_row<eT> subvec(const uword start_col,
                                      const SizeMat &s) const;
