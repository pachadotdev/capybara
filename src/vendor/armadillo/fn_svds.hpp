...
inline bool svds_helper(
  ...
  arma_debug_check((((void *)(&U) == (void *)(&S)) || (&U == &V) ||
                    ((void *)(&S) == (void *)(&V))),
  ...
  if (S.n_elem < k) {
    arma_debug_warn_level(1,
                          "svds(): found fewer singular values than specified");
  }
...
template <typename T1>
inline bool
svds_helper(Mat<typename T1::elem_type> &U, Col<typename T1::pod_type> &S,
            Mat<typename T1::elem_type> &V,
            const SpBase<typename T1::elem_type, T1> &X, const uword k,
            const typename T1::pod_type tol, const bool calc_UV,
  ...
  if (arma_config::arpack == false) {
    arma_stop_logic_error("svds(): use of ARPACK must be enabled for "
                          "decomposition of complex matrices");
    return false;
  }
...
//! find the k largest singular values and corresponding singular vectors of
//! sparse matrix
//! X
template <typename T1>
inline bool
svds(Mat<typename T1::elem_type> &U, Col<typename T1::pod_type> &S,
     Mat<typename T1::elem_type> &V,
     const SpBase<typename T1::elem_type, T1> &X, const uword k,
...
template <typename T1>
inline bool
svds(Col<typename T1::pod_type> &S, const SpBase<typename T1::elem_type, T1> &X,
...
svds(const SpBase<typename T1::elem_type, T1> &X, const uword k,
     const typename T1::pod_type tol = 0.0,
     const typename arma_real_or_cx_only<typename T1::elem_type>::result *junk =
         nullptr) {
  arma_extra_debug_sigprint();
