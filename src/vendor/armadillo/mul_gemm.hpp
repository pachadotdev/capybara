...
//! emulation of gemm(), for non-complex matrices only, as it assumes only
//! simple transposes (ie. doesn't do hermitian transposes)
    ...
    if ((do_trans_A == false) && (do_trans_B == false)) {
      ...
      for (uword row_A = 0; row_A < A_n_rows; ++row_A) {
        ...
        for (uword col_B = 0; col_B < B_n_cols; ++col_B) {
          const eT acc =
              op_dot::direct_dot_arma(B_n_rows, A_rowdata, B.colptr(col_B));
      ...
      for (uword col_A = 0; col_A < A_n_cols; ++col_A) {
        ...
        for (uword col_B = 0; col_B < B_n_cols; ++col_B) {
          const eT acc =
              op_dot::direct_dot_arma(B_n_rows, A_coldata, B.colptr(col_B));
      ...
      gemm_emul_large<false, false, use_alpha, use_beta>::apply(C, A, BB, alpha,
                                                                beta);
    } else if ((do_trans_A == true) && (do_trans_B == true)) {
      // mat B_tmp = trans(B);
      // dgemm_arma<true, false,  use_alpha, use_beta>::apply(C, A, B_tmp,
      // alpha, beta);
      ...
      for (uword row_B = 0; row_B < B_n_rows; ++row_B) {
        ...
        for (uword col_A = 0; col_A < A_n_cols; ++col_A) {
          const eT acc =
              op_dot::direct_dot_arma(A_n_rows, B_rowdata, A.colptr(col_A));
...
public:
  ...
  apply(Mat<eT> &C, const TA &A, const TB &B, const eT alpha = eT(1),
        const eT beta = eT(0),
        const typename arma_not_cx<eT>::result *junk = nullptr) {
    arma_extra_debug_sigprint();
    arma_ignore(junk);

    gemm_emul_large<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(
        C, A, B, alpha, beta);
  }

  template <typename eT>
  arma_hot inline static void
  apply(Mat<eT> &C, const Mat<eT> &A, const Mat<eT> &B, const eT alpha = eT(1),
        const eT beta = eT(0),
        const typename arma_cx_only<eT>::result *junk = nullptr) {
    arma_extra_debug_sigprint();
    arma_ignore(junk);

    // "better than nothing" handling of hermitian transposes for complex number
    // matrices
    ...
    gemm_emul_large<false, false, use_alpha, use_beta>::apply(C, AA, BB, alpha,
                                                              beta);
  }
};

//! \brief
//! Wrapper for BLAS dgemm function, using template arguments to control the
//! arguments passed to dgemm. Matrix 'C' is assumed to have been set to the
//! correct size (ie. taking into account transposes)
...
public:
  ...
  inline static void apply_blas_type(Mat<eT> &C, const TA &A, const TB &B,
    ...
    if ((A.n_rows <= 4) && (A.n_rows == A.n_cols) && (A.n_rows == B.n_rows) &&
      ...
      if (do_trans_B == false) {
        gemm_emul_tinysq<do_trans_A, use_alpha, use_beta>::apply(C, A, B, alpha,
                                                                 beta);
      } else {
...
#if defined(ARMA_USE_ATLAS)
      {
        ...
        atlas::cblas_gemm<eT>(
            atlas_CblasColMajor,
            (do_trans_A)
                ? (is_cx<eT>::yes ? atlas_CblasConjTrans : atlas_CblasTrans)
                : atlas_CblasNoTrans,
            (do_trans_B)
                ? (is_cx<eT>::yes ? atlas_CblasConjTrans : atlas_CblasTrans)
                : atlas_CblasNoTrans,
            ...
            (do_trans_A) ? A.n_rows : C.n_rows, B.mem,
...
#elif defined(ARMA_USE_BLAS)
      {
        ...
        const blas_int k =
            (do_trans_A) ? blas_int(A.n_rows) : blas_int(A.n_cols);
        ...
        arma_extra_debug_print(arma_str::format("blas::gemm(): trans_A = %c") %
                               trans_A);
        arma_extra_debug_print(arma_str::format("blas::gemm(): trans_B = %c") %
                               trans_B);

        blas::gemm<eT>(&trans_A, &trans_B, &m, &n, &k, &local_alpha, A.mem,
                       &lda, B.mem, &ldb, &local_beta, C.memptr(), &m);
      }
#else
      {
        gemm_emul<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(
            C, A, B, alpha, beta);
      }
#endif
    }
  ...
  template <typename eT, typename TA, typename TB>
  inline static void apply(Mat<eT> &C, const TA &A, const TB &B,
                           const eT alpha = eT(1), const eT beta = eT(0)) {
    gemm_emul<do_trans_A, do_trans_B, use_alpha, use_beta>::apply(C, A, B,
                                                                  alpha, beta);
  }
  ...
  arma_inline static void apply(Mat<float> &C, const TA &A, const TB &B,
    ...
    gemm<do_trans_A, do_trans_B, use_alpha, use_beta>::apply_blas_type(
        C, A, B, alpha, beta);
  }
  ...
  apply(Mat<std::complex<float>> &C, const TA &A, const TB &B,
  ...
  template <typename TA, typename TB>
  arma_inline static void
  apply(Mat<std::complex<double>> &C, const TA &A, const TB &B,
