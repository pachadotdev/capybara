...
inline void
spop_var::apply(SpMat<typename T1::pod_type> &out,
  ...
  arma_debug_check((norm_type > 1),
                   "var(): parameter 'norm_type' must be 0 or 1");
  ...
  if (dim == 0) // find variance in each column
  {
    ...
    for (uword col = 0; col < p_n_cols; ++col) {
      if (SpProxy<T1>::use_iterator) {
        ...
        // in_eT is used just to get the specialization right (complex /
        // noncomplex)
        out.at(0, col) =
            spop_var::iterator_var(it, end, n_zero, norm_type, in_eT(0));
      } else {
...
template <typename T1>
inline typename T1::pod_type spop_var::var_vec(const T1 &X,
...
inline eT spop_var::direct_var(const eT *const X, const uword length,
...
template <typename T>
inline T spop_var::direct_var(const std::complex<T> *const X,
                              const uword length, const uword N,
                              const uword norm_type) {
  arma_extra_debug_sigprint();
...
inline eT
spop_var::iterator_var(T1 &it, const T1 &end, const uword n_zero,
...
spop_var::iterator_var(T1 &it, const T1 &end, const uword n_zero,
  ...
  const T norm_val =
      (norm_type == 0) ? T(n_zero + n_nonzero - 1) : T(n_zero + n_nonzero);
