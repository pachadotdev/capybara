...
inline SpMat<eT>::SpMat()
    : n_rows(0), n_cols(0), n_elem(0), n_nonzero(0), vec_state(0),
...
//! indices, and values should be the corresponding values. If sort_locations is
//! false, then it is assumed that the locations and values are already sorted
//! in column-major ordering.
template <typename eT>
template <typename T1, typename T2>
inline SpMat<eT>::SpMat(const Base<uword, T1> &locations_expr,
                        const Base<eT, T2> &vals_expr,
...
//! indices, and values should be the corresponding values. If sort_locations is
//! false, then it is assumed that the locations and values are already sorted
//! in column-major ordering. In this constructor the size is explicitly given.
template <typename eT>
template <typename T1, typename T2>
inline SpMat<eT>::SpMat(const Base<uword, T1> &locations_expr,
...
template <typename eT>
template <typename T1, typename T2>
inline SpMat<eT>::SpMat(const bool add_values,
                        const Base<uword, T1> &locations_expr,
                        const Base<eT, T2> &vals_expr, const uword in_n_rows,
                        const uword in_n_cols, const bool sort_locations,
                        const bool check_for_zeros)
    : n_rows(0), n_cols(0), n_elem(0), n_nonzero(0), vec_state(0),
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::operator=(const eT val) {
  arma_extra_debug_sigprint();
...
inline SpMat<eT>::SpMat(const SpBase<typename SpMat<eT>::pod_type, T1> &A,
  ...
  arma_debug_assert_same_size(X.n_rows, X.n_cols, Y.n_rows, Y.n_cols,
                              "SpMat()");
...
inline SpMat<eT> &SpMat<eT>::operator=(const SpSubview<eT> &X) {
  ...
  if (alias) {
    ...
    if (X.n_rows == X.m.n_rows) {
      ...
      typename SpMat<eT>::const_col_iterator m_it =
          X.m.begin_col_no_sync(sv_col_start);
      typename SpMat<eT>::const_col_iterator m_it_end =
          X.m.end_col_no_sync(sv_col_end);
...
inline SpMat<eT>::SpMat(const SpOp<T1, spop_type> &X)
    : n_rows(0), n_cols(0), n_elem(0), n_nonzero(0), vec_state(0),
      ...
      ,
      row_indices(nullptr), col_ptrs(nullptr) {
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::rows(const uword in_row1,
                                          const uword in_row2) {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::cols(const uword in_col1,
                                          const uword in_col2) {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline SpSubview<eT>
SpMat<eT>::submat(const uword in_row1, const uword in_col1, const uword in_row2,
                  const uword in_col2) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::submat(const span &row_span,
                                       const span &col_span) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpMat<eT>::operator()(const span &row_span,
                                           const span &col_span) {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline SpSubview<eT> SpMat<eT>::operator()(const uword in_row1,
                                                const uword in_col1,
                                                const SizeMat &s) {
  arma_extra_debug_sigprint();
...
inline SpSubview<eT> SpMat<eT>::head_rows(const uword N) {
  ...
  arma_debug_check_bounds((N > n_rows),
                          "SpMat::head_rows(): size out of bounds");
...
inline SpSubview<eT> SpMat<eT>::tail_rows(const uword N) {
  ...
  arma_debug_check_bounds((N > n_rows),
                          "SpMat::tail_rows(): size out of bounds");
...
inline SpSubview<eT> SpMat<eT>::head_cols(const uword N) {
  ...
  arma_debug_check_bounds((N > n_cols),
                          "SpMat::head_cols(): size out of bounds");
...
inline SpSubview<eT> SpMat<eT>::tail_cols(const uword N) {
  ...
  arma_debug_check_bounds((N > n_cols),
                          "SpMat::tail_cols(): size out of bounds");
...
template <typename eT>
template <typename T1>
arma_inline SpSubview_col_list<eT, T1>
...
template <typename eT>
template <typename T1>
arma_inline const SpSubview_col_list<eT, T1>
...
inline void SpMat<eT>::swap_rows(const uword in_row1, const uword in_row2) {
  ...
  // The easier way to do this, instead of collecting all the elements in one
  // row and then swapping with the other, will be to iterate over each column
  // of the matrix (since we store in column-major format) and then swap the two
  // elements in the two rows at that time. We will try to avoid using the at()
  // call since it is expensive, instead preferring to use an iterator to track
  // our position.
  ...
  for (uword lcol = 0; lcol < n_cols; lcol++) {
    ...
    // There are four cases: we found both elements; we found one element
    // (loc1); we found one element (loc2); we found zero elements. If we found
    // zero elements no work needs to be done and we can continue to the next
    // column.
    if ((loc1 != (n_nonzero + 1)) && (loc2 != (n_nonzero + 1))) {
      // This is an easy case: just swap the values.  No index modifying
      // necessary.
      eT tmp = values[loc1];
    ...
    {
      // We need to find the correct place to move our value to.  It will be
      // forward (not backwards) because in_row2 > in_row1. Each iteration of
      // the loop swaps the current value (loc1) with (loc1 + 1); in this manner
      // we move our value down to where it should be.
      while (((loc1 + 1) < col_ptrs[lcol + 1]) &&
             (row_indices[loc1 + 1] < in_row2)) {
      ...
      // We need to find the correct place to move our value to.  It will be
      // backwards (not forwards) because in_row1 < in_row2. Each iteration of
      // the loop swaps the current value (loc2) with (loc2 - 1); in this manner
      // we move our value up to where it should be.
      while (((loc2 - 1) >= col_ptrs[lcol]) &&
             (row_indices[loc2 - 1] > in_row1)) {
        // Swap both the values and the indices.  The column should not change.
        eT tmp = values[loc2];
...
template <typename eT> inline void SpMat<eT>::shed_row(const uword row_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(row_num >= n_rows,
                          "SpMat::shed_row(): out of bounds");

  shed_rows(row_num, row_num);
}

template <typename eT> inline void SpMat<eT>::shed_col(const uword col_num) {
  arma_extra_debug_sigprint();

  arma_debug_check_bounds(col_num >= n_cols,
                          "SpMat::shed_col(): out of bounds");
...
inline void SpMat<eT>::shed_cols(const uword in_col1, const uword in_col2) {
  ...
  if (diff > 0) {
    ...
    if (col_end != n_nonzero) {
      arrayops::copy(new_values + col_beg, values + col_end,
                     n_nonzero - col_end);
...
/**
 * Element access; acces the i'th element (works identically to the Mat
 * accessors). If there is nothing at element i, 0 is returned.
 */
...
template <typename eT> arma_inline eT SpMat<eT>::at(const uword i) const {
  return get_value(i);
}
...
template <typename eT>
arma_inline eT SpMat<eT>::operator[](const uword in_row,
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT> SpMat<eT>::at(const uword in_row,
                                               const uword in_col) {
  return SpMat_MapMat_val<eT>((*this), cache, in_row, in_col);
}
 ...
 */
template <typename eT> arma_inline bool SpMat<eT>::is_empty() const {
  return (n_elem == 0);
}

//! returns true if the object can be interpreted as a column or row vector
template <typename eT> arma_inline bool SpMat<eT>::is_vec() const {
  return ((n_rows == 1) || (n_cols == 1));
}

//! returns true if the object can be interpreted as a row vector
template <typename eT> arma_inline bool SpMat<eT>::is_rowvec() const {
  return (n_rows == 1);
}

//! returns true if the object can be interpreted as a column vector
template <typename eT> arma_inline bool SpMat<eT>::is_colvec() const {
  return (n_cols == 1);
}

//! returns true if the object has the same number of non-zero rows and columnns
template <typename eT> arma_inline bool SpMat<eT>::is_square() const {
  return (n_rows == n_cols);
}

template <typename eT> inline bool SpMat<eT>::is_symmetric() const {
  arma_extra_debug_sigprint();
...
inline bool SpMat<eT>::is_symmetric(
  ...
  arma_debug_check((tol < T(0)),
                   "is_symmetric(): parameter 'tol' must be >= 0");
...
template <typename eT> inline bool SpMat<eT>::is_hermitian() const {
  arma_extra_debug_sigprint();
...
inline bool SpMat<eT>::is_hermitian(
  ...
  arma_debug_check((tol < T(0)),
                   "is_hermitian(): parameter 'tol' must be >= 0");
...
template <typename eT> inline bool SpMat<eT>::internal_is_finite() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline bool SpMat<eT>::internal_has_inf() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline bool SpMat<eT>::internal_has_nan() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline bool SpMat<eT>::internal_has_nonfinite() const {
  arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline bool SpMat<eT>::in_range(const uword in_row,
                                     const uword in_col) const {
  return ((in_row < n_rows) && (in_col < n_cols));
}

template <typename eT>
arma_inline bool SpMat<eT>::in_range(const span &row_span,
                                     const uword in_col) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpMat<eT> &SpMat<eT>::set_size(const uword in_rows,
                                      const uword in_cols) {
  arma_extra_debug_sigprint();

  invalidate_cache(); // placed here, as set_size() is used during matrix
                      // modification
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::set_size(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
inline SpMat<eT> &SpMat<eT>::resize(const uword in_rows, const uword in_cols) {
  ...
  if (tmp.n_elem > 0) {
    ...
    tmp.submat(0, 0, last_row, last_col) =
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::resize(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
inline SpMat<eT> &SpMat<eT>::reshape(const uword in_rows, const uword in_cols) {
  ...
  if (vec_state == 1) {
    arma_debug_check((in_cols != 1),
                     "SpMat::reshape(): object is a column vector; requested "
                     "size is not compatible");
  }
  if (vec_state == 2) {
    arma_debug_check((in_rows != 1),
                     "SpMat::reshape(): object is a row vector; requested size "
                     "is not compatible");
  }
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::reshape(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::reshape_helper_generic(const uword in_rows,
                                              const uword in_cols) {
  arma_extra_debug_sigprint();
  ...
  // We have to modify all of the relevant row indices and the relevant column
  // pointers. Iterate over all the points to do this.  We won't be deleting any
  // points, but we will be modifying columns and rows. We'll have to store a
  // new set of column vectors.
...
template <typename eT> inline void SpMat<eT>::reshape_helper_intovec() {
  arma_extra_debug_sigprint();
...
inline SpMat<eT> &SpMat<eT>::replace(const eT old_val, const eT new_val) {
  ...
  if (old_val == eT(0)) {
    arma_debug_warn_level(
        1, "SpMat::replace(): replacement not done, as old_val = 0");
  } else {
    sync_csc();
    invalidate_cache();
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::zeros() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::zeros(const uword in_elem) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::zeros(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).zeros(s.n_rows, s.n_cols);
}

template <typename eT> inline SpMat<eT> &SpMat<eT>::eye() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::eye(const SizeMat &s) {
  arma_extra_debug_sigprint();

  return (*this).eye(s.n_rows, s.n_cols);
}

template <typename eT> inline SpMat<eT> &SpMat<eT>::speye() {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpMat<eT> &SpMat<eT>::speye(const uword in_n_rows,
...
template <typename eT> inline SpMat<eT> &SpMat<eT>::speye(const SizeMat &s) {
  arma_extra_debug_sigprint();
...
inline SpMat<eT> &SpMat<eT>::sprandu(const uword in_rows, const uword in_cols,
  ...
  const uword new_n_nonzero =
      uword(density * double(in_rows) * double(in_cols) + 0.5);
  ...
  for (uword i = 1; i < new_n_nonzero - 1; ++i) {
    ...
    uword index_new =
        uword(double(center) + double(min_delta) * (2.0 * randu() - 1.0));
...
template <typename eT> inline void SpMat<eT>::reset() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::reset_cache() {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::save(const std::string name,
...
inline bool SpMat<eT>::save(const csv_name &spec, const file_type type) const {
  ...
  if ((type != csv_ascii) && (type != ssv_ascii)) {
    arma_stop_runtime_error(
        "SpMat::save(): unsupported file type for csv_name()");
    return false;
  }
  ...
  if (with_header) {
    if ((spec.header_ro.n_cols != 1) && (spec.header_ro.n_rows != 1)) {
      arma_debug_warn_level(
          1, "SpMat::save(): given header must have a vector layout");
      return false;
    }
    ...
    for (uword i = 0; i < spec.header_ro.n_elem; ++i) {
      ...
      if (token.find(separator) != std::string::npos) {
        arma_debug_warn_level(1,
                              "SpMat::save(): token within the header contains "
  ...
  if (save_okay == false) {
    arma_debug_warn_level(3,
                          "SpMat::save(): write failed; file: ", spec.filename);
  }
...
inline bool SpMat<eT>::load(const csv_name &spec, const file_type type) {
  ...
  if ((type != csv_ascii) && (type != ssv_ascii)) {
    arma_stop_runtime_error(
        "SpMat::load(): unsupported file type for csv_name()");
    return false;
  }
  ...
  if (strict) {
    arma_debug_warn_level(
        1,
        "SpMat::load(): option 'strict' not implemented for sparse matrices");
  }
  ...
  if (do_trans) {
    ...
    load_okay = diskio::load_csv_ascii(tmp_mat, spec.filename, err_msg,
                                       spec.header_rw, with_header, separator);
    ...
    load_okay = diskio::load_csv_ascii(*this, spec.filename, err_msg,
                                       spec.header_rw, with_header, separator);
  }
  ...
  if (load_okay == false) {
    if (err_msg.length() > 0) {
      ...
      arma_debug_warn_level(
          3, "SpMat::load(): read failed; file: ", spec.filename);
    }
  } else {
...
template <typename eT>
inline bool SpMat<eT>::quiet_save(const std::string name,
                                  const file_type type) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::quiet_save(std::ostream &os,
                                  const file_type type) const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline bool SpMat<eT>::quiet_load(const std::string name,
...
/**
 * Initialize the matrix to the specified size.  Data is not preserved, so the
 * matrix is assumed to be entirely sparse (empty).
 */
template <typename eT>
inline void SpMat<eT>::init(uword in_rows, uword in_cols,
                            const uword new_n_nonzero) {
  arma_extra_debug_sigprint();

  invalidate_cache(); // placed here, as init() is used during matrix
                      // modification
...
inline void SpMat<eT>::init_cold(uword in_rows, uword in_cols,
  ...
  if (vec_state > 0) {
    if ((in_rows == 0) && (in_cols == 0)) {
      ...
      if (vec_state == 1) {
        arma_debug_check((in_cols != 1),
                         "SpMat::init(): object is a column vector; requested "
                         "size is not compatible");
      }
      if (vec_state == 2) {
        arma_debug_check((in_rows != 1),
                         "SpMat::init(): object is a row vector; requested "
                         "size is not compatible");
      }
    }
  }
...
template <typename eT> inline void SpMat<eT>::init(const std::string &text) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::init(const SpMat<eT> &x) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::init(const MapMat<eT> &x) {
  arma_extra_debug_sigprint();
  ...
  for (uword i = 0; i < x_n_nz; ++i) {
    ...
    // } if(x_col != tmp_x_col)  { cout << "x_col != tmp_x_col" << endl;
    // exit(-1); }
...
template <typename eT> inline void SpMat<eT>::init_simple(const SpMat<eT> &x) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline void SpMat<eT>::init_batch_std(const Mat<uword> &locs,
                                      const Mat<eT> &vals,
                                      const bool sort_locations) {
  arma_extra_debug_sigprint();
  ...
  if (sort_locations) {
    ...
    if (actually_sorted == false) {
      // see op_sort_index_bones.hpp for the definition of
      // arma_sort_index_packet and arma_sort_index_helper_ascend
  ...
  if ((sort_locations == false) || (actually_sorted == true)) {
    ...
    // Increment the column pointers in each column (so they are column
    // "counts").
...
template <typename eT>
inline void SpMat<eT>::init_batch_add(const Mat<uword> &locs,
                                      const Mat<eT> &vals,
                                      const bool sort_locations) {
  arma_extra_debug_sigprint();
  ...
  if (sort_locations) {
    ...
    if (actually_sorted == false) {
      // This may not be the fastest possible implementation but it maximizes
      // code reuse.
...
inline SpMat<eT>::SpMat(const arma_vec_indicator &, const uword in_vec_state)
    : n_rows(0), n_cols(0), n_elem(0), n_nonzero(0), vec_state(in_vec_state),
      values(nullptr), row_indices(nullptr), col_ptrs(nullptr) {
  arma_extra_debug_sigprint_this(this);
...
inline void SpMat<eT>::mem_resize(const uword new_n_nonzero) {
  ...
  invalidate_cache(); // placed here, as mem_resize() is used during matrix
                      // modification
  ...
  // Set the "fake end" of the matrix by setting the last value and row index to
  // 0. This helps the iterators work correctly.
  access::rw(values[new_n_nonzero]) = 0;
  access::rw(row_indices[new_n_nonzero]) = 0;
...
template <typename eT> inline void SpMat<eT>::sync() const {
  arma_extra_debug_sigprint();

  sync_csc();
}

template <typename eT> inline void SpMat<eT>::remove_zeros() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::steal_mem(SpMat<eT> &x) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::steal_mem_simple(SpMat<eT> &x) {
  arma_extra_debug_sigprint();
...
inline void SpMat<eT>::init_xform(const SpBase<eT, T1> &A,
                                  const Functor &func) {
  arma_extra_debug_sigprint();

  // if possible, avoid doing a copy and instead apply func to the generated
  // elements
...
inline void SpMat<eT>::init_xform_mt(const SpBase<eT2, T1> &A,
                                     const Functor &func) {
  arma_extra_debug_sigprint();
  ...
  if (P.is_alias(*this) ||
      (is_SpMat<typename SpProxy<T1>::stored_type>::value)) {
    // NOTE: unwrap_spmat will convert a submatrix to a matrix, which in effect
    // takes care of aliasing with submatrices; NOTE: however, when more delayed
    // ops are implemented, more elaborate handling of aliasing will be
    // necessary
    ...
    // initialise the elements array with a transformed version of the elements
    // from x
    ...
    for (uword i = 0; i < nnz; ++i) {
      ...
      t_values_i =
          func(x_values[i]); // NOTE: func() must produce a value of type eT
    ...
    while (it != it_end) {
      const eT val = func(*it); // NOTE: func() must produce a value of type eT
                                // (ie. act as a convertor between eT2 and eT)
...
template <typename eT> inline typename SpMat<eT>::iterator SpMat<eT>::begin() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline typename SpMat<eT>::iterator SpMat<eT>::end() {
  sync_csc();
...
template <typename eT>
inline typename SpMat<eT>::col_iterator
...
template <typename eT>
inline typename SpMat<eT>::const_col_iterator
...
template <typename eT>
inline typename SpMat<eT>::row_iterator
...
template <typename eT> inline void SpMat<eT>::clear() { (*this).reset(); }

template <typename eT> inline bool SpMat<eT>::empty() const {
  return (n_elem == 0);
}

template <typename eT> inline uword SpMat<eT>::size() const { return n_elem; }

template <typename eT> arma_inline SpMat_MapMat_val<eT> SpMat<eT>::front() {
  arma_debug_check((n_elem == 0), "SpMat::front(): matrix is empty");

  return SpMat_MapMat_val<eT>((*this), cache, 0, 0);
}

template <typename eT> arma_inline eT SpMat<eT>::front() const {
  arma_debug_check((n_elem == 0), "SpMat::front(): matrix is empty");

  return get_value(0, 0);
}

template <typename eT> arma_inline SpMat_MapMat_val<eT> SpMat<eT>::back() {
  arma_debug_check((n_elem == 0), "SpMat::back(): matrix is empty");

  return SpMat_MapMat_val<eT>((*this), cache, n_rows - 1, n_cols - 1);
}

template <typename eT> arma_inline eT SpMat<eT>::back() const {
  arma_debug_check((n_elem == 0), "SpMat::back(): matrix is empty");

  return get_value(n_rows - 1, n_cols - 1);
}

template <typename eT> inline eT SpMat<eT>::get_value(const uword i) const {
  const MapMat<eT> &const_cache =
      cache; // declare as const for clarity of intent
...
template <typename eT> inline eT SpMat<eT>::get_value_csc(const uword i) const {
...
template <typename eT>
inline const eT *SpMat<eT>::find_value_csc(const uword in_row,
                                           const uword in_col) const {
  ...
  const uword *pos_ptr =
      std::lower_bound(start_ptr, end_ptr, in_row); // binary search
...
template <typename eT>
inline eT SpMat<eT>::get_value_csc(const uword in_row,
                                   const uword in_col) const {
...
inline bool SpMat<eT>::try_set_value_csc(const uword in_row, const uword in_col,
  ...
  // element not found, ie. it's zero; fail if trying to set it to non-zero
  // value
  if (val_ptr == nullptr) {
    return (in_val == eT(0));
  }
...
inline bool SpMat<eT>::try_sub_value_csc(const uword in_row, const uword in_col,
  ...
  // element not found, ie. it's zero; fail if trying to subtract a non-zero
  // value
  if (val_ptr == nullptr) {
    return (in_val == eT(0));
  }
...
inline bool SpMat<eT>::try_mul_value_csc(const uword in_row, const uword in_col,
  ...
  // element not found, ie. it's zero; succeed if given value is finite; zero
  // multiplied by anything is zero, except for nan and inf
  if (val_ptr == nullptr) {
...
inline bool SpMat<eT>::try_div_value_csc(const uword in_row, const uword in_col,
  ...
  // element not found, ie. it's zero; succeed if given value is not zero and
  // not nan; zero divided by anything is zero, except for zero and nan
  if (val_ptr == nullptr) {
...
inline eT &SpMat<eT>::insert_element(const uword in_row, const uword in_col,
  ...
  arrayops::copy(new_row_indices + pos + 1, row_indices + pos,
                 (old_n_nonzero - pos) + 1);
  ...
  if (colptr != next_colptr) {
    ...
    for (uword pos = colptr; pos < next_colptr; pos++) {
      if (in_row == row_indices[pos]) {
        --access::rw(
            n_nonzero); // Remove one from the count of nonzero elements.
...
template <typename eT> arma_inline void SpMat<eT>::invalidate_cache() const {
  arma_extra_debug_sigprint();
...
template <typename eT> arma_inline void SpMat<eT>::invalidate_csc() const {
  arma_extra_debug_sigprint();

  sync_state = 1;
}

template <typename eT> inline void SpMat<eT>::sync_cache() const {
  arma_extra_debug_sigprint();
  ...
  // flush is also implicitly executed at the entrance and the exit of critical
  // section; data races are prevented by the 'critical' directive
  //
  // C++11  mode:
  // underlying type for sync_state is std::atomic<int>;
  // reading and writing to sync_state uses std::memory_order_seq_cst which has
  // an implied fence; data races are prevented via the mutex
...
template <typename eT> inline void SpMat<eT>::sync_cache_simple() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::sync_csc() const {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpMat<eT>::sync_csc_simple() const {
  arma_extra_debug_sigprint();
...
inline void SpMat_aux::set_real(SpMat<std::complex<T>> &out,
                                const SpBase<T, T1> &X) {
  arma_extra_debug_sigprint();
...
template <typename T, typename T1>
inline void SpMat_aux::set_imag(SpMat<std::complex<T>> &out,
                                const SpBase<T, T1> &X) {
  arma_extra_debug_sigprint();
