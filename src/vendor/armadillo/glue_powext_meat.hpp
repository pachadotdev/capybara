...
inline void glue_powext::apply(Mat<eT> &out, const Mat<eT> &A,
                               const Mat<eT> &B) {
  arma_extra_debug_sigprint();
...
glue_powext::apply(const subview_each1<parent, mode> &X,
  ...
  if (mode == 0) // each column
  {
    if (arma_config::openmp && mp_gate<eT>::eval(A.n_elem)) {
      ...
      {
        const int n_threads =
            int((std::min)(uword(mp_thread_limit::get()), A_n_cols));
...
template <typename eT>
inline void glue_powext::apply(Cube<eT> &out, const Cube<eT> &A,
                               const Cube<eT> &B) {
  arma_extra_debug_sigprint();
...
inline Cube<eT> glue_powext::apply(const subview_cube_each1<eT> &X,
  ...
  if (arma_config::openmp && mp_gate<eT>::eval(A.n_elem)) {
    ...
    {
      const int n_threads =
          int((std::min)(uword(mp_thread_limit::get()), A_n_slices));
...
template <typename parent, unsigned int mode, typename T2>
inline Mat<typename parent::elem_type>
glue_powext_cx::apply(const subview_each1<parent, mode> &X,
...
inline Cube<std::complex<T>>
glue_powext_cx::apply(const subview_cube_each1<std::complex<T>> &X,
                      const Base<T, T2> &Y) {
  arma_extra_debug_sigprint();
