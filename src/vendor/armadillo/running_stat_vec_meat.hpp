...
inline running_stat_vec<obj_type>::running_stat_vec(
    ...
    : calc_cov(in_rsv.calc_cov), counter(in_rsv.counter), r_mean(in_rsv.r_mean),
      r_var(in_rsv.r_var), r_cov(in_rsv.r_cov), min_val(in_rsv.min_val),
      max_val(in_rsv.max_val), min_val_norm(in_rsv.min_val_norm),
...
template <typename obj_type>
inline typename running_stat_vec<obj_type>::T
...
inline void running_stat_vec_aux::update_stats(
    ...
    const typename arma_cx_only<typename running_stat_vec<obj_type>::eT>::result
        *junk) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);
