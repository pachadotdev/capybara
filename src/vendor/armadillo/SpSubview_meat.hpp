...
inline SpSubview<eT>::SpSubview(const SpMat<eT> &in_m, const uword in_row1,
    ...
    : m(in_m), aux_row1(in_row1), aux_col1(in_col1), n_rows(in_n_rows),
      n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols), n_nonzero(0) {
  arma_extra_debug_sigprint_this(this);
  ...
  for (uword i = m.col_ptrs[in_col1]; i < lend; ++i) {
    ...
    const bool condition =
        (m_row_indices_i >= in_row1) && (m_row_indices_i < lend_row);
...
inline SpSubview<eT>::SpSubview(const SpSubview<eT> &in)
    : m(in.m), aux_row1(in.aux_row1), aux_col1(in.aux_col1), n_rows(in.n_rows),
      n_cols(in.n_cols), n_elem(in.n_elem), n_nonzero(in.n_nonzero) {
...
inline const SpSubview<eT> &SpSubview<eT>::operator/=(const Base<eT, T1> &x) {
  ...
  for (uword c = 0; c < B_n_cols; ++c) {
    for (uword r = 0; r < B_n_rows; ++r) {
      // a zero in B and A at the same location implies the division result is
      // NaN; hence a zero in A (not stored) needs to be changed into a non-zero

      // for efficiency, an element in B is checked before checking the
...
template <typename eT> inline void SpSubview<eT>::fill(const eT val) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpSubview<eT>::zeros() {
  arma_extra_debug_sigprint();
  ...
  for (; m_it != m_it_end; ++m_it) {
    ...
    const bool inside_box =
        ((m_it_row >= sv_row_start) && (m_it_row <= sv_row_end)) &&
...
template <typename eT> inline void SpSubview<eT>::ones() {
  arma_extra_debug_sigprint();

  (*this).fill(eT(1));
}

template <typename eT> inline void SpSubview<eT>::eye() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpSubview<eT>::randu() {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpSubview<eT>::randn() {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline eT SpSubview<eT>::operator()(const uword in_row,
...
template <typename eT> inline eT SpSubview<eT>::at(const uword i) const {
  const uword lrow = i % n_rows;
  const uword lcol = i / n_cols;
...
inline SpSubview_MapMat_val<eT> SpSubview<eT>::at(const uword in_row,
                                                  const uword in_col) {
  return SpSubview_MapMat_val<eT>((*this), m.cache, aux_row1 + in_row,
                                  aux_col1 + in_col);
}
...
template <typename eT> inline bool SpSubview<eT>::is_vec() const {
...
inline SpSubview_row<eT> SpSubview<eT>::row(const uword row_num) {
  ...
  return SpSubview_row<eT>(const_cast<SpMat<eT> &>(m), row_num + aux_row1,
                           aux_col1, n_cols);
}
...
inline SpSubview_col<eT> SpSubview<eT>::col(const uword col_num) {
  ...
  return SpSubview_col<eT>(const_cast<SpMat<eT> &>(m), col_num + aux_col1,
                           aux_row1, n_rows);
}
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::rows(const uword in_row1,
                                         const uword in_row2) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::cols(const uword in_col1,
                                         const uword in_col2) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpSubview<eT>
SpSubview<eT>::submat(const uword in_row1, const uword in_col1,
  ...
  return access::rw(m).submat(in_row1 + aux_row1, in_col1 + aux_col1,
                              in_row2 + aux_row1, in_col2 + aux_col1);
}
...
template <typename eT>
inline SpSubview<eT> SpSubview<eT>::submat(const span &row_span,
                                           const span &col_span) {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline typename SpSubview<eT>::iterator
...
template <typename eT>
inline typename SpSubview<eT>::row_iterator
...
inline eT &SpSubview<eT>::insert_element(const uword in_row, const uword in_col,
  ...
  eT &retval = access::rw(m).insert_element(in_row + aux_row1,
                                            in_col + aux_col1, in_val);
...
template <typename eT>
inline void SpSubview<eT>::delete_element(const uword in_row,
                                          const uword in_col) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline void SpSubview<eT>::invalidate_cache() const {
  arma_extra_debug_sigprint();
...
template <typename eT>
inline SpSubview_col<eT>::SpSubview_col(const SpMat<eT> &in_m,
                                        const uword in_col)
...
template <typename eT>
inline SpSubview_col<eT>::SpSubview_col(const SpMat<eT> &in_m,
                                        const uword in_col, const uword in_row1,
...
template <typename eT>
inline const SpOp<SpSubview_col<eT>, spop_htrans>
SpSubview_col<eT>::ht() const {
  return SpOp<SpSubview_col<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpSubview_col<eT>, spop_strans>
...
template <typename eT>
inline SpSubview_row<eT>::SpSubview_row(const SpMat<eT> &in_m,
                                        const uword in_row)
...
template <typename eT>
inline SpSubview_row<eT>::SpSubview_row(const SpMat<eT> &in_m,
                                        const uword in_row, const uword in_col1,
                                        const uword in_n_cols)
...
template <typename eT>
inline const SpOp<SpSubview_row<eT>, spop_htrans>
SpSubview_row<eT>::ht() const {
  return SpOp<SpSubview_row<eT>, spop_htrans>(*this);
}

template <typename eT>
inline const SpOp<SpSubview_row<eT>, spop_strans>
