...
template <typename eT, typename gen_type>
arma_inline eT GenCube<eT, gen_type>::at(const uword, const uword,
                                         const uword) const {
  if (is_same_type<gen_type, gen_zeros>::yes) {
    return eT(0);
  } else if (is_same_type<gen_type, gen_ones>::yes) {
...
inline void GenCube<eT, gen_type>::apply(Cube<eT> &out) const {
  ...
  // size; this is done by either the Cube contructor or operator=()
...
inline void GenCube<eT, gen_type>::apply(subview_cube<eT> &out) const {
  ...
  // NOTE: we're assuming that the subcube has the same dimensions as the
  // GenCube object this is checked by subview_cube::operator=()
