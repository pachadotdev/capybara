...
template <typename T1>
inline bool
op_inv_gen_default::apply_direct(Mat<typename T1::elem_type> &out,
                                 const Base<typename T1::elem_type, T1> &expr,
                                 const char *caller_sig) {
  arma_extra_debug_sigprint();

  return op_inv_gen_full::apply_direct<T1, false>(out, expr, caller_sig,
                                                  uword(0));
}
...
inline bool
op_inv_gen_full::apply_direct(Mat<typename T1::elem_type> &out,
                              const Base<typename T1::elem_type, T1> &expr,
  ...
  const bool allow_approx =
      has_user_flags && bool(flags & inv_opts::flag_allow_approx);
  ...
  if (no_ugly) {
    ...
    if ((status == false) ||
        (local_rcond < ((std::max)(local_size, uword(1)) *
  ...
  const bool is_tril_mat =
      (is_triu_expr || is_tril_expr)
          ? false
          : ((is_triu_mat) ? false : trimat_helper::is_tril(out));

  if (is_triu_expr || is_tril_expr || is_triu_mat || is_tril_mat) {
    return auxlib::inv_tr(
        out, ((is_triu_expr || is_triu_mat) ? uword(0) : uword(1)));
  }

  const bool try_sympd =
      arma_config::optimise_sym && sym_helper::guess_sympd(out);
...
template <typename eT> inline bool op_inv_gen_full::apply_tiny_2x2(Mat<eT> &X) {
  arma_extra_debug_sigprint();
...
template <typename eT> inline bool op_inv_gen_full::apply_tiny_3x3(Mat<eT> &X) {
  arma_extra_debug_sigprint();
  ...
  Ym[pos<0, 0>::n3] = (Xm[pos<2, 2>::n3] * Xm[pos<1, 1>::n3] -
                       Xm[pos<2, 1>::n3] * Xm[pos<1, 2>::n3]) /
                      det_val;
  Ym[pos<1, 0>::n3] = -(Xm[pos<2, 2>::n3] * Xm[pos<1, 0>::n3] -
                        Xm[pos<2, 0>::n3] * Xm[pos<1, 2>::n3]) /
                      det_val;
  Ym[pos<2, 0>::n3] = (Xm[pos<2, 1>::n3] * Xm[pos<1, 0>::n3] -
                       Xm[pos<2, 0>::n3] * Xm[pos<1, 1>::n3]) /
                      det_val;

  Ym[pos<0, 1>::n3] = -(Xm[pos<2, 2>::n3] * Xm[pos<0, 1>::n3] -
                        Xm[pos<2, 1>::n3] * Xm[pos<0, 2>::n3]) /
                      det_val;
  Ym[pos<1, 1>::n3] = (Xm[pos<2, 2>::n3] * Xm[pos<0, 0>::n3] -
                       Xm[pos<2, 0>::n3] * Xm[pos<0, 2>::n3]) /
                      det_val;
  Ym[pos<2, 1>::n3] = -(Xm[pos<2, 1>::n3] * Xm[pos<0, 0>::n3] -
                        Xm[pos<2, 0>::n3] * Xm[pos<0, 1>::n3]) /
                      det_val;

  Ym[pos<0, 2>::n3] = (Xm[pos<1, 2>::n3] * Xm[pos<0, 1>::n3] -
                       Xm[pos<1, 1>::n3] * Xm[pos<0, 2>::n3]) /
                      det_val;
  Ym[pos<1, 2>::n3] = -(Xm[pos<1, 2>::n3] * Xm[pos<0, 0>::n3] -
                        Xm[pos<1, 0>::n3] * Xm[pos<0, 2>::n3]) /
                      det_val;
  Ym[pos<2, 2>::n3] = (Xm[pos<1, 1>::n3] * Xm[pos<0, 0>::n3] -
                       Xm[pos<1, 0>::n3] * Xm[pos<0, 1>::n3]) /
                      det_val;
...
inline bool op_inv_gen_rcond::apply_direct(
  ...
  if (is_op_diagmat<T1>::value || out.is_diagmat()) {
    ...
    for (uword i = 0; i < N; ++i) {
      ...
      max_abs_src_val =
          (abs_src_val > max_abs_src_val) ? abs_src_val : max_abs_src_val;
      max_abs_inv_val =
          (abs_inv_val > max_abs_inv_val) ? abs_inv_val : max_abs_inv_val;
  ...
  if (try_sympd) {
    ...
    const bool status =
        auxlib::inv_sympd_rcond(tmp, sympd_state, out_state.rcond);
