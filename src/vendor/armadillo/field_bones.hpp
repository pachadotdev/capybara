...
private:
  ...
  arma_aligned oT *mem_local[field_prealloc_n_elem::val]; //!< local storage,
                                                          //!< for small fields
...
public:
  ...
  inline field &
  operator=(const std::initializer_list<std::initializer_list<oT>> &list);

  inline field(field &&X);
  inline field &operator=(field &&X);

  template <typename oT2> inline field &copy_size(const field<oT2> &x);
  ...
  arma_warn_unused arma_inline const oT &at(const uword row,
                                            const uword col) const;

#if defined(__cpp_multidimensional_subscript)
  arma_warn_unused arma_inline oT &operator[](const uword row, const uword col,
                                              const uword slice);
  arma_warn_unused arma_inline const oT &
  ...
  inline const subview_field<oT> rows(const uword in_row1,
                                      const uword in_row2) const;

  inline subview_field<oT> cols(const uword in_col1, const uword in_col2);
  inline const subview_field<oT> cols(const uword in_col1,
  ...
  inline const subview_field<oT> subfield(const uword in_row1,
                                          const uword in_col1,
                                          const uword in_row2,
                                          const uword in_col2) const;
  ...
  subfield(const uword in_row1, const uword in_col1, const uword in_slice1,
           const uword in_row2, const uword in_col2,
           const uword in_slice2) const;
  ...
  inline subview_field<oT> subfield(const uword in_row1, const uword in_col1,
                                    const SizeMat &s);
  inline const subview_field<oT>
  subfield(const uword in_row1, const uword in_col1, const SizeMat &s) const;
  ...
  inline subview_field<oT> operator()(const span &row_span,
                                      const span &col_span);
  ...
  inline subview_field<oT> operator()(const uword in_row1, const uword in_col1,
                                      const uword in_slice1, const SizeCube &s);
  inline const subview_field<oT> operator()(const uword in_row1,
                                            const uword in_col1,
                                            const uword in_slice1,
                                            const SizeCube &s) const;
...
public:
  ...
  template <typename oT>
  inline static bool save(const field<oT> &x, std::ostream &os,
                          const file_type type, std::string &err_msg);
  template <typename oT>
  inline static bool load(field<oT> &x, const std::string &name,
                          const file_type type, std::string &err_msg);
  template <typename oT>
  ...
  inline static bool load(field<Mat<eT>> &x, const std::string &name,
                          const file_type type, std::string &err_msg);
  template <typename eT>
  inline static bool load(field<Mat<eT>> &x, std::istream &is,
                          const file_type type, std::string &err_msg);
  ...
  inline static bool load(field<Col<eT>> &x, const std::string &name,
                          const file_type type, std::string &err_msg);
  template <typename eT>
  inline static bool load(field<Col<eT>> &x, std::istream &is,
                          const file_type type, std::string &err_msg);
  ...
  inline static bool load(field<Row<eT>> &x, const std::string &name,
                          const file_type type, std::string &err_msg);
  template <typename eT>
  inline static bool load(field<Row<eT>> &x, std::istream &is,
                          const file_type type, std::string &err_msg);
  ...
  inline static bool load(field<Cube<eT>> &x, const std::string &name,
                          const file_type type, std::string &err_msg);
  template <typename eT>
  inline static bool load(field<Cube<eT>> &x, std::istream &is,
                          const file_type type, std::string &err_msg);
  ...
  inline static bool load(field<std::string> &x, const std::string &name,
                          const file_type type, std::string &err_msg);
  inline static bool load(field<std::string> &x, std::istream &is,
                          const file_type type, std::string &err_msg);
};
