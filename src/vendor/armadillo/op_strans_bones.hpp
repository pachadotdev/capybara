...
//! 'matrix transpose' operation (simple transpose, ie. without taking the
...
public:
  ...
  template <const bool do_flip, const uword row, const uword col> struct pos {
    static constexpr uword n2 =
        (do_flip == false) ? (row + col * 2) : (col + row * 2);
    static constexpr uword n3 =
        (do_flip == false) ? (row + col * 3) : (col + row * 3);
    static constexpr uword n4 =
        (do_flip == false) ? (row + col * 4) : (col + row * 4);
  };

  template <typename eT, typename TA>
  arma_cold inline static void apply_mat_noalias_tinysq(Mat<eT> &out,
                                                        const TA &A);

  template <typename eT>
  arma_hot inline static void
  block_worker(eT *Y, const eT *X, const uword X_n_rows, const uword Y_n_rows,
               const uword n_rows, const uword n_cols);

  template <typename eT>
  arma_hot inline static void apply_mat_noalias_large(Mat<eT> &out,
                                                      const Mat<eT> &A);
  ...
  template <typename T1>
  inline static void apply_direct(Mat<typename T1::elem_type> &out,
                                  const T1 &X);
