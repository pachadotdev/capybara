...
inline void glue_cor::apply(Mat<typename T1::elem_type> &out,
  ...
  const Mat<eT> &AA = (A.n_rows == 1)
                          ? Mat<eT>(const_cast<eT *>(A.memptr()), A.n_cols,
                                    A.n_rows, false, false)
                          : Mat<eT>(const_cast<eT *>(A.memptr()), A.n_rows,
                                    A.n_cols, false, false);

  const Mat<eT> &BB = (B.n_rows == 1)
                          ? Mat<eT>(const_cast<eT *>(B.memptr()), B.n_cols,
                                    B.n_rows, false, false)
                          : Mat<eT>(const_cast<eT *>(B.memptr()), B.n_rows,
                                    B.n_cols, false, false);
  ...
  out /= conv_to<Mat<eT>>::from(stddev(AA).t() *
                                stddev(BB)); // TODO: check for zeros?
}
