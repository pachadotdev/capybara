...
inline xvec_htrans<eT>::xvec_htrans(const eT *const in_mem,
...
template <typename eT> inline eT xvec_htrans<eT>::at_alt(const uword ii) const {
  return access::alt_conj(mem[ii]);
}
...
inline eT xvec_htrans<eT>::at(const uword in_row, const uword in_col) const {
  ...
  return access::alt_conj(
      mem[in_row + in_col]); // either in_row or in_col must be zero,
