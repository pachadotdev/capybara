...
// TODO: this implementation of conv() is rudimentary; replace with faster
// version
...
// inline
// void
// glue_conv::apply(Mat<eT>& out, const Mat<eT>& A, const Mat<eT>& B, const bool
// A_is_col)
//   {
//   arma_extra_debug_sigprint();
//
...
//
//
//   Col<eT> xx( (x_n_elem + 2*h_n_elem_m1), arma_zeros_indicator() );  // zero
//   padded version of x
//
//   const eT*  x_mem =  x.memptr();
//         eT* xx_mem = xx.memptr();
...
inline void glue_conv::apply(Mat<typename T1::elem_type> &out,
  ...
  } else if (mode == 1) // same size as A
  {
    ...
    if ((tmp.is_empty() == false) && (A.is_empty() == false) &&
      ...
      out = (A_is_col) ? tmp(start, 0, arma::size(A))
                       : tmp(0, start, arma::size(A));
    } else {
...
// TODO: this implementation of conv2() is rudimentary; replace with faster
// version
template <typename eT>
inline void glue_conv2::apply(Mat<eT> &out, const Mat<eT> &A,
                              const Mat<eT> &B) {
  arma_extra_debug_sigprint();

  const Mat<eT> &G =
      (A.n_elem <= B.n_elem) ? A : B; // unflipped filter coefficients
  ...
  Mat<eT> H(G.n_rows, G.n_cols,
            arma_nozeros_indicator()); // flipped filter coefficients
  ...
  X(H_n_rows_m1, H_n_cols_m1, arma::size(W)) =
      W; // zero padded version of 2D image
      ...
      for (uword col = 0; col < out_n_cols; ++col) {
        ...
        for (uword row = 0; row < out_n_rows; ++row) {
          ...
          for (uword H_col = 0; H_col < H_n_cols; ++H_col) {
            ...
            acc +=
                op_dot::direct_dot(H_n_rows, H.colptr(H_col), &(X_colptr[row]));
          }
