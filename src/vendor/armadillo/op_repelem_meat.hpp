...
inline void op_repelem::apply_noalias(Mat<typename obj::elem_type> &out,
                                      const obj &X, const uword copies_per_row,
    ...
    for (uword row = 0; row < X_n_rows; ++row) {
      ...
      if (copies_per_col != 1) {
        for (uword col_copy = 1; col_copy < copies_per_col; ++col_copy) {
          ...
          arrayops::copy(&out_colptr[out_row_offset],
                         &out_colptr_first[out_row_offset], copies_per_row);
        }
      }
    }
  }
