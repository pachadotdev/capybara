...
arma_inline SpValProxy<T1>::SpValProxy(uword in_row, uword in_col,
                                       T1 &in_parent, eT *in_val_ptr)
    : row(in_row), col(in_col), val_ptr(in_val_ptr), parent(in_parent) {
  // Nothing to do.
}

template <typename T1>
arma_inline SpValProxy<T1> &
...
inline SpValProxy<T1> &SpValProxy<T1>::operator=(const eT rhs) {
  ...
  {
    if (val_ptr) {
      // The element exists, but we need to remove it, because it is being set
      // to 0.
  ...
  if (rhs != eT(0)) // I hope this is true!
  {
    ...
    if (val_ptr) {
      *val_ptr /= rhs; // That is where it gets ugly.
      ...
      eT val =
          eT(0) / rhs; // This may vary depending on type and implementation.
...
template <typename T1> inline SpValProxy<T1> &SpValProxy<T1>::operator++() {
  if (val_ptr) {
    (*val_ptr) += eT(1);
    parent.invalidate_cache();
    check_zero();
...
template <typename T1> arma_inline void SpValProxy<T1>::check_zero() {
  if (*val_ptr == eT(0)) {
    parent.delete_element(row, col);
    val_ptr = nullptr;
  }
