...
arma_inline typename enable_if2<is_arma_type<T1>::value, const T1 &>::result
operator+(const T1 &X) {
...
arma_inline typename enable_if2<is_arma_type<T1>::value,
                                const eOp<T1, eop_scalar_plus>>::result
...
arma_inline
    typename enable_if2<(is_arma_type<T1>::value &&
                        ...
                        const mtOp<typename std::complex<typename T1::pod_type>,
                                   T1, op_cx_scalar_plus>>::result
    operator+(const T1 &X, const std::complex<typename T1::pod_type> &k) {
  arma_extra_debug_sigprint();

  return mtOp<typename std::complex<typename T1::pod_type>, T1,
              op_cx_scalar_plus>('j', X, k);
}
...
arma_inline
  ...
  return mtOp<typename std::complex<typename T1::pod_type>, T1,
              op_cx_scalar_plus>('j', X, k); // NOTE: order is swapped
}
...
inline typename enable_if2<
    ...
    const mtSpGlue<typename promote_type<typename T1::elem_type,
                                         typename T2::elem_type>::result,
                   T1, T2, spglue_plus_mixed>>::result
operator+(const T1 &X, const T2 &Y) {
  arma_extra_debug_sigprint();
...
inline typename enable_if2<
    (is_arma_type<T1>::value && is_arma_sparse_type<T2>::value &&
     is_same_type<typename T1::elem_type, typename T2::elem_type>::no),
    Mat<typename promote_type<typename T1::elem_type,
...
operator+(const Base<typename parent::elem_type, T1> &X,
  ...
  return subview_each1_aux::operator_plus(Y,
...
operator+(const Base<typename parent::elem_type, T1> &X,
  ...
  return subview_each2_aux::operator_plus(Y,
                                          X.get_ref()); // NOTE: swapped order
}
