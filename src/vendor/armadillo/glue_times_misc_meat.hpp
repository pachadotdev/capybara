...
arma_inline typename arma_not_cx<eT>::result
...
template <typename eT>
arma_inline typename arma_cx_only<eT>::result
dense_sparse_helper::dot(const eT *A_mem, const SpMat<eT> &B, const uword col) {
  arma_extra_debug_sigprint();
...
template <typename T1, typename T2>
inline void
glue_times_dense_sparse::apply_noalias(Mat<typename T1::elem_type> &out,
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
      for (uword i = 0; i < B_n_cols; ++i) {
        ...
        const Col<eT> B_col(const_cast<eT *>(&(B.values[col_offset_1])),
                            col_offset_delta, false, false);
...
inline void glue_times_dense_sparse::apply_mixed(
  ...
  } else if ((is_same_type<eT1, out_eT>::yes) &&
             (is_same_type<eT2, out_eT>::no)) {
    // upgrade T2
...
template <typename T1, typename T2>
inline void
glue_times_sparse_dense::apply_noalias(Mat<typename T1::elem_type> &out,
                                       const T1 &x, const T2 &y) {
  arma_extra_debug_sigprint();
  ...
  if ((resolves_to_colvector<T2>::value) || (B_n_cols == 1)) {
    ...
    if (A_n_rows == B_n_cols) {
      ...
      op_strans::apply_mat(out, out); // since 'out' is square-sized, this will
                                      // do an inplace transpose
    } else {
      Mat<eT> tmp;
...
template <typename T1, typename T2>
inline void
glue_times_sparse_dense::apply_noalias_trans(Mat<typename T1::elem_type> &out,
                                             const T1 &x, const T2 &y) {
  arma_extra_debug_sigprint();
  ...
  const unwrap_spmat<T1> UA(x);
  const SpMat<eT> &A = UA.M; // NOTE: this is the given matrix without the
                             // transpose operation applied
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
    ...
    arma_extra_debug_print(
        "using standard multiplication (avoiding transpose of A)");
