...
// Partly based on algorithm 11.9 (inverse scaling and squaring algorithm with
// Schur decomposition) in: Nicholas J. Higham. Functions of Matrices: Theory
// and Computation. SIAM, 2008. ISBN 978-0-89871-646-7
...
template <typename T1>
inline bool
op_logmat::apply_direct(Mat<std::complex<typename T1::elem_type>> &out,
  ...
  out.zeros(
      N,
      N); // aliasing can't happen as op_logmat is defined as cx_mat = op(mat)
...
op_logmat::apply_direct(Mat<std::complex<typename T1::elem_type>> &out,
  ...
  const bool try_sympd =
      arma_config::optimise_sym && sym_helper::guess_sympd(A);
...
template <typename T1>
inline bool
op_logmat_cx::apply_direct(Mat<typename T1::elem_type> &out,
                           const Base<typename T1::elem_type, T1> &expr,
                           const uword n_iters) {
  arma_extra_debug_sigprint();
  ...
  const bool try_sympd =
      arma_config::optimise_sym && sym_helper::guess_sympd(S);
...
inline bool op_logmat_cx::apply_common(Mat<std::complex<T>> &out,
  ...
  while (iter < n_iters) {
    ...
    if (tau <= theta[6]) {
      ...
      arma_debug_check((j2 > j1),
                       "internal error: op_logmat::apply_direct(): j2 > j1");
  ...
  if (iter >= n_iters) {
    arma_debug_warn_level(
        2, "logmat(): reached max iterations without full convergence");
  }
...
inline bool op_logmat_cx::helper(Mat<eT> &A, const uword m) {
  ...
  for (uword i = 0; i < m; ++i) {
    ...
    // solve_opts::fast );
    const bool solve_ok = solve(X, trimatu(nodes(i) * A + eye<Mat<eT>>(N, N)),
                                A, solve_opts::no_approx);
...
template <typename T1>
inline bool
op_logmat_sympd::apply_direct(Mat<typename T1::elem_type> &out,
                              const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_LAPACK)
  {
    ...
    const bool status =
        eig_sym_helper(eigval, eigvec, X, 'd', "logmat_sympd()");
