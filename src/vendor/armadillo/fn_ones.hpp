...
arma_warn_unused arma_inline const Gen<obj_type, gen_ones>
ones(const uword n_elem, const arma_empty_class junk1 = arma_empty_class(),
...
arma_warn_unused arma_inline const
    GenCube<typename cube_type::elem_type, gen_ones>
    ones(const uword n_rows, const uword n_cols, const uword n_slices,
         const typename arma_Cube_only<cube_type>::result *junk = nullptr) {
  arma_extra_debug_sigprint();
  arma_ignore(junk);

  return GenCube<typename cube_type::elem_type, gen_ones>(n_rows, n_cols,
                                                          n_slices);
}
...
arma_warn_unused arma_inline const
  ...
  return GenCube<typename cube_type::elem_type, gen_ones>(s.n_rows, s.n_cols,
                                                          s.n_slices);
}
