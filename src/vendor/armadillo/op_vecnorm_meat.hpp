...
inline void
op_vecnorm::apply(Mat<typename T1::pod_type> &out,
...
inline void
op_vecnorm::apply_noalias(Mat<typename get_pod_type<in_eT>::result> &out,
...
template <typename in_eT>
inline void
op_vecnorm::apply_rawmem(typename get_pod_type<in_eT>::result &out_val,
...
template <typename in_eT>
inline void
op_vecnorm_ext::apply_noalias(Mat<typename get_pod_type<in_eT>::result> &out,
  ...
  if (dim == 0) {
    ...
    if (X_n_rows > 0) {
      ...
      for (uword col = 0; col < X_n_cols; ++col) {
        op_vecnorm_ext::apply_rawmem(out_mem[col], X.colptr(col), X_n_rows,
                                     method_id);
      }
    }
  } else if (dim == 1) {
    ...
    if (X_n_cols > 0) {
      ...
      for (uword row = 0; row < X_n_rows; ++row) {
        ...
        op_vecnorm_ext::apply_rawmem(out_mem[row], dat_mem, X_n_cols,
                                     method_id);
      }
    }
  }
...
template <typename in_eT>
inline void
op_vecnorm_ext::apply_rawmem(typename get_pod_type<in_eT>::result &out_val,
