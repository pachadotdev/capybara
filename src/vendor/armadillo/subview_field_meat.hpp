...
arma_inline
subview_field<oT>::subview_field(const field<oT> &in_f, const uword in_row1,
                                 const uword in_col1, const uword in_n_rows,
                                 const uword in_n_cols)
    : f(in_f), aux_row1(in_row1), aux_col1(in_col1), aux_slice1(0),
...
subview_field<oT>::subview_field(const field<oT> &in_f, const uword in_row1,
    ...
    : f(in_f), aux_row1(in_row1), aux_col1(in_col1), aux_slice1(in_slice1),
...
inline void subview_field<oT>::operator=(const field<oT> &x) {
  ...
  arma_debug_check((t.n_rows != x.n_rows) || (t.n_cols != x.n_cols) ||
                       (t.n_slices != x.n_slices),
...
template <typename oT>
arma_inline oT &subview_field<oT>::operator()(const uword in_row,
                                              const uword in_col) {
...
template <typename oT> arma_inline bool subview_field<oT>::is_empty() const {
  return (n_elem == 0);
}
...
inline bool subview_field<oT>::check_overlap(const subview_field<oT> &x) const {
  ...
  if (&t.f != &x.f) {
    ...
    if ((t.n_elem == 0) || (x.n_elem == 0)) {
      ...
      const bool outside_slices = ((x_slice_start >= t_slice_end_p1) ||
                                   (t_slice_start >= x_slice_end_p1));
...
template <typename oT> inline void subview_field<oT>::fill(const oT &x) {
  arma_extra_debug_sigprint();
...
inline void subview_field<oT>::extract(field<oT> &actual_out,
  ...
  arma_extra_debug_print(
      ...
      out.n_rows % out.n_cols % out.n_slices % in.f.n_rows % in.f.n_cols %
      in.f.n_slices);
