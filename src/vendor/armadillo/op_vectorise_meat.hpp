...
inline void op_vectorise_col::apply_direct(Mat<typename T1::elem_type> &out,
  ...
  if (is_Mat<T1>::value || (arma_config::openmp && Proxy<T1>::use_mp)) {
    ...
    if (&out == &(U.M)) {
      ...
      out.set_size(out.n_elem,
                   1); // set_size() doesn't destroy data as long as the
...
inline void op_vectorise_col::apply_subview(Mat<eT> &out,
                                            const subview<eT> &sv) {
  arma_extra_debug_sigprint();
...
template <typename T1>
inline void
op_vectorise_cube_col::apply(Mat<typename T1::elem_type> &out,
  ...
  if (is_same_type<T1, subview_cube<eT>>::yes) {
    op_vectorise_cube_col::apply_subview(
        out, reinterpret_cast<const subview_cube<eT> &>(in.m));
  } else {
...
template <typename T1>
inline void
op_vectorise_cube_col::apply_unwrap(Mat<typename T1::elem_type> &out,
                                    const T1 &expr) {
  arma_extra_debug_sigprint();
