...
inline bool
op_log_det::apply_direct(typename T1::elem_type &out_val,
  ...
  // const bool try_sympd = arma_config::optimise_sym &&
  // sym_helper::guess_sympd(A);
  //
  // if(try_sympd)
  //   {
...
template <typename T1>
inline bool
op_log_det::apply_diagmat(typename T1::elem_type &out_val,
                          typename T1::pod_type &out_sign,
                          const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
  ...
  T sign =
      (is_cx<eT>::no) ? ((access::tmp_real(x) < T(0)) ? T(-1) : T(1)) : T(1);
  eT val = (is_cx<eT>::no)
...
template <typename T1>
inline bool
op_log_det::apply_trimat(typename T1::elem_type &out_val,
                         typename T1::pod_type &out_sign,
                         const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
  ...
  arma_debug_check((N != P.get_n_cols()),
                   "log_det(): given matrix must be square sized");
...
template <typename T1>
inline bool
op_log_det_sympd::apply_direct(typename T1::pod_type &out_val,
                               const Base<typename T1::elem_type, T1> &expr) {
  arma_extra_debug_sigprint();
  ...
  if ((arma_config::debug) && (arma_config::warn_level > 0) &&
      (is_cx<eT>::yes) && (sym_helper::check_diag_imag(A) == false)) {
  ...
  if ((arma_config::debug) && (auxlib::rudimentary_sym_check(A) == false)) {
    if (is_cx<eT>::no) {
      arma_debug_warn_level(1,
                            "log_det_sympd(): given matrix is not symmetric");
    }
    if (is_cx<eT>::yes) {
      arma_debug_warn_level(1,
                            "log_det_sympd(): given matrix is not hermitian");
    }
  }
