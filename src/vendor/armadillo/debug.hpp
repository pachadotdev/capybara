...
arma_deprecated inline std::ostream &get_stream_err1() {
  return get_cerr_stream();
}

arma_deprecated inline std::ostream &get_stream_err2() {
  return get_cerr_stream();
}

arma_frown("this function does nothing; instead use ARMA_COUT_STREAM or "
           "ARMA_WARN_LEVEL; see "
           "documentation") inline void set_cout_stream(const std::ostream &) {}

arma_frown("this function does nothing; instead use ARMA_CERR_STREAM or "
           "ARMA_WARN_LEVEL; see "
...
template <typename T>
arma_frown(
    "this function does nothing; instead use ARMA_COUT_STREAM or "
    "ARMA_WARN_LEVEL; see "
...
template <typename T>
arma_frown(
    "this function does nothing; instead use ARMA_CERR_STREAM or "
    "ARMA_WARN_LEVEL; see "
...
arma_cold arma_noinline static void arma_stop_logic_error(const char *x,
                                                          const char *y) {
...
arma_hot inline void arma_check(const bool state, const char *x,
                                const Functor &fn) {
  if (state) {
    fn();
...
arma_cold arma_noinline static std::string
arma_incompat_size_string(const uword A_n_rows, const uword A_n_cols,
                          const uword B_n_rows, const uword B_n_cols,
                          const char *x) {
  std::ostringstream tmp;

  tmp << x << ": incompatible matrix dimensions: " << A_n_rows << 'x'
      << A_n_cols << " and " << B_n_rows << 'x' << B_n_cols;
...
arma_incompat_size_string(const uword A_n_rows, const uword A_n_cols,
  ...
  tmp << x << ": incompatible cube dimensions: " << A_n_rows << 'x' << A_n_cols
      << 'x' << A_n_slices << " and " << B_n_rows << 'x' << B_n_cols << 'x'
      << B_n_slices;
...
arma_cold arma_noinline static std::string
arma_incompat_size_string(const subview_cube<eT> &Q, const Mat<eT> &A,
                          const char *x) {
  std::ostringstream tmp;

  tmp << x << ": interpreting matrix as cube with dimensions: " << A.n_rows
      << 'x' << A.n_cols << 'x' << 1 << " or " << A.n_rows << 'x' << 1 << 'x'
      << A.n_cols << " or " << 1 << 'x' << A.n_rows << 'x' << A.n_cols
      << " is incompatible with cube dimensions: " << Q.n_rows << 'x'
      << Q.n_cols << 'x' << Q.n_slices;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Proxy<eT1> &A,
                                           const Proxy<eT2> &B, const char *x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const subview<eT1> &A,
                                           const subview<eT2> &B,
                                           const char *x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Mat<eT1> &A,
                                           const Proxy<eT2> &B, const char *x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const SpMat<eT1> &A,
                                           const SpMat<eT2> &B, const char *x) {
  const uword A_n_rows = A.n_rows;
  const uword A_n_cols = A.n_cols;
...
arma_hot inline void
arma_assert_same_size(const uword A_n_rows, const uword A_n_cols,
                      const uword A_n_slices, const uword B_n_rows,
                      const uword B_n_cols, const uword B_n_slices,
                      const char *x) {
  if ((A_n_rows != B_n_rows) || (A_n_cols != B_n_cols) ||
      (A_n_slices != B_n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_same_size(const Cube<eT1> &A,
                                           const Cube<eT2> &B, const char *x) {
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) ||
      (A.n_slices != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(
        A.n_rows, A.n_cols, A.n_slices, B.n_rows, B.n_cols, B.n_slices, x));
  }
}
...
arma_hot inline void arma_assert_same_size(const Cube<eT1> &A,
  ...
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (A.n_slices != 1)) {
    arma_stop_logic_error(arma_incompat_size_string(
        A.n_rows, A.n_cols, A.n_slices, B.n_rows, B.n_cols, 1, x));
  }
}
...
arma_hot inline void arma_assert_same_size(const Mat<eT1> &A,
  ...
  if ((A.n_rows != B.n_rows) || (A.n_cols != B.n_cols) || (1 != B.n_slices)) {
    arma_stop_logic_error(arma_incompat_size_string(
        A.n_rows, A.n_cols, 1, B.n_rows, B.n_cols, B.n_slices, x));
  }
}
...
template <typename eT, typename T1>
inline void arma_assert_cube_as_mat(const Mat<eT> &M, const T1 &Q,
                                    const char *x,
    ...
    if (M_vec_state == 0) {
      ...
      if (Q_n_slices == 1) {
        if ((M_vec_state == 1) && (Q_n_rows != M_n_rows)) {
          ...
          tmp << x << ": can't interpret cube with dimensions " << Q_n_rows
              << 'x' << Q_n_cols << 'x' << Q_n_slices
              << " as a column vector with dimensions " << M_n_rows << 'x'
              << M_n_cols;
        ...
        if ((M_vec_state == 2) && (Q_n_cols != M_n_cols)) {
          ...
          tmp << x << ": can't interpret cube with dimensions " << Q_n_rows
              << 'x' << Q_n_cols << 'x' << Q_n_slices
              << " as a row vector with dimensions " << M_n_rows << 'x'
              << M_n_cols;
        ...
        if (((M_n_cols == Q_n_slices) || (M_n_rows == Q_n_slices)) == false) {
          ...
          tmp << x << ": can't interpret cube with dimensions " << Q_n_rows
              << 'x' << Q_n_cols << 'x' << Q_n_slices
              << " as a vector with dimensions " << M_n_rows << 'x' << M_n_cols;
...
//
// functions for checking whether two matrices have dimensions that are
// compatible with the matrix multiply operation

arma_hot inline void arma_assert_mul_size(const uword A_n_rows,
                                          const uword A_n_cols,
                                          const uword B_n_rows,
...
arma_hot inline void arma_assert_mul_size(const Mat<eT1> &A, const Mat<eT2> &B,
  ...
  if (final_A_n_cols != final_B_n_rows) {
    ...
    arma_stop_logic_error(arma_incompat_size_string(
        final_A_n_rows, final_A_n_cols, final_B_n_rows, final_B_n_cols, x));
  }
}

template <const bool do_trans_A, const bool do_trans_B>
arma_hot inline void
arma_assert_trans_mul_size(const uword A_n_rows, const uword A_n_cols,
                           const uword B_n_rows, const uword B_n_cols,
                           const char *x) {
...
template <typename eT1, typename eT2>
arma_hot inline void arma_assert_mul_size(const subview<eT1> &A,
                                          const Mat<eT2> &B, const char *x) {
  if (A.n_cols != B.n_rows) {
    arma_stop_logic_error(
        arma_incompat_size_string(A.n_rows, A.n_cols, B.n_rows, B.n_cols, x));
...
template <typename T1> arma_hot inline void arma_assert_blas_size(const T1 &A) {
  if (sizeof(uword) >= sizeof(blas_int)) {
    ...
    if (overflow) {
      arma_stop_runtime_error("integer overflow: matrix dimensions are too "
                              "large for integer type used by "
                              "BLAS and LAPACK");
    }
  }
}
...
#define arma_debug_assert_trans_mul_size                                       \
  true ? (void)0 : arma_assert_trans_mul_size
