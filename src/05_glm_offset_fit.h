...
inline FeglmOffsetFitResult feglm_offset_fit(
  vec eta,
  const vec &y,
  const vec &offset,
  const vec &wt,
  ...
  vec mu = link_inv_(eta, family_type), mu_eta(n, fill::none), yadj(n, fill::none), w(n, fill::none), eta_upd(n, fill::none), eta_old(n, fill::none);
  double dev = dev_resids_(y, mu, 0.0, wt, family_type), dev_old, dev_ratio, dev_ratio_inner, rho;
  ...
  for (iter = 0; iter < iter_max; ++iter) {
    ...
    Myadj += yadj;
    center_variables_(Myadj, w, k_list, center_tol, iter_center_max, iter_interrupt, iter_ssr);
