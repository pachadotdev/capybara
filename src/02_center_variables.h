...
// FENmlm-style centering: direct group mean subtraction using Armadillo
void center_variables_(mat &V, const vec &w, const list &klist,
  ...
  // Main centering loop (FENmlm style)
  for (size_t p = 0; p < P; ++p) {
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
      x0 = x;
      // For each group, subtract group mean
      for (size_t k = 0; k < K; ++k) {
      ...
      // Convergence check (weighted mean abs diff)
