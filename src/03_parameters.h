...
inline InferenceBeta get_beta(const mat &X, const vec &y, const vec &y_orig,
  ...
  if (has_weights) {
    for (size_t i = 0; i < p; ++i) {
      for (size_t j = i; j < p; ++j) {
        XtX(i, j) = dot(X.col(i) % w, X.col(j));
        if (i != j)
          XtX(j, i) = XtX(i, j);
      }
    }
    for (size_t i = 0; i < p; ++i) {
      XtY(i) = dot(X.col(i) % w, y);
    }
  } else {
    XtX = X.t() * X;
    XtY = X.t() * y;
...
inline InferenceAlpha get_alpha(const vec &sumFE,
  ...
  if (max_groups == 0) {
    ...
    for (size_t q = 0; q < Q; ++q) {
      result.Alpha(q) = vec(0, fill::zeros);
    }
  ...
  while (iter < iter_max && !converged) {
    iter++;
    std::memcpy(old_fe_ptr, current_fe_ptr, N * sizeof(double));
    for (size_t q = 0; q < Q; ++q) {
      if (cluster_sizes(q) == 0)
        continue;
      std::memcpy(residual_ptr, sumFE_ptr, N * sizeof(double));
      ...
      std::fill(group_sums_ptr, group_sums_ptr + n_groups, 0.0);
      std::fill(group_counts_ptr, group_counts_ptr + n_groups, 0);
    ...
    std::fill(current_fe_ptr, current_fe_ptr + N, 0.0);
    for (size_t q = 0; q < Q; ++q) {
      const uword *obs_group_ptr = obs_to_group.colptr(q);
