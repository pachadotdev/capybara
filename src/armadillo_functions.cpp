...
// Helper function for 2-FE special case
// This avoids creating N-sized temporary vectors
void center_variables_2fe_(mat &V, const vec &w, const list &klist,
                           const double &tol, const size_t &max_iter,
                           const int &iter_interrupt, const size_t &iter_ssr) {
  ...
  // Precompute weights
  for (j = 0; j < n_fe1; ++j) {
    fe1_groups(j) = as_uvec(fe1_list[j]);
    sum_w = accu(w(fe1_groups(j)));
    fe1_weights(j) = sum_w > 0 ? 1.0 / sum_w : 0.0;
  }

  for (j = 0; j < n_fe2; ++j) {
    fe2_groups(j) = as_uvec(fe2_list[j]);
    sum_w = accu(w(fe2_groups(j)));
    fe2_weights(j) = sum_w > 0 ? 1.0 / sum_w : 0.0;
  }

  // Lambda for applying projections (optimized for 2 FEs)
...
void center_variables_(mat &V, const vec &w, const list &klist,
  ...
  // Special case for 2 fixed effects
  if (K == 2) {
    center_variables_2fe_(V, w, klist, tol, max_iter, iter_interrupt, iter_ssr);
    return;
  }
  ...
  for (k = 0; k < K; ++k) {
    ...
    for (j = 0; j < J; ++j) {
      ...
      sum_w = accu(w(idxs(j)));
      weights(j) = sum_w > 0 ? 1.0 / sum_w : 0.0;
    }

    group_indices(k) = std::move(idxs);
    group_weights(k) = std::move(weights);
  }

  // Lambda for applying projections
  auto project = [&](vec &v) {
    for (k = 0; k < K; ++k) {
