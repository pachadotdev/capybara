...
inline bool pivoted_cholesky(mat &L, uvec &perm, uvec &excluded, const mat &XtX, double tol) {
...
inline vec solve_lower_triangular(const mat &L, const vec &b, const uvec &valid_cols) {
...
inline bool modified_gram_schmidt(mat &Q, mat &R, uvec &excluded, const mat &X, double tol) {
...
inline vec robust_solve(const mat &XtX, const vec &XtY, double tol, uvec &excluded) {
  const size_t p = XtX.n_cols;
