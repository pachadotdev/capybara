...
struct ObsToGroupMapping {
  ...
  void build_index(const field<field<uvec>> &group_indices) {
    ...
    for (uword g = 0; g < g0.n_elem; ++g) {
      if (!g0(g).is_empty())
        max_idx = std::max(max_idx, arma::max(g0(g)));
    }
    n_obs = max_idx + 1;
  ...
  void update_weights(const vec &w) {
    ...
    for (uword k = 0; k < K; ++k) {
      ...
      // Invert
      for (uword g = 0; g < n_groups; ++g) {
        if (inv_w_ptr[g] > 1e-12)
          inv_w_ptr[g] = 1.0 / inv_w_ptr[g];
        else
          inv_w_ptr[g] = 0.0;
      }
    }
...
template <int B_SZ>
inline void center_2fe_block_iter(const uword n_obs, const uword p_start,
                                  const uword real_b_sz, const double *w_ptr,
                                  const uword *g1, const uword *g2, mat &V,
                                  mat &alpha1, mat &alpha2) {
...
template <int B_SZ>
inline void center_2fe_block_iter_a2(const uword n_obs, const uword p_start,
                                     const uword real_b_sz, const double *w_ptr,
                                     const uword *g1, const uword *g2, mat &V,
                                     mat &alpha1, mat &alpha2) {
  const double *v_ptrs[B_SZ];
...
inline void center_accel_2fe(mat &V, const vec &w,
                             const ObsToGroupMapping &mapping, double tol,
                             uword max_iter, CenteringWorkspace &ws) {
  ...
  for (uword iter = 0; iter < max_iter; ++iter) {
    ...
    for (uword p = 0; p < P; p += block_size) {
      uword b_sz = std::min(block_size, P - p);
      center_2fe_block_iter_a2<4>(n_obs, p, b_sz, p_w, g1, g2, V, alpha1,
                                  alpha2);
    }
        ...
        if (j < (int)real_b_sz) {
          ...
          for (uword o = 0; o < K; ++o) {
            if (o != k_idx) {
              ...
              // Let's pass: array of pointers where ptrs[o] =
              // alpha[o].colptr(p_start+j) But we have multiple j.
...
inline void center_kfe_accel(mat &V, const vec &w,
  ...
  // But keep persistent if workspace is reused? The caller might expect 0
  // start. center_variables doesn't promise warm start across calls usually.
  Alpha.zeros();
  ...
  for (uword k = 0; k < K; ++k)
    maps_ptrs[k] = mapping.maps[k].data();
  ...
  for (uword iter = 0; iter < max_iter; ++iter) {
    ...
    for (uword k = 0; k < K; ++k) {
      ...
      for (uword p = 0; p < P; p += block_size) {
        ...
        for (uword j = 0; j < b_sz; ++j) {
          ...
          for (uword o = 0; o < K; ++o) {
            all_alphas_ptrs[o * block_size + j] =
                Alpha.colptr(p + j) + offsets[o];
          }
        }

        project_kfe_block_iter<4>(n_obs, p, b_sz, w_ptr, maps_ptrs[k],
                                  maps_ptrs, all_alphas_ptrs, k, K, V,
                                  write_ptrs);
      }
    ...
    if (iter >= 3) {
      if (use_accel) {
        ...
        if (ssq > 1e-14) {
          ...
          double coef =
              accu(Delta_G % Delta2) / ssq; // Re-check original code logic
    ...
    if (norm(Alpha - Alpha_old, "fro") < tol)
      break;
  }
...
void center_variables(mat &V, const vec &w,
                      ...
                      const double &tol, const uword &max_iter,
                      const uword &iter_interrupt,
