...
public:
  ...
  // through indexing 
  FGCov(const arma::mat& A_inv_,
	const arma::mat& X_,
	const arma::vec& u_hat_,
	const arma::uvec& from_id_,
	const arma::uvec& to_id_,
    ...
    : A_inv(A_inv_),
      ...
      N(X_.n_rows), 
      K(X_.n_cols),
      G(std::max(arma::max(from_id_), arma::max(to_id_))),
      verbose(verbose_)
  {

    omp_set_num_threads(ncores);

    if (u_hat_.n_elem != (unsigned)N ||
	from_id.n_elem != (unsigned)N ||
	to_id.n_elem != (unsigned)N) {
      Rcpp::stop("Dimensions mismatch: nrow(X) must match lengths of u_hat, from_id, to_id.");
    }
  ...
  arma::mat meatIndexed() const {
    if (verbose)
      std::cout << "Computing 'meat' of covariance sandwich (chunked parallel)\n";
    arma::mat B(K, K, arma::fill::zeros);

    const int nthreads = omp_get_max_threads();   
    int nchunks = nthreads * 10; // Every thread gets about 10 jobs 
    if(nchunks > static_cast<int>(N)) nchunks = static_cast<int>(N);;
...
// [[Rcpp::export]]
arma::mat vcovFG_Rcpp(const arma::mat& A_inv,
		      const arma::mat& X,
		      const arma::vec& u_hat,
		      const arma::uvec& from_id,
		      const arma::uvec& to_id,
		      const int ncores = 1,
