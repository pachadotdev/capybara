...
second_order_derivative_ <- function(eta, mu_eta, family) {
  ...
  if (link == "logit") {
    ...
    return(-2.0 * mu_eta / linkinv_eta)
  } else if (link == "sqrt") {
    return(2)
  } else if (link == "cauchit") {
    return(-2.0 * eta / (1.0 + eta^2) * mu_eta)
  } else {
    # For any other link, try to compute numerically or throw an error
    stop(paste("Second derivative not implemented for link:", link))
  }
}
