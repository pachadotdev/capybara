...
# Extract and normalize fixed effects to match fixest/Stata convention
# Normalization: set first level of first FE to zero
fe_raw <- fit_baseline_app1$fixed_effects
first_fe_val <- fe_raw$exporter[1]

fe_exporter_bln <- (fe_raw$exporter - first_fe_val) |>
  tibble::enframe(name = "exporter", value = "fe_exporter_bln")

fe_importer_bln <- (fe_raw$importer + first_fe_val) |>
...
# Extract and normalize fixed effects to match fixest/Stata convention
fe_raw_cfl <- fit_counterfactual_app1$fixed_effects
first_fe_val_cfl <- fe_raw_cfl$exporter[1]

fe_exporter_cfl <- (fe_raw_cfl$exporter - first_fe_val_cfl) |>
  tibble::enframe(name = "exporter", value = "fe_exporter_cfl")

fe_importer_cfl <- (fe_raw_cfl$importer + first_fe_val_cfl) |>
