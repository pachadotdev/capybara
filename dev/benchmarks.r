...
if (run[2]) {
  ...
  if (!file.exists(fout)) {
    ...
    bench_trade_diversion <- mark(
      round(alpaca::feglm(form, data = ch1_application3, family = poisson())$coefficients["rta"], 2),
      round(fixest::fepois(form, data = ch1_application3)$coefficients["rta"], 2),
      round(capybara::fepoisson(form, data = ch1_application3)$coefficients["rta"], 2),
      iterations = iterations_pkgs
    )
