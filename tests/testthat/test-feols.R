...
#' @srrstats {RE3.1} Validates consistency between `feols` and base R `lm` in terms of coefficients, R-squared, and fitted values.
...
test_that("feols works", {
  ...
  m1 <- feols(mpg ~ wt + qsec | cyl, mtcars)
  ...
  m1 <- feols(mpg ~ wt + qsec | cyl + am, mtcars)
  ...
  m1 <- feols(mpg ~ wt + qsec | cyl + am, mtcars2)
  ...
  m1 <- feols(mpg ~ wt + qsec | cyl + am | carb, mtcars)

  expect_equal(coef(m1), coef(m2)[c(2, 3)], tolerance = 1e-2)

  # 3-FE ----

  m1 <- feols(mpg ~ wt + qsec | cyl + am + gear, mtcars)
...
test_that("feols time is the minimally affected when adding noise to the data", {
  mtcars2 <- mtcars[, c("mpg", "wt", "cyl")]
  set.seed(200100)
  mtcars2$mpg <- mtcars2$mpg + rbinom(nrow(mtcars2), 1, 0.5) *
    .Machine$double.eps
  m1 <- feols(mpg ~ wt | cyl, mtcars)
  m2 <- feols(mpg ~ wt | cyl, mtcars2)
...
test_that("Inf values are dropped", {
  ...
  mtcars2$mpg[1] <- 0
  expect_error(feols(log(mpg) ~ log(wt) | cyl, mtcars2), "Infinite values")
  ...
  mtcars2$wt[2] <- 1
  m1 <- feols(log(mpg) ~ log(wt) | cyl, mtcars2)
  m2 <- lm(log(mpg) ~ log(wt) + as.factor(cyl), mtcars2)
  expect_equal(coef(m2)[2], coef(m1), tolerance = 1e-2)
})

test_that("feols correctly predicts values outside the inter-quartile range", {
  ...
  m1_lm <- feols(mpg ~ wt + disp | cyl, mtcars)
